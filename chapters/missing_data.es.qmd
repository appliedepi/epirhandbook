---
aliases:
  - /new_pages/missing_data.es.html
---
...
Consulta la sección de [datos faltantes](importing.es.qmd#import_missing) de la página de Importación para obtener más detalles, ya que la sintaxis exacta varía según el tipo de archivo. 
...
Entonces, representamos la proporción que falta como una línea, por semana. Si no estás familiarizado con el paquete de gráficas **ggplot2**, consulta la página de [fundamentos de ggplot](ggplot_basics.es.qmd).
...
Puedes listar las columnas una tras otra, o utilizar [las funciones de ayuda "tidyselect"](cleaning.es.qmd#clean_tidyselect):
...
Si tu columna de interés es un factor, utiliza `fct_explicit_na()` del paquete **forcats** para convertir los valores `NA` en un valor de carácter. Mira más detalles en la página de [Factores](factors.es.qmd). Por defecto, el nuevo valor es "(Missing)" pero puede ajustarse mediante el argumento `na_level = `.
