...
Recuerda de la página de [importación y exportación](#import-and-export) que cuando se utiliza en libros de Excel, `import()` puede aceptar el argumento `which = ` especificando la hoja a importar. Dentro de la función `.f` en  `import()`, proporcionamos `which = .x`, cuyo valor cambiará con cada iteración a través del vector `sheet_names` - primero "Central Hospital", luego "Military Hospital", etc. 
