...
La sintaxis básica es: `for (elemento in secuencia) {hacer operaciones con el elemento}`. Fíjate en los paréntesis y las llaves. Los resultados pueden ser impresos en la consola, o almacenados en un objeto R contenedor. 
...
La estructura básica de una declaración de secuencia es `elemento in vector`. 
...
Hemos elegido el término `hosp` para representar los valores del vector `hospital_names`. Para la primera iteración del bucle, el valor de `hosp` será `hospital_names[[1]]`. Para la segunda iteración del bucle será `hospital_names[[2]]`. Y así sucesivamente... 
...
Si quieres que la secuencia sea numérica, pero partes de un vector (no de un dataframe), utiliza el atajo `seq_along()` para devolver una secuencia de números para cada elemento del vector. Por ejemplo, `for (i in seq_along(hospital_names) {Código_de_operaciones}`. 
...
Para reunir los tres componentes (contenedor, secuencia y operaciones) vamos a intentar trazar una epicurva para cada hospital (véase la página sobre [curvas epidémicas](epicurves.es.qmd)). 
...
Para producir un gráfico separado para los casos de cada hospital, podemos poner este código de epicurva dentro de un *bucle for*. 
...
Ten en cuenta que **purrr** es una *herramienta de programación funcional*. Es decir, las operaciones que se van a aplicar de forma iterativa están envueltas en *funciones*. Consulta la página [Escribir funciones](writing_functions.es.qmd) para aprender a escribir tus propias funciones. 
...
```{r, echo=F}
hospital_linelists_dir <- tempfile()
dir.create(hospital_linelists_dir)
appliedepidata::save_data(name = "example_hospital_linelists", path = hospital_linelists_dir)
sheet_names <- readxl::excel_sheets(fs::path(hospital_linelists_dir, "example_hospital_linelists.xlsx"))

```

```{r, eval=F}
appliedepidata::save_data(name = "example_hospital_linelists", path = getwd())
sheet_names <- readxl::excel_sheets("example_hospital_linelists.xlsx")
```
...
Recuerda de la página de [importación y exportación](importing.es.qmd) que cuando se utiliza en libros de Excel, `import()` puede aceptar el argumento `which = ` especificando la hoja a importar. Dentro de la función `.f` en  `import()`, proporcionamos `which = .x`, cuyo valor cambiará con cada iteración a través del vector `sheet_names` - primero "Central Hospital", luego "Military Hospital", etc. 
...
combined <- sheet_names %>%
  purrr::set_names() %>%
  map(.f = ~import(fs::path(hospital_linelists_dir, "example_hospital_linelists.xlsx"), which = .x))
```
...
combined <- sheet_names %>%
  purrr::set_names() %>%
  map(.f = ~import("example_hospital_linelists.xlsx", which = .x))
```
...
```{r, eval=F}
appliedepidata::save_data(name = "example_hospital_linelists", path = getwd())
sheet_names <- readxl::excel_sheets("example_hospital_linelists.xlsx")  # extrae los nombres de las hojas

combined <- sheet_names %>%                                     # comienza con los nombres de las hojas
  purrr::set_names() %>%                                        # establece sus nombres
  map(.f = ~import("example_hospital_linelists.xlsx", which = .x)) %>% # iterativamente, importa, guarda en una lista
...
combined <- sheet_names %>%
     ...
     map_at(.f = ~import(fs::path(hospital_linelists_dir, "example_hospital_linelists.xlsx"), which = .x),
            .at = c(-1))
```
...
combined <- sheet_names %>%
     purrr::set_names() %>%
     # exclude the first sheet
     map_at(.f = ~import("example_hospital_linelists.xlsx", which = .x),
            .at = c(-1))
```
...
Digamos que tenemos una `linelist` de casos completa como un dataframe, y ahora queremos crear un listado separado para cada hospital y exportar cada una como un archivo CSV separado. A continuación, hacemos los siguientes pasos: 
...
names(linelist_split) <- groupings %>% 
     mutate(across(everything(), ~ replace_na(.x, "Missing"))) %>%  # sustituye NA por "Missing" en todas las columnas
...
De nuevo utilizamos `map()`: tomamos el vector de nombres de elementos de la lista (mostrado arriba) y utilizamos `map()` para iterar a través de ellos, aplicando `export()` (del paquete **rio**, véase la página [Importar y exportar](importing.es.qmd)) en el dataframe de lista `linelist_split` que tiene ese nombre. También utilizamos el nombre para crear un nombre de archivo único. Así es como funciona: 
...
* La ruta del archivo proporcionada a `export()` es dinámica mediante el uso de `str_glue()` (ver página de [caracteres y cadenas](characters_strings.es.qmd)): 
...
Recuerda de la página sobre [Test estadísticos simples](stat_tests.es.qmd) que `t.test()` puede tomar entradas en un formato de fórmula, como `t.test(columna numérica ~ columna binaria)`. En este ejemplo, hacemos lo siguiente: 
...
* Cada uno de esos cinco elementos son a su vez listas, con elementos dentro de ellas como `p.value` y `conf.int`. Algunos de estos elementos, como `p.value`, son números individuales, mientras que otros, como `estimate`, constan de dos o más elementos (`mean in group f` y `mean in group m`). 
...
Nota: Recuerda que si deseas aplicar una función sólo a determinadas columnas de un dataframe, puedes utilizar simplemente `mutate()` y `across()`, como se explica en la página [Limpieza de datos y funciones básicas](cleaning.es.qmd). A continuación se muestra un ejemplo de aplicación de `as.character()` sólo a las columnas "age". Observa la colocación de los paréntesis y las comas.   
...
Tendríamos que extraer el elemento `estimate`, pero éste contiene en realidad *dos* elementos en su interior (`mean in group f` y `mean in group m`). Por lo tanto, no se puede simplificar en un vector con `map_chr()` o `map_dbl()`. En su lugar, utilizamos `map()`, que usado dentro de `tibble()` creará *una columna de tipo lista dentro del tibble*! ¡Sí, esto es posible! 
...
* `hoist()` - actúa como `unnest_wider()` pero se especifica qué elementos se van a desanidar 
