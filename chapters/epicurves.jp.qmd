...
解析に必要な標準パッケージを読み込みます。 このハンドブックでは、パッケージを読み込むために、**pacman** パッケージの `p_load()` を主に使用しています。`p_load()` は、必要に応じてパッケージをインストールし、現在の R セッションで使用するためにパッケージを読み込む関数です。また、すでにインストールされたパッケージは、R の基本パッケージである **base** の `library()` を使用して読み込むこともできます。R パッケージについての詳細は、[R の基礎](basics.jp.qmd) の章を参照してください。
...
データセットは、**rio** パッケージの `import()` を使ってインポートします。データをインポートする様々な方法については、[データのインポート・エクスポート](importing.jp.qmd) の章を参照してください。
...
エボラ出血熱の流行をシミュレートしたデータセットをインポートします。この章の内容をお手元の環境で実行したい方は、[ハンドブックとデータのダウンロード](data_used.jp.qmd) の章をご覧ください。ファイルが作業ディレクトリにある場合、ファイルパスにはファイル名のみ指定します。
...
`ggplot()` コマンドでは、データセットを `data =` で指定します。 この土台の上に、ヒストグラムの形状を `+` で追加します。`geom_histogram()` では、`date_onset` 列が x 軸にマッピングされるように指定します。また、`geom_histogram()` 内では、`aes()` 内ではなく、ヒストグラムの `binwidth =` を日単位で設定します。この **ggplot2** の構文が分かりにくい場合は、[ggplot の基礎](ggplot_basics.jp.qmd) の章を参照して下さい。

[<u>**注意**</u>**：**`binwidth = 7` **を使って週別の症例数をプロットすると、7日間の最初のビンは、最初の症例で開始されるため、週の始まりが何曜日になるかは分かりません。特定の週を作成するには、以下のセクションを参照してください。**]{style="color: orange;"}
...
この Central Hospital のデータセットの最初の症例の発症日に注目してみましょう。
...
* 日付が正しいビンにカウントされるようにするため `geom_histogram()` の `closed = "left"` オプションを指定します。
...
ヒストグラムのビンは、グループごとに色分けして積み重ねることができます。グループ化する列を指定するには、以下の変更を行います。詳しくは、[ggplot の基礎](ggplot_basics.jp.qmd) の章を参照してください。
...
-   色やパレットに関する詳しい情報は、[ggplot の基礎](ggplot_basics.jp.qmd) の章を参照してください。
...
グループ化されたヒストグラムの積み重ねの順番は、グループ化された列を因子型にし、各因子のレベルの順番（とその表示ラベル）を指定することで調整します。詳しくは、[因子（ファクタ）型データ](factors.jp.qmd) の章、または、[ggplot の基礎](ggplot_basics.jp.qmd) の章をご覧ください。

[因子（ファクタ）型データ](factors.jp.qmd) の章で説明されているように、グラフを作成する前に **forcats** パッケージの `fct_relevel()` 関数を使用し、グループ化したい列を因子型に変換してレベル順をマニュアルで調整してください。
...
凡例やスケールについては、[ggplot のヒント](ggplot_tips.jp.qmd) の章で詳しく説明していますが、以下にその要約を記載します。
...
3 つ以上のグループがある場合、読みにくくなることがあるため、代わりにファセット化された可視化（グループごとにヒストグラムを表示する）を行った方が良いかも知れません。以下の例では、見やすくするために性別の欠測値を削除しています。
...
-   **ヒストグラムのビン分割が日、月、月曜始まりの週、月、年である場合：**
    ...
    -   日付のラベルの書式を指定するには `date_labels =` を使用します。[日付型データ](dates.jp.qmd) の章では、役立つヒントを紹介しています（改行には `\n` を使用など）
...
-   `seq.Date()` を使用して日付のベクトルを作成する方法については、本章冒頭の「ggplot2 を用いた流行曲線」のセクションを参照して下さい。

-   日付ラベルを作成するためのヒントについては、[こちら](https://rdrr.io/r/base/strptime.html) のウェブサイト、または、本ハンドブックの [日付型データ](dates.jp.qmd) の章を参照してください。
...
ラインリストではなく、施設や地域などの集計データのみがあることが多くあります。集計データのみでも `ggplot()` で流行曲線を作成することができますが、コードが少し異なります。このセクションでは、先述のデータ準備のセクションでインポートした `count_data` データセットを使用します。このデータセットは、`linelist` を日別・病院別の症例数に集計したデータです。最初の 50 行を以下に表示します。
...
詳細な説明については、[移動平均](moving_average.jp.qmd) の章を参照して下さい。以下は、**slider** パッケージで移動平均を計算する方法の 1 つです。この方法では、移動平均はプロットする前にデータセット上で計算されます。
...
他の ggplot と同様に、ファセット化されたグラフ（「複数の小さなグラフを並べたグラフ」）を作成することができます。[ggplot の基礎](ggplot_basics.jp.qmd) の章で説明されているように、 `facet_wrap()` と `facet_grid()` のどちらかを使用することができますが、ここでは `facet_wrap()` を使って説明します。流行曲線の作成においては、1 つの列に対してのみファセットするだけでよいので、一般的に `facet_wrap()` の方が簡単です。
...
各ファセットの背景に全体の流行曲線を表示するには、**ggplot** で作成した図に **gghighlight** パッケージの `gghighlight()` を追加します。すべてのファセットにおける y 軸の最大値が、全体の流行曲線の頂点になっていることに注意して下さい。**gghighlight** パッケージの例は、[ggplot の基礎](ggplot_basics.jp.qmd) の章で多く紹介されています。
...
-   必要であれば：縦に積み重ねたファセットを実現するために、ファセット列を式の行側に移動し、右側を "." (`facet_wrap(facet~.)`) に置き換え、`ncol = 1`とします。保存される png ファイルの幅と高さの調整が必要になります（[ggplot の基礎](ggplot_basics.jp.qmd) の章で紹介している `ggsave()` を参照してください）。
...
上記のファセット化の方法は、stackoverflow.com の [こちら](https://stackoverflow.com/questions/44616530/axis-labels-on-two-lines-with-nested-x-variables-year-below-months) と [こちら](https://stackoverflow.com/questions/20571306/multi-row-x-axis-labels-in-ggplot-line-chart) の投稿を参考に作成されました。
...
データ可視化のコミュニティー内では、2 軸グラフの有効性について激しい議論が交わされていますが、流行曲線やそれに類するグラフで、2 軸目に割合を重ねたものを見たいと思う疫学専門家は依然として多くいます。これについては [ggplot のヒント](ggplot_tips.jp.qmd) の章で詳しく説明していますが、**cowplot** メソッドを使った一例を以下に示します。
...
また、上記や [ggplot の基礎](ggplot_basics.jp.qmd) の章で説明されている **cowplot** を使って、2 軸で流行曲線に重ねることもできます。
