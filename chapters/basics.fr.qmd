...
Voir la page sur la [transition a R](transition_to_r.fr.qmd) pour des conseils sur le passage de STATA, SAS ou Excel à R.
...
Pour savoir quelles versions de R, RStudio ou des paquets ont été utilisées lors de la réalisation de ce manuel, consultez la page sur [Notes techniques et choix éditoriaux](editorial_style.fr.qmd).
...
Ce volet, situé par défaut en haut à droite, est le plus souvent utilisé pour afficher de brefs résumés des [objets](#objects) de l'environnement R dans la session en cours. Ces objets peuvent inclure des ensembles de données importés, modifiés ou créés, des paramètres que vous avez définis (par exemple, une semaine épi spécifique pour l'analyse), ou des vecteurs ou des listes que vous avez définis pendant l'analyse (par exemple, les noms des régions). Vous pouvez cliquer sur la flèche à côté du nom d'un cadre de données pour voir ses variables.

Dans Stata, cette fenêtre est très similaire à celle du gestionnaire de variables.

Ce volet contient également l'onglet "Historique" où vous pouvez voir les commandes que vous avez exécutées précédemment. Il comporte également un onglet "Tutoriel" où vous pouvez suivre des tutoriels R interactifs si vous avez installé le paquet **learnr**. En outre, il existe un volet "Connexions" pour les connexions aux bases de données externes. Il peut également y avoir un volet "Git" si vous choisissez de vous interfacer avec Github.
...
-   Spécifier les deux couleurs à utiliser (`pal =` est l'abréviation de "palette" et est fourni avec un vecteur de deux noms de couleurs. Voir la page [objets](basics.fr.qmd#objects) pour savoir comment la fonction `c()` fabrique un vecteur).
...
L'écriture d'une fonction est traitée en détail à la page [Écriture de fonctions](writing_functions.fr.qmd).
...
Vous pouvez imprimer le chemin d'accès à votre bibliothèque en entrant`.libPaths()` (parenthèses vides). Ceci devient particulièrement important si vous travaillez avec [R sur des lecteurs réseau](network_drives.fr.qmd).
...
Voir la page [Paquets suggérés](packages_suggested.fr.qmd) pour une liste de paquets que nous recommandons pour l'épidémiologie quotidienne.
...
Il convient de noter que les utilisateurs débutants de R peuvent également utiliser R Markdown - ne vous laissez pas intimider !Pour en savoir plus, consultez la page du manuel consacrée aux [rapports avec R Markdown](rmarkdown.fr.qmd).
...
**Voir la page sur** [projets R](r_projects.fr.qmd) pour plus de détails sur notre approche recommandée pour gérer votre répertoire de travail.

Un moyen commun, efficace et sans problème de gérer votre répertoire de travail et vos chemins de fichier consiste à combiner ces trois éléments dans un flux de travail du [projets R](r_projects.fr.qmd) orienté comme expliqué ci-dessous:

1.  Un projet R pour stocker tous vos fichiers (voir page sur [projets R](r_projects.fr.qmd))
2.  Le paquet **here** pour localiser les fichiers (voir page sur [importer et exporter](importing.fr.qmd))
3.  Le paquet **rio** pour importer ou exporter des fichiers (voir page sur [importer et exporter](importing.fr.qmd))

<!-- ======================================================= -->

### Définir le répertoire de travail par commande {.unnumbered}

Jusqu'à récemment, de nombreuses personnes apprenant R ont appris à commencer leurs scripts avec une commande `setwd()`. Veuillez plutôt envisager d'utiliser un flux de travail orienté par [projets R](r_projects.fr.qmd) et lire les [raisons de ne pas utiliser `setwd()`](https://www.tidyverse.org/blog/2017/12/workflow-vs-script/).
...
Dans un script R Markdown, le répertoire de travail par défaut est le dossier ou le fichier RMarkdown (`.Rmd`) est enregistré. Si vous utilisez un projet R et le paquet **here**, cela ne s'applique pas et le répertoire de travail sera `here()`, comme expliqué dans la page [projets R](r_projects.fr.qmd).
...
La commande suivante redéfinira la valeur de `current_week`:
...
Vous pouvez en savoir plus sur l'importation et l'exportation d'ensembles de données dans la section sur [importer et exporter](importing.fr.qmd).
...
Le graphique ci-dessous est emprunté à [ce tutoriel R en ligne](http://venus.ifca.unican.es/Rintro/dataStruct.html). Il montre certaines structures de données courantes et leurs noms. Les données spatiales ne sont pas incluses dans cette image, qui sont abordées dans la page [bases de GIS](gis.fr.qmd).
...
|                  | [dates](dates.fr.qmd) pour plus de      |
|                  | détails                                   |
+------------------+-------------------------------------------+
...
De même, il existe des fonctions **base** R pour vérifier si un objet EST d'une classe spécifique, comme `is.numeric()`, `is.character()`, `is.double()`, `is.factor()`, `is.integer()`
...
-   `hospitals`, un vecteur de caractères
-   `addresses`, une trame de données d'adresses
...
Si [Fonctions d'écriture](writing_functions.fr.qmd), ou si vous utilisez R de
...
Il s'agit d'un "tuyau d'affectation" du paquet **magrittr**, qui dirige un objet vers l'avant et *redéfinit également l'objet*. Il doit être le premier opérateur pipe de la chaîne. Il s'agit d'un raccourci, comme le montrent les deux exemples équivalents ci-dessous :
...
Ceci est utilisé pour ajouter des données aux arbres phylogénétiques avec le package **ggtree**. Voir la page sur les [arbres phylogénétiques](phylogenetic_trees.fr.qmd) ou ce [livre de ressources en ligne](https://yulab-smu.top/treedata-book/).
...
`rdt_result`, un résultat d'un test rapide, et `other_cases_in_home`,
...
|                                                    | "case_def"     |
+====================================================+================+
| Si la valeur des variables `rdt_result` et         | `NA`           |
| `other_cases_in_home` est manquante                | (manquante)    |
+----------------------------------------------------+----------------+
| Si la valeur dans `rdt_result` est "Positive"      | "Confirmé"     |
+----------------------------------------------------+----------------+
| Si la valeur dans `rdt_result` n'est pas           | "Probable"     |
| "Positive" **ET** la valeur dans                   |                |
|  `other_cases_in_home` est "Oui"                   |                |
+----------------------------------------------------+----------------+
...
dans la page [importer et exporter](importing.fr.qmd).
...
impossibles dans la page sur [les valeur manquantes](missing_data.fr.qmd).
Découvrez comment convertir les valeurs manquantes lors de l'importation
de données dans la page sur [importer et exporter](importing.fr.qmd).
...
Remarque: Si vous souhaitez détecter une chaîne partielle, en utilisant peut-être `str_detect()` de **stringr**, il n'acceptera pas un vecteur de caractères tel que `c("1", "Yes", "yes", "y")`. Au lieu de cela, il doit recevoir une *expression régulière* - une chaîne condensée avec des barres OU, telle que "1\|Yes\|yes\|y". Par exemple, `str_detect(hospitalized, "1|Yes|yes|y")`. Voir la page sur [les caractères et les chaîne de caractères](characters_strings.fr.qmd) pour plus d'informations.
...
Les erreurs et avertissements courants ainsi que des conseils de dépannage sont disponibles sur la page [erreurs frequentes](errors.fr.qmd).
