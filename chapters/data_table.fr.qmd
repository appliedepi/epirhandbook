...
Nous importons le jeu de données des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_excel")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails). A partir de là, nous utilisons `data.table()` pour convertir le cadre de données en un tableau de données.
...
Le premier exemple récupère les 5 premières lignes de la table de données, le deuxième exemple sélectionne le sous-ensemble des cas âgés de 18 ans ou plus, et le troisième exemple sélectionne le sous-ensemble des cas âgés de 18 ans ou plus mais non diagnostiqués à l'hôpital central :
