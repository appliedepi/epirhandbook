...
Si después de mucho buscar no encuentras una respuesta a tu problema, considera la posibilidad de crear un ejemplo *reproducible* ("reprex") y publicar tú mismo la pregunta. Consulta la página sobre [Cómo obtener ayuda](help.es.qmd) para obtener consejos sobre cómo crear y publicar un ejemplo reproducible en los foros.
...
Si el mensaje de error contiene algo como `Error in options[[sprintf("fig.%s", i)]]`, comprueba que tus opciones knitr en la parte superior de cada chunk utilizan correctamente ` out.width = ` o `out.height = `y *no* `fig.width= ` y `fig.height= `.
