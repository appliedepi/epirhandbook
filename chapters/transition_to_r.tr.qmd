...
**STATA**                        | **R**  
-------------------------------- | ---------------------------------------------
Çalışma dizinleri mutlak dosya yollarını içerir (örneğin, "C:/username/documents/projects/data/"). | Çalışma dizinleri ya mutlak olabilir ya da **here** paketi kullanılarak proje ana klasörüne göreli olabilir (bkz. [İçe aktarma ve dışa aktarma](importing.tr.qmd)).
...
Faktör değişkenleri, **label define** gibi bir dizi komut kullanılarak etiketlenebilir. | Değerleri etiketleme, sütunu Faktör sınıfına dönüştürerek ve seviyeler belirleyerek yapılabilir.([Faktörler](factors.tr.qmd) ile ilgili sayfaya bakınız.) Sütun adları genellikle Stata'da olduğu gibi etiketlenmez.
...
-------------------------------- | ---------------------------------------------
Veri kümesi sütunlarına genellikle "değişkenler" denir. | Daha sıklıkla "sütunlar" veya bazen "vektörler" veya "değişkenler" olarak anılır.
Bir değişken oluşturmak için özel bir prosedüre gerek yoktur. Yeni değişkenler, yalnızca yeni değişken adı, ardından eşittir işareti ve ardından değer için bir ifade yazılarak oluşturulur. | `mutate()` fonksiyonu kullanılarak yeni değişkenler oluşturabilir. Bütün **dplyr** fonksiyonlarıyla ilgili ayrıntılar için [Verileri temizleme ve temel fonksiyonlar](cleaning.tr.qmd) sayfasına bakınız.
...
R **rio** paketinin, STATA .dta dosyaları, SAS .xpt ve.sas7bdat dosyaları, SPSS .por ve.sav dosyaları gibi dosyaları gibi pek çok dosyanın nasıl içe ve dışa aktarabileceğiyle ilgili ayrıntılar için [İçe ve dışa aktar](importing.tr.qmd) sayfasına bakınız.
