...
Importamos los datos de casos de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_excel")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles). A partir de aquí utilizamos `data.table()` para convertir el dataframe en una data.table.
