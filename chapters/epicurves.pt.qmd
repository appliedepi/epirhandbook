...
O pacote **incidence2** oferece uma abordagem alternativa, com comandos mais simples, mas, no momento em que este texto foi escrito, estava passando por revisões e não estava estável. Ele será novamente incluído neste capítulo quando estiver estável.
...
Os conjuntos de dados são importados utilizando a função `import()` do pacote **rio**. Veja a página sobre [Importar e  exportar](importing.pt.qmd)  para conhecer as diferentes formas de importar dados.
...
Você precisa controlar *manualmente* a agregação dos casos pelo tempo (em semanas, meses, etc) *e* os intervalos dos rótulos no eixo de data. Isto precisa ser cuidadosamente gerenciado.
...
* Utilizamos `closed = "left"` em `geom_histogram()` para garantir que as datas sejam contadas nos containers corretos
...
ggplot(data = central_data) + 
  ...
  geom_histogram(
    ...
    closed = "left",  # conta os casos a partir do início do ponto de quebra
...
As barras do histograma podem ser coloridas por grupo e podem ser "empilhadas" (o inglês *stacked*). Para designar a coluna de agrupamento, faça as seguintes modificações. Veja a página sobre [básico do ggplot](ggplot_basics.pt.qmd) para mais detalhes.
...
* Para mais dicas sobre escalas de cor e paletas, veja a página sobre [básico do ggplot](ggplot_basics.pt.qmd).  
...
A ordem em que as barras agrupadas estão empilhadas é melhor ajustada ao se classificar a coluna de agrupamento como sendo da classe fator (*Factor*). Você poderá, então, designar a ordem dos níveis desse fator (e os rótulos mostrados). Veja a página sobre [Fatores](factors.pt.qmd) ou [dicas do ggplot](ggplot_tips.pt.qmd) para mais detalhes.

Antes de criar o gráfico, utilize a função `fct_relevel()` do pacote **forcats** para converter a coluna de agrupamento para a classe factor, e manualmente ajustar a ordem dos níveis, como detalhado na página sobre [Fatores](factors.pt.qmd).
...
Leia mais sobre legendas e escalas no página sobre [dicas do ggplot](ggplot_tips.pt.qmd). Aqui estão alguns destaques:
...
* Veja [esta página](https://rdrr.io/r/base/strptime.html) ou a página [Trabalhando com datas](dates.pt.qmd) para dicas sobre como criar rótulos de data.
...
Veja a página sobre [médias móveis](moving_average.pt.qmd) para uma descrição detalhada e diferentes opções. Abaixo, uma alternativa para calcular as médias móveis com o pacote **slider** é utilizada. Nesta abordagem, *a média móvel é calculada no conjunto de dados antes de criar os gráficos*:  

1) Agregue os dados em contagens conforme necessário (diárias, semanais, etc.) (veja a página [Agrupando dados](grouping.pt.qmd))  
...
Como em outros ggplots, você pode criar gráficos facetados ("múltiplos pequenos"). Como explicado na página [dicas do ggplot](ggplot_tips.pt.qmd) deste manual, você pode utilizar tanto `facet_wrap()` quanto `facet_grid()`. Aqui, nós demonstramos com o `facet_wrap()`. Para epicurvas, `facet_wrap()` é tipicamente mais fácil, uma vez que provavelmente você só precisa facetar uma coluna.
...
Para mostrar o total da epidemia no fundo de cada faceta, adicione a função `gghighlight()` com parênteses vazios ao ggplot. Isto é do pacote **gghighlight**. Observe que o máximo do eixo y em todas as facetas é agora baseado no pico da epidemia inteira. Existem mais exemplos deste pacote na página sobre [dicas do ggplot](ggplot_tips.pt.qmd).  
...
* Opcional: para obter facetas empilhadas verticalmente: a coluna facetada é movida para o lado das linhas na equação e no lado direito é substituído por "." (`facet_wrap(facet~.)`), e `ncol = 1`. Você também pode precisar ajustar o comprimento e altura da imagem do gráfico salvo (veja `ggsave()` no [dicas do ggplot](ggplot_tips.pt.qmd)).  
...
Lembre - você pode utilizar ferramentas como `\n` *dentro* dos argumentos de `date_labels` ou `labels` para colocar partes de cada etiqueta em uma nova linha abaixo da atual. Entretanto, o código abaixo o ajuda a colocar anos ou meses (por exemplo) em uma linha abaixo *e apenas uma vez*.

O método mais fácil é atribuir o argumento `labels = `, de `scale_x_date()`, à função `label_date_short()`, do pacote **scales** (atenção: não se esqueça de incluir os parênteses vazios ()). Essa função constrói automaticamente etiquetas de data eficientes (leia mais [aqui](https://scales.r-lib.org/reference/label_date.html)). Um benefício adicional dessa função é que as etiquetas se ajustam automaticamente à medida que seus dados se estendem no tempo: de dias, para semanas, para meses e anos.
...
A segunda opção é utilizar faceteamento. Algumas notas sobre o código abaixo:
...
Embora existam discussões intensas sobre a validade de eixos duplos na comunidade de visualização de dados, muitos supervisores de epidemiologia ainda querem ver uma epicurva ou gráfico similar com um percentual sobreposto com um segundo eixo. Isto é discutido mais extensivamente na página [dicas do ggplot](ggplot_tips.pt.qmd), mas um exemplo utilizando o método de **cowplot** é mostrado abaixo:
...
Ela também pode ser sobreposta em uma epicurva, com dois eixos, utilizando o método **cowplot** descrito acima e na página [dicas do ggplot](ggplot_tips.pt.qmd):
