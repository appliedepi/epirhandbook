---
aliases:
  - /new_pages/ggplot_tips.pt.html
---

# Dicas do ggplot {#ggplot-tips}

Nesta página, iremos abordar dicas e truques para criar gráficos inteligentes e bonitos com o ggplot. Para aprender os fundamentos do ggplot, veja a página sobre [básico do ggplot](ggplot_basics.pt.qmd).
...
O código abaixo realiza o carregamento dos pacotes necessários para a análise dos dados. Neste manual, enfatizamos o uso da função `p_load()`, do **pacman**, que instala os pacotes, caso não estejam instalados, *e* os carrega no R para utilização. Também é possível carregar os pacotes instalados utilizando a função `library()`, do R **base**. Para mais informações sobre  pacotes do R, veja a página [Introdução ao R](basics.pt.qmd).  
...
Nesta página, nós importamos um conjunto de dados de casos de uma simulação de epidemia de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.pt.qmd) para mais detalhes).    
...
Uma coisa que pode ser inicialmente difícil de entender com o **ggplot2**, é o controle dos esquemas de cores. Observe que esta seção discute a cor dos *objetos no gráfico* (geometrias/formatos), como pontos, barras, linhas, bases, etc. Para ajustar a cor de textos adicionais, títulos, ou plano de fundo, veja a seção [Temas](ggplot_basics.pt.qmd#ggplot_basics_themes) da página sobre [Básico do ggplot](ggplot_basics.pt.qmd).
...
3) A terceira parte, o METHOD, será ou `_discrete()`, `continuous()`, `_date()`, `_gradient()`, ou `_manual()`, de acordo com a classe da coluna e *como* você quer controlar ela. Existem outros, mas estes são frequentemente os mais utilizados.
...
As nuances de escalas de datas são abordados mais extensivamente na página [Curvas epidêmicas](epicurves.pt.qmd).
...
Para demonstrar como ajustar uma escala contínua de cor, nós iremos utilizar um conjunto de dados da página [Rastreamento de contatos](contact_tracing.pt.qmd), que contém as idades dos casos e dos seus casos-fonte.
...
Abaixo, produzimos um gráfico "raster" de densidade em blocos de calor. Não iremos discorrer sobre como criar o gráfico (veja o link no parágrafo acima), mas sim como ajustar a escala de cor. Veja mais sobre a função `stat_density2d()` do **ggplot2** [aqui](https://ggplot2.tidyverse.org/reference/geom_density_2d.html). Observe como a escala `fill` é *contínua*.  
...
Aqui é um exemplo da página [Básico do ggplot](ggplot_basics.pt.qmd), utilizando diferentes esquemas de cores.
...
Uma solução fácil para obter rótulos de data eficientes no eixo x é atribuir o argumento `labels = `, de `scale_x_date()`, à função `label_date_short()`, do pacote **scales**. Essa função constrói automaticamente rótulos de data eficientes (leia mais [aqui](https://scales.r-lib.org/reference/label_date.html)). Um benefício adicional dessa função é que os rótulos se ajustam automaticamente à medida que seus dados se estendem no tempo, de dias, para semanas, para meses e anos.

Veja um exemplo completo na seção sobre etiquetas multiníveis de data, na página [Curvas epidêmicas](epicurves.pt.qmd), mas um exemplo rápido é mostrado abaixo como referência:
...
Enquanto facetear (descrito na página [Básico do ggplot](ggplot_basics.pt.qmd)) é uma abordagem conveniente para criar gráficos, as vezes não é possível obter os resultados desejados com essa abordagem. Aqui, você pode escolher combinar os gráficos ao uni-los em um gráfico maior. Existem três pacotes bem conhecidos, que são ótimos para isso - **cowplot**, **gridExtra**, e **patchwork**. Entretanto, estes pacotes fazem basicamente a mesma coisa, de forma que iremos focar no **cowplot** nesta seção.
...
Note que existe outro exemplo dessa técnica na página [Curvas epidêmicas](epicurves.pt.qmd) - a sobreposição da incidência acumulada acima da epicurva.
