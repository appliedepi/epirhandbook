---
aliases:
  - /new_pages/grouping.es.html
---

# Agrupar datos {#grouping}
...
Véase la sección **dplyr** de la página [Tablas descriptivas](tables_descriptive.es.qmd) para una descripción detallada de cómo producir tablas de resumen con `summarise()`. Aquí abordamos brevemente cómo cambia su comportamiento cuando se aplica a datos agrupados. 
...
Cuando se aplica junto con funciones que evalúan el dataframe (como `max()`, `min()`, `mean()`), estas funciones se aplicarán ahora a los grupos. Por ejemplo, si deseas filtrar y mantener las filas en las que los pacientes están por encima de la mediana de edad, esto se aplicará ahora por grupo, filtrando para mantener las filas por encima de la mediana de edad del *grupo*. 
...
Consulta la página de [De-duplicación](deduplication.es.qmd) para ver más ejemplos y detalles sobre `slice()`. 
