...
*Cette page explique comment* créer les tableaux de base, tandis que la page [Tableaux pour la présentation](tables_presentation.fr.qmd) explique comment les mettre en forme et les imprimer.
...
Nous importons le jeu de données des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails).
...
Vous pouvez renvoyer des statistiques individuelles avec des fonctions **extension/package R base** comme `max()`, `min()`, `median()`, `mean()`, `quantile()`, `sd()`, et `range()`. Consultez la page [Bases de R](basics.fr.qmd) pour obtenir une liste complète.
...
Si la colonne est un facteur de classe et que seuls certains niveaux sont présents dans vos données, tous les niveaux apparaîtront quand même dans le tableau. Vous pouvez supprimer cette fonctionnalité en spécifiant `show_missing_levels = FALSE`. Pour en savoir plus, consultez la page [Facteurs](factors.fr.qmd).
...
Consultez la page sur les [Tests statistiques simples](stat_tests.fr.qmd) pour obtenir plus de code et de conseils sur les statistiques.
...
Les résultats de la mise en tableau de deux colonnes de regroupement ou plus sont toujours renvoyés au format "long", avec les effectifs dans la colonne `n`. Consultez la page [Pivoter les données](pivoting.fr.qmd) pour en savoir plus sur les formats de données "long" et "large".
...
Consultez la page sur les [Facteurs](factors.fr.qmd) pour plus d'informations.
...
Afficher un tableau "long" comme celui ci-dessus avec `ggplot()` est relativement simple. Les données sont naturellement au format "long", qui est naturellement accepté par `ggplot()`. Voir d'autres exemples dans les pages [ggplot basics](ggplot_basics.fr.qmd) et [Astuces de ggplot](ggplot_tips.fr.qmd).
...
Dans la page [Caractères et chaînes de caractères](characters_strings.fr.qmd), diverses options pour combiner des colonnes sont discutées, notamment `unite()`, et `paste0()`. Dans ce cas d'utilisation, nous préconisons `str_glue()` parce qu'il est plus flexible que `unite()` et a une syntaxe plus simple que `paste0()`.
...
Maintenant, vous pouvez passer à **flextable** et imprimer le tableau dans Word, .png, .jpeg, .html, Powerpoint, RMarkdown, etc. ! (voir la page [Tableaux pour la présentation](tables_presentation.fr.qmd)).
...
Si vous préférez votre tableau en format "large", vous pouvez le transformer en utilisant la fonction **tidyr** `pivot_wider()`. Vous devrez probablement renommer les colonnes avec `rename()`. Pour plus d'informations, consultez la page sur [le pivotement des données](pivoting.fr.qmd).
...
Et vous pouvez ensuite afficher ce tableau sous la forme d'une image. Vous trouverez ci-dessous le résultat imprimé avec **flextable**. Vous pouvez lire plus en détail cet exemple et la façon d'obtenir ce "joli" tableau sur la page [Tableaux pour la présentation](tables_presentation.fr.qmd).
...
Vous pouvez également ajouter les résultats des tests statistiques aux tableaux **gtsummary**. Ce processus est décrit dans la section **gtsummary** de la page [Tests statistiques simples](stat_tests.fr.qmd).
...
-   Le côté droit de l'équation utilise la syntaxe de `str_glue()` de **stringr** (voir [Caractères et chaînes de caractères](characters_strings.fr.qmd)), avec la chaîne d'affichage souhaitée entre guillemets et les statistiques elles-mêmes entre accolades. Vous pouvez inclure des statistiques comme "n" (pour les comptes), "N" (pour le dénominateur), "mean", "median", "sd", "max", "min", les percentiles comme "p##" comme "p25", ou le pourcentage du total comme "p". Voir `?tbl_summary` pour plus de détails.
...
5)  Affichez, visualisez ou exportez comme vous le souhaitez. Dans cet exemple, nous utilisons `flextable()` du package **flextable** comme décrit dans la page [Tableaux pour la présentation](tables_presentation.fr.qmd). Cela permettra d'afficher dans le volet de visualisation de RStudio une jolie image HTML.
