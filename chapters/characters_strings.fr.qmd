---
aliases:
  - /new_pages/characters_strings.fr.html
---
...
L'argument `collapse = ` est pertinent si vous mettez plusieurs *vecteurs* comme arguments à `str_c()`. L'argument `collapse = ` est utilisé pour séparer les éléments inclus dans le vecteur produit par `str_c()`, de sorte que le vecteur produit ne contient qu'un seul long élément de type caractère.
...
Il est parfois utile d'extraire des données d'un dataframe et de les coller ensemble en séquence.Vous trouverez ci-dessous un exemple de dataframe. Nous allons l'utiliser pour faire une description résumée du nombre de nouveaux cas et du nombre total de cas par juridiction. 
...
Si vous essayez "d'enrouler" des valeurs dans une colonne d'un dataframe, c'est-à-dire de combiner des valeurs de plusieurs lignes en une seule ligne en les collant ensemble avec un séparateur. Consultez la section de la page [Deduplication](deduplication.fr.qmd) sur les valeurs ["enroulées"](deduplication.fr.qmd#str_rollup).    
...
En supposant que les données sont passées ('pipe', en utilisant %>%) dans `separate()`, vous devez d'abord fournir la colonne à séparer dans la fonction `separate()`. Ensuite, fournissez `into = ` comme un vecteur `c( )` contenant les noms des *nouvelles* colonnes, comme indiqué ci-dessous.  
...
Voici un exemple avec `extra = "merge"` - aucune donnée n'est perdue. Deux nouvelles colonnes sont définies mais tous les troisiémes symptômes ou plus sont combinés dans la deuxiéme colonne:  
...
La fonction `cat()` de **base** R peut être enroulée autour de la commande ci-dessus afin d'imprimer le résultat dans la console R, en affichant les nouvelles lignes ajoutées.
...
Si vous avez besoin de construire une longue liste de termes de recherche, vous pouvez les combiner en utilisant `str_c()` et `sep = |`, et assigner ceci à un objet. Vous pouvez ensuite référencer le vecteur par le nom de l'objet. L'exemple ci-dessous combine en un seul objet les termes de recherche d'occupations possibles pour les prestataires de soins de première ligne.  
