...
上記のプロット例では、5 つの症状が示されており、それぞれの縦棒の下には、反映されている症状の組み合わせを示す線と点があります。左下の横棒は、各症状の頻度を示しています。
...
以下のコードで使用されている **stringr** パッケージの `unite()` 関数についての詳細は、[文字型データ](characters_strings.jp.qmd)をご覧ください。
...
より効率的に処理する関数に興味がある場合は、論理構文に基づいて値を 1 と 0 に置き換える関数 `+()` を利用できます。この関数は、`across()` を利用し、複数の列を一度に置き換えます。（詳しくは[データクリーニングと主要関数](cleaning.jp.qmd)を参照してください）。
