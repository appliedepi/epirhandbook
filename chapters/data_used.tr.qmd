...
* "Shigella_tree.txt" filogenetik ağaç dosyasını indirmek için  ```{r, eval=F}
tree <- appliedepidata::get_data(name = "Shigella_tree")
``` sağ fare tuşuyla tıklayın (Mac için Cmd + tıkla) ve "Farklı Kaydet" seçeneğini seçin.  
* Her örnekle ilgili ek bilgiler içeren "sample_data_Shigella_tree.csv" dosyasını indirmek için  ```{r, eval=F}
sample_data <- appliedepidata::get_data(name = "sample_data_Shigella_tree")
``` sağ fare tuşuyla tıklayın (Mac için Cmd + tıkla) ve "Farklı Kaydet" seçeneğini seçin.  
* Yeni oluşturulan altküme ağacını görmek için  ```{r, eval=F}
