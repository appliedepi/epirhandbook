---
aliases:
  - /new_pages/moving_average.jp.html
---
...
全ての日付に値がある時系列のデータがある場合、[Time series and outbreak detection](time_series.jp.qmd) の章で紹介されているように、`slide_dbl()` を使用してもよいでしょう。
...
データのグループ化に関する詳細は、[データのグループ化](grouping.jp.qmd) の章を参照してください。
...
[***警告：*** *"slide() was deprecated in tsibble 0.9.0 and is now defunct. Please use slider::slide() instead."* というエラーが出た場合、**tsibble** パッケージの `slide()` が **slider** パッケージの `slide()` をマスクしていることを意味します。 `slider::slide_dbl()` のように、コマンドパッケージを指定して修正してください。]{style="color: red;"}
