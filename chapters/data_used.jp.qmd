...
最も簡単にすべてのデータをダウンロードする方法は、前述した **appliedepidata** パッケージをインストールすることです。このパッケージには、すべてのサンプルデータをパソコンの任意のフォルダに保存する関数 `get_data()` が含まれています。

**appliedepidata** パッケージをインストールするには、次のコードを実行してください。このパッケージは CRAN にないため、インストールする際には `p_install_gh()` という関数を使用し、このハンドブックの Github Organization（"<u>appliedepi</u>"）と **appliedepidata** パッケージを指定してください。
...
次に、**appliedepidata** パッケージに含まれている関数 `get_data()` を使って、サンプルデータをパソコンにダウンロードします。`appliedepidata::save_data()` を実行すると<u>すべての</u>サンプルデータがダウンロードできます。特定のファイルのみをダウンロードしたい場合は、引用符の中でファイル名と拡張子を指定してください。
...
また、ご希望の場合は、**appliedepidata** パッケージの Github リポジトリの[「data」フォルダ](https://github.com/appliedepi/appliedepidata)で元データファイルを確認することができます。
...
先述したオプション 1 の方法に従って **appliedepidata** パッケージを利用してすべてのサンプルデータをダウンロードする場合、すべてのシェイプファイルが含まれます。
