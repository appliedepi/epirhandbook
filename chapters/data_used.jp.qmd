...
-   系統樹ファイル「Shigella_tree.txt」をダウンロードするには、この ```{r, eval=F}
tree <- appliedepidata::get_data(name = "Shigella_tree")
``` を右クリック（Mac の場合は Cmd を押しながらクリック）し、「リンク先を別名で保存（Save link as）」を選択してください。
-   各サンプルの追加情報を含む「sample_data_Shigella_tree.csv」をダウンロードするには、この```{r, eval=F}
sample_data <- appliedepidata::get_data(name = "sample_data_Shigella_tree")
```を右クリック（Mac の場合は Cmd を押しながらクリック）し、「リンク先を別名で保存（Save link as）」を選択してください。
-   新しく作成されたサブセット・ツリーを見るには、この```{r, eval=F}
subtree <- appliedepidata::get_data(name = "Shigella_subtree_2")
```を右クリック（Mac の場合は Cmd を押しながらクリック）し、「リンク先を別名で保存（Save link as）」を選択してください。.txt ファイルがあなたのパソコンにダウンロードされます。
