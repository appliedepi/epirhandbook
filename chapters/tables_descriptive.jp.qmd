...
**rstatix** パッケージの `get_summary_stats()` を使用すると <u>データフレーム形式</u> で要約統計量を取得できます。これは、それ以降の演算を行ったり、数値をプロットするのに役立ちます。**rstatix** パッケージとその関数の詳細については、 [簡単な統計的検定](#stat-tests) 章を参照してください。
