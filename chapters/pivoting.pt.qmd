...
**Nessa página, iremos nos focar na segunda definição**. A primeira é um passo crucial em análise de dados que está coberto em outras partes do livro, nos capítulos de [Agrupamento de Dados](grouping.pt.qmd) e [Tabelas Descritivas](tables_descriptive.pt.qmd).
...
Nesta página, iremos utilizar um banco fictício de casos diários de malária, divididos por local e grupos de idade. Carregue diretamente com `appliedepidata::get_data(name = "malaria_facility_count_data")` (consulte a página [Baixar manual e dados](data_used.pt.qmd) para mais detalhes).    
...
Nas seções finais dessa página também iremos utilizar dados de uma epidemia simulada de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.pt.qmd) para mais detalhes).    
```{r, echo=F}
...
Note que a coluna `date` está atualmente com a classe *caractere* - nós podemos convertê-la facilmente em sua classe apropriada utilizando as funções `mutate()` e `as_date()` descritas na página [Trabalhando com datas](dates.pt.qmd).  

Também podemos converter a coluna `observation` para o formato `numeric` removendo o prefixo "obs" e convertendo para numérico. Podemos fazer isso com a função `str_remove_all()` do pacote **stringr** (veja a página [Caracteres and strings](characters_strings.pt.qmd)).  
...
Um caso de uso típico é quando queremos transformar o resultado de uma análise em um formato mais "palatável" ao leitor (tal como em [Tabelas para apresentação](tables_presentation.pt.qmd)). Normalmente, isso envolve transformar uma base em que a informação para um sujeito está espalhada em múltiplas linhas em um formato em que aquela informação esteja armazenada em apenas uma.

### Dados {.unnumbered}

Para essa seção da página, vamos utilizar o caso da linelist (veja a seção de [Preparação](pivoting.pt.qmd)), que contém uma linha por caso.  
...
Essa tabela é muito mais legível e assim, melhor para utilização em relatórios. Você pode convertê-la em tabelas elegantes e bonitas utilizando vários pacotes, incluindo **flextable** e **knitr**. Esse processo é elaborado na página [Tabelas para apresentação](tables_presentation.pt.qmd).  
