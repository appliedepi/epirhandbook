...
Bu sayfada, simüle edilmiş bir Ebola salgını vakalarının temizlenmiş "satır listesine" ara sıra atıfta bulunulacaktır. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.tr.qmd) sayfasına bakın).
...
Çıktı kaydedilirse, parantez sözdizimi ile n'inci değerine erişebilirsiniz. Belirli bir değere erişmek için `the_returned_object[[1]][2]` argümanı kullanılır. Bu fonksiyonla listedeki ("fever") ikinci değere erişir. Değerlere erişim hakkında daha fazla ayrıntı için [R temelleri](basics.tr.qmd) sayfasına bakabilirsiniz. 
