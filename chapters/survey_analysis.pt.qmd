...
[OpenEpi](https://www.openepi.com/Menu/OE_Menu.htm). A página [Introdução do GIS](gis.pt.qmd) (<https://epirhandbook.com/gis-basics.html>) do manual terá eventualmente uma seção sobre 
...
se necessário *e* o carrega para utilização. Você também pode carregar pacotes usando `library()` do **R Base**. Veja a página [Introdução ao R](basics.pt.qmd) para mais informações
sobre os pacotes R.
...
O conjunto de dados é importado utilizando a função `import()` do pacote **rio**. Veja a página [Importar e exportação](importing.pt.qmd) para outrasmaneiras de importar dados.
...
Abaixo, garantimos que a coluna de datas esteja no formato apropriado. Há várias outras maneiras de fazer isso (veja a página [Trabalhando com datas](dates.pt.qmd) para mais detalhes), porém a utilização do dicionário para definir datas é rápida e fácil.

Também criamos uma variável de faixa etária utilizando a função `age_categories()` do pacote **epikit** - veja a seção [limpeza de dados](cleaning.pt.qmd) para mais detalhes. Além disso, criamos uma variável do tipo caractere, que define em
...
exemplo, com `drop_na(uid)`) ou remover duplicidades (consulte a seção do manual sobre [Eliminando duplicidades](deduplication.pt.qmd) para maiores detalhes). Aqueles sem consentimento
...
capítulos [Tabelas Descritivas](tables_descriptive.pt.qmd), [Testes Estatísticos](stat_tests.pt.qmd),
[Tabelas para Apresentação](tables_presentation.pt.qmd),[Introdução ao ggplot](ggplot_basics.pt.qmd) e [Relatórios em R markdown](rmarkdown.pt.qmd).
...
de [Tabelas Descritivas](tables_descriptive.pt.qmd) de idade e sexo por estratos de pesquisa. Demonstraremos utilizando o pacote **apyramid**,
pois ele permite proporções ponderadas utilizando nosso objeto de pesquisa criado acima. Outras opções para criar [Pirâmides Demográficas](age_pyramid.pt.qmd) são amplamente cobertas nesse capítulo do manual. Também utilizaremos uma função *wrapper* da
...
substancialmente diferente da população original e se a ponderação corrige esta diferença. Para isso, usaremos o pacote **patchwork** para mostrar nossas visualizações **ggplot** lado a lado; para detalhes, veja a seção sobre combinação de lotes no capítulo [dicas ggplot](ggplot_tips.pt.qmd) do manual. Visualizaremos nossa população original, nossa população não ponderada de pesquisa e nossa população ponderada de pesquisa. Você
...
conhecidos por vários nomes diferentes, incluindo aluvial, sankey e conjuntos paralelos - os detalhes estão no capítulo sobre [Diagramas e Gráficos](diagrams.pt.qmd).
...
Existem três opções, utilizando funções dos seguintes pacotes: **survey**, **srvyr** e **gtsummary**. 
O código **survey** será provavelmente o mais estável a longo prazo, enquanto 
...
para repetir sobre várias variáveis e criar uma tabela. Veja o capítulo [Iterações e loops](iteration.pt.qmd) para mais detalhes do
pacote **purrr**.
...
Também aqui poderíamos escrever uma função para então reiterar sobre múltiplas variáveis usando o pacote **purrr**. Veja o capítulo do manual [Iterações e loops](iteration.pt.qmd) para detalhes sobre **purrr**.
