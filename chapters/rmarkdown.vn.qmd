...
Để tìm kiếm các tệp này ở chỗ khác, bạn sẽ cần sử dụng đường dẫn tệp đầy đủ hoặc sử dụng package **here**. Package **here** cài đặt thư mục làm việc tới thư mục gốc của R project và sẽ được giải thích chi tiết trong các chương [Dự án R](r_projects.vn.qmd) và [Nhập xuất dữ liệu](importing.vn.qmd) của sổ tay này. Ví dụ, để nhập một tệp tên "data.csv" từ trong thư mục `projectX`, code sẽ là `import(here(“data.csv”))`.  
