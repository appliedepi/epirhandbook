---
aliases:
  - /new_pages/collaboration.fr.html
---
...
*Remarque : comme les interfaces utilisent toutes Git en interne, vous pouvez en essayer plusieurs, passer de l'une à l'autre en fonction de vos besoins, utiliser ponctuellement la console pour une action que votre interface ne prend pas en charge, ou même effectuer un certain nombre d'actions en ligne sur Github.*
...
5.  Lorsque vous êtes satisfait de votre branche, vous pouvez fusionner la version en ligne de la branche de travail avec la branche "principale" en ligne afin de transférer les modifications.
...
*Note : jusqu'à récemment, le terme de branche "master" était utilisé, mais on parle maintenant de branche "principale" ("main").
...
Pour mettre en scène les changements, il suffit de cocher la petite case à côté des noms de fichiers. Lorsque vous avez sélectionné les fichiers que vous voulez ajouter à cette livraison, donnez un nom à la livraison, éventuellement une description, puis cliquez sur le bouton **commit**.
...
git add chapters/collaboration.Rmd # sélectionner les fichiers à livrer (= mettre en scène les changements)
...
**Note:** il est beaucoup plus facile de revenir sur des modifications qui ont été commises mais pas poussées (c'est-à-dire qu'elles sont toujours locales) que de revenir sur des changements qui ont été poussés vers le dépôt distant (et peut-être déjà extraites par quelqu'un d'autre), il est donc préférable de pousser une fois que vous avez terminé d'apporter les modifications liées à la tâche sur laquelle vous travailliez.
...
- **git config --global user.email " [you\@example.com](mailto:you@example.com){.email} "** (votre adresse électronique Github
    ), et
...
Vous pouvez soumettre une demande de modification (PR) directement à partir du site Web (comme illustré ci-dessous) ou à partir de Github Desktop.
...
Il suffit d'aller (_checkout_) dans la branche que vous voulez supprimer, puis de vous rendre dans le menu *Branche \> Supprimer*.
...
Si vous n'avez pas encore configuré le référentiel amont, vous devriez voir deux lignes, commençant par _origin_. Elles montrent le dépôt distant  vers lequel `fetch` et `push` pointent. Rappelez-vous, _origin_ est le surnom conventionnel pour votre propre version du dépôt sur Github. Par exemple :  
