---
aliases:
  - /new_pages/collaboration.pt.html
---
...
*Mudanças preparadas (staged)?* Preparar mudanças é adicioná-las à *área de preparação* (staging area), em preparação para o próximo commit. A ideia é que você possa decidir com precisão quais alterações incluir em um determinado commit. Por exemplo, se você trabalhou na especificação do modelo em um script e, posteriormente, trabalhou em uma figura em outro script, faria sentido ter dois commits diferentes (seria mais fácil no caso de você querer reverter as mudanças na figura, mas não o modelo).
...
A próxima etapa pode ser realizada em qualquer interface. Vamos ilustrar com o Rstudio e com o Github Desktop.
...
Cada uma de suas alterações será rastreada, *uma vez que o respectivo arquivo for salvo*. Os arquivos alterados irão aparecer na aba Git do RStudio, no Github Desktop, ou usando o comando `git status` no terminal (veja abaixo).
...
git add chapters/collaboration.Rmd  # selecione arquivos para o commit (= "stage" mudanças)
...
Repita esse ciclo enquanto estiver trabalhando no repositório.
...
Na imagem abaixo, o branch "forests" foi selecionado para ser mesclado no branch "main":
...
Basta fazer o checkout do branch que deseja excluir e ir até o menu *Branch\>Delete*.
