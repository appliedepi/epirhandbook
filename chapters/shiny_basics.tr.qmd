---
aliases:
  - /new_pages/shiny_basics.tr.html
---

# Shiny ile Gösterge Panelleri { }  

Gösterge paneli, genellikle analizlerden elde edilen sonuçları başkalarıyla paylaşmanın iyi bir yoludur. **shiny** ile bir gösterge paneli üretmek, nispeten ileri düzeyde R dili bilgisi gerektirir, ancak inanılmaz özelleştirme seçenekleri sunar. 
...
Bu sayfa, shiny ve uzantıları olan panellerin nasıl oluşturulacağına dair kısa bir genel bakış sağlayacaktır. Panelleri daha hızlı, daha kolay, ancak daha az özelleştirilebilir hale getirmenin alternatif bir yöntemi için, flextable [R Markdown ile gösterge panelleri](flexdashboard.tr.qmd) sayfasına bakabilirsiniz. 
...
Bir uygulama oluşturmaya başlamadan önce, ne oluşturmak istediğinizi bilmek son derece yararlıdır. Kullanıcı arayüzünüz kodla yazılacağından, belirli bir şeyi hedeflemediğiniz sürece ne inşa ettiğinizi gerçekten görselleştiremezsiniz. Bu nedenle, neler yapabileceğiniz hakkında bir fikir edinmek için önceden shiny uygulama örneklerine bakmak son derece yararlıdır - bu uygulamaların arkasındaki kaynak koduna bakabilirseniz daha da iyi olur! Bunun için bazı harika kaynaklar şunlardır:
...
• Çıktılar genellikle xxxOutput() olarak biçimlendirilir, ör. plotOutput()
...
• checkboxInput(), sunucuya mantık nesnesi iletir (TRUE veya FALSE)
...
```{r, echo = TRUE}
pacman::p_load("tidyverse", "lubridate", "appliedepidata")
...
Dikkat edilmesi gereken bir diğer nokta da, bölge girdisini değerlendirirken %in% operatörünün kullanılmasıdır. Yukarıda bahsedildiği gibi bu girdi birden çok değere sahip bir karakter vektörü olarak gelebilir ,bu nedenle %in% operatörünü kullanmak, == operatöründen daha esnektir.
...
2. Uygulamanızın dizininde _global.R_ adlı yeni bir dosya oluşturun ve başlangıç kodunu bu dosyaya yerleştirin.
...
pacman::p_load("tidyverse", "lubridate", "shiny", "appliedepidata")
...
Shiny'nin harika bir özelliği, app.R, server.R, ui.R ve global.R adlı dosyaların ne için olduğunu anlamasıdır, bu nedenle herhangi bir kod aracılığıyla bunları birbirine bağlamaya gerek yoktur. Bu yüzden sadece global.R'deki bu kodu dizinde bulundurmak uygulamamızı başlatmadan önce çalışması için yeterlidir!
...
Artık kodumuzun çoğunu yazdığımıza göre, sadece sunucumuzu geliştirmemiz gerekiyor. Bu, uygulamamızın son parçasıdır ve muhtemelen anlaşılması en zor olanıdır. Sunucu büyük bir R işlevidir, ancak onu bir dizi daha küçük işlev veya uygulamanın gerçekleştirebileceği görevler olarak düşünmek yararlıdır. Bu işlevlerin doğrusal bir sırada yürütülmediğini anlamak önemlidir. Aralarında bir sıra vardır ancak shiny’de başlangıç seviyesinde bu süreci tam olarak anlamak gerekli değildir. Çok temel düzeyde, bu görevler veya işlevler, geliştirici farklı davranacak şekilde ayarlamadıkça, sadece kullanıcı girdilerinde kendilerini etkileyen bir değişiklik olduğunda etkinleşir. Yine, bunların hepsi oldukça soyut kavramlardır. Ama önce üç temel shiny nesne türünü inceleyelim.
...
• Bir çıktı:
• Salgın eğrisi (malaria_epicurve çıkış kimliği ile)
...
• shinyapps.io: Bu seçenekte shiny uygulamalarını yayınlamak için yapılandırma iş yükü düşüktür ve kısmen ücretsizdir, (ancak sınırlı lisanslara sahiptir) bu nedenle, shiny uygulamaları yayınlamak için en kolay yerdir.
...
- **shinyWidgets** - bu paket size uygulamanızda kullanabileceğiniz çok daha fazla widget sunar. Bu paketteki mevcut widgetların bir seçimini görmek için `shinyWidgets::shinyWidgetsGallery()` fonksiyonunu çalıştırın. [Buradaki](https://github.com/dreamRs/shinyWidgets) örneklere bakabilirsiniz.
