---
aliases:
  - /new_pages/interactive_plots.jp.html
---
...
この章では、既存の `ggplot()` で作成したプロットを **plotly** パッケージで動的な図に変換することに焦点を当てます。**shiny** パッケージについては、 [Shiny で作るダッシュボード](shiny_basics.jp.qmd)の章を参照してください。特筆すべき点は、動的な図は HTML 形式の R  Markdown 文書でのみ使用可能であり、PDF や Word 文書では使用できないということです。
...
まず始めに、エボラ出血熱のシミュレーションで得られた症例がクリーニングされたラインリストの取り込みをします。`appliedepidata::get_data(name = "linelist_cleaned_rds")` で直接読み込めます（詳細は[ハンドブックとデータのダウンロード](data_used.jp.qmd)のページを参照）。
...
標準的な plotly のボタンの中には、余計なものや邪魔なものがあるので、それらを取り除くことができます。これは、作成した図を **plotly** パッケージの `config()` 関数にパイプでつなぎ、削除したいボタンを指定するだけで可能です。以下の例では、削除するボタンの名前をあらかじめ指定し、引数の `modeBarButtonsToRemove =` に渡しています。 また、`displaylogo = FALSE` とすることで、 **plotly** パッケージのロゴを削除しています。
...
ヒートタイルを含むほとんどの `ggplot()` プロットオブジェクトを動的に変えることができます。ある施設が州へデータを報告した日数の週あたりの割合を表示する下記の図の作成方法について[ヒートマップ](heatmaps.jp.qmd)の章で説明しています。
...
Plotly はR だけではなく、Python でも問題なく動作します（Plotly は JavaScript で作られているため、実際にはあらゆるデータサイエンス言語で動作します）。詳しくは [plotly のウェブサイト](https://plotly.com/r/)をご覧ください。
