...
To load the shapefile we use `get_data()` from **appliedepidata**, which returns it as an **sf** object. To read your own shapefile use `read_sf()` from **sf** with its file path (see [Import and export](importing.qmd) and [R projects](r_projects.qmd)).  
...
Here is what the population file looks like. Scroll to the right to see how each jurisdiction has columns with `male` population, `female` population, `total` population, and the population break-down in columns by age group.  
...
We load the facility points with `get_data()`. We again clean the column names, then filter to keep only the points tagged as either "hospital", "clinic", or "doctors".  
