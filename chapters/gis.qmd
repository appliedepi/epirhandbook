...
For an .rds file, `import()` warns unless you pass `trust = TRUE`, which says that you trust the source of the file. Use it only for files you made yourself or got from a source you trust.  
...
These data can again be downloaded from HDX (link [here](https://data.humdata.org/dataset/sierra-leone-population)) or via our **appliedepidata** R package as explained [in this page](data_used.qmd). We use `import()` to load the .csv file. We also pass the imported file to `clean_names()` to standardize the column name syntax.   
