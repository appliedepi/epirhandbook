...
Çok fazla arama yaptıktan sonra sorununuza bir yanıt bulamıyorsanız, *tekrarlanabilir bir örnek* ("reprex") oluşturmayı ve soruyu kendiniz göndermeyi düşünün. Tekrarlanabilir bir örneğin nasıl oluşturulacağı ve forumlara nasıl gönderileceğiyle ilgili ipuçları için [Yardım alma](help.tr.qmd) hakkındaki sayfaya bakabilirsiniz.
...
```
"unexpected symbol" görürseniz, eksik virgül olup olmadığını kontrol edin
...
```
Bu muhtemelen fonksiyon adını yanlış yazdığınız ya da bir paketi kurmayı veya yüklemeyi unuttuğunuz anlamına gelir.

```
Error in select(data, var) : unused argument (var)
```
'dplyr::select()' kullandığınızı düşünüyorsunuz, ancak 'select()' fonksiyonu 'MASS::select()' tarafından maskelenmiştir - ya 'dplyr::' ön ekini açıkça yazın ya da paket yükleme sıranızı dplyr diğerlerinin hepsinden sonra yüklenecek şekilde değiştirin.
...
```
# mutate(x = recode(x, OLD= NEW) içindeki x değişkeni yeniden belirtilmeden recode çalıştırıldı
...
```
#Bir faktöre bir değer ("Missing") eklenmeye çalışıldı (bir faktör üzerinde çalışan replace_na ile)
...
ggplot() scale_fill_manual() değerleri = c("orange", "purple") ... faktör düzeyi sayısı için yetersiz ... NA'nın artık bir faktör düzeyi olup olmadığını düşünün...
...
Hata mesajı şöyle bir şey içeriyorsa `Error in options[[sprintf("fig.%s", i)]]`, her bir parçanın üstündeki knitr seçeneklerinizin `out.width = ` veya `out.height = ` seçeneklerini doğru şekilde kullandığını, `fig.width=` ve `fig.height=` seçeneklerini *kullanmadığını* kontrol edin.
...
Bu bölüm, [yeni başlayanların karşılaştığı yaygın R programlama hatalarını](https://www.r-bloggers.com/2016/06/common-r-programming-errors-faced-by-beginners/) listeleyen başka bir blog gönderisidir