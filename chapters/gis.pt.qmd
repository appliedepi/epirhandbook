...
Importe os dados com a função `import()` do pacote **rio** (ela lida com vários tipos de arquivo como .xlsx, .csv, .rds - veja a página [Importar and exportar](importing.pt.qmd) para mais detalhes).  
...
Para um arquivo .rds, `import()` emite um aviso a menos que você passe `trust = TRUE`, o que indica que você confia na origem do arquivo. Use-o apenas com arquivos que você mesmo criou ou que vêm de uma fonte confiável.  
...
Para importar um shapefile usamos a função `read_sf()` do pacote **sf**. À ela é fornecido o caminho do arquivo através da função `here()`. - em nosso caso o arquivo está em nosso projeto R, nas subpastas "data", "gis", e "shp", com o nome de arquivo "sle_adm3.shp" (veja as páginas [Importar e exportar](importing.pt.qmd) e [Projetos R](r_projects.pt.qmd) para mais informações). Você precisará fornecer seu próprio caminho de arquivo.
...
Novamente, esses dados podem ser baixadados do site da HDX (link [aqui](https://data.humdata.org/dataset/sierra-leone-population)) ou pelo pacote **appliedepidata** explicado [nessa página](data_used.pt.qmd). Usamos `import()` para carregar o arquivo .csv. Também passamos o arquivo importado para `clean_names()` para padronizar a sintaxe dos nomes das colunas.   
