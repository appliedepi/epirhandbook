...
Para fins de demonstração, iremos trabalhar com uma amostra aleatória de 1000 casos do dataframe da `linelist` de uma epidemia simulada de Ebola (computacionalmente, trabalhar com menos casos é mais fácil para mostrar nesse handbook). A `linelist` limpa vem do pacote **appliedepidata**.  
...
Carregue a `linelist` limpa com `get_data()` do **appliedepidata** (veja [Baixe o livro e os dados](data_used.pt.qmd)).  
...
As fronteiras administrativas de Serra Leoa vêm do site da Humanitarian Data Exchange (HDX) [que você encontra aqui](https://data.humdata.org/dataset/sierra-leone-all-ad-min-level-boundaries). O capítulo as carrega com `get_data()` do **appliedepidata**, conforme explicado na página [Download dos dados no manual](data_used.pt.qmd).  
...
Para ler seu próprio shapefile use `read_sf()` do **sf** com o caminho do arquivo (veja as páginas [Importar e exportar](importing.pt.qmd) e [Projetos R](r_projects.pt.qmd)).
...
Esses dados vêm do pacote **appliedepidata** (veja [Baixe o livro e os dados](data_used.pt.qmd)). Eles também estão no site da HDX (link [aqui](https://data.humdata.org/dataset/sierra-leone-population)). Nós os carregamos com `get_data()` e depois passamos o resultado para `clean_names()` para padronizar a sintaxe dos nomes das colunas.   
...
Essa é a aparência do arquivo de população. Role para a direita para ver as colunas de população masculina (`male`), feminina (`female`) e `total` de cada jurisdição. As demais colunas subdividem a população por grupo de idade.
...
Os locais das unidades de saúde vêm do HDX [aqui](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities). O capítulo os carrega com `get_data()` do **appliedepidata**, conforme explicado na página [manual de Download e dados](data_used.pt.qmd).   

Novamente limpamos os nomes das colunas e depois filtramos para manter apenas os pontos marcados com "hospital", clínica ("clinic"), ou doutores ("doctors").  
...
tm_shape(sle_adm3) +               # shapefile das fronteiras administrativas
  tm_polygons(fill = "#F7F7F7")+    # show polygons in light grey
...
tm_shape(linelist_sf) +
  ...
  tm_title("Distribution of Ebola cases")   # Dá título ao gráfico 
...
Você deve estar familiarizado com o conceito de fazer *join* dos dados de uma base em outra. Vários métodos são discutidos na página [Juntando dados](joining_matching.pt.qmd) desse manual. Um "join espacial" tem um propósito similar, mas lidando com os relacionamentos espaciais dos dados. Em vez de utilizar valores comuns às colunas para parear corretamente as observações, você pode utilizar as relações espaciais deles, tais como o fato de uma característica estar *contida* em outra, ou de um ponto ser o *vizinho mais próximo* de outro, ou contido na área de influência (*buffer*) de um certo raio de outro, etc.  
...
Nesse exemplo, iniciamos o `ggplot()` com `linelist_adm`, de forma que podemos aplicar as funções de fatores como `fct_infreq()` que ordena as barras por frequência (veja a página de [Fatores](factors.pt.qmd) para dicas).  
...
tm_view(set_view = c(-13.2284, 8.4699, 13), # ajusta o zoom (centra as coordenadas, e define o zoom)
...
Também podemos explorar quantos casos estão localizados em uma distância a pé de 2 km (~30 min) da unidade de saúde mais próxima.

*Nota: o sf trata um objeto lat/long como esférico. Portanto, uma distância passada para `st_buffer()` ou `st_distance()` está em metros. Para cálculos de área ou de distância precisos, re-projete para um sistema de coordenadas projetado. Escolha esse sistema conforme o local e a medida, por exemplo a zona UTM local.*  
...
**Primeiro**, crie uma área de influência de 2 km ao redor de cada unidade de saúde com `st_buffer()` do pacote **sf**. O objeto está em lat/long, então o **sf** mede essa distância em metros sobre a esfera.  

```{r, warning=F, message=F}
sle_hf_2k <- sle_hf %>%
  st_buffer(dist = 2000)     # metros: o sf usa geometria esférica para objetos lat/long 
```

Abaixo criamos o gráfico das zonas de influência propriamente ditas, com os pontos das unidades de saúde:
...
linelist_sf_hf_2k <- linelist_sf_hf %>%
  st_join(sle_hf_2k, join = st_intersects, left = TRUE) %>%          # uma linha por caso e por área de influência que o contém; osm_id.y é NA se nenhuma o contém
  select(case_id, osm_id.x, nearest_clinic, amenity.x, osm_id.y) %>%
  distinct(case_id, .keep_all = TRUE)                                # uma linha por caso, assim um caso fica fora apenas se nenhuma área de influência o contém
```

Agora podemos contar os resultados: `r nrow(linelist_sf_hf_2k[is.na(linelist_sf_hf_2k$osm_id.y),])` dos 1000 casos não intersectam com nenhuma área de influência (esses valores estão faltando), e então residem a mais de 30 min a pé da da unidade de saúde mais próxima.
...
1) Os dados populacionais `sle_adm3_pop` são obtidos utilizando `left_join()` do pacote **dplyr** com base nos valores comuns ao longo das colunas `admin3pcod` no dataframe `case_adm3`, e coluna `adm_pcode` no dataframe `sle_adm3_pop`. Veja a página [Juntando dados](joining_matching.pt.qmd).  
...
tm_shape(case_adm3_sf) +                # adiciona polígonos ao mapa 
  tm_polygons("case_10kpop",            # colore pelas colunas contendo a taxa de casos
              fill.scale = tm_scale_intervals(
                ...
                values = "brewer.purples")  # utiliza uma paleta de cor roxa
              ) +
...
Este código baixa os tiles atuais do servidor de tiles do OpenStreetMap, portanto precisa de conexão à internet. Isso é permitido quando você mesmo visualiza um mapa. Não execute o código em uma construção automatizada. Não salve os tiles para uso off-line. Veja a [política de uso de tiles do OpenStreetMap](https://operations.osmfoundation.org/policies/tiles/).  

O manual mostra imagens dos mapas que este código produz. Elas foram criadas em 2 de setembro de 2026 para que a página seja construída sem conexão à internet. Dados do mapa © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
Dados do mapa © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
A matriz exibida acima mostra as relações entre as 9 regiões de nossa base `sle_adm3_dat`. Um score de 0 indica duas regiões que não são vizinhas, enquanto quaisquer outros valores diferentes de zero indicam uma relação de vizinhança. Os valores na matriz estão escalados de forma que a linha de cada região tenha um peso total de 1. 
...
Para dar um exemplo, vamos testar se um padrão espacial dos casos de Ebola da epidemia simulada é correlacionado com o padrão espacial da população. Para iniciar, precisamos de uma variável `population` em nossa base `sle_adm3_dat`. Podemos utilizar a variável `total` do dataframe `sle_adm3_pop` que carregamos anteriormente.
