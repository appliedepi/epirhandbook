---
aliases:
  - /new_pages/transmission_chains.ru.html
---
...
Попробуйте воспользоваться приведенным ниже интерактивным графиком, наводя курсор на вершины для получения дополнительной информации, перетаскивая их для перемещения и нажимая на них для выделения последующих случаев в цепочке передачи.
...
Вам потребуется версия разработчиков **epicontacts**, которая может быть установлена из github с помощью функции `p_install_github()` из **pacman**. Эту команду, приведенную ниже, необходимо
выполнить только один раз, а не при каждом использовании пакета (в дальнейшем можно использовать `p_load()` как обычно).
...
что в кластерах, содержащих `2ae019` и `71577a`, задействовано в общей сложности 13 случаев из построчного списка.
