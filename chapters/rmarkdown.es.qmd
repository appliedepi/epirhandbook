...
* La página [Organización de informes rutinarios](reportfactory.es.qmd) muestra cómo "rutinizar" la producción de informes con carpetas autogeneradas con marca de tiempo.
* La página [Dashboards con R Markdown](flexdashboard.es.qmd) explica cómo formatear un informe de R Markdown como un cuadro de mando o tablero de control.
...
Para referirse a archivos en otro lugar, tendrá que utilizar la ruta completa del archivo o utilizar el paquete **here**. El paquete **here** establece el directorio de trabajo en la carpeta raíz del proyecto R y se explica en detalle en las páginas de [proyectos R](r_projects.es.qmd) e [importación y exportación](importing.es.qmd) de este manual. Por ejemplo, para importar un archivo llamado "data.csv" desde la carpeta `projectX`, el código sería `import(here("data.csv"))`.
...
Puedes utilizar código R en valores YAML escribiéndolo como código en línea (precedido por `r` entre acentos graves) pero también entre comillas (véase el ejemplo anterior para `date: `).
...
Al igual que con "knit", la configuración predeterminada guardará la salida Rmd en la misma carpeta que el script Rmd, con el mismo nombre de archivo (excepto la extensión del archivo). Por ejemplo, "my_report.Rmd" cuando se procese creará "my_report.docx" si se procesa a un documento de Word. Sin embargo, al usar `render()` existe la opción de usar diferentes configuraciones. `render()` puede aceptar argumentos que incluyen:
...
Como ejemplo, para mejorar el control de versiones, el siguiente comando guardará el archivo de salida dentro de una subcarpeta 'outputs', con la fecha actual en el nombre del archivo. Para crear el nombre del archivo, se utiliza la función `str_glue()` del paquete **stringr** para "pegar" las cadenas estáticas (escritas sin formato) con el código dinámico de R (escrito entre corchetes). Por ejemplo, si es 10 de abril de 2021, el nombre del archivo será "Report_2021-04-10.docx". Consultá la página sobre [Caracteres y cadenas](characters_strings.es.qmd) para obtener más detalles sobre `str_glue()`.
...
Leé más sobre este flujo de trabajo en la página sobre la [organización de informes rutinarios](reportfactory.es.qmd). 
...
A continuación, introducimos estos valores uno a uno en el comando `render()` mediante un bucle, que ejecuta el comando una vez por cada valor del vector `hospitals`. La letra "i" representa la posición del índice (del 1 al 4) del hospital que se está utilizando en esa iteración, de modo que `hospital_list[1]` sería "Central Hospital". Esta información se suministra en dos lugares en el comando `render()`:  

1) Al nombre del archivo, de forma que el nombre del archivo de la primera iteración, si se produce el 10 de abril de 2021, sería "Report_Central Hospital_2021-04-10.docx", guardado en la subcarpeta 'output' del directorio de trabajo.  
...
* Plotly (utilizado en la página de este manual y en la página de [Gráficos interactivos](interactive_plots.es.qmd)
* visNetwork (utilizado en la página de [Cadenas de transmisión](transmission_chains.es.qmd) de este manual)
* Leaflet (Folleto) (utilizado en la página [Conceptos básicos de los SIG](gis.es.qmd) de este manual)
...
La función `ggplotly()` de **plotly** es particularmente fácil de usar. Consúltalo en la sección en la página de [Gráficos interactivos](interactive_plots.es.qmd).
