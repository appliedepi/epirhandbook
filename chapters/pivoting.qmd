...
In this page, we will use a fictional dataset of daily malaria cases, by facility and age group. If you want to follow along, load it directly with `appliedepidata::get_data(name = "malaria_facility_count_data")` (see the [Download handbook and data](data_used.qmd) page for details).
...
In the later part of this page, we will also use the dataset of cases from a simulated Ebola epidemic. If you want to follow along, load it directly with `appliedepidata::get_data(name = "linelist_cleaned_rds")` (see the [Download handbook and data](data_used.qmd) page for details).
...
For this section of the page, we will use the case linelist (see the [Preparation](pivoting.qmd) section), which contains one row per case.  
