---
aliases:
  - /new_pages/collaboration.es.html
---
...
Por ejemplo, puedes *clonar* un repositorio en línea _desde_ Github a tu ordenador, o empezar con un repositorio local y clonarlo en línea _hacia_ Github.

Cuando has clonado un repositorio, los archivos del proyecto existen en dos lugares:
...
Hay muchas formas de crear nuevos repositorios. Puedes hacerlo desde la consola/terminal, desde Github, desde una interfaz gráfica.
...
Cuando creas un nuevo repositorio, puedes crear opcionalmente
todos los archivos siguientes, o puedes añadirlos a tu repositorio más adelante.
Normalmente estarían en la carpeta "raíz" del repositorio.
...
Fíjate en los botones rodeados con un círculo en la imagen anterior, ya que se mencionarán más adelante (de izquierda a derecha):
...
A continuación, repasamos el proceso anterior paso a paso con más detalle. Es un esquema que hemos desarrollado - está en el formato de una tabla de doble entrada, por lo que debería ayudarnos a entenderlo.
...
git add chapters/collaboration.Rmd  # selecciona los ficheros a (= stage los cambioss)
...
_Nota: piensa antes de modificar confirmaciones (commits) que ya son públicas y están compartidas con tus colaboradores_.
...
- Confirmar tus cambios, obtener los cambios remotos, extraerlos, resolver los conflictos si es necesario (ver la sección más abajo), y hacer push de todo en línea
...
Sólo tienes que cambiarte (checkout) a la rama que quieres eliminar, e ir al menú
*Branch \> Delete*.
...
En los comandos de abajo, <name> representa un nombre de rama. <commit_hash> representa el hash ID de un commit específico. <num> representa un número. No escriba los símbolos \< o \>.
...
| `git switch`             | Una alternativa a `git checkout` que se está introduciendo progresivamente en Git |
