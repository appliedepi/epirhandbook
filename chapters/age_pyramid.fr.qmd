...
Pour commencer, nous importons la liste de cas nettoyée d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails).
...
Pour la démonstration, nous agrégeons les données de la liste de cas en nombre de cas par catégorie d'âge et par sexe, dans un format "large". Cela fera comme si vos données étaient aggrégées dès le départ. Pour en savoir plus sur le [Regroupement des données](grouping.fr.qmd) et le [Pivotage des données](pivoting.fr.qmd), consultez leurs pages respectives.
...
Notez dans l'exemple ci-dessus que l'ordre des facteurs "m" et "f" est différent (pyramide inversée). Pour ajuster l'ordre, vous devez redéfinir le sexe dans les données agrégées comme un facteur et ordonner les niveaux comme vous le souhaitez. Voir la page [Facteurs](factors.fr.qmd).
...
Importez et visualisez les données de population (voir la page [Télécharger le manuel et les données](data_used.fr.qmd)) :
...
-   Un graphique en barres des données de cas (petites barres, plus solides)
...
Importez les données (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) si vous le souhaitez).
...
Commencez avec des données qui ressemblent à ceci, avec une classification catégorielle de chaque répondant (`status`) et leurs réponses à 8 questions sur une échelle de type Likert à 4 points ("Très mauvais", "Mauvais", "Bon", "Très bon").
