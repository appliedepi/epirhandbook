...
Para acceder rápidamente, **clica con el botón derecho** [en este enlace](https://github.com/appliedepi/epirhandbook/raw/main/offline_long/index.es.html) **y selecciona "Guardar enlace como"**.
...
El método más sencillo para descargar todos los datos es instalar nuestro paquete **appliedepidata**. Contiene una función `get_data()` que guarda todos los datos del ejemplo en una carpeta de tu elección en tu ordenador.

Para instalar nuestro paquete **appliedepidata**, ejecuta el siguiente código. Este paquete no está en CRAN, así que utiliza la función `p_install_gh()` para instalarlo. La entrada hace referencia a nuestra organización de Github ("*appliedepi*") y al paquete **appliedepidata**.
...
A continuación, utiliza la función `get_data()` del paquete para descargar los datos de ejemplo en tu ordenador. Ejecuta appliedepidata::save_data() para obtener *todos los datos de ejemplo*, o escribe un nombre de archivo específico y una extensión entre comillas para recuperar sólo un archivo.
...
Si lo deseas, puedes revisar los archivos de datos originales en la **[carpeta "data"](https://github.com/appliedepi/appliedepidata)** del repositorio de Github del paquete **appliedepidata**.
...
Si sigues la opción 1 para descargar todos los datos de ejemplo (a través de nuestro paquete **appliedepidata**), todos los shapefiles están incluidos en el paquete.
...
Para cargar el árbol filogenético "Shigella_tree.txt" en R:

```{r, eval=F}
tree <- appliedepidata::get_data(name = "Shigella_tree")
```

Para cargar "sample_data_Shigella_tree.csv" con información adicional sobre cada muestra en R:

```{r, eval=F}
sample_data <- appliedepidata::get_data(name = "sample_data_Shigella_tree")
```

Para ver el nuevo árbol de subconjuntos creado:
...
Puedes [clicar aquí para descargar el archivo facility_count_data.rds](https://github.com/appliedepi/epirhandbook/raw/main/data/malaria_app/data/facility_count_data.rds) que contiene datos sobre la malaria para la aplicación Shiny. Ten en cuenta que puede ser necesario almacenarlo dentro de una carpeta "data" para que las rutas de los archivos here() funcionen correctamente.
