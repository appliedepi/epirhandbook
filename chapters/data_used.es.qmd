...
# Descargando el manual y los datos {#data-used}
...
Se trata de datos ficticios de una encuesta tipo Likert, utilizados en la página sobre [Pirámides de población y escalas de Likert](age_pyramid.es.qmd). Puedes cargar estos datos directamente en R ejecutando los siguientes comandos:
...
A continuación se encuentran los enlaces a los archivos asociados a la página sobre [Dashboards con R Markdown](flexdashboard.es.qmd). Son archivos fuente de R Markdown/HTML, no conjuntos de datos de ejemplo, por lo que no forman parte del paquete **appliedepidata**. Descárgalos directamente desde Github:
...
La página de [rastreo de contactos](contact_tracing.es.qmd) muestra el análisis de los datos de rastreo de contactos, utilizando como ejemplo datos de [Go.Data](https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting). Estos datos no forman parte de **appliedepidata**; los datos utilizados en la página pueden descargarse como archivos .rds clicando en los siguientes enlaces:
...
<span style="color: black;">**_NOTA:_** Los datos de rastreo de contactos estructurados de otro software (por ejemplo, KoBo, DHIS2 Tracker, CommCare) pueden tener un aspecto diferente. Si quieres contribuir con una muestra de datos alternativos o con contenido para esta página, [por favor, ponte en contacto, con nosotros](../index.es.qmd).</span> 

<span style="color: darkgreen;">**_CONSEJO:_** Si estás utilizando Go.Data y quieres conectarte a tu instancia de la API, consulta la página de importación y exportación [(sección API)](importing.es.qmd#import_api) y la [Comunidad de Prácticas de Go.Data](https://community-godata.who.int/).</span>
...
Consulta la página de [tasas estandarizadas](standardization.es.qmd). Puedes cargar los datos directamente con los siguientes comandos:
...
Para el capítulo sobre [análisis de encuestas](survey_analysis.es.qmd) utilizamos datos ficticios de encuestas de mortalidad basados en las plantillas de encuestas de MSF OCA. Estos datos ficticios se generaron como parte del [proyecto "R4Epis"](https://r4epis.netlify.app/).
