...
Você pode baixar todos os dados utilizados neste manual através das instruções na página de [download do manual e dos dados](data_used.pt.qmd).  

Os dados utilizados nessa seção para fins de exemplificação são as contagens semanais de casos de campylobacter notificados na Alemanha entre 2001 e 2011. Carregue diretamente com `appliedepidata::get_data(name = "campylobacter_germany")` (consulte a página [Baixar manual e dados](data_used.pt.qmd) para mais detalhes).  
...
maneiras de fazer isso (veja a página [trabalhando com datas](dates.pt.qmd)
...
Você pode baixar cada um desses arquivos de dados climáticos da página [Download do manual e dos dados](data_used.pt.qmd).  
...
O bloco de código abaixo grava os 10 anos de arquivos de dados climáticos ".nc" em uma pasta temporária. Ele usa `save_data()` do **appliedepidata** e depois descompacta o arquivo.
...
Veja a página sobre [Eliminando duplicidades](deduplication.pt.qmd)
...
Veja a página sobre [Dados faltantes](missing_data.pt.qmd) para mais informações sobre imputação.

Outra alternativa seria calcular a média móvel, para tentar suavizar
esses problemas aparentes de notificação (veja a próxima seção, e a página sobre [médias móveis](moving_average.pt.qmd)). 
...
Veja a página sobre [Médias móveis](moving_average.pt.qmd) para mais detalhes.
...
Abaixo, nós definimos uma função customizada para criar um periodograma. Veja a página sobre [Escrevendo funções](writing_functions.pt.qmd) para informações sobre como escrever funções no R.
...
nas maiores ou nas menores quantidades) e que ele pode ter maior probabilidade de subestimar
...
manual a seção sobre [unindo dados](joining_matching.pt.qmd).
...
Para mais sobre o pivoteamento dos dados, veja a seção sobre [pivoteando dados](pivoting.pt.qmd) neste manual. 
...
`add_row()`. Então, nós especificamos a sequência de semanas entre a semana seguinte à última semana
atualmente disponível nos dados e a semana final.
...
Veja a página sobre [Iteração, loops, e listas](iteration.pt.qmd) para aprender mais sobre o pacote **purrr**.  
