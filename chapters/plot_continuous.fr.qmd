...
linelist <- appliedepidata::get_data(name = "linelist_cleaned_rds") %>% #Charger les données
...
Pour les exemples de cette page, nous importons la linelist nettoyée des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails).    
