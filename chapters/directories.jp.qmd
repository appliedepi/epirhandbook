...
ディレクトリ内のファイル名だけをリストアップするには、**base** R の `dir()` を使用します。 例えば、下記のコマンドは R プロジェクトの "data" フォルダの "population" サブフォルダ内のファイル名をリストアップしています。相対的なファイルパスは `here()` を使ってえられます (詳しくは [データのインポート・エクスポート](importing.jp.qmd) 章を参照してください)。
