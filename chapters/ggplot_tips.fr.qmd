...
Dans cette page, nous couvrirons les trucs et astuces pour rendre vos ggplots nets et esthétiques. Voir la page sur les [bases de ggplot](ggplot_basics.fr.qmd) pour les principes de base.
...
Les nuances des "scales" au format date sont traitées plus en détail dans la page [Courbes épidémiques](epicurves.fr.qmd).
...
Pour illustrer comment ajuster une "scale" de couleur continue, nous utiliserons un ensemble de données de la page [Suivi des contacts](contact_tracing.fr.qmd) qui contient les âges des cas et de leurs cas sources.
...
Voici un exemple tiré de la page [ggplot basics](ggplot_basics.fr.qmd), utilisant différents schémas de couleurs.
...
Vous trouverez un exemple complet dans la section de la page [Courbes épidémiques](epicurves.fr.qmd) sur les étiquettes de date à plusieurs niveaux, mais un exemple rapide est présenté ci-dessous pour référence :
...
Notez qu'il existe un autre exemple de cette technique sur la page [Courbes épidémiques](epicurves.fr.qmd) - superposition de l'incidence cumulée sur l'épicurve.
