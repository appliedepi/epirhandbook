...
<span style="color: darkgreen;">**<u>ヒント：</u>** Kobo のデータ辞書形式（例、[MSF-survey-dict.xlsx](https://github.com/R4EPI/epidict/blob/master/inst/extdata/MSF-survey-dict.xlsx)）では、調査ごとシートに分かれており、各シートの "name" 列に変数名（質問）が記載されています。各変数がとり得る値（回答の選択肢）は、"choices" シートで設定されています。"choices" シートでは、"name" 列には選択肢の省略形が、"label::english" および "label::french" 列にはそれぞれの言語で適切な長さの選択肢が表示されています。**Epidict** パッケージの `msf_dict_survey()` 関数を使って、Kobo データ辞書形式の Excel ファイルをインポートすると、対象の調査シートがデータフレームに変換されるので、調査データをコード記述に簡単に利用できます。</span>
