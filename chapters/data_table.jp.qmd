...
エボラ出血熱の流行をシミュレートしたデータセットをインポートします。お手元の環境でこの章の内容を実行したい方は、[ハンドブックとデータのダウンロード](#data-used) の章のダウンロード手順を参照してください。データは **rio** パッケージの `import()` を利用してインポートしましょう（詳細は、[インポートとエクスポート](#importing) の章をご覧ください。）`data.table()` を使用してデータフレームをデータテーブルに変換します。
