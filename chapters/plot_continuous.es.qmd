...
linelist <- appliedepidata::get_data(name = "linelist_cleaned_rds") %>% #Load the data
...
For the examples in this page, we import the cleaned linelist of cases from a simulated Ebola epidemic. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles).    
