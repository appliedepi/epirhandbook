...
Dans ce chapitre, nous allons nous focaliser sur le second processus. Résumer ses données dans des tableau est une étape cruciale de l'analyse des données et est traitée dans les chapitres sur [le regroupement des données](grouping.fr.qmd) et les [tableaux descriptifs](tables_descriptive.fr.qmd). 
...
A la fin de ce chapitre, nous utiliserons également une liste des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails).    
...
La colonne `date` est actuellement sous la forme d'une *chaîne de caractères* mais nous pouvons facilement la convertir en classe date en utilisant les fonctions `mutate()` et `as_date()` décrites dans la page [travailler avec des dates](dates.fr.qmd).  

Nous pouvons aussi améliorer la colonne `observation` en supprimant le préfixe "obs"  et en la convertissant en format `numeric`. Nous pouvons le faire avec `str_remove_all()` du paquet **stringr** (voir la page sur les [chaînes de caractères](characters_strings.fr.qmd)). 
...
Un cas d'utilisation typique est lorsque qu'il faut transformer les résultats d'une analyse dans un format plus digeste pour le lecteur, tel qu'un [tableau résumé](tables_presentation.fr.qmd). En général, il s'agit de transformer un dataframe dans lequel les informations relatives à un sujet sont réparties sur plusieurs lignes en un format dans lequel ces informations sont stockées sur une seule ligne.
