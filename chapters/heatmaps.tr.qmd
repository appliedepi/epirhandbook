...
Simüle edilmiş bir Ebola salgınının vaka listesiyle başlıyoruz. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.tr.qmd) sayfasına bakın).
...
4) `summarise()` işlevi, tesis-hafta grubu başına özet istatistikleri yansıtmak için yeni sütunlar oluşturur:
    ...
    + Tesis-haftadaki verilerin raporlandığı *benzersiz* günlerin sayısı  
...
`expanded_weeks` burada; `r nrow(expanded_weeks)` satır içerir:

```{r, echo=F}
DT::datatable(expanded_weeks, rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap')
```

Bu kodu çalıştırmadan önce `agg_weeks`, `r nrow(agg_weeks)` satırlarını içerir.
