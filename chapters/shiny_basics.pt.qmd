---
aliases:
  - /new_pages/shiny_basics.pt.html
---
...
Para um método alternativo de fazer painéis, que é mais rápido e mais fácil, mas talvez menos personalizável, consulte a página **flextable** ([Painéis com R Markdown](flexdashboard.pt.qmd)).  
...
Neste manual, enfatizamos `p_load()` de **pacman**, que instala o pacote se necessário *e* o carrega para uso. Você também pode carregar pacotes instalados com `library()` do R **base**. Veja a página em [Introdução ao R](basics.pt.qmd) para mais informações sobre pacotes R.  
...
Se você gostaria de acompanhar esta página, consulte esta seção do [Baixar manual e dados](data_used.pt.qmd). Existem links para baixar os scripts R e arquivos de dados que produzem o aplicativo Shiny final.  
...
Será mais fácil trabalhar com esses dados se usarmos padrões de dados organizados, então também devemos transformar em um formato de dados mais longo, em que a faixa etária é uma coluna e os casos é outra coluna. Podemos fazer isso facilmente usando o que aprendemos na página [Pivoteando dados](pivoting.pt.qmd).  
...
O que segue representa uma seleção de extensões shiny de alta qualidade que podem ajudá-lo a obter muito mais do shiny. Em nenhuma ordem particular:

- **shinyWidgets** - este pacote oferece muitos, muitos mais widgets que podem ser usados em seu aplicativo. Execute `shinyWidgets::shinyWidgetsGallery()` para ver uma seleção de widgets disponíveis com este pacote. Veja exemplos [aqui](https://github.com/dreamRs/shinyWidgets)  

- **shinyjs** - este é um excelente pacote que dá ao usuário a habilidade de estender muito a utilidade do shiny por meio de uma série de javascript. Os aplicativos deste pacote variam de muito simples a altamente avançado, mas você pode querer usá-lo primeiro para manipular a interface do usuário de maneiras simples, como ocultar / mostrar elementos ou habilitar / desabilitar botões. Saiba mais [aqui](https://deanattali.com/shinyjs/basic)

- **shinydashboard** - este pacote expande enormemente a interface do usuário disponível que pode ser usada em shiny, permitindo especificamente que o usuário crie um painel complexo com uma variedade de layouts complexos. Veja mais [aqui](https://rstudio.github.io/shinydashboard/)

- **shinydashboardPlus** - obtenha ainda mais recursos da estrutura do **shinydashboard**! Veja mais [aqui](https://rinterface.github.io/shinydashboardPlus/articles/shinydashboardPlus.html)

- **shinythemes** - mude o tema CSS padrão para seu aplicativo shiny com uma ampla gama de modelos predefinidos! Veja mais [aqui](https://rstudio.github.io/shinythemes/)
