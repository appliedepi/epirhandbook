...
Để tải file cây phả hệ “Shigella_tree.txt” vào R:

```{r, eval=F}
tree <- appliedepidata::get_data(name = "Shigella_tree")
```

Để tải file "sample_data_Shigella_tree.csv" với thông tin bổ sung cho từng mẫu vào R:

```{r, eval=F}
sample_data <- appliedepidata::get_data(name = "sample_data_Shigella_tree")
```

Để xem subset-tree mới được tạo:
