...
-   Để tải xuống file cây phả hệ “Shigella_tree.txt”, nhấn chuột phải vào ```{r, eval=F}
tree <- appliedepidata::get_data(name = "Shigella_tree")
``` (Cmd+click đối với Mac) và chọn "Save link as".\
-   Để tải xuống file "sample_data_Shigella_tree.csv" với thông tin bổ sung cho từng mẫu, nhấn chuột phải vào ```{r, eval=F}
sample_data <- appliedepidata::get_data(name = "sample_data_Shigella_tree")
``` (Cmd+click đối với Mac) và chọn "Save link as".\
-   Để xem subset-tree mới được tạo, nhấn chuột phải vào ```{r, eval=F}
subtree <- appliedepidata::get_data(name = "Shigella_subtree_2")
``` (Cmd+click đối với Mac) và chọn "Save link as". Tệp .txt sẽ được tải xuống máy tính của bạn.
