...
Nếu bạn muốn một lệnh hiệu quả hơn, bạn có thể tận dụng hàm `+()`, hàm này chuyển giá trị thành 1 và 0 dựa trên một mệnh đề logic. Lệnh này dùng kết hợp với hàm `across()` để thực hiện thay đổi trên nhiều cột cùng lúc (đọc thêm tại chương [Các hàm làm sạch dữ liệu](cleaning.vn.qmd#clean_across)).  
