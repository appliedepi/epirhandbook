---
aliases:
  - /new_pages/phylogenetic_trees.es.html
output:
...
Los datos de esta página pueden descargarse con las instrucciones de la página [Descargando el manual y los datos](data_used.es.qmd).
...
En primer lugar, utilizamos la función read.tree() del paquete **ape** para importar un archivo de árbol filogenético de Newick en formato .txt, y lo almacenamos en un objeto tipo lista de clase "phylo". Si es necesario, utiliza la función `here()` del paquete **here** para especificar la ruta relativa del archivo.
...
A continuación, añadimos las mutaciones de resistencia a la ciprofloxacina después de añadir otra capa de esquema de colores:
