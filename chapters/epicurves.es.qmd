...
En esta página mostramos cómo hacer curvas epidémicas con el paquete **ggplot2**, que permite una personalización avanzada.
...
El paquete **incidence2** ofrece un enfoque alternativo con comandos más sencillos, pero en el momento de escribir esto estaba siendo revisado y no era estable. Se volverá a añadir a este capítulo cuando sea estable.
...
Los datos se importan mediante la función `import()` del paquete **rio**. Consulta la página sobre [importación y exportación](importing.es.qmd) para conocer las distintas formas de importar datos.
...
Hay que controlar *manualmente* la agregación de los casos por tiempo (en semanas, meses, etc.) *y* los intervalos de las etiquetas en el eje de fechas. Esto debe gestionarse cuidadosamente.
...
* Usamos `closed = "left"` en `geom_histogram()` para asegurar que las fechas se cuenten en las franjas correctas
...
Las barras del histograma pueden colorearse por grupos y "apilarse". Para designar la columna de agrupación, haz los siguientes cambios. Consulta la página de [Conceptos básicos de ggplot](ggplot_basics.es.qmd) para más detalles.
...
* Consulta la página sobre [Conceptos básicos de ggplot](ggplot_basics.es.qmd) para obtener más información sobre escalas y paletas de colores.  
...
El orden en que se apilan las barras agrupadas se ajusta mejor clasificando la columna de agrupación como tipo Factor. A continuación, puedes designar el orden de los niveles de los factores (y sus etiquetas de visualización). Consulta la página sobre [Factores](factors.es.qmd) o [consejos de ggplot](ggplot_tips.es.qmd) para obtener más detalles.

Antes de realizar el gráfico, utiliza la función `fct_relevel()` del paquete **forcats** para convertir la columna de agrupación en de tipo factor y ajustar manualmente el orden de los niveles, como se detalla en la página sobre [Factores](factors.es.qmd).
...
Lee más sobre las leyendas y las escalas en la página [Consejos de ggplot](ggplot_tips.es.qmd). Aquí hay algunos puntos destacados:
...
* Consulta [esta página](https://rdrr.io/r/base/strptime.html) o la página [Trabajar con fechas](dates.es.qmd) para obtener consejos sobre la creación de etiquetas con fechas.
...
Consulta la página sobre [medias móviles](moving_average.es.qmd) para obtener una descripción detallada y varias opciones. A continuación se muestra una opción para calcular medias móviles con el paquete **slider**. En este enfoque, *la media móvil se calcula antes de representarla*:

1.  Agrega los datos en recuentos según sea necesario (diario, semanal, etc.) (véase la página de [Agrupar datos](grouping.es.qmd))
...
ggplot(data = ll_counts_7day) +  # comienza con el nuevo conjunto de datos definido anteriormente 
    ...
    geom_line(                   # crea la linea de la media móvil
      ...
      linewidth = 1) +                # anchura de la línea
...
* Opcional: para conseguir facetas apiladas verticalmente: la columna de la faceta se mueve al lado de las filas de la ecuación y a la derecha se sustituye por "." (`facet_wrap(facet~.)`), y `ncol = 1 `. También puede ser necesario ajustar la anchura y la altura de la imagen png guardada (ver `ggsave()` en [Conceptos básicos de ggplot](ggplot_basics.es.qmd)).
...
Recuerda - puedes utilizar herramientas como `\n` *dentro de* los argumentos `date_labels` o `labels` para poner partes de cada etiqueta en una nueva línea inferior. Sin embargo, el código de abajo le ayuda a usar años o meses (por ejemplo) en una línea inferior *y sólo una vez*.

El método más sencillo es asignar el argumento `labels = ` de `scale_x_date()` a la función `label_date_short()` del paquete **scales** (nota: no olvides incluir los paréntesis vacíos ()). Esta función construye automáticamente etiquetas de fecha eficientes (lee más [aquí](https://scales.r-lib.org/reference/label_date.html)). Otra ventaja de esta función es que las etiquetas se ajustan automáticamente a medida que tus datos se extienden en el tiempo: de días, a semanas, a meses y años.
...
Una segunda opción es utilizar facetas. A continuación:
...
También se puede superponer a la curva epidémica, con doble eje utilizando el método **cowplot** descrito anteriormente y en la página [Consejos de ggplot](ggplot_tips.es.qmd):
