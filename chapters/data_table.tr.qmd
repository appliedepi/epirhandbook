...
Simüle edilmiş bir Ebola salgınından vakaların veri setini içe aktarıyoruz. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_excel")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.tr.qmd) sayfasına bakın). Buradan veri çerçevesini bir veri tablosuna dönüştürmek için `data.table()` kullanıyoruz.
