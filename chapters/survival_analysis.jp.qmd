---
aliases:
  - /new_pages/survival_analysis.jp.html
---
...
<span style="color: darkgreen;">**<u>ヒント：</u>** `futime` の要約（`summary()`）と、`event` とその作成元である `outcome` のクロス集計を行うことで、作成した新しい列を検証することができます。この検証の他にも、生存時間解析の結果を解釈する時には、この集計に加えて追跡期間中央値を示すことは良い習慣です。</span>
...
新しい年齢分類の性別ごとのより詳細な分布を得るために、列 `age_cat_small` と `gender` のクロス集計も行うことができます。ここでは、[記述統計表の作り方](tables_descriptive.jp.qmd)の章で説明した **janitor** パッケージの `tabyl()` と *adorn* 関数を使用します。
...
<span style="color: orange;">**<u>注意：</u>** **survminer** パッケージの関数では survfit オブジェクト <u>に加えて</u> そのオブジェクトを作成するのに用いたデータを再度指定する必要があります。エラーメッセージが表示されてしまうので、忘れずに指定してください。</span>
