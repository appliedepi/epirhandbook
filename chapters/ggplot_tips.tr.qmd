...
Bu sayfa için, simüle edilmiş bir Ebola salgınından vakaların veri setini içe aktarıyoruz. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.tr.qmd) sayfasına bakın).
...
**ggplot2** ile başlangıçta anlaşılması zor olabilecek bir şey, renk şemalarının kontrolüdür. Bu bölümün noktalar, çubuklar, çizgiler, döşemeler vb. *çizim nesnelerinin* (geomlar/şekiller) rengini tartıştığını unutmayın. Aksesuar metninin, başlıkların veya arka plan renginin rengini ayarlamak için  [ggplot temelleri] sayfasının bölümü olan [Temalar](ggplot_basics.tr.qmd#ggplot_basics_themes) bölümüne bakınız.

*Arsa nesnelerinin* "rengini" kontrol etmek için, ya `color = ` estetiğini (*dış* renk) ya da `fill = ` estetiğini (*iç* renk) ayarlayacaksınız. Bu modelin bir istisnası, gerçekten yalnızca noktanın rengini (iç ve dış) kontrol eden 'color = ' öğesini kontrol edebileceğiniz 'geom_point()' fonksiyonudur.

Renk veya dolguyu ayarlarken "kırmızı" gibi R tarafından tanınan renk adlarını kullanabilir (bkz. [tam liste](http://sape.inf.usi.ch/quick-reference/ggplot2/colour) veya `?colors`) veya "#ff0505"` gibi belirli bir onaltılık renkler kullanılabilir.
...
[Gggplot temelleri] bölümünde [verileri çizime eşleme](ggplot_basics.tr.qmd#ggplot_basics_mapping) ile ilgili olarak açıklandığı gibi, 'fill = ' ve 'color = ' gibi estetikler bir 'mapping = aes()' ifadesinin *dışında*  veya birinin *içinde* tanımlanabilir. "aes()" * dışında* ise, atanan değer statik olmalıdır (ör. 'color = "blue"') ve geom tarafından çizilen *tüm* veriler için geçerli olacaktır. *içeride* ise, estetik, 'color = hospital' gibi bir sütuna eşlenmelidir ve ifade, verilerdeki o satırın değerine göre değişecektir. Birkaç örnek:
...
1) İlk kısım olan `scale_()` sabittir.
...
`scale_y_continuous()` kullanarak ggplot'taki değerlerin kesintilerini veya gösterimini ayarlamak isteyebiliriz. Yukarıda belirtildiği gibi, ölçek boyunca "kesmeler" olarak hizmet edecek bir değerler dizisi sağlamak için "breaks =" bağımsız değişkenini kullanın. Bunlar sayıların görüntüleneceği değerlerdir. Bu bağımsız değişkene, istenen kesme değerlerini içeren bir "c()" vektörü sağlayabilir veya R tabanı fonksiyonu "seq()" kullanarak düzenli bir sayı dizisi sağlayabilirsiniz. Bu 'seq()' fonksiyonu 'to = ', 'from = ' ve 'by = ' kabul eder.
...
* `scale_fill_gradientn()` herhangi bir uzunluktaki bir renk vektörünü "values =" olarak kabul eder (ara değerler eklemlenmiş olacaktır) 
...
x eksenindeki tarih etiketlerini verimli hale getirmenin kolay bir yolu, `scale_x_date()` içindeki `labels = ` argümanına **scales** paketinden `label_date_short()` fonksiyonunu atamaktır. Bu fonksiyon, verimli tarih etiketlerini otomatik olarak oluşturur (daha fazlası için [buraya](https://scales.r-lib.org/reference/label_date.html) bakın). Bu fonksiyonun ek bir faydası da, verileriniz zaman içinde günlerden haftalara, aylara ve yıllara doğru genişledikçe etiketlerin kendiliğinden ayarlanmasıdır.

Tam bir örnek için salgın eğrileri sayfasındaki [çok seviyeli tarih etiketleri](https://epirhandbook.com/en/epidemic-curves.html#multi-level-date-labels) bölümüne bakın, ancak referans olması için aşağıda hızlı bir örnek gösterilmiştir:
...
Grafikleriniz aynı göstergelere sahipse, bunları birleştirmek nispeten basittir. Grafikleri birleştirmek için yukarıdaki **cowplot** yaklaşımını kullanın, ancak göstergeyi bunlardan birinden kaldırın (yinelenmeyi önlemek için).
...
* Salgın eğrisi ve çizgi, çizimden önceki haftalarda toplanır *ve* "date_breaks" ve "date_labels" aynıdır - bunu, üst üste bindirildiklerinde iki grafiğin x-eksenlerinin aynı olması için yaparız.
...
Yardımcı sayfaları
[ggplot2 ile güzel çizim](http://zevross.com/blog/2014/08/04/beautiful-plotting-in-r-a-ggplot2-cheatsheet-3/)  
