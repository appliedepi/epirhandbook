---
aliases:
  - /new_pages/contact_tracing.fr.html
editor_options: 
  markdown: 
    wrap: 72
---
...
Vous pouvez télécharger tous les exemples de données pour ce manuel à partir de la page [Télécharger le manuel et les données](data_used.fr.qmd).
...
Comme le montre la page consacrée aux [Pyramides démographiques](age_pyramid.fr.qmd), vous pouvez visualiser la répartition par âge et par sexe (nous utilisons ici le package **apyramid**).
...
Ces données peuvent exister sous différents formats. Elles peuvent exister sous la forme d'une feuille Excel au format "large" avec une ligne par contact et une colonne par "jour" de suivi. Voir [Pivoter les données](pivoting.fr.qmd) pour obtenir des descriptions des données "longues" et données "larges" et comment faire pivoter des données plus larges ou plus longues.
...
Un mécanisme de visualisation pratique (si le nombre de cas n'est pas trop important) peut être une carte de chaleur (heat plot), réalisée avec `geom_tile()`. Voir plus de détails dans la page [heat plot](heatmaps.fr.qmd).
...
| contact     | élicitation | `date_of_followup`  |             |
|             |             | -                   |             |
|             |             | `                   |             |
|             |             | da te_of_reporting` |             |
|             |             | \< 1 jour           |             |
...
Nous vous proposons ci-dessous un exemple de création d'un tableau visuel pour afficher le suivi des contacts dans les différentes zones d'administration. À la fin, nous le convertirons en tableau de présentation avec le package **formattable** (mais vous pouvez utiliser d'autres packages comme **flextable** - voir [Tableaux de présentation](tables_presentation.fr.qmd)).

La manière de créer un tel tableau dépend de la structure de vos données de suivi des contacts. Utilisez la page [Tableaux descriptifs](tables_descriptive.fr.qmd) pour apprendre à résumer les données à l'aide des fonctions **dplyr**.
...
Comme indiqué sur la page [Heat plots](heatmaps.fr.qmd), vous pouvez créer une matrice de "qui a infecté qui" en utilisant `geom_tile()`.
