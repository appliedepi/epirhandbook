...
Chúng tôi nhập bộ số liệu của các ca bệnh được mô phỏng từ một vụ dịch Ebola. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.vn.qmd) để biết chi tiết).
...
Cũng giống như chương [Bảng mô tả](tables_descriptive.vn.qmd), mục đích sử dụng của bạn sẽ quyết định package nào trong R mà bạn dùng. Chúng tôi trình bày hai chọn lựa để thực hiện các phân tích đơn biến:
...
Để thay đổi giá trị tham chiếu của một biến bất kỳ, hãy đảm bảo cột đó thuộc lớp Factor và chuyển giá trị mong muốn lên vị trí đầu tiên bằng hàm `fct_relevel()` (xem chương [Factors](factors.vn.qmd)). Ở ví dụ bên dưới, chúng tôi lấy biến `age_cat` và đặt nhóm tuổi "20-29" làm giá trị tham chiếu trước khi chuyển số liệu đã sửa đổi vào hàm `glm()`.
