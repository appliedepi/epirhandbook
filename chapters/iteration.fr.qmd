...
Pour rassembler les trois composants (conteneur, séquence et opérations), essayons de tracer une épicurve pour chaque hôpital (voir la page sur les [Courbes épidémiques](epicurves.fr.qmd)).  
...
Gardez à l'esprit que **purrr** est un outil de programmation *fonctionnel*. C'est-à-dire que les opérations qui doivent être appliquées de manière itérative sont regroupées dans des *fonctions*. Consultez la page [Écrire des fonctions](writing_functions.fr.qmd) pour apprendre à écrire vos propres fonctions.  
...
* Le chemin du fichier fourni à `export()` est dynamique grâce à l'utilisation de `str_glue()` (voir la page [Caractères et chaînes de caractères](characters_strings.fr.qmd)) :  
...
Rappelez-vous de la page sur les [Tests statistiques simples](stat_tests.fr.qmd) que `t.test()` peut prendre des entrées dans un format de formule, comme `t.test(colonne numérique ~ colonne binaire)`. Dans cet exemple, nous faisons ce qui suit :    
...
Pour extraire les éléments d'une liste par nom ou par position, vous pouvez utiliser des crochets "[[ ]]" comme décrit dans la page [bases de R](basics.fr.qmd). Ci-dessous, nous utilisons des doubles crochets pour indexer la liste `t.tests_results` et afficher le premier élément qui est le résultat du test t sur `age`.  
