...
Le paquet **incidence2** offre une approche alternative avec des commandes plus simples, mais au moment de la rédaction de ce texte, il était en cours de révision et n'était pas stable. Il sera réintégré dans ce chapitre lorsqu'il sera stable.
...
Les jeux de données sont importés à l'aide de la fonction `import()` du paquetage **rio**. Voir la page [Importation et exportation](importing.fr.qmd) pour les différentes maniéres d'importer des données.  
...
Vous devez *manuellement* contrôler l'agrégation des cas par temps (en semaines, mois, etc.) *et* les intervalles des étiquettes sur l'axe des dates. Ceci doit étre soigneusement géré.  
...
Les barres de l'histogramme peuvent étre colorées par groupe et "empilées". Pour désigner la colonne de regroupement, effectuez les modifications suivantes. Voir la page [ggplot basics](ggplot_basics.fr.qmd) pour plus de détails.  
...
* Pour plus d'informations sur les échelles et les palettes de couleurs, consultez la page sur [les bases de ggplot](ggplot_basics.fr.qmd).  
...
Le meilleur moyen d'ajuster l'ordre dans lequel les barres groupées sont empilées est de classer la colonne de groupage en tant que classe Facteur. Vous pouvez alors désigner l'ordre des niveaux de facteurs (et leurs étiquettes d'affichage). Voir la page sur [facteurs](factors.fr.qmd) ou [ggplot tips](ggplot_tips.fr.qmd) pour plus de détails.  

Avant de réaliser le tracé, utilisez la fonction `fct_relevel()` du paquet **forcats** pour convertir la colonne de regroupement en classe facteur et ajuster manuellement l'ordre des niveaux, comme détaillé dans la page sur les [facteurs](factors.fr.qmd).  
...
Pour en savoir plus sur les légendes et les échelles, consultez la page [ggplot tips](ggplot_tips.fr.qmd). Voici quelques points saillants :  
...
* Voir [cette page](https://rdrr.io/r/base/strptime.html) ou la page [travailler avec des dates](dates.fr.qmd) pour des conseils sur la création d'étiquettes de date.  
...
Voir la page sur les [Moyennes mobiles](moving_average.fr.qmd) pour une description détaillée et plusieurs options. Vous trouverez ci-dessous une option pour calculer des moyennes mobiles avec le package **slider**. Dans cette approche, *la moyenne mobile est calculée dans l'ensemble de données avant le tracé* :  

1) Regroupez les données en comptes si nécessaire (quotidien, hebdomadaire, etc.) (voir la page sur [groupage des données](grouping.fr.qmd)).  
...
ggplot(data = ll_counts_7day) + # commencer avec le nouvel ensemble de données défini ci-dessus 
    ...
    geom_line( # créer une ligne pour la moyenne mobile
      ...
      linewidth = 1) + # largeur de la ligne
...
On peut aussi le superposer à l'épicurve, avec un double axe en utilisant la méthode **cowplot** décrite ci-dessus et dans la page [ggplot tips](ggplot_tips.fr.qmd) :
