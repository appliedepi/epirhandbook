...
Esto puede ser especialmente útil si quieres extraer la última hora de modificación del archivo, por ejemplo si quieres importar la versión más reciente de un archivo. Para ver un ejemplo de esto, consulta la página de [importación y exportación](importing.es.qmd).
...
Mira la página sobre [Informes con R Markdown](rmarkdown.es.qmd) para más detalles. También consulta la documentación de `render()` [aquí](https://rmarkdown.rstudio.com/docs/reference/render.html) o escribiendo `?render`.
...
Consulta la página sobre [importación y exportación](importing.es.qmd) para importar y exportar archivos individuales.

Consulta también la página de [importación y exportación](importing.es.qmd) para conocer los métodos para importar automáticamente el archivo más reciente, basándose en una fecha del nombre del archivo *o* mirando los metadatos del mismo.

Consulta la página sobre [Iteración, bucles y listas](iteration.es.qmd) para ver un ejemplo con el paquete **purrr** demostrando:
