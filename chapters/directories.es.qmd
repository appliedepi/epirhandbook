...
Proporciona la ruta de la carpeta a `path = ` y decide si quieres mostrar sólo un nivel (`recurse = FALSE`) o todos los archivos en todos los subniveles (`recurse = TRUE`). A continuación utilizamos `here()` como abreviatura del proyecto R y especificamos su subcarpeta "data", que contiene los datos de ejemplo de este manual. Lo configuramos para que muestre todos los archivos dentro de "data" y sus subcarpetas.
