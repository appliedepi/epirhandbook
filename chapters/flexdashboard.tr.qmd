---
aliases:
  - /new_pages/flexdashboard.tr.html
---
...
Bu sayfa, **flexdashboard** paketinin temel kullanımını kapsayacaktır. Bu paket, R Markdown çıktısını paneller ve sayfalar içeren bir pano olarak kolayca biçimlendirmenize olanak tanır. Pano içeriği metin, statik şekiller/tablolar veya etkileşimli grafikler olabilir.
...
Simüle edilmiş bir Ebola salgını vakalarının veri setini içe aktarıyoruz. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.tr.qmd) sayfasına bakın).  
...
YAML değerlerinde R kodunu, satır içi kod gibi (backticks içinde r'den önce gelir) veya tırnak işaretleri içine koyarak (Tarih için yukarıya bakın) kullanabilirsiniz. Zorunlu bir YAML parametresi olan `output: `, üretilecek dosyanın türünü belirtir (ör. html_document, pdf_document, word_document veya powerpoint_presentation).
...
Örneğin, hastaneye göre vakaların özet bir tablosunu oluşturmak için aşağıda linelist()'i bir count() komutuyla besliyoruz. Sonuç olarak, tablo knitr::kable()'a yönlendirilir ve sağ tarafında bir kaydırma çubuğu bulunur. Tablonuzu kable() ve kableExtra ile özelleştirme hakkında daha fazla bilgiyi burada bulabilirsiniz. [buradan](https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html).  
...
Shiny etkileşimi ve **leaflet** haritalama widget'ını kullanan bir Shiny-flexdashboard'un sağlıkla ilgili bir örneğini incelemek için  [Geospatial Health Data: R-INLA ve Shiny ile Modelleme ve Görselleştirme çevrimiçi kitabının bu bölümüne](https://www.paulamoraga.com/book-geospatial/sec-dashboardswithshiny.html)  bakabilirsiniz.  
