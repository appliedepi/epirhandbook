...
Veja a seção sobre como importar os dados na página [Campos em branco](importing.pt.qmd#import_missing) para mais detalhes, uma vez que a sintaxe muda de acordo com o tipo de arquivo.
...
Esta é uma função do pacote **tidyr**, que é útil em um [pipeline de limpeza de dados](cleaning.pt.qmd). Se executada com os parênteses vazios, ela remove linhas com *qualquer* um dos campos esteja em brancos. Se o nome das colunas são especificados em parênteses, linhas com campos em branco apenas nestas colunas serão excluídas. Você pode também utilizar a sintaxe do "tidyselect" para especificar as colunas.
...
Então nóstraçamos um gráfico das proporções dos campos em branco como uma linha, de acordo com a semana. A página sobre [básico do ggplot](ggplot_basics.pt.qmd) pode ser utilizada se você não tiver familiaridade com o pacote **ggplot2** de visuzalização de dados.
...
Você pode listar as colunas uma após a outra, ou utilizar as [funções auxiliares do "tidyselect"](cleaning.pt.qmd#clean_tidyselect):  
