...
Las variables factoriales pueden etiquetarse en el Data Step mediante la sentencia `Label`|El etiquetado de los valores puede hacerse convirtiendo la columna en una de tipo Factor y especificando los niveles. Véase la página sobre [Factores](#factors). Los nombres de las columnas no se suelen etiquetar.
...
Los datos se combinan utilizando la sentencia `Merge` en el Data Step. Los datos que se van a combinar deben ordenarse primero mediante el procedimiento `Proc Sort`.|El paquete **dplyr** ofrece algunas funciones para fusionar conjuntos de datos. Para más detalles, consulta la página de [Unir datos](#joining-data).
