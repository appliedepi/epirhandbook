---
aliases:
  - /new_pages/contact_tracing.pt.html
---
...
que instala o pacote se necessário *e* carrega-o para utilização. Também se pode carregar pacotes instalados com `library()` a partir do **R base**. Veja a página [Introdução ao R](basics.pt.qmd) para mais informações sobre pacotes R.
...
Você pode fazer o download de todos os dados de exemplo para este manual a partir da página [Download do manual e dados](data_used.pt.qmd).
...
Na sua forma original nos arquivos passíveis de download, os dados são exibidos como fornecidos pela API do Go.Data (saiba mais sobre [APIs aqui](importing.pt.qmd#import_api)). Para fins de exemplo, aqui vamos limpar os dados para facilitar a leitura nesta página. Se estiver usando uma extensão Go.Data, pode ver instruções completas sobre como recuperar os seus dados [aqui](https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting).

Abaixo, os conjuntos de dados são importados utilizando a função `import()` do pacote **rio**. Veja a página [Importar e exportar](importing.pt.qmd) para várias formas de importação de dados. Utilizamos `here()` para especificar o caminho do arquivo - forneça o caminho específico do arquivo para o seu computador. Em seguida, utilizamos `select()` para selecionar apenas certas colunas dos dados, simplificando para efeitos de demonstração.
...
-   Selecionar apenas algumas colunas específicas, ao mesmo tempo que se
    renomeia uma delas;
-   Atribuir artificialmente o nome "Djembe" às linhas cuja coluna `admin_2_name` estiver faltante, melhorando a clareza da visualização de alguns exemplos;
...
Como demonstrado neste manual [Pirâmides demográficas](age_pyramid.pt.qmd), é possível visualizar a idade e a distribuição por sexo (aqui utilizamos o pacote **apyramid**).
...
Agora podemos traçar este conjunto de dados transformados com a função `age_pyramid()` como antes, mas substituindo `gender` por `category`
(Caso, ou Contato).
...
occ_plot_data <- cases %>% 
  mutate(occupation = forcats::fct_na_value_to_level(occupation, level = "(Missing)"),  # fazer valores NA e vazios uma categoria
...
casos, mas se isso existir, você pode precisar computá-los antes de criar o gráfico (e explorá-los também!).
...
Estes dados podem existir em uma variedade de formatos, como um formato Excel "largo/amplo", com uma linha por contato e uma coluna por dia de acompanhamento. Veja [Pivoteando dados](pivoting.pt.qmd) para descrições de dados "longos" e "largos", e como pivotar dados mais amplos/largos ou mais longos.
...
Para ver todos os valores, podemos executar um rápido `tabyl()` (do **janitor**) ou `table()` (do **R Base**) (ver [Tabelas descritivas](tables_descriptive.pt.qmd)) por 'followup_status' para ver a frequência de cada um dos resultados.
...
Veja mais detalhes na página [Gráfico de calor](heatmaps.pt.qmd).
...
| velocidade  | 24h da            | == "SEEN_NOT_OK"   |              |
| de rastreio | identificação     | OR "SEEN_OK" AND   |              |
...
Abaixo, iremos passar por um exercício de exemplo de criação de uma tabela que mostra o acompanhamento dos contatos ao longo de áreas administrativas. No final, tornaremos essa tabela apresentável com o pacote **formattable** (mas você poderia usar outros pacotes, como **flextable** - veja [Tabelas para apresentação](tables_presentation.pt.qmd)).

A criação de tabelas como esta, dependerá da sua estrutura de dados de rastreamento de contatos. Visite a página [Tabelas descritivas](tables_descriptive.pt.qmd) para aprender como resumir os dados usando as funções do **dplyr**.
...
Como foi discutido na página [Gráficos de calor](heatmaps.pt.qmd), você pode criar uma matriz de "quem infectou quem" utilizando `geom_tile()`.
