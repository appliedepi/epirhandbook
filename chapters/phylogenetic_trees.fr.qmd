---
aliases:
  - /new_pages/phylogenetic_trees.fr.html
---
...
Les données pour cette page peuvent être téléchargées suivant les instructions de la page [Télécharger le manuel et les données](data_used.fr.qmd).
...
L'opérateur **%\<+%** est utilisé pour connecter le tableau de donnees `sample_data` avec le fichier `tree`. L'annotation la plus facile de votre arbre est l'ajout des noms des échantillons aux extrémités, ainsi que la coloration des points d'extrémité et si désiré des branches :
...
p1 <- p + geom_hilight(  # met en évidence le nœud 39 en bleu, "extend =" nous permet de définir la longueur du bloc de couleur
  ...
  fill = "steelblue",
  extend = 0.0017) +  
geom_hilight(            # met en évidence le nœud 37 en jaune
  node = 37,
  fill = "yellow",
  extend = 0.0017) +               
