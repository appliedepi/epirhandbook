...
Bu el kitabında, gerekirse paketi kuran *ve* kullanım için yükleyen **pacman**'dan `p_load()` vurgusu yapılmaktadır. R tabanı'ndan `library()` ile kurulu paketler de yüklenebilir. R paketleri hakkında daha fazla bilgi için [R'ın temelleri](basics.qmd) sayfasına bakabilirsiniz.  
...
Bu yeni sütunun cinsiyete göre dağılımı hakkında daha fazla ayrıntıya sahip olmak için `age_cat_small` ve `gender` sütunları da çapraz tablo haline getirilebilir. [Açıklayıcı tablolar](tables_descriptive.qmd) sayfasında açıklandığı gibi **janitor**'dan `tabyl()` ve *adorn* fonksiyonları kullanılmaktadır. 
