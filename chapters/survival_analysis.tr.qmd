---
aliases:
  - /new_pages/survival_analysis.tr.html
---
...
Bu el kitabında, gerekirse paketi kuran *ve* kullanım için yükleyen **pacman**'dan `p_load()` vurgusu yapılmaktadır. R tabanı'ndan `library()` ile kurulu paketler de yüklenebilir. R paketleri hakkında daha fazla bilgi için [R'ın temelleri](basics.tr.qmd) sayfasına bakabilirsiniz.  
...
Simüle edilmiş bir Ebola salgınından vakaların veri seti içe aktarılmaktadır. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.tr.qmd) sayfasına bakın).    
...
Bu yeni sütunun cinsiyete göre dağılımı hakkında daha fazla ayrıntıya sahip olmak için `age_cat_small` ve `gender` sütunları da çapraz tablo haline getirilebilir. [Açıklayıcı tablolar](tables_descriptive.tr.qmd) sayfasında açıklandığı gibi **janitor**'dan `tabyl()` ve *adorn* fonksiyonları kullanılmaktadır. 
