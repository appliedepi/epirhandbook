...
Pour charger le shapefile, nous utilisons `get_data()` de **appliedepidata**, qui le renvoie comme objet **sf**. Pour lire votre propre shapefile, utilisez `read_sf()` de **sf** avec son chemin de fichier (voir [Importation et exportation](importing.fr.qmd) et [Projets R](r_projects.fr.qmd)).  
```{r}
...
Nous chargeons les points des établissements avec `get_data()`. Nous nettoyons à nouveau les noms des colonnes, puis nous filtrons pour ne garder que les points étiquetés comme "hôpital", "clinique", ou "médecins".  
