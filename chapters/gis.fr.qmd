...
Chargez la `linelist` propre avec `get_data()` de **appliedepidata** (voir [Télécharger le manuel et les données](data_used.fr.qmd)).  
...
Les limites administratives de la Sierra Leone proviennent du Humanitarian Data Exchange (HDX) [site Web ici](https://data.humdata.org/dataset/sierra-leone-all-ad-min-level-boundaries). Le chapitre les charge avec `get_data()` de **appliedepidata**, comme expliqué dans la page [Télécharger le manuel et les données](data_used.fr.qmd).  
...
Pour lire votre propre shapefile, utilisez `read_sf()` de **sf** avec son chemin de fichier (voir [Importation et exportation](importing.fr.qmd) et [Projets R](r_projects.fr.qmd)).  
```{r}
...
Ces données proviennent du package **appliedepidata** (voir [Télécharger le manuel et les données](data_used.fr.qmd)). Elles sont aussi sur HDX (lien [ici](https://data.humdata.org/dataset/sierra-leone-population)). Nous les chargeons avec `get_data()`, puis nous passons le résultat à `clean_names()` pour standardiser la syntaxe des noms de colonnes.   
...
Voici à quoi ressemble le fichier de la population. Faites défiler vers la droite pour voir les colonnes de population "masculine", "féminine" et "totale" de chaque juridiction. Les autres colonnes répartissent la population par groupe d'âge.  
...
Les emplacements des établissements de santé proviennent de HDX [site Web ici](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities). Le chapitre les charge avec `get_data()` de **appliedepidata**, comme expliqué dans la page [Télécharger le manuel et les données](data_used.fr.qmd).   

Nous nettoyons à nouveau les noms des colonnes, puis nous filtrons pour ne garder que les points étiquetés comme "hôpital", "clinique", ou "médecins".  
...
tm_shape(sle_adm3) +               # Shapefiles des limites administratives
  tm_polygons(fill = "#F7F7F7")+    # afficher les polygones en gris clairs
...
tm_shape(linelist_sf) +
  ...
  tm_title("Distribution of Ebola cases")   # donner un titre à la carte
...
Dans cet exemple, nous commençons le `ggplot()` avec la `linelist_adm`, afin de pouvoir appliquer des fonctions de facteurs comme `fct_infreq()` qui ordonne les barres par fréquence (voir la page sur les [Facteurs](factors.fr.qmd) pour des conseils).  
...
tm_view(set_view = c(-13.2284, 8.4699, 13), # ajuster le zoom (coordonnées du centre, zoom)
...
Nous pouvons également explorer combien de cas sont situés à moins de 2 km (~30 minutes) de distance de marche de l'établissement de santé le plus proche.

*Remarque : sf traite un objet lat/long comme sphérique. Une distance passée à `st_buffer()` ou `st_distance()` est donc en mètres. Pour les calculs de surface ou de distance précis, reprojetez vers un système de coordonnées projeté. Choisissez ce système selon le lieu et la mesure, par exemple la zone UTM locale.*  
...
**Tout d'abord**, créez un tampon de 2 km autour de chaque établissement de santé avec `st_buffer()` de **sf**. L'objet est en lat/long, donc **sf** mesure cette distance en mètres sur la sphère.  

```{r, warning=F, message=F}
sle_hf_2k <- sle_hf %>%
  st_buffer(dist = 2000)     # mètres : sf utilise une géométrie sphérique pour les objets lat/long 
```

Ci-dessous, nous traçons les zones tampons elles-mêmes, avec les points des établissements de santé :  
...
linelist_sf_hf_2k <- linelist_sf_hf %>%
  st_join(sle_hf_2k, join = st_intersects, left = TRUE) %>%          # une ligne par cas et par zone tampon qui le contient ; osm_id.y vaut NA si aucune ne le contient
  select(case_id, osm_id.x, nearest_clinic, amenity.x, osm_id.y) %>%
  distinct(case_id, .keep_all = TRUE)                                # une ligne par cas, donc un cas est hors zone seulement si aucune zone tampon ne le contient
```

Maintenant, nous pouvons compter les résultats : `r nrow(linelist_sf_hf_2k[is.na(linelist_sf_hf_2k$osm_id.y),])` sur 1000 cas ne recoupent aucun tampon (cette valeur est manquante), et vivent donc à plus de 30 minutes de marche de l'établissement de santé le plus proche.
...
1) Les données de population `sle_adm3_pop` sont jointes à l'aide d'un `left_join()` de **dplyr** sur la base des valeurs communes à la colonne `admin3pcod` dans le dataframe `case_adm3`, et à la colonne `adm_pcode` dans le dataframe `sle_adm3_pop`. Voir la page [Joindre des données](joining_matching.fr.qmd).  
...
tm_shape(case_adm3_sf) +                # visualiser lespolygons
  tm_polygons("case_10kpop",            # colorier en fonction du colonnes contenant le pourcentages des cas
              fill.scale = tm_scale_intervals(
                ...
                values = "brewer.purples")  # utiliser une palette de couleurs violettes
              ) +
...
Ce code télécharge les tuiles actuelles depuis le serveur de tuiles d'OpenStreetMap, il a donc besoin d'une connexion internet. Cela est autorisé lorsque vous consultez vous-même une carte. Ne l'exécutez pas dans une compilation automatisée. N'enregistrez pas les tuiles pour une utilisation hors ligne. Consultez la [politique d'utilisation des tuiles d'OpenStreetMap](https://operations.osmfoundation.org/policies/tiles/).  

Le manuel affiche des images des cartes que ce code produit. Elles ont été créées le 2 septembre 2026 afin que la page se construise sans connexion internet. Données cartographiques © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
Données cartographiques © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
Maintenant, nous introduisons simplement le facettage via **ggplot2** à la carte thermique de densité. `facet_wrap()` est appliqué, en utilisant les nouvelles colonnes comme lignes. Nous avons fixé le nombre de colonnes de facettes à 4 pour plus de clarté.  
...
Nous pouvons quantifier les relations d'adjencence entre les polygones des régions administratives dans les données `sle_adm3_dat` que nous avons utilisées avec le package **spdep**. Nous allons spécifier une contiguïté *queen*, ce qui signifie que les voisins possèdent au moins un segment de frontière commune. Dans notre cas, avec des polygones irréguliers, la distinction est triviale, mais dans certains cas, le choix entre la reine et la tour peut ne pas etre evident
...
La matrice affiché ci-dessus montre les relations entre les 9 régions de nos données `sle_adm3_dat`. Un score de 0 indique que deux régions ne sont pas voisines, tandis que toute valeur différente de 0 indique une relation de voisinage. Les valeurs de la matrice sont normalisées afin que chaque région ait un poids total  par ligne egale à 1.
...
Pour donner un exemple, testons si la configuration spatiale des cas d'Ebola de l'épidémie simulée est corrélée à la configuration spatiale de la population. Pour commencer, nous devons avoir une variable `population` dans nos données `sle_adm3_dat`. Nous pouvons utiliser la variable `total` du dataframe `sle_adm3_pop` que nous avons chargé précédemment.
...
Le résultat ci-dessus montre que la statistique L de Lee pour nos deux variables était `r round(lee_test$estimate[1],2)`, ce qui indique une faible corrélation négative. Cela confirme notre évaluation visuelle selon laquelle le schéma des cas et la population ne sont pas liés l'un à l'autre, et fournit la preuve que le schéma spatial des cas n'est pas strictement le résultat de la densité de population dans les zones à haut risque.
