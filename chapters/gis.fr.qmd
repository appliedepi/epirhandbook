...
À des fins de démonstration, nous travaillerons avec un échantillon aléatoire de 1000 cas provenant du dataframe `linelist` de l'épidémie d'Ebola simulée (d'un point de vue computationnel, travailler avec moins de cas est plus facile à afficher dans ce manuel). Si vous souhaitez suivre le processus, <a href='https://github.com/appliedepi/epirhandbook_eng/raw/master/data/case_linelists/linelist_cleaned.rds' class='download-button'>cliquez pour télécharger la linelist "propre"</a> (en tant que fichier .rds).  
...
Importez les données avec la fonction `import()` du paquet **rio** (elle gère de nombreux types de fichiers comme .xlsx, .csv, .rds - voir la page [Importation et exportation](importing.fr.qmd) pour plus de détails).  
...
```{r, eval=F}
# importer le jeu de données linelist nettoyé
linelist <- import("linelist_cleaned.rds", trust = TRUE)
```

Pour un fichier .rds, `import()` émet un avertissement sauf si vous passez `trust = TRUE`, qui indique que vous faites confiance à l'origine du fichier. Utilisez-le seulement pour des fichiers que vous avez créés vous-même ou qui viennent d'une source de confiance.  
...
Pour importer un fichier shapefile, nous utilisons la fonction `read_sf()` de **sf**. Le chemin du fichier est fourni via `here()`. - Dans notre cas, le fichier se trouve dans notre projet R dans les sous-dossiers "data", "gis" et "shp", avec le nom de fichier "sle_adm3.shp" (voir les pages sur [Importation et exportation](importing.fr.qmd) et [Projets R](r_projects.fr.qmd) pour plus d'informations). Vous devrez fournir votre propre chemin de fichier.  
```{r, echo=F}
...
Ces données peuvent être téléchargées sur HDX (lien [ici](https://data.humdata.org/dataset/sierra-leone-population)) ou via notre package R **appliedepidata** comme expliqué [dans la page Télécharger le manuel et les données](data_used.fr.qmd). Nous utilisons `import()` pour charger le fichier .csv. Nous passons également le fichier importé à `clean_names()` pour standardiser la syntaxe des noms de colonnes.   
