...
```{r, echo=FALSE}
# The handbook renders with no internet access, so this page reads a saved copy of the
# basemap that the code above fetches: OpenStreetMap tiles, fetched 2026-09-02, ODbL,
# (c) OpenStreetMap contributors. Re-create the file with data/gis/osm_basemap.R
map <- readRDS(here::here("data", "gis", "osm_basemap.rds"))
```

Le manuel affiche une copie enregistrée de ce fond de carte, téléchargée une fois avec le code ci-dessus le 2 septembre 2026, afin que la page se construise sans connexion internet. Données cartographiques © les contributeurs d'OpenStreetMap.  
