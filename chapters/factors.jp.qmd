...
既存の `days_onset_hosp` （症状発現から入院までの日数）列を用いて、各行を複数カテゴリのいずれかに分類し、新しく `delay_cat` 列を作成します。**dplyr** の `case_when()` を使い、左側には各行に適用する論理基準を書き、右側には新しい列 `delay_cat` の値として返される値を書きます。`case_when()` については、[データクリーニングと主要関数](cleaning.jp.qmd) の章で詳しく説明しています。
...
同様に、棒グラフを作成する場合も、この順序で x 軸の値が表示されます（R で最も一般的なデータ視覚化のパッケージである **ggplot2** の詳細については、[ggplot の基礎](ggplot_basics.jp.qmd) の章を参照ください）。
...
以下に、 `ggplot()` で 2 つのプロットを作成します（プロットの作成についての詳細は、[ggplot の基礎](ggplot_basics.jp.qmd) の章を参照ください）。最初の例では、 `delay_cat` の列がプロットの x 軸にプロットされ、`linelist` データセットにあるデフォルトの順序でレベルが表示されます。二つ目の例では、`fct_relevel()` の内で `delay_cat` の列を指定してプロット内で順序を変更しています。
...
プロットの凡例<u>のみ</u>を反転し、実際の因子レベルは反転させたくない場合は、`guides()` を使用して反転できます（詳しくは、[ggplot のヒント](ggplot_tips.jp.qmd) の章を参照ください）。
...
`fct_recode()` を使い、レベルの表示を手動で調整することができます。これは、**dplyr** パッケージの `recode()`（[データクリーニングと主要関数](cleaning.jp.qmd) の章を参照）に似ていますが、`fct_recode()` を使用すると新たな因子レベルを作成できます。因子型の列に `recode()` を使用する場合、新しく因子として作成された値は、あらかじめ許容レベルとして設定されていない限り、拒否されます。
...
詳細については、[記述統計表の作り方](tables_descriptive.jp.qmd) の章、scale_discrete について説明した [こちら](https://ggplot2.tidyverse.org/reference/scale_discrete.html) のページ、または count() について説明した [こちら](https://dplyr.tidyverse.org/reference/count.html) のページを参照してください。また、[接触者の追跡](contact_tracing.jp.qmd) の章では他の例を紹介しています。

## 疫学週

疫学週作成方法については、[データのグループ化](grouping.jp.qmd) の章で詳しく説明していますので、そちらをご覧ください。また、[日付型データ](dates.jp.qmd) の章では、疫学週の作成方法と書式を紹介していますので、あわせてご覧ください。

### プロット内での疫学週 {.unnumbered}

疫学週を作成してプロットに表示することが目的である場合は、[データのグループ化](grouping.jp.qmd) の章で説明されているように、**lubridate** パッケージの `floor_date()` を使用して簡単に行うことができます。その際作成される値は、YYYY-MM-DD 形式の日付型になります。`floor_date()` で作成された列をプロットで使用する場合は、日付が正しく順序付けられているため、因子レベルの調整や因子型への変換を気にする必要はありません。以下に表示されている、発症日の `ggplot()` ヒストグラムを参照してください。

`floor_date()` で作成された列をプロットする際は、`scale_x_date()` を使用して x 軸上の日付の<u>表示形式</u>を調整できます。詳細については、[流行曲線（エピカーブ）](epicurves.jp.qmd) の章を参照してください。

`scale_x_date()` の `date_labels =` 引数に表示したい形式を、"strptime" 形式で指定ください。[日付型データ](dates.jp.qmd) の章で詳しく説明されていますが、"strptime" 形式では "%" 記号を使用します。また、"％Y" を使用して 4 桁の年を表し、"％W" または "％U" を使用して週番号（それぞれ月曜日または日曜日の週）を表します。
...
まず、**base** R の `format()` を使用して、日付表示を YYYY-MM-DD から YYYY-Www 形式に変換します（詳しくは、[日付型データ](dates.jp.qmd) の章を参照ください）。表示形式の変換時に、データ型は文字型に変換されます。次に、`factor()` を使用して文字型から因子型に変換します。
...
**aweek** パッケージの詳細は、[日付型データ](dates.jp.qmd) の章を参照してください。また、`date2week()` と逆の働きをする `week2date()` についても触れられています。
