...
Il s'agit d'une fonction de **tidyr** utile pour [nettoyer des données dans un pipeline](#cleaning_data). Si elle est exécutée sans argument, elle supprime également les lignes dont *au moins une* valeur est manquante. Mais si des noms de colonnes sont spécifiés comme arguments, seules les lignes avec des valeurs manquantes dans ces colonnes seront supprimées.   
