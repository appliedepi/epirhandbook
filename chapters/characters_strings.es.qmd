---
aliases:
  - /new_pages/characters_strings.es.html
---
...
En esta página haremos referencia de vez en cuando a la lista limpia de casos de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles).  
...
Si estás intentando "enrollar" valores en una columna del dataframe, por ejemplo, combinar valores de varias filas en una sola fila pegándolos con un separador, consulta la sección ["combinación de valores"](deduplication.es.qmd#str_rollup) de la página de **De-duplicación**. 
...
Para dividir una cadena basada en un patrón, utiliza `str_split()`. Esta función evalúa las cadenas y devuelve una `list` de vectores de caracteres formada por los valores recién divididos. 
...
Asumiendo que los datos son enlazados con pipe en `separate()`, primero proporciona la columna a separar. A continuación, proporcione `into = ` como un vector `c()` que contiene los nombres de las *nuevas* columnas, como se muestra a continuación. 
...
Utiliza `toTitleCase()` del paquete de **tools** para lograr una capitalización más matizada (palabras como "to", "the" y "of" no se escriben en mayúsculas). 
...
Al igual que otras funciones `str`, existe una versión "\_all" (`str_locate_all()`) que devolverá las posiciones de *todas* las instancias del patrón dentro de cada cadena. La salida es una `list`. 
...
`str_extract_all()` devuelve los patrones coincidentes en sí mismos, lo que resulta muy útil cuando se han ofrecido varios patrones mediante condiciones "OR". Por ejemplo, buscando en el vector de cadenas de ocupaciones (véase la pestaña anterior) *cualquiera* "teach", "prof" o "tutor". 

`str_extract_all()` devuelve una `list` que contiene *todas las coincidencias* de cada cadena evaluada. Mira a continuación cómo la ocupación 3 tiene dos coincidencias de patrón dentro de ella. 
