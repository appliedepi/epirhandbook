...
Trong chương này, chúng ta sẽ sử dụng một bộ dữ liệu giả định về các trường hợp ghi nhận sốt rét hàng ngày, theo cơ sở y tế và nhóm tuổi. Tải trực tiếp bằng `appliedepidata::get_data(name = "malaria_facility_count_data")` (xem trang [Tải sách và dữ liệu](data_used.vn.qmd) để biết chi tiết).
...
Trong phần cuối của chương này, chúng ta cũng sẽ sử dụng tập dữ liệu mô phỏng về các trường hợp từ một vụ dịch Ebola. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.vn.qmd) để biết chi tiết).
...
Trong chương này, chúng ta sẽ sử dụng bộ dữ liệu ca bệnh `linelist` (xem mục [Chuẩn bị](pivoting.vn.qmd)), với mỗi trường hợp nằm trên một hàng.
