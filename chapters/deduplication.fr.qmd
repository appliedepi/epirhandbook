---
aliases:
  - /new_pages/deduplication.fr.html
---
...
Utilisez les boîtes de filtre en haut du tableau pour examiner les rencontres pour chaque personne.  
...
Pour garder que les lignes uniques d'un tableau de données, utilisez `distinct()` de **dplyr** (comme démontré dans la page [CNettoyage de données et fonctions essentielles](cleaning.fr.qmd)). Les lignes qui sont dupliquées sont enlevées de sorte que seule la première ligne est retenue. Par défaut, la première ligne correspond au plus grand `rownumber` (ordre des lignes de haut en bas). Seules les lignes uniques sont retenues.  
...
Vous pouvez également spécifier des colonnes qui seront la base de la déduplication. Ainsi, la déduplication ne s'applique qu'aux lignes qui sont des duplications dans les colonnes spécifiées. A moins que vous ne définissiez `.keep_all = TRUE`, toutes les colonnes non mentionnées seront supprimées.    
...
Pour ne retourner que les éléments dupliqués, vous pouvez utiliser des crochets pour sous-titrer le vecteur original : 
...
Voir la page [Joindre des données](joining_matching.fr.qmd) pour une explication de cette méthode. La section sur l'Appariement Probabiliste contient un exemple d'application de ces algorithmes pour comparer un tableau de données à *soi-même*, effectuant ainsi une déduplication probabiliste.    
