...
```{r, fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "gis_dot_density.png"))
```
...
```{r, fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "gis_proportional_symbols.png"))
```
...
Bu kod parçası, analizler için gereken paketlerin yüklenmesini gösterir. Bu el kitabında, gerekirse paketi kuran *ve* kullanım için yükleyen **pacman**'dan `p_load()` vurgusunu yapıyoruz. R tabanından `library()` ile kurulu paketleri de yükleyebilirsiniz. R paketleri hakkında daha fazla bilgi için [R basics](basics.tr.qmd) sayfasına bakabilirsiniz.
...
Gösteri amacıyla, simüle edilmiş Ebola salgını "satır listesi" veri çerçevesinden rastgele 1000 vakalık bir örneklemle çalışacağız (hesaplamalı olarak, daha az vakayla çalışmak bu el kitabında daha kolaydır). Temizlenmiş `linelist` **appliedepidata** paketinden gelir.
...
Temizlenmiş `linelist` verisini **appliedepidata** paketindeki `get_data()` ile yükleyin ([El kitabını ve verileri indirin](data_used.tr.qmd) sayfasına bakın).

```{r, echo=F}
# temizlenmiş vaka satır listesini içe aktarmak
linelist <- appliedepidata::get_data(name = "linelist_cleaned_rds")
# fix the random sample below, so every language build shows the same numbers
set.seed(1)
```
...
Sierra Leone için tüm idari sınırlar İnsani Veri Değişimi'nden (HDX) [web sitesi burada](https://data.humdata.org/dataset/sierra-leone-all-ad-min-level-boundaries) gelmektedir. Bu bölüm, [El kitabını ve verileri indir](data_used.tr.qmd) sayfasında açıklandığı gibi, bunları **appliedepidata** paketindeki `get_data()` ile yükler.
...
Kendi şekil dosyanızı okumak için **sf** paketindeki `read_sf()` fonksiyonunu dosya yoluyla kullanın (daha fazla bilgi için [İçe aktarma ve dışa aktarma](importing.tr.qmd) ve [R projeleri](r_projects.tr.qmd) sayfalarına bakabilirsiniz).

```{r}
sle_adm3_raw <- appliedepidata::get_data(name = "sle_adm3")
```
...
Bu veriler **appliedepidata** R paketimizden gelir ([El kitabını ve verileri indirin](data_used.tr.qmd) sayfasına bakın). Veriler HDX'te de bulunur (bağlantı [buradan](https://data.humdata.org/dataset/sierra-leone-population)). Verileri `get_data()` ile yükler, ardından sütun adı sözdizimini standartlaştırmak için sonucu `clean_names()` öğesine iletiriz.
...
Popülasyon dosyası böyle görünüyor. Her yetki alanının `male`, `female` ve `total` nüfus sütunlarını görmek için sağa kaydırın. Diğer sütunlar nüfusu yaş grubuna göre ayırır.
...
Sağlık tesislerinin konumları HDX'ten [buradan](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities) gelmektedir. Bu bölüm, [el kitabını ve verileri indir](data_used.tr.qmd) sayfasında açıklandığı gibi, bunları **appliedepidata** paketindeki `get_data()` ile yükler.

Sütun adlarını tekrar temizliyoruz ve ardından yalnızca "hastane", "klinik" veya "doktorlar" olarak etiketlenmiş noktaları tutmak için filtreliyoruz.
...
tm_shape(linelist_sf) + tm_dots(size=0.08, fill='blue')
```
...
tm_shape(sle_adm3) +               # yönetici sınırları şekil dosyası
  tm_polygons(fill = "#F7F7F7")+    # çokgenleri açık gri olarak göster
...
tm_shape(linelist_sf) +
  tm_dots(size=0.08, fill='blue', fill_alpha = 0.5) +
  tm_title("Distribution of Ebola cases")   # haritaya isim vermek
...
`sle_adm3`'deki tüm sütunlar satır listesine eklendi! Artık her vaka, içinde bulunduğu idari seviyeleri detaylandıran sütunlara sahiptir. Bu örnekte, yeni sütunlardan yalnızca ikisini (yönetici düzeyi 3) tutmak istiyoruz, bu nedenle eski sütun adlarını ve yalnızca iki ek ilgiyi `select()` yapıyoruz: 
...
Bu örnekte, ggplot()'a 'linelist_adm' ile başlıyoruz, böylece çubukları frekansa göre sıralayan 'fct_infreq()' gibi faktör fonksiyonlarını uygulayabiliriz (ipuçları için [Faktörler](factors.tr.qmd) sayfasına bakınız).
...
tm_shape(sle_hf) +                    # klinik tesisleri büyük siyah noktalarla çizin
  tm_dots(size=0.3, fill='black', fill_alpha = 0.4) +
  tm_text("name") +                   # tesis adı ile üzerine bindirme
tm_view(set_view = c(-13.2284, 8.4699, 13), # yakınlaştırmayı ayarla (merkez kodları, yakınlaştırma)
        set_zoom_limits = c(13,14))+
tm_title("Cases, colored by nearest clinic")
```


### Tamponlar {.unnumbered}

Ayrıca en yakın sağlık kuruluşuna 2 km (~30 dakika) yürüme mesafesinde kaç vaka bulunduğunu da keşfedebiliriz.

*Not: sf, enlem/boylam nesnesini küresel olarak ele alır. Bu nedenle `st_buffer()` veya `st_distance()` fonksiyonuna verdiğiniz mesafe metre cinsindendir. Alan veya hassas mesafe hesapları için bir projeksiyon koordinat sistemine yeniden yansıtın. Bu sistemi konuma ve ölçüme göre seçin, örneğin yerel UTM dilimini.*
...
**İlkin**, her sağlık tesisinin çevresinde 2 km'lik bir tampon oluşturmak için **sf**'teki `st_buffer()` fonksiyonunu kullanın. Nesne enlem/boylam cinsindendir, bu nedenle **sf** bu mesafeyi küre üzerinde metre olarak ölçer.

```{r, warning=F, message=F}
sle_hf_2k <- sle_hf %>%
  st_buffer(dist = 2000)     # metre: sf, enlem/boylam nesneleri için küresel geometri kullanır 
```
...
linelist_sf_hf_2k <- linelist_sf_hf %>%
  st_join(sle_hf_2k, join = st_intersects, left = TRUE) %>%          # vakanın içine düştüğü her arabellek için bir satır; hiçbirine düşmezse osm_id.y NA olur
  select(case_id, osm_id.x, nearest_clinic, amenity.x, osm_id.y) %>%
  distinct(case_id, .keep_all = TRUE)                                # vaka başına bir satır, böylece bir vaka yalnızca hiçbir arabelleğe düşmezse dışarıda kalır
```

Şimdi sonuçları sayabiliriz: `r nrow(linelist_sf_hf_2k[is.na(linelist_sf_hf_2k$osm_id.y),])` 1000 vakadan herhangi biri arabellekle kesişmedi (bu değer eksik) ve bu yüzden en yakın sağlık kuruluşundan 30 dakikadan fazla yürüyün.
...
tm_shape(linelist_sf_hf) +
  tm_dots(size=0.08, fill='nearest_clinic') +
...
tm_view(set_view = c(-13.2284,8.4699, 13), set_zoom_limits = c(13,14))+

# Başlık ekleyin
tm_title("Cases by clinic catchment area")
...
1) `sle_adm3_pop` popülasyon verileri, `case_adm3` veri çerçevesindeki `admin3pcod` sütunundaki ve `sle_adm3_pop` içindeki `adm_pcode` sütunundaki ortak değerler temelinde **dplyr**'den bir `left_join()` kullanılarak birleştirilir. [Birleştirme verileri](joining_matching.tr.qmd) ile ilgili sayfaya bakın.
2) Yalnızca yararlı sütunları tutmak için yeni veri çerçevesine `select()` uygulanır - `total` toplam nüfustur
...
tm_shape(case_adm3_sf) +                # polygonlqrı çiz
  tm_polygons("case_10kpop",            # vaka oranını içeren sütuna göre renklendir
              fill.scale = tm_scale_intervals(
                ...
                values = "brewer.purples")  # mor renk paleti kullanın
              ) +
...
Ardından, **OpenStreetMap** paketinden ([dokümantasyon](https://www.rdocumentation.org/packages/OpenStreetMap/versions/0.3.4/topics/openmap)) `openmap()` fonksiyonunu kullanarak tanımladığımız `harita` nesnesini oluşturuyoruz. Aşağıdakileri sağlıyoruz:

* `upperLeft` ve `lowerRight` altlık harita döşemesinin sınırlarını belirten iki koordinat çifti
...
Bu kod, güncel harita döşemelerini OpenStreetMap döşeme sunucusundan indirir, bu nedenle internet bağlantısı gerekir. Haritayı kendiniz görüntülediğinizde buna izin verilir. Kodu otomatik bir derlemede çalıştırmayın. Döşemeleri çevrimdışı kullanım için kaydetmeyin. [OpenStreetMap döşeme kullanım politikasına](https://operations.osmfoundation.org/policies/tiles/) bakın.  

El kitabı, bu kodun ürettiği haritaların görüntülerini gösterir. Görüntüler, sayfanın internet bağlantısı olmadan oluşturulabilmesi için 2 Eylül 2026 tarihinde hazırlandı. Harita verileri © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
```{r, fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "gis_basemap_01_mercator.png"))
```

Harita verileri © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
```{r, fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "gis_basemap_02_latlon.png"))
```

Harita verileri © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  

Daha fazla bilgi için Eğitimlere [buradan](http://data-analytics.net/cep/Schedule_files/geospatial.html) ve [buradan](https://www.rdocumentation.org/packages/OpenStreetMap/versions/0.3.4/topics/autoplot.OpenStreetMap) ulaşabilirsiniz.
...
```{r, fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "gis_basemap_03_points.png"))
```
...
```{r, fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "gis_basemap_04_density.png"))
```
...
Şimdi, yoğunluk ısı haritasına **ggplot2** aracılığıyla yüzey oluşturmayı tanıtıyoruz. Yeni sütun satır olarak kullanılarak `facet_wrap()` uygulanır. Anlaşılır olması için yüzey sütunlarının sayısını 4 olarak ayarladık.
...
```{r, fig.align = "center", echo=F}
knitr::include_graphics(here::here("images", "gis_basemap_05_density_month.png"))
```
...
Yukarıda yazdırılan matris, `sle_adm3_dat` verilerimizdeki 9 bölge arasındaki ilişkileri göstermektedir. 0 puan, iki bölgenin komşu olmadığını gösterirken, 0 dışında herhangi bir değer, bir komşu ilişkisini gösterir. Matristeki değerler, her bölgenin toplam satır ağırlığı 1 olacak şekilde ölçeklendirilir.
...
'moran.test()' fonksiyonunun çıktısı bize `r round(moran_i$estimate[1],2)` Moran I istatistiğini gösterir. Bu, verilerimizde uzamsal otokorelasyonun varlığını gösterir - özellikle, benzer sayıda Ebola vakası olan bölgelerin birbirine yakın olması muhtemeldir. "moran.test()" tarafından sağlanan p değeri, uzaysal otokorelasyonun olmadığı sıfır hipotezi altındaki beklentiyle karşılaştırılarak oluşturulur ve resmi bir hipotez testinin sonuçlarını bildirmeniz gerekiyorsa kullanılabilir.
...
Bir örnek vermek gerekirse, simüle edilmiş salgındaki Ebola vakalarının mekansal düzeninin, nüfusun mekansal düzeniyle ilişkili olup olmadığını test edelim. Başlamak için, `sle_adm3_dat` verilerimizde bir `population` değişkenine ihtiyacımız var. Daha önce yüklediğimiz `sle_adm3_pop` veri çerçevesinden `total` değişkenini kullanabiliriz.
...
cases_map <- tm_shape(sle_adm3_dat) + tm_polygons("cases") + tm_title("Cases")
pop_map <- tm_shape(sle_adm3_dat) + tm_polygons("population") + tm_title("Population")
