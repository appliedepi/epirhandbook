...
Şekil dosyasını yüklemek için **appliedepidata** paketindeki `get_data()` fonksiyonunu kullanırız; fonksiyon dosyayı bir **sf** nesnesi olarak döndürür. Kendi şekil dosyanızı okumak için **sf** paketindeki `read_sf()` fonksiyonunu dosya yoluyla kullanın (daha fazla bilgi için [İçe aktarma ve dışa aktarma](importing.tr.qmd) ve [R projeleri](r_projects.tr.qmd) sayfalarına bakabilirsiniz).
...
Popülasyon dosyası böyle görünüyor. Her yetki alanında `male` nüfus, `female` nüfus, `total` nüfus ve yaş grubuna göre nüfus dökümü sütunlarının nasıl olduğunu görmek için sağa kaydırın.
...
Tesis noktalarını `get_data()` ile yüklüyoruz. Sütun adlarını tekrar temizliyoruz ve ardından yalnızca "hastane", "klinik" veya "doktorlar" olarak etiketlenmiş noktaları tutmak için filtreliyoruz.
