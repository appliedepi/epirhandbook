...
Bir .rds dosyası için `import()`, dosyanın kaynağına güvendiğinizi belirten `trust = TRUE` argümanı verilmezse uyarı verir. Bunu yalnızca kendi oluşturduğunuz veya güvendiğiniz bir kaynaktan aldığınız dosyalar için kullanın.  
...
Ardından, **OpenStreetMap** paketinden ([dosyalar](https://www.rdocumentation.org/packages/OpenStreetMap/versions/0.3.4/topics/openmap) `openmap()` fonksiyonunu kullanarak tanımladığımız `harita` nesnesini oluşturuyoruz). Aşağıdakileri sağlıyoruz:
