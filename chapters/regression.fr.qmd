...
Pour la régression des risques proportionnels de Cox, voir la page [Analyse d'enquête](survival_analysis.fr.qmd).
...
Tout comme dans la page sur les [Tableaux descriptifs](tables_descriptive.fr.qmd), votre cas d'utilisation déterminera le package R que vous utiliserez. Nous vous présentons deux options pour effectuer une analyse univariée :  
...
2) Combinez les résultats de ces modèles avec un tableau de comptage. Ci-dessous, nous créons un tableau de comptage croisé avec la fonction `tabyl()` de **janitor**, comme indiqué dans la page [Tableaux descriptifs](tables_descriptive.fr.qmd).  
...
Voici à quoi ressemble le dataframe combiné, affiché joliment comme une image avec une fonction de **flextable**. La section [Tableau pour la presentationn](tables_presentation.fr.qmd) explique comment personnaliser de tels tableaux avec **flextable**, ou vous pouvez utiliser de nombreux autres packages tels que **knitr** ou **GT**.  
...
Pour exécuter les modèles sur plusieurs variables d'explicative afin de produire des odds ratios univariés (c'est-à-dire sans contrôle des autres variables), vous pouvez utiliser l'approche ci-dessous. Elle utilise `str_c()` de **stringr** pour créer des formules univariées (voir [Caractères et chaînes de caractères](characters_strings.fr.qmd)), exécute la régression `glm()` sur chaque formule, passe chaque sortie `glm()` à `tidy()` et enfin rassemble toutes les sorties du modèle avec `bind_rows()` de **tidyr**. Cette approche utilise `map()` du packages **purrr** pour itérer - voir la page sur [Iteration, loops, and lists] pour plus d'informations sur cet outil.  
...
Voici à quoi ressemble le dataframe. Voir la page sur les [Tableau pour la presentationn](tables_presentation.fr.qmd) pour des idées sur la façon de convertir ce tableau en une jolie sortie HTML (par exemple avec **flextable**).  
...
Consultez la page sur [Les bases de ggplot](ggplot_basics.fr.qmd) si vous n'êtes pas familier avec le package de traçage **ggplot2**.  
