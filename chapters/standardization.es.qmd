---
aliases:
  - /new_pages/standardization.es.html
---

# Tasas estandarizadas {#standardised-rates}

Esta página te mostrará cómo estandarizar un resultado, como las hospitalizaciones o la mortalidad, por características como la edad y el sexo.

Esta página utiliza el paquete **PHEindicatormethods**.
...
Consulta la página de [descargando el manual y los datos](data_used.es.qmd) para obtener instrucciones sobre cómo descargar todos los datos de ejemplo del manual. Puedes cargar los datos de la página de estandarización directamente en R con las siguientes llamadas a `appliedepidata::get_data()`:
...
En primer lugar, combinamos los datos de las poblaciones de los países, los pivotamos "largo" y realizamos una pequeña limpieza. Para más detalles, consulta la página sobre [pivotar datos](pivoting.es.qmd).
...
Actualmente, los valores de la columna age_cat5 del dataframe `standard_pop_data` contienen la palabra "years" y "plus", mientras que los del dataframe `country_data` no. Tendremos que hacer coincidir los valores de la categoría de edad. Usamos `str_replace_all()` del paquete **stringr**, como se describe en la página sobre [Caracteres y cadenas](characters_strings.es.qmd), para reemplazar estos patrones sin espacio `""`.

Además, la función `phe_dsr()` espera que en la población estándar, la columna que contiene los recuentos se llame `"pop"`. Así que cambiaremos el nombre de esa columna.
...
standard_pop_clean <- standard_pop_data %>%
     ...
     rename(pop = WorldStandardPopulation)   # cambia el nombre de la columna por "pop", ya que es lo que espera phe_dsr()
```
...
Calculamos las tasas estandarizadas con el paquete **PHEindicatormethods**. Este paquete permite calcular las tasas estandarizadas tanto directa como indirectamente. Mostraremos ambos métodos.
