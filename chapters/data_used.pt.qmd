...
Para acesso rápido, **clique com o botão direito** [neste link](https://github.com/appliedepi/epirhandbook/raw/main/offline_long/index.pt.html) **e selecione "Salvar link como"**.  
...
A abordagem mais fácil para baixar todos os dados é instalar nosso pacote R **appliedepidata**. Ele contém uma função `get_data()` que salva todos os dados de exemplo em uma pasta de sua escolha em seu computador.  

Para instalar nosso pacote R **appliedepidata**, execute o seguinte código. Este pacote não está no CRAN, então use a função `p_install_gh()` para instalá-lo. A entrada faz referência à nossa organização Github ("*appliedepi*") e o pacote **appliedepidata**.  
...
A seguir, use a função do pacote `get_data()` para baixar os dados de exemplo para o seu computador. Execute `appliedepidata::save_data()` para obter *todos* os dados de exemplo ou forneça um nome de arquivo específico e extensão entre aspas para recuperar apenas um arquivo.  
...
Se desejar, você pode revisar os arquivos de dados originais na **[pasta "data"](https://github.com/appliedepi/appliedepidata)** do repositório Github do pacote **appliedepidata**.  
...
Se você seguir a Opção 1 para baixar todos os dados de exemplo (por meio de nosso pacote R **appliedepidata**), todos os shapefiles serão incluídos.  
