...
Para carregar a árvore filogenética "Shigella_tree.txt" em R:

```{r, eval=F}
tree <- appliedepidata::get_data(name = "Shigella_tree")
```

Para carregar o "sample_data_Shigella_tree.csv" com informações adicionais sobre cada amostra em R:

```{r, eval=F}
sample_data <- appliedepidata::get_data(name = "sample_data_Shigella_tree")
```

Para ver a nova árvore de subconjunto criada:
