---
aliases:
  - /new_pages/ggplot_tips.es.html
---

# Consejos de ggplot {#ggplot-tips}

En esta página cubriremos consejos y trucos para hacer que tus ggplots sean nítidos y elegantes. Consulta la página sobre [Conceptos básicos de ggplot](ggplot_basics.es.qmd) para conocer los fundamentos.
...
Para esta página, importamos el conjunto de datos de casos de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles).
...
Los matices de las escalas de fechas se tratan más ampliamente en la página de [curvas epidémicas](epicurves.es.qmd).
...
Para demostrar cómo ajustar una escala de colores continua, utilizaremos unos datos de la página de [Rastreo de contactos](contact_tracing.es.qmd) que contiene las edades de los casos y de sus casos de origen.
...
Aquí hay un ejemplo de la página de [Conceptos básicos de ggplot](ggplot_basics.es.qmd), utilizando varios esquemas de color. 
...
Una solución sencilla para obtener etiquetas de fecha eficientes en el eje x es asignar el argumento `labels = ` de `scale_x_date()` a la función `label_date_short()` del paquete **scales**. Esta función construye automáticamente etiquetas de fecha eficientes (lee más [aquí](https://scales.r-lib.org/reference/label_date.html)). Otra ventaja de esta función es que las etiquetas se ajustan automáticamente a medida que tus datos se extienden en el tiempo, de días, a semanas, a meses y años.

Consulta un ejemplo completo en la sección sobre [etiquetas de fecha de varios niveles](epicurves.es.qmd) de la página de curvas epidémicas, pero a continuación se muestra un ejemplo rápido como referencia:
...
Observa que hay otro ejemplo de esta técnica en la página de [curvas epidémicas](epicurves.es.qmd): la superposición de la incidencia acumulada sobre la curva epidemica.
