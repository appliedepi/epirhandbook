---
aliases:
  - /new_pages/joining_matching.fr.html
---
...
Nous importons un jeu de données de cas d'une épidémie d'ébola fictive. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails).    
...
Avant une jointure, il est souvent rassurant de convertir une colonne tout en minuscules ou majuscules. Pour cela, on peut utiliser `mutate()` et une de ces fonctions de **stringr** (voir le chapitre [sur les chaînes de caractères](characters_strings.fr.qmd)):  
...
<span style="color: orange;">**_ATTENTION:_** Les jointures respectent les majuscules/minuscules ! Il peut donc être utile de convertir les colonnes utilisées comme clefs en minuscules ou majuscules. Voir le chapitre [sur les chaînes de caractères](characters_strings.fr.qmd)</span>
...
Exemple animé d'une jointure interne : ([image source](https://github.com/gadenbuie/tidyexplain/tree/master/images))
...
2) Utiliser ensuite un autre `left_join()` pour joindre `results` à `cases` (en faisant correspondre les "inds.b" nouvellement acquis dans `cases` aux rownames dans `results`).  
...
* Il est possible d'utiliser `bind_rows()` sur une liste de dataframes possédant la même structure pour les combiner en un seul. Vous trouverez un exemple de cette action dans la page [Itération, boucles et listes](iteration.fr.qmd) qui importe plusieurs linelists avec le paquet **purrr**.  
...
Vous trouverez d'autres exemples de ce type dans les pages [Tableaux descriptifs](tables_descriptive.fr.qmd) et [Tableaux de présentation](tables_presentation.fr.qmd).  
