---
aliases:
  - /new_pages/writing_functions.pt.html
---
...
Este "chuk" (pedaço) de código mostra o carregamento de pacotes necessários para as análises. Neste manual, enfatizamos `p_load()` de **pacman**, que instala o pacote, se necessário, *e* carrega ele para ser utilizado. Você também pode carregar pacotes instalados com  `library()` do R **base**. Veja em [Introdução ao R](basics.pt.qmd) para mais informações sobre pacotes R.  
...
Importamos os dados de casos de uma simulação de epidemia de Ebola. Se desejar fazer o download dos dados para seguir passo a passo, veja as instruções na página [Baixar livro e dados](data_used.pt.qmd). O conjunto de dados é importado usando a função `import()` do pacote **rio**. Consulte a página [Importar e exportar](importing.pt.qmd) para várias maneiras de importar dados.
...
Podemos criar nossa primeira função que será chamada `contain_covid19()`.
...
Então, vamos ver o que acontece se um dos valores for `"no"` ou **diferente de** `"yes"`.
...
Outra função poderia selecionar uma a cada duas observações do nosso conjunto de dados. Pode ser relevante quando temos dados longitudinais com muitos registros por paciente, por exemplo, após ter solicitado por data ou visita). Nesse caso, a função apropriada para escrever fora do dplyr seria `function (x) (x%%2 == 0)` para aplicar ao vetor contendo todos os números de linha.
...
O mesmo passo pode ser feito usando a função `map_if()` do pacote **purrr**
...
Podemos verificar executando primeiro a `mean()` como função e, em seguida, executando com `safely()`.
