...
A muestra de ejemplo, trabajaremos con una muestra aleatoria de 1000 casos del brote de ébola simulado en el dataframe `linelist` (computacionalmente, una base de datos pequeña hace que sea más fácil trabajar con este ejemplo). El `linelist` limpio proviene del paquete **appliedepidata**.
...
Carga el `linelist` limpio con `get_data()` de **appliedepidata** (véase [Descargando el manual y los datos](data_used.es.qmd)).
...
Los límites administrativos de Sierra Leona proceden del sitio web de [Humanitarian Data Exchange (HDX)](https://data.humdata.org/dataset/sierra-leone-all-ad-min-level-boundaries). El capítulo los carga con `get_data()` de **appliedepidata**, como se explica en la página [descargando el manual y los datos](data_used.es.qmd).
...
Para leer tu propio shapefile utiliza `read_sf()` de **sf** con su ruta de archivo (véanse [Importación y exportación](importing.es.qmd) y [Proyectos en R](r_projects.es.qmd)).
...
Estos datos provienen del paquete **appliedepidata** (véase [Descargando el manual y los datos](data_used.es.qmd)). También están en HDX (enlace [aquí](https://data.humdata.org/dataset/sierra-leone-population)). Los cargamos con `get_data()` y después pasamos el resultado a `clean_names()` para estandarizar la sintaxis de los nombres de las columnas.
...
Este es el aspecto del archivo de población. *Desplázate a la derecha* para ver las columnas de población `male`, `female` y `total` de cada jurisdicción. Las demás columnas desglosan la población por grupos de edad.
...
La localización de los centros sanitarios procede del HDX [aquí](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities). El capítulo la carga con `get_data()` de **appliedepidata**, como se explica en la página [descarga de manuales y datos](data_used.es.qmd).

Limpiamos de nuevo los nombres de las columnas y filtramos para mantener sólo los puntos etiquetados como "hospital", "clinic" o "doctors".
...
tm_shape(sle_adm3) +               # shapefile de límites administrativos
  tm_polygons(fill = "#F7F7F7")+    # muestra los polígonos en gris claro
...
tm_shape(linelist_sf) +
  ...
  tm_title("Distribution of Ebola cases")   # da título al mapa
...
En este ejemplo, utilizamos `ggplot()` con `linelist_adm` para poder aplicar funciones que manejan factores, como `fct_infreq()` que ordena las barras por frecuencia (véase la página sobre [Factores](factors.es.qmd) para ver algunos consejos).
...
tm_view(set_view = c(-13.2284, 8.4699, 13), # ajusta el zoom (coordenadas centrales, zoom)
...
También podemos explorar cuántos casos se encuentran a menos de 2 km (~30 minutos) de distancia a pie del centro sanitario más cercano. Esto se conoce como buffer o zona o ámbito de influencia.

*Nota: sf trata un objeto lat/long como esférico. Por tanto, una distancia que pases a `st_buffer()` o `st_distance()` está en metros. Para calcular áreas o distancias precisas, reproyecta a un sistema de coordenadas proyectado. Elige ese sistema según la ubicación y la medida, por ejemplo la zona UTM local.*
...
**En primer lugar**, se crea un buffer de 2 km alrededor de cada centro sanitario con `st_buffer()` de **sf**. El objeto está en lat/long, por lo que **sf** mide esa distancia en metros sobre la esfera.

```{r, warning=F, message=F}
sle_hf_2k <- sle_hf %>%
  st_buffer(dist = 2000)     # metros: sf usa geometría esférica para objetos lat/long 
```
...
linelist_sf_hf_2k <- linelist_sf_hf %>%
  st_join(sle_hf_2k, join = st_intersects, left = TRUE) %>%          # una fila por caso y por cada buffer que lo contiene; osm_id.y es NA si no lo contiene ninguno
  select(case_id, osm_id.x, nearest_clinic, amenity.x, osm_id.y) %>%
  distinct(case_id, .keep_all = TRUE)                                # una fila por caso, así un caso queda fuera solo si ningún buffer lo contiene
```

Ahora podemos contabilizar cuántos de nuestros casos no estan dentro de ningun buffer (sus puntos no se *cruzan* con ningún buffer): `r nrow(linelist_sf_hf_2k[is.na(linelist_sf_hf_2k$osm_id.y),])`. Con este código vemos que falta información para este valor (estamos buscando `NA`), y por tanto, inferimos que estos casos viven a más de 30 minutos a pie del centro sanitario más cercano.
...
1)  Los datos de la población `sle_adm3_pop` se unen utilizando un `left_join()` de **dplyr** utilizando la columna `admin3pcod` del dataframe `case_adm3`, y la columna `adm_pcode` en el dataframe `sle_adm3_pop`, que tienen la misma información. Véase la página sobre la [unión de datos](joining_matching.es.qmd).
...
tm_shape(case_adm3_sf) +                # polígonos del gráfico
  tm_polygons("case_10kpop",            # colorea según la columna que contiene la tasa de casos
              fill.scale = tm_scale_intervals(
                ...
                values = "brewer.purples")  # usa una paleta de colores púrpura
              ) +
...
Este código descarga los mosaicos actuales del servidor de mosaicos de OpenStreetMap, por lo que necesita una conexión a internet. Eso está permitido cuando tú mismo consultas un mapa. No lo ejecutes en una compilación automatizada. No guardes los mosaicos para usarlos sin conexión. Consulta la [política de uso de los mosaicos de OpenStreetMap](https://operations.osmfoundation.org/policies/tiles/).  

El manual muestra imágenes de los mapas que produce este código. Se crearon el 2 de septiembre de 2026 para que la página se construya sin conexión a internet. Datos del mapa © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
Datos del mapa © OpenStreetMap contributors ([openstreetmap.org/copyright](https://www.openstreetmap.org/copyright)).  
...
Ahora, simplemente introducimos el facetado a través de **ggplot2** en el mapa de calor de densidad. Se aplica `facet_wrap()`, utilizando la nueva columna como filas. Fijamos el número de columnas de facetas en 4 para que se vea mejor.
...
La matriz de arriba muestra las relaciones entre las 9 regiones de nuestros datos `sle_adm3_dat`. Una puntuación de 0 indica que dos regiones no son vecinas, mientras que cualquier valor distinto de 0 indica una relación de vecindad. Los valores de la matriz se han escalado para que cada región tenga un peso total de 1 en la fila.
...
El resultado de la función `moran.test()` nos muestra una estadística I de Moran de `r round(moran_i$estimate[1],2)`. Esto indica la presencia de autocorrelación espacial en nuestros datos; en concreto, sugiere que es probable que las regiones con un número similar de casos de Ébola estén próximas entre sí. El valor p proporcionado por `moran.test()` se genera mediante la comparación con la expectativa bajo la hipótesis nula de ausencia de autocorrelación espacial, y puede utilizarse si se necesita informar de los resultados de una prueba de hipótesis formal.
