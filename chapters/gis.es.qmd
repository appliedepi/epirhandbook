...
Importa los datos con la función `import()` del paquete **rio** (este paquete puede manejar muchos tipos de archivos, como .xlsx, .csv, .rds - véase la página de [importación y exportación](importing.es.qmd) para más detalles).
...
Para un archivo .rds, `import()` emite una advertencia a menos que pases `trust = TRUE`, lo que indica que confías en el origen del archivo. Úsalo solo con archivos que hayas creado tú o que provengan de una fuente de confianza.  
...
Para importar un shapefile utilizamos la función `read_sf()` del paquete **sf**, y usamos la función `here()` para que R encuentre el archivo. En nuestro caso, el archivo se encuentra dentro de nuestro proyecto R en las subcarpetas "data", "gis" y "shp", con nombre de archivo "sle_adm3.shp" (para más información ver las páginas sobre [Importación y exportación](importing.es.qmd) y [Proyectos en R](r_projects.es.qmd)). En tu caso, tendrás que indicar la localización específica donde tienes tus archivos en tu ordenador.
...
de [HDX](https://data.humdata.org/dataset/sierra-leone-population)) o a través de nuestro paquete R **appliedepidata** como se explica [en esta página](data_used.es.qmd). Utilizamos `import()` para cargar el archivo .csv. Al igual que antes, pasamos el archivo importado a `clean_names()` para estandarizar la sintaxis de los nombres de las columnas.
