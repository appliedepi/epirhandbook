...
Para cargar el shapefile utilizamos `get_data()` de **appliedepidata**, que lo devuelve como un objeto **sf**. Para leer tu propio shapefile utiliza `read_sf()` de **sf** con su ruta de archivo (véanse [Importación y exportación](importing.es.qmd) y [Proyectos en R](r_projects.es.qmd)).
...
Cargamos los puntos de las instalaciones con `get_data()`. Limpiamos de nuevo los nombres de las columnas y filtramos para mantener sólo los puntos etiquetados como "hospital", "clinic" o "doctors".
