---
aliases:
  - /new_pages/joining_matching.es.html
---
...
Es poco frecuente que tu análisis o flujo de trabajo epidemiológico no implique múltiples fuentes de datos y el enlace de varios conjuntos de datos. Quizá necesites conectar datos de laboratorio con los resultados clínicos de los pacientes, o los datos de movilidad de Google con las tendencias de las enfermedades infecciosas, o incluso unos datos en una fase del análisis con una versión transformada de sí mismos.

En esta página mostramos código para:

* Realizar *uniones* ("joins") de dos dataframes de modo que las filas se emparejen según valores comunes en columnas identificadoras
* Unir dos dataframes basándose en coincidencias *probabilísticas* (probables) entre valores
* Ampliar un dataframe *vinculando* ("binding") o ("añadiendo") directamente filas o columnas de otro dataframe
...
Para empezar, importamos la lista de casos limpiada de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles).  
...
* Las dos columnas de `my_matches` se convierten en tipo carácter, por lo que pueden unirse a los rownames, que son de tipo carácter
...
La coincidencia probabilística también puede utilizarse para de-duplicar unos datos. Consulta la página sobre [de-duplicación](deduplication.es.qmd) para conocer otros métodos de de-duplicación. 
...
* Puedes utilizar `bind_rows()` en una lista de dataframes de estructura similar para combinarlos en un dataframe. Mira un ejemplo en la página [Iteración, bucles y listas](iteration.es.qmd) que implica la importación de múltiples listas de líneas con **purrr**. 
...
Mira otros ejemplos trabajados como éste en las páginas de [Tablas descriptivas](tables_descriptive.es.qmd) y [Tablas para presentaciones](tables_presentation.es.qmd). 
...
Ahora podemos ver el resultado. Observa cómo en la última fila se rellena un valor `NA` vacío para la columna hospital que no estaba en hosp_summary. Como se explica en la página de [Tablas para presentaciones](tables_presentation.es.qmd), podrías "rellenar" esta celda con "Total" utilizando replace_na().   
...
Observa que los hospitales son los mismos, pero están en diferente orden en cada dataframe. La solución más sencilla sería utilizar un `left_join()` en la columna `hospital`, pero también podría utilizar `bind_cols()` con un paso adicional. 
...
Las [páginas de tidyverse sobre join](https://dplyr.tidyverse.org/reference/join.html) 
