...
Başlamak için, Ebola salgını simulasyonuna ait temizlenmiş vaka listesini içe aktarıyoruz. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.tr.qmd) sayfasına bakın).
...
Eğer eksik veriler `NA` olarak kodlanmışsa, `split_by = ` veya `age_group = ` sütunlarındaki eksik veri olan satırlar, yukarıda gösterilen yön ayrımını tetiklemez. Varsayılan olarak bu satırlar gösterilmeyecektir. Bununla birlikte, `na.rm = FALSE` argümanıyla bunların bitişik bir çubuk grafiğinde ve en üstte ayrı bir yaş grubu olarak görünmelerini sağlayabilirsiniz.
...
Demografik piramitler yapmak üzere `ggplot()`u kullanmak için, iki çubuk grafiği (her cinsiyet için bir tane) oluşturursunuz, bir grafikteki değerleri negatife çevirirsiniz ve son olarak çubuk grafiklerini, tabanları grafiğin ortasında birleşecek şekilde dikey olarak görüntülemek için x ve y eksenlerini çevirirsiniz.
