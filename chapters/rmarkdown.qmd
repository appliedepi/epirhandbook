...
* The page [Organizing routine reports](reportfactory.qmd) demonstrates how to routinize your report production with auto-generated time-stamped folders.  
* The page [Dashboards with R Markdown](flexdashboard.qmd) explains how to format a R Markdown report as a dashboard.  
...
Install the **rmarkdown** R package. In this handbook we emphasize `p_load()` from **pacman**, which installs the package if necessary *and* loads it for use. You can also load installed packages with  `library()` from **base** R. See the page on [R basics](basics.qmd) for more information on R packages.  
...
To refer to files elsewhere, you will either need to use the full file path or use the **here** package. The **here** package sets the working directory to the root folder of the R project and is explained in detail in the [R projects](r_projects.qmd) and [Import and export](importing.qmd) pages of this handbook. For instance, to import a file called "data.csv" from within the `projectX` folder, the code would be `import(here(“data.csv”))`. 
...
Workflow also concerns the overall folder structure, such as having an 'output' folder for created documents and figures, and 'data' or 'inputs' folders for cleaned data. We do not go into further detail here, but check out the [Organizing routine reports](reportfactory.qmd) page.  
...
Read more about this work flow in the page on [Organizing routine reports](reportfactory.qmd).  
...
The `ggplotly()` function from **plotly** is particularly easy to use. See the [Interactive plots](interactive_plots.qmd) page. 
