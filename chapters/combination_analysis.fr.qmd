---
aliases:
  - /new_pages/combination_analysis.fr.html
---
...
Pour commencer, nous importons la linelist nettoyée des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails).    
...
Voir la page sur [Caractères et chaînes de caractères](characters_strings.fr.qmd) pour en savoir plus sur la fonction `unite()` de **stringr**.
...
Si vous êtes intéressé par une commande plus efficace, vous pouvez profiter de la fonction `+()`, qui convertit en 1 et en 0 en fonction d'une déclaration logique. Cette commande utilise la fonction `across()` pour modifier plusieurs colonnes à la fois (pour en savoir plus, lisez la section [Nettoyage des données et des fonctions de base](cleaning.fr.qmd#clean_across)).  
