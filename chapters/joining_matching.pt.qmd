---
aliases:
  - /new_pages/joining_matching.pt.html
---
...
Para iniciar, importaremos a versão limpa da linelist de casos de uma epidemia simulada de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.pt.qmd) para mais detalhes).    
...
1) Utilizar `left_join()` para fazer o join de `my_matches` em `cases` (combinando os nomes das linhas em `cases` com "inds.a" em  `my_matches`)  
...
* Ambas as colunas de `my_matches` devem ser convertidas para a classe *character*, para que o join possa ser feito com os nomes das linhas que são caracteres
...
Para retornar os números das linhas que são potenciais duplicidades, você pode contar o número de linhas por valores únicos na coluna `dedupe.ids`, e aí filtrar para manter apenas aquelas com mais de uma linha. Nesse caso, isso deixaria as linhas 2 e 3.  
...
* Você pode usar `bind_rows()` em uma `list` de dataframes que tenham estrutura semelhante para combiná-los em um só. Veja um exemplo na página [Iteração, loops e listas](iteration.pt.qmd) que envolve a importação de múltimplas linelists com **purrr**.  
...
Veja outros exemplos práticos como esse nas páginas [Tabelas descritivas](tables_descriptive.pt.qmd) e [Tabelas para apresentação](tables_presentation.pt.qmd).  
...
Agora podemos visualizar o resultado. Veja como na última linha, um valor vazio, `NA`, preenche a coluna `hospital` que não estava em `hosp_summary`. Como explicado na página [Tabelas para apresentação](tables_presentation.pt.qmd), você pode preencher essa célula com "Total" utilizando `replace_na()`.  
