...
Bunun için tipik senaryo, **dplyr** fonksiyonu "case_when()" ile yeni bir sütun oluştururken gerçekleşmektedir. [Verileri ve temel fonksiyonları temizleme](#clean_case_when) sayfasında açıklandığı gibi, bu fonksiyon veri çerçevesindeki her satırı değerlendirir, satırların belirtilen mantıksal ölçütlerini (kodun sağ tarafı) karşılayıp karşılamadığını değerlendirir ve doğru yeni değerini atar ( kodun sol tarafı). *Önemli olan: sağ taraftaki tüm değerler aynı sınıf olmalıdır*.
