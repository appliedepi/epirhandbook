...
Tam sözdizimi dosya türüne göre değiştiğinden, ayrıntılar için [Eksik veriler](importing.tr.qmd#import_missing) ile ilgili sayfanın içe aktarılmasıyla ilgili sayfaya bakın.
...
Verilerinizin "99" veya "Eksik" veya "Bilinmeyen" gibi eksikleri temsil etmenin başka yolları olabilir - "boş" görünen boş karakter değerini "" veya tek bir boşluk " " bile olabilir. Bunların farkında olun ve [içe aktarma sırasında bunları 'NA'ya dönüştürün](importing.tr.qmd#import_missing) veya 'na_if()' ile veri temizleme sırasında yapabilirsiniz.
...
Bunun için tipik senaryo, **dplyr** fonksiyonu "case_when()" ile yeni bir sütun oluştururken gerçekleşmektedir. [Verileri ve temel fonksiyonları temizleme](cleaning.tr.qmd#clean_case_when) sayfasında açıklandığı gibi, bu fonksiyon veri çerçevesindeki her satırı değerlendirir, satırların belirtilen mantıksal ölçütlerini (kodun sağ tarafı) karşılayıp karşılamadığını değerlendirir ve doğru yeni değerini atar ( kodun sol tarafı). *Önemli olan: sağ taraftaki tüm değerler aynı sınıf olmalıdır*.
...
Sütunları arka arkaya listeleyebilir veya ["tidyselect" yardımcı işlevlerini](cleaning.tr.qmd#clean_tidyselect) kullanabilirsiniz:
