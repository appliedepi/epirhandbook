...
La fonction `ymd()` convertit de maniére flexible les valeurs de date fournies sous la forme **année, mois, jour**.  
...
Comme nous l'avons déjà mentionné, R supporte également une classe `datetime` - une colonne qui contient des informations de date **et** d'heure. Comme pour la classe `Date`, il faut souvent convertir les objets `character` en objets `datetime`. 
...
Pour convertir des nombres à un chiffre en nombres à deux chiffres (par exemple, pour ajouter des zéros aux heures ou aux minutes afin d'obtenir deux chiffres), consultez la section ["Longueur des caractères" de la page caractères et chaînes de caractères](characters_strings.fr.qmd#str_pad). 
...
Pour en savoir plus sur la fonction `case_when()`, consultez la page [Nettoyage des données et des fonctions de base](cleaning.fr.qmd). 
...
Allez à la page [Regroupement des données](grouping.fr.qmd) pour une explication détaillée du regroupement des données avec `count()`, `group_by()`, et `summarise()`. Voici un bref exemple.  
...
Une autre alternative pour les séries temporelles qui fonctionne également bien pour afficher un format "semaine" ("2020 W12") est `yearweek()` du paquet **tsibble**, comme démontré dans la page sur [Série temporelle et détection des épidémies](time_series.fr.qmd).  
