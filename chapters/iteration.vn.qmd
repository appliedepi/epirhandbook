...
Chúng tôi nhập dataset về các trường hợp từ một vụ dịch Ebola mô phỏng. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.vn.qmd) để biết chi tiết).    
...
Dưới đây là một số ví dụ sử dụng danh sách `combined` được tạo trong phần trên về [sử dụng map() để nhập và kết hợp nhiều tệp](iteration.vn.qmd#iter_combined) (nó chứa 6 trường hợp linelist thuộc data frames):  
