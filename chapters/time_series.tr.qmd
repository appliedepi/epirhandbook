...
for (i in 2002:2011) {
 ...
 # dosyayı indirin ve mevcut çalışma dizininde saklayın
  file <- wf_request(user     = "XXXXX",  # kullanıcı ID (kimlik doğrulama için)
                     ...
                     path     = here::here("data", "time_series", "weather")) ## dosyayı kaydetmek için yolak
  }
...
Aşağıdaki kod bloğu, 10 yıllık ".nc" iklim veri dosyalarını geçici bir klasöre yazar. **appliedepidata** paketindeki `save_data()` fonksiyonunu çağırır, ardından arşivi açar.
