...
Na [seção](#iter_purrr) **purrr** também forneceremos vários exemplos de criação e manipulação de `lists`.
...
A sintaxe básica é: `for (item in sequência) {fazer operações usando item}`. Observe os parênteses e as chaves. Os resultados podem ser impressos no console ou armazenados em um objeto R (o container).
...
A estrutura básica de uma instrução de sequência é `item in vetor`.
...
Para fins de exemplo, incluímos o código de operações dentro do *loop for*, que é executado para cada valor na sequência. Neste código, os valores de sequência (nomes das colunas) são usados para *indexar* (subconjunto) `linelist`, um de cada vez. Conforme ensinado na página [Introdução ao R](basics.pt.qmd), colchetes duplos `[[ ]]` são usados para subconjunto. A coluna resultante é passada para `is.na()`, então para `sum()` para produzir o número de valores na coluna que estão faltando. O resultado é impresso no console - um número para cada coluna.

Uma nota sobre indexar com nomes de colunas: sempre que fizer referência à coluna em si, *não escreva apenas "col"!* `col` representa apenas o nome da coluna, como caractere! Para se referir à coluna inteira, você deve usar o nome da coluna como um *índice* em `linelist`, por meio de `linelist[[col]]`.
...
Para testar seu loop, você pode executar um comando para fazer uma atribuição temporária do "item", como `i <- 10` ou `hosp <- "Central Hospital"`. Faça isso *fora do loop* e execute apenas seu código de operações (o código dentro das chaves) para ver se os resultados esperados são produzidos.

### Gráficos em loop {.unnumbered}

Para juntar todos os três componentes (contêiner, sequência e operações), vamos tentar traçar uma epicurva para cada hospital (consulte a página em [Curvas epidêmicas](epicurves.pt.qmd)).
...
Tenha em mente que o **purrr** é uma *ferramenta de programação funcional*. Ou seja, as operações que devem ser aplicadas iterativamente são agrupadas em *funções*. Consulte a página [Escrevendo funções](writing_functions.pt.qmd) para aprender a escrever suas próprias funções.
...
-   Se a função não precisar de mais argumentos especificados, ela pode ser escrita sem parênteses e sem til (por exemplo, `.f = mean`). Para fornecer argumentos que terão o mesmo valor para cada iteração, forneça-os dentro da `map()` mas fora do argumento `.f =`, como `na.rm = T` em `map(.x = my_list, .f = mean, na.rm=T)`.
...
Lembre-se da página [Importar e exportar](importing.pt.qmd) que quando usado em arquivos do Excel, `import()` pode aceitar o argumento `which =` (qual) especificando a aba a ser importada. Dentro da função `.f` `import()`, fornecemos `which = .x`, cujo valor mudará a cada iteração através do vetor `sheet_names` - primeiro "Central Hospital" ("Hospital Central"), depois "Military Hospital" ("Hospital Militar") etc.
...
```{r, eval=F}
appliedepidata::save_data(name = "example_hospital_linelists", path = getwd())
sheet_names <- readxl::excel_sheets("example_hospital_linelists.xlsx")  # extraindo o nome das abas

combined <- sheet_names %>%                                     # começando com os nomes das abas
  purrr::set_names() %>%                                        # definindo seus nomes
  map(.f = ~import("example_hospital_linelists.xlsx", which = .x)) %>% # iterando, importando, salvando na lista
...
Novamente usamos `map()`: pegamos o vetor de nomes de elementos da lista (mostrado acima) e usamos `map()` para iterar por eles, aplicando `export()` (do pacote **rio**, veja página [Importar e exportar](importing.pt.qmd)) no data frame na lista `linelist_split` que tem esse nome. Também usamos o nome para criar um nome de arquivo exclusivo. Aqui está como funciona:
...
-   O caminho do arquivo fornecido para `export()` é dinâmico através do uso de `str_glue()` (consulte a página [Caracteres e strings](characters_strings.pt.qmd)):
...
Se este código `map()` parecer muito confuso, você pode obter o mesmo resultado salvando seu comando `ggplot()` específico como uma função personalizada definida pelo usuário, por exemplo, podemos chamá-lo de `make_epicurve())`. Esta função é então usada dentro do `map()`. `.x` será substituído iterativamente pelo nome do hospital e usado como `hosp_name` na função `make_epicurve()`. Consulte a página sobre [Escrevendo funções](writing_functions.pt.qmd).
...
Lembre-se da página em [Testes estatísticos simples](stat_tests.pt.qmd) que `t.test()` pode receber entradas em um formato de fórmula, como `t.test(coluna numérica ~ coluna binária)`. Neste exemplo, fazemos o seguinte:
...
-   Cada um desses cinco elementos são listas, com elementos dentro deles, como `p.value` e `conf.int`. Alguns desses elementos como `p.value` são números únicos, enquanto alguns como `estimate` consistem em dois ou mais elementos (`mean in group f` e `mean in group m`).

```{r, out.height="150%", echo=F}
knitr::include_graphics(here::here("images", "purrr_ttest.png"))
```

Nota: Lembre-se que se você deseja aplicar uma função a apenas certas colunas em um data frame, você também pode simplesmente usar `mutate()` e `across()`, conforme explicado na página [Limpeza de dados e principais funções](cleaning.pt.qmd). Abaixo está um exemplo de aplicação de `as.character()` apenas para as colunas "age". Observe o posicionamento dos parênteses e vírgulas.
...
Para extrair elementos da lista por nome ou por posição, você pode usar colchetes `[[ ]]` conforme descrito na página [Introdução ao R](basics.pt.qmd). Abaixo usamos colchetes duplos para indexar a lista `t.tests_results` e exibir o primeiro elemento que é o resultado do teste t em `age` (idade).
...
Precisaríamos extrair o elemento `estimate`, mas na verdade ele contém *dois* elementos dentro dele (`mean in group f` e `mean in group m`). Portanto, não pode ser simplificado em um vetor com `map_chr()` ou `map_dbl()`. Em vez disso, usamos `map()`, que usado dentro de `tibble()` criará *uma coluna da classe lista dentro do tibble*! Sim, isso é possível!
