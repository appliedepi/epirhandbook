---
aliases:
  - /new_pages/transmission_chains.pt.html
---
...
arrastando para movê-los e clicando para destacar os casos subsequentes na cadeia de transmissão.
...
Vamos importar a base de dados de casos da epidemia simulada de Ebola. Se você quiser fazer download dos dados para seguir o passo a passo, veja as instruções na página [Baixando dados do manual](data_used.pt.qmd). A base será importada utilizando a função `import()` do pacote **rio**. Veja a página sobre [Importação e exportação](importing.pt.qmd) para aprender várias formas de importar dados.
...
* uma linelist documentando casos onde colunas são variáveis e linhas correspondem a casos únicos
...
Você pode salvar um gráfico como um arquivo html interativo e auto-contido com a função `visSave` do pacote **VisNetwork**:
