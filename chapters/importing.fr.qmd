---
aliases:
  - /new_pages/importing.fr.html
---

# Importer et exporter des données {#importing}
...
L'importation/exportation de shapefile nécessite d'autres packages, tel que détaillé dans la page sur [Introduction aux SIG](gis.fr.qmd).
...
Par exemple, ci-dessous, la fonction `import()` reçoit un chemin d'accès construit avec `here()`. Nous créons d'abord une copie réelle de la linelist brute dans un sous-dossier "linelists" du dossier "data" avec `appliedepidata::save_data()`, afin que le code ci-dessous s'exécute réellement tel qu'il est écrit.
...
La commande `here("data", "linelists", "linelist_raw.xlsx")` fournit en fait le chemin d'accès complet au fichier qui est *unique à l'ordinateur de l'utilisateur* :
...
Plus d'informations sur le package **here** [en cliquant sur ce lien](https://here.r-lib.org/).
...
Voici un exemple de chemin d'accès relatif construit avec `here()`. Nous supposons que le travail se trouve dans un projet R qui contient un sous-dossier "data" et dans celui-ci un sous-dossier "linelists", dans lequel se trouve le fichier .xlsx qui nous intéresse (créez-le avec `appliedepidata::save_data()` comme indiqué ci-dessus).
...
La feuille peut également être importée en utilisant uniquement l'ID de la feuille, une partie plus courte de l'URL :
...
Une fois enregistré sur votre ordinateur, vous pouvez importer le shapefile comme indiqué sur la page [Bases de SIG](gis.fr.qmd) `st_read()` du paquet **sf**. Il vous suffit de fournir le chemin d'accès et le nom du fichier " .shp ", à condition que les fichiers associés se trouvent dans le même dossier sur votre ordinateur.
...
Pour plus de détails, consultez la documentation sur [LoopBack Explorer](https://loopback.io/doc/en/lb4/index.html), la page [Suivi des contacts](contact_tracing.fr.qmd) ou les astuces API sur [Go.Data Github repository](https://worldhealthorganization.github.io/godata/api-docs)
...
Les insTIPtions sur la façon de sauvegarder les tracés, tels que ceux créés par `ggplot()`, sont discutées en profondeur dans la page [bases de ggplot](ggplot_basics.fr.qmd).
...
L'enregistrement d'un graphe de réseau, tel qu'un arbre de transmission, est abordé dans la page [Chaînes de transmission](transmission_chains.fr.qmd).
