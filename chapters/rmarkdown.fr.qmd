...
-   La page [Organisation des rapports de routine](reportfactory.fr.qmd) montre comment organiser la production de vos rapports avec des dossiers horodatés générés automatiquement.\
-   La page [Tableaux de bord avec R Markdown](flexdashboard.fr.qmd) explique comment formater un rapport R Markdown en tant que tableau de bord.
...
Installez le "package" R **rmarkdown**.  Dans ce manuel, nous mettons l'accent sur la fonction `p_load()` du "package" **pacman**, qui installe le "package" si nécessaire (uniquement si le package n'est pas déjà installé) *et* le charge pour l'utiliser . On peut également charger des "packages" avec `library()` à partir de R **base**. Voir la page sur [R - les bases](basics.fr.qmd) pour plus d'informations sur les packages R.
...
Le répertoire de travail d'un fichier markdown est l'endroit où le fichier Rmd lui-même est enregistré. Par exemple, si le projet R se trouve dans `~/Documents/projectX` et que le fichier Rmd lui-même se trouve dans un sous-dossier `~/Documents/projectX/markdownfiles/markdown.Rmd`, le code `read.csv("data.csv")` dans le markdown cherchera un fichier csv dans le dossier `markdownfiles`, et non dans le dossier racine du projet où les scripts dans les projets chercheraient normalement automatiquement.


Pour faire référence à des fichiers ailleurs, vous devrez soit utiliser le chemin complet du fichier, soit utiliser le package **here**. Le package **here** définit le répertoire de travail comme étant le dossier racine du projet R et est expliqué en détail dans les pages [Projets R](r_projects.fr.qmd) et [Importer et exporter des données](importing.fr.qmd) de ce manuel. Par exemple, pour importer un fichier appelé "data.csv" depuis le dossier `projectX`, le code serait `import(here("data.csv"))`.
...
Vous pouvez ajouter une option supplémentaire `.tabset-pills` après `.tabset` pour donner aux onglets eux-mêmes un aspect de "pilules". Sachez que, dans la sortie HTML à onglets, la recherche Ctrl+f ne portera que sur les onglets "actifs", et non sur les onglets masqués.
...
* Des messages d'erreur plus informatifs - ces messages seront générés par le script R, et non par le Markdown R. Les erreurs du Markdown R ont tendance à vous indiquer quel "chunk" a un problème, mais ne vous diront pas quelle ligne.
...
Pour en savoir plus sur ce flux de travail, consultez la page [Organisation des rapports de routine](reportfactory.fr.qmd).
...
Editez le YAML pour inclure une option `params:`, avec des déclarations indentées pour chaque paramètre que vous voulez définir. Dans cet exemple, nous créons les paramètres `date` et `hospital`, pour lesquels nous spécifions des valeurs. Ces valeurs sont susceptibles de changer à chaque fois que le rapport est exécuté. Si vous utilisez le bouton "Knit" pour produire le résultat, les paramètres auront ces valeurs par défaut. De même, si vous utilisez `render()`, les paramètres auront ces valeurs par défaut, sauf indication contraire dans la commande `render()`.
...
Ci-dessous, ces spécifications sont écrites pour le paramètre `hospital`.
...
Le code suivant crée des paramètres pour `date` et '`hospital`, qui sont utilisés dans le R Markdown comme `params$date` et `params$hospital`, respectivement.
...
- Leaflet (utilisé dans la page [Bases des GIS](gis.fr.qmd) de ce manuel)
...
La fonction `ggplotly()` de **plotly** est particulièrement facile à utiliser. Voir la page [Graphiques interactifs](interactive_plots.fr.qmd).
