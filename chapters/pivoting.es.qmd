...
**En esta página, nos centraremos en la última definición.** La primera es un paso crucial en el análisis de datos, y se trata en las páginas [Agrupar datos](grouping.es.qmd) y [Tablas descriptivas](tables_descriptive.es.qmd). 
...
En esta página, utilizaremos unos datos ficticios de casos diarios de malaria, por centro y grupo de edad. Cárguelo directamente con `appliedepidata::get_data(name = "malaria_facility_count_data")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles).  
...
En la parte posterior de esta página, también utilizaremos los datos de casos de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles).  
...
Dado que los nombres de estas cuatro columnas comienzan con el prefijo `malaria_`, podríamos haber hecho uso de la práctica función "tidyselect" `starts_with()` para conseguir el mismo resultado (véase la página [Limpieza de datos y funciones básicas](cleaning.es.qmd) para conocer más sobre estas funciones de ayuda). 
...
Ten en cuenta que la columna de fecha es actualmente de tipo *carácter* - podemos convertirla fácilmente en tipo fecha utilizando las funciones `mutate()` y `as_date()` descritas en la página [Trabajar con fechas](dates.es.qmd). 

También podemos convertir la columna de `observation` a un formato `numeric` eliminando el prefijo "obs" y convirtiendo a numérico. Podemos hacer esto con `str_remove_all()` del paquete **stringr** (véase la página [Caracteres y cadenas](characters_strings.es.qmd)).   
...
Esta tabla es mucho más fácil de leer y, por tanto, mejor para incluirla en nuestros informes. Se puede convertir en una tabla bonita con varios paquetes, como **flextable** y **knitr**. Este proceso se elabora en la página [Tablas para presentaciones](tables_presentation.es.qmd).  
