...
データフレームを結合する前に、列の値をすべて小文字または大文字に変換すると、結合作業がより簡単になります。列のすべての値を大文字または小文字に変換する必要がある場合は、[文字型・文字列型データ](#characters-strings) の章で言及したように、`mutate()` を使用して、さらに **stringr** の以下のいずれかの関数に変換したい列を適用します。
