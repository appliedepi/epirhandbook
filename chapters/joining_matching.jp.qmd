...
あるいは、`cases` データセットの「元の」9 行のみを残したまま、`results` データセットの新しい列のみを追加したい場合は、結合を行う前に `results` データセットの列を選別しましょう。`select()` を使用して `results` データセットで結合後も残したい列のみを選択し（この例では、`rowname` 列と `results` 列）、選択された列のみを `cases` データセットに結合することができます。
