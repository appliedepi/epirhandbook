...
**SAS**                          | **R**  
-------------------------------- | ---------------------------------------------
Os diretórios de trabalho podem ser absolutos ou relativos a uma pasta raiz do projeto, definindo a pasta raiz utilizando `%let rootdir=/root path; %include "&rootdir/subfoldername/nome_arquivo"`|Os diretórios de trabalho podem ser absolutos ou relativos a uma pasta raiz do projeto utilizando o pacote **here** (ver [Importar e exportar](#importing))
...
**SAS**                          | **R**  
-------------------------------- | ---------------------------------------------
Utilize o procedimento `Proc Import` ou utilize a declaração `Data Step Infile`.|Utilize a função `import()` do pacote **rio** para quase todos os tipos de arquivos. Existem funções específicas como alternativas (ver [Importar e exportar](#importing)).
...
Os conjuntos de dados são combinados utilizando a declaração `Merge` na Etapa de Dados. Os conjuntos de dados a serem fundidos precisam ser ordenados primeiro utilizando o procedimento `Proc Sort`.|O pacote **dplyr** oferece algumas funções para fundir conjuntos de dados. Consulte a página [Juntando dados](#joining-matching) para obter detalhes.
