---
aliases:
  - /new_pages/transition_to_R.pt.html
---
...
Os conjuntos de dados "tidy" podem não ser tão legíveis a um olho humano, mas tornam a limpeza e análise dos dados muito mais fácil! Dados "tidy" podem ser armazenados em vários formatos, por exemplo "longo/comprido" (*long*)  ou "largo/amplo" (*wide*) (ver página em [Pivotando dados](pivoting.pt.qmd)), mas os princípios acima serão sempre observados.
...
*O script R consiste de instruções passo a passo.* Isto permite que qualquer colega leia o script e veja facilmente os passos que você deu. Isto também ajuda a eliminar erros ou cálculos imprecisos. Veja a seção [Introdução ao R](basics.pt.qmd) sobre scripts para exemplos.  
...
Aqui estão alguns links para tutoriais que ajudarão na sua transição do Excel para o R:
...
Quase todos os comandos são nativos ao Stata. Funções novas/escritas pelo usuário podem ser instaladas como arquivos **ado*** usando **ssc install** [pacote] | A instalação do R vem com funções **base** , mas o uso típico envolve a instalação de outros pacotes do CRAN (veja página em [Introdução ao R](basics.pt.qmd))
...
**STATA**                        | **R**  
-------------------------------- | ---------------------------------------------
Os diretórios de trabalho envolvem caminhos de arquivo absolutos (por exemplo, "C:/ nome do usuário/documentos/projetos/dados/")| Os diretórios de trabalho podem ser absolutos, ou relativos a uma pasta raiz do projeto usando o pacote **here** (ver [Importar e exportar](importing.pt.qmd)) 
...
**STATA**                    | **R**  
-------------------------------- | ---------------------------------------------
Comandos específicos por tipo de arquivo | Utilize `import()` do pacote **rio** para quase todos os tipos de arquivo. Existem funções específicas como alternativas (ver [Importar e exportar](importing.pt.qmd))
A leitura em arquivos csv é feita por **import delimited** "filename.csv". | Use `import("nomedoarquivo.csv")`
...
Não é necessário especificar o conjunto de dados | Em cada um dos comandos abaixo, você precisa especificar o conjunto de dados - veja a página em [Limpeza de dados e principais funções](cleaning.pt.qmd) para exemplos
Novas variáveis são criadas utilizando o comando **generate** *nome_var* = | Gerar novas variáveis utilizando a função `mutate(nome_var = )`. Consulte a página [Limpeza de dados e principais funções](cleaning.pt.qmd) para obter detalhes sobre todas as funções abaixo **dplyr***.
...
As variáveis fatoriais podem ser etiquetadas usando uma série de comandos como **label define** | Os valores de etiquetagem podem ser feitos convertendo a coluna para a classe fator e especificando níveis. Veja a página em [Fatores](factors.pt.qmd). Os nomes das colunas não são tipicamente etiquetados como estão na Stata.

**Análise descritiva**  

**STATA**                    | **R**  
-------------------------------- | ---------------------------------------------
Tabula contagens de uma variável utilizando **tab** *nome_var* | Forneça o conjunto de dados e nome da coluna para `table()` tal como `table(dataset$nome_da_coluna)`. Alternativamente, utilize `count(nome_var)` do pacote **dplyr**, como explicado em [Agrupando dados](grouping.pt.qmd).
...
Quase todos os comandos são nativos.  Os usuários podem escrever novas funções utilizando a macro SAS, SAS/IML, SAS Component Language (SCL) e, mais recentemente, procedimentos `Proc Fcmp` e `Proc Proto`|A instalação do R vem com as funções **base**, mas o uso típico envolve a instalação de outros pacotes do CRAN (veja a página em [Introdução ao R](basics.pt.qmd))
...
**SAS**                          | **R**  
-------------------------------- | ---------------------------------------------
Os diretórios de trabalho podem ser absolutos ou relativos a uma pasta raiz do projeto, definindo a pasta raiz utilizando `%let rootdir=/root path; %include "&rootdir/subfoldername/nome_arquivo"`|Os diretórios de trabalho podem ser absolutos ou relativos a uma pasta raiz do projeto utilizando o pacote **here** (ver [Importar e exportar](importing.pt.qmd))
Veja o diretório de trabalho atual com `%put %sysfunc(getoption(work));`|Use `getwd()` ou `here()` (se utilizar o pacote **here**), com parênteses vazios
...
**SAS**                          | **R**  
-------------------------------- | ---------------------------------------------
Utilize o procedimento `Proc Import` ou utilize a declaração `Data Step Infile`.|Utilize a função `import()` do pacote **rio** para quase todos os tipos de arquivos. Existem funções específicas como alternativas (ver [Importar e exportar](importing.pt.qmd)).
A leitura em arquivos csv é feita utilizando `Proc Import datafile="nome_arquivo.csv" out=work.nome_arquivo dbms=CSV; run;` ou usando [Data Step Infile statement](http://support.sas.com/techsup/technote/ts673.pdf)|Use `import("nome_arquivo.csv")`
A leitura em arquivos xslx é feita utilizando `Proc Import datafile="nome_arquivo.xlsx" out=work.nome_arquivo dbms=xlsx; run;` ou usando [Data Step Infile statement](http://support.sas.com/techsup/technote/ts673.pdf)|Use import("nome_arquivo.xlsx")
...
As variáveis do tipo fator podem ser etiquetadas na Etapa de Dados, utilizando a declaração de 'Label' | Os valores do rótulo podem ser feitos convertendo a coluna para a classe Fator e especificando níveis. Veja a página em [Fatores](factors.pt.qmd). Os nomes das colunas não são tipicamente rotulados.
...
Os conjuntos de dados são combinados utilizando a declaração `Merge` na Etapa de Dados. Os conjuntos de dados a serem fundidos precisam ser ordenados primeiro utilizando o procedimento `Proc Sort`.|O pacote **dplyr** oferece algumas funções para fundir conjuntos de dados. Consulte a página [Juntando dados](joining_matching.pt.qmd) para obter detalhes.

**Análise descritiva**  

**SAS**                          | **R**  
-------------------------------- | ---------------------------------------------
Obtenha uma visão geral  do seu conjunto de dados utilizando o procedimento "Proc Summary", que fornece os nomes das variáveis e as estatísticas descritivas.| Obtenha uma visão geral do seu conjunto de dados utilizando o "summary(conjunto_de_dados)" ou "skim(conjunto_de_dados)" do pacote **skimr**.
Tabular contagens de uma variável utilizando `proc freq data=Dataset; Tables varname; Run;`| Ver a página em [Tabelas descritivas](tables_descriptive.pt.qmd). As opções incluem `table()` do R **base**, e `tabyl()` do pacote **janitor** , entre outras. Observe que você precisará especificar o conjunto de dados e o nome da coluna, pois R contém vários conjuntos de dados.
A tabulação cruzada (tabela de contingência) de duas variáveis em uma tabela 2x2 é feita com `proc freq data=Dataset; Tables rowvar*colvar; Run;`|Novamente, você pode utilizar `table()`, `tabyl()` ou outras opções como descritas na página [Tabelas descritivas](tables_descriptive.pt.qmd).   
...
Veja a página [Importar e exportar](importing.pt.qmd) para detalhes sobre como o pacote **rio** do R pode importar e exportar arquivos como arquivos STATA .dta, arquivos SAS .xpt e.sas7bdat, arquivos SPSS .por e.sav, e muitos outros.   
