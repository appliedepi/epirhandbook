---
aliases:
  - /new_pages/ggplot_basics.pt.html
---
...
Nesta página cobriremos os fundamentos da criação de gráficos com o pacote **ggplot2**. Veja a página [Dicas para o ggplot](ggplot_tips.pt.qmd) para sugestões e técnicas avançadas para tornar seus gráficos realmente bonitos.  
...
Este trecho de código mostra o carregamento dos pacotes necessários para as análises. Neste manual, enfatizamos `p_load()` de **pacman**, que instala o pacote se necessário *e* o carrega para utilização. Você também pode carregar os pacotes instalados com `library()` do R **base***. Veja a página em [Introdução ao R](basics.pt.qmd) para mais informações sobre os pacotes R.  
...
Importamos o conjunto de dados de casos de uma epidemia simulada de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.pt.qmd) para mais detalhes).  
...
* Limpar algumas colunas para que os valores adequados à manipulações (_"data friendly"_), tais como valores que possuam sublinhado (_"underline"_), por exemplo, sejam alterados para texto normal ou padrão título (ver [Caracteres e strings](characters_strings.pt.qmd))  
...
Por uma questão de estrutura de dados, para **ggplot2** muitas vezes também queremos *pivotar* nossos dados para formatos *mais longos*. Leia mais sobre isto na página em [Pivoteando dados](pivoting.pt.qmd).  
...
Aqui estão as primeiras 50 linhas. Observe que o caso tem 5 fileiras - uma para cada sintoma possível. As novas colunas `symptom_name` e `symptom_is_present` são o resultado do pivoteamento. Note que este formato pode não ser muito útil para outras operações, mas é útil para a criação do gráfico.
...
Normalmente, o comando `ggplot()` inclui o argumento `data = ` para o gráfico. Isto define o conjunto de dados padrão a ser utilizado para as camadas subsequentes do gráfico.  
...
Para conseguir isso, você mapeia esse *aesthetics* do gráfico para um nome de *coluna* (não entre aspas). Isto deve ser feito *em uma função `mapping = aes()` (nota: há vários lugares no código que você pode fazer estas atribuições de mapeamento, como discutido [abaixo](ggplot_basics.pt.qmd#ggplot_basics_map_loc))).  
...
Torna-se importante acompanhar as camadas e a *aesthetics* de seus gráficos à medida que vão ficando mais complexos - por exemplo, gráficos com múltiplos *geoms*. No exemplo abaixo, `geom_point()` define `size = 1` e `geom_smooth()` define `linewidth = 2`. Ambos são valores estáticos.   
...
ggplot(data = linelist,
  ...
  geom_smooth(                  # adiciona uma linha de tendência 
    ...
    linewidth = 2)                   # tamanho (grossura da linha) de 2
```
...
Para ajustar a ordem dos grupos em um gráfico, veja a página [Dicas para o ggplot](ggplot_tips.pt.qmd) ou a página em [Fatores](factors.pt.qmd). Há muitos exemplos de gráficos agrupados nas seções abaixo sobre a plotagem de dados contínuos e categóricos.    
...
O Facetamento (*Faceting*) é uma funcionalidade que vem com **ggplot2**, portanto as legendas e eixos dos "painéis" de faceta são automaticamente alinhados. Há outros pacotes discutidos na página [Dicas para o ggplot](ggplot_tips.pt.qmd) que são usados para combinar gráficos completamente diferentes (**cowplot** e **patchwork**) em uma única figura.  
...
As facetas podem rapidamente conter uma quantidade avassaladora de informações - é bom garantir que você não tenha muitos níveis de cada variável que você escolher facetar. Aqui estão alguns exemplos rápidos com o conjunto de dados sobre malária (ver [Baixar manual e dados](data_used.pt.qmd)) que consiste em contagens diárias de casos de malária para estabelecimentos, por faixa etária.  
...
malaria_data <- appliedepidata::get_data(name = "malaria_facility_count_data") %>%  # importar
...
Podemos utilizar uma abordagem `facet_grid()` para cruzar duas variáveis. Digamos que queremos cruzar o `District` (*Distrito*) e a idade (*age*). Bem, precisamos fazer algumas transformações de dados nas colunas de idade para obter esses dados no formato "longo", preferido pelo ggplot. Todos os grupos etários têm suas próprias colunas - queremo-los em uma única coluna chamada `age_group` e outra chamada `num_cases`. Consulte a página em [Pivoteamento de dados](pivoting.pt.qmd) para obter mais informações sobre este processo.  
...
Quando você passa as duas variáveis para `facet_grid()`, o mais fácil é utilizar notação de fórmula (por exemplo `x ~ y`) onde x corresponde às linhas e y às colunas. Aqui está o gráfico, utilizando `facet_grid()` para mostrar os gráficos para cada combinação das colunas `age_group` e `District`.
...
Lembre-se de que você pode utilizar a sintaxe da função `here()` para fornecer o caminho de arquivo desejado. Consulte a página [Importar e exportar](importing.pt.qmd) para obter mais informações.    
...
* O alinhamento horizontal da legenda do gráfico (`caption`) com `element_text(hjust = 0)` (da direita para a esquerda)  
...
`axis.text = element_text()`       |Texto dos eixos (ambos x e y)
...
Por favor veja [a seção sobre escala de cores na página de dicas do ggplot](ggplot_tips.pt.qmd#ggplot_tips_colors).  
...
*Gráficos para uma variável contínua:  
  ...
  * **Gráfico Sina**, são uma combinação de gráficos de jitter e violino, onde são mostrados pontos individuais mas na forma simétrica da distribuição (via **ggforce** pacote).
  * **Gráfico de dispersão** Para duas variáveis contínuas.  
  * **Gráficos de calor** para três variáveis contínuas (ligados à página [gráficos de calor](heatmaps.pt.qmd))    
...
Dados categóricos podem ser variáveis do tipo caractere ou variáveis lógicas (TRUE/FALSE. VERDADEIRO/FALSO), ou ainda fatores (veja a página [Fatores](factors.pt.qmd)). 
...
* Forneça apenas uma atribuição de coluna de um eixo (tipicamente eixo x). Se você fornecer x e y, você receberá `Error: stat_count() can only have an x or y aesthetic.`  
...
Abaixo está o código utilizando `geom_col` para criar gráficos de barras simples para mostrar a distribuição dos resultados dos pacientes com Ebola. Com o geom_col, tanto x como y precisam ser especificados. Aqui x é a variável categórica ao longo do eixo x, e y é a coluna de proporções geradas `proportion`.  
...
Para mostrarmos as detalhamentos por hospital, precisaríamos que nossa tabela contivesse mais informações e que estivesse em formato "longo". Criamos esta tabela com as freqüências das categorias combinadas `outcome` (Desfecho) e  `hospital` (ver página [Agrupando dados](grouping.pt.qmd) para dicas de agrupamento). 
