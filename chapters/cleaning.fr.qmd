...
Si vous souhaitez comparer ces fonctions aux commandes de Stata ou de SAS, consultez la page [Transition vers R](transition_to_r.fr.qmd).  

Vous pouvez rencontrer une méthode alternative de gestion des données à partir du package  R **data.table** avec des opérateurs comme `:=` et l'utilisation fréquente de crochets `[ ]`. Cette approche et cette syntaxe sont brièvement expliquées dans la page [Data Table](data_table.fr.qmd).  
...
Chargez-les directement avec `appliedepidata::get_data(name = "case_linelists_linelist_raw")` (voir la page [Télécharger le manuel et les données](data_used.fr.qmd) pour plus de détails).    
...
Comme raccourci, vous pouvez aussi renommer les colonnes dans les fonctions **dplyr** `select()` et `summarise()`. `select()` est utilisé pour ne garder que certaines colonnes (et est couvert plus loin dans cette page). La fonction `summarise()` est traitée dans les pages [Regroupement des données](grouping.fr.qmd) et [Tableaux descriptifs](tables_descriptive.fr.qmd). Ces fonctions utilisent également le format `nouveau_nom = ancien_nom`. Voici un exemple :  
...
La fonction `matches()` fonctionne de la même manière que `contains()` mais on peut lui fournir une expression régulière (voir la page sur les [Caractères et chaînes de caractères](characters_strings.fr.qmd)), comme plusieurs chaînes de caractères séparées par des barres OR à l'intérieur des parenthèses :  
...
Il existe des moyens de définir la classe des colonnes avec les commandes d'importation, mais cela est souvent fastidieux. Consultez la section [Bases de R](basics.fr.qmd) sur les classes d'objets pour en savoir plus sur la conversion de la classe des objets et des colonnes.  
...
Si votre dataframe est déjà *groupée* (voir la page sur [Travailler sur des données groupées](grouping.fr.qmd)), `mutate()` peut se comporter différemment que si la base de données n'est pas groupée. Toutes les fonctions de résumé, comme `mean()`, `median()`, `max()`, etc. seront calculées par groupe, et non par toutes les lignes.     
...
<span style="color: red;">**_DANGER:_** **Les valeurs du côté droit doivent toutes être de la même classe** - soit numérique, caractère, date, logique, etc. Pour attribuer des valeurs manquantes (`NA`), Il est dnas certaine situation important d'utiliser  des variantes spéciales de `NA` telles que `NA_character_`, `NA_real_` (pour les numériques ou POSIX), et `as.Date(NA)`. Pour en savoir plus, lisez [Manipuler les dates](dates.fr.qmd).</span>  
...
2) Importez le fichier du dictionnaire dans R. Cet exemple peut être téléchargé via les instructions de la page [Télécharger le manuel et les données](data_used.fr.qmd).  
...
Tout d'abord, examinez la distribution de vos données, afin de définir limites  appropriés. Voir la page sur [les bases de ggplot](ggplot_basics.fr.qmd).  
...
Pour une manière rapide de faire des pauses et de labelliser des vecteurs, utilisez quelque chose comme ci-dessous. Voir la page [Bases de R](basics.fr.qmd) pour les références sur `seq()` et `rep()`.  
...
Pour combiner des ensembles de données ensemble en liant les lignes d'un cadre de données au bas d'un autre cadre de données, vous pouvez utiliser `bind_rows()` de **dplyr**. Ceci est expliqué plus en détail dans la page [Joindre des données](joining_matching.fr.qmd).  
...
*(Voir la page [Manipuler les dates](dates.fr.qmd) pour plus d'informations sur le travail avec des dates et le paquet **lubridate**)*.
...
Listez simplement les colonnes dans l'ordre où elles doivent être triées. Spécifiez `.by_group = TRUE` si vous voulez que le tri se fasse d'abord par tout *groupement* appliqué aux données (voir la page sur [Travailler sur des données groupées](grouping.fr.qmd)).  
...
Le tri des données avec `arrange()` est particulièrement utile lorsque vous créez des [Tableaux de présentation](tables_presentation.fr.qmd), lorsque vous utilisez `slice()` pour prendre les lignes "supérieures" par groupe, ou lorsque vous définissez l'ordre des niveaux de facteurs par ordre d'apparition.  
