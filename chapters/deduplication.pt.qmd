---
aliases:
  - /new_pages/deduplication.pt.html
---
...
Os dados são registros de encontros telefônicos COVID-19, incluindo encontros com contatos e casos. As colunas incluem `recordID` (gerado por computador), `personID`, `name`, `date` do encontro, `time` do encontro, o `purpose` do encontro (para entrevistar como um caso ou como um contato ), e `symptoms_ever` (se a pessoa nesse encontro relatou *sempre* ter sintomas).  
...
Para manter apenas linhas exclusivas de um data frame, use `distinct()` de **dplyr** (conforme demonstrado na página [Limpeza de dados e funções principais](cleaning.pt.qmd)). As duplicidades são removidas de forma que apenas a primeira dessas linhas seja mantida. Por padrão, "primeiro" significa o maior `rownumber` (ordem das linhas de cima para baixo). Apenas linhas exclusivas permanecem.  
...
<span style="color: darkgreen;">**_DICA:_** Ao usar `slice_max()` e `slice_min()`, certifique-se de especificar/escrever o `n = ` (por exemplo, `n = 2` , não apenas `2`). Caso contrário, você pode receber um erro `Error: `...` is not empty.` </span>
...
*No exemplo acima, também seria possível dividir por número de `encounter`, mas mostramos a fatia em `data` e `time` para fins de exemplo.*  
...
Agora você pode ver na coluna `symptoms_ever` que se a pessoa ALGUMA VEZ disse "Yes" aos sintomas, então apenas "Yes" é exibido.
...
Consulte a página em [Juntar dados](joining_matching.pt.qmd) para obter uma explicação sobre este método. A seção sobre Correspondência Probabilística contém um exemplo de aplicação desses algoritmos para comparar um data frame com o *próprio*, realizando assim a desduplicação probabilística.  
