---
aliases:
  - /new_pages/tables_presentation.pt.html
---
...
Instalar e carregar o pacote **flextable**. Neste manual, nós destacamos o `p_load()` do **pacman**, o qual instala o pacote, se necessário, *e* o carrega para ser utilizado. Você também pode carregar os pacotes com `library()` presente no R **base**. Veja a seção sobre [Introdução ao R](basics.pt.qmd) para obter mais informações sobre os pacotes “R”.
...
Para começar, nós importamos a *linelist* dos casos que simulam uma epidemia de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.pt.qmd) para mais detalhes).
...
*Antes* de começar a utilizar o pacote **flextable** você precisará *criar* a sua tabela como um data frame. Veja a seção em [Tabelas descritivas](tables_descriptive.pt.qmd) e [Pivotando dados](pivoting.pt.qmd) para aprender a criar um quadro de dados utilizando pacotes como **janitor*** e **dplyr***. 
...
Abaixo está um exemplo da página [Tabelas descritivas](tables_descriptive.pt.qmd) de conversão dos casos na `linelist` dentro de um conjunto de dados que resume os resultados dos pacientes e os valores de CT por hospital, com uma linha de Totais na parte inferior. A saída é salva como o objeto `table`.
...
Nesta página, por uma questão de clareza, nós devemos salvar a tabela em níveis intermediários como `my_table`, e adicionando as funções **flextable**  uma a uma. Se você quiser ver *todos* os comandos escritos do início ao fim em um único bloco de código, visite a seção logo abaixo [Todos os códigos juntos](#tbl_pres_all).  
...
Por isso, podemos especificar larguras com a função `width()`. Pode ser necessário testar alguns valores para saber qual largura inserir. No exemplo abaixo, especificamos larguras diferentes para cada coluna 1, coluna 2 e colunas 4 a 8.
...
Alinhamos ao centro todas as colunas, exceto a coluna mais à esquerda com os nomes dos hospitais, utilizando a função `align()` de **flextable**.
...
Veja detalhes na seção [Relatórios com R Markdown](rmarkdown.pt.qmd) deste manual.
