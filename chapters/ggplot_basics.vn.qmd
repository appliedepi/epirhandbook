...
Để đạt được điều này, bạn gán thuộc tính biểu đồ của biểu đồ với một *tên cột* (không trong dấu ngoặc kép). Điều này phải được thực hiện *bên trong một hàm `mapping = aes()`* (lưu ý: có một số vị trí trong code mà bạn có thể thực hiện các phép gán, như được thảo luận [bên dưới](ggplot_basics.vn.qmd#ggplot_basics_map_loc)).
...
malaria_data <- appliedepidata::get_data(name = "malaria_facility_count_data") %>%  # import
