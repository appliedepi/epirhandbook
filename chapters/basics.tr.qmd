...
**Functions** - Fonksiyon, girdileri kabul eden ve çıktıya dönüştüren bir kod işlemidir. Daha fazlasını [Fonksiyonlar bölümünde](basics.tr.qmd#functions) okuyabilirsiniz.
...
Fonksiyonlar, R'ı kullanmanın merkezinde yer alır. Fonksiyonlar, görevleri ve işlemleri nasıl gerçekleştirdiğinizdir. Pek çok fonksiyon R ile kurulu olarak gelir, daha pek çoğu *paketlerde* indirilebilir ([paketler](basics.tr.qmd#packages) bölümünde açıklanmıştır) ve hatta kendi özel Fonksiyonlarınızı bile yazabilirsiniz!
...
- Kullanılacak iki rengi belirtin (`pal =`, "palet"in kısaltmasıdır ve iki renk adından oluşan bir vektörle sağlanır. ""c()" fonksiyonunun nasıl vektör oluşturduğunu öğrenmek için [nesneler](basics.tr.qmd#objects) sayfasına bakın)
...
**Çalışma dizininizi yönetmek için önerdiğimiz yaklaşımın ayrıntıları için [R projeleri](r_projects.tr.qmd) sayfasına bakın.**\
Çalışma dizininizi ve dosya yollarınızı yönetmenin yaygın, verimli ve sorunsuz bir yolu, şu 3 öğeyi bir [R projesi](r_projects.tr.qmd) odaklı iş akışında bir araya getirmektir:

1)  Tüm dosyalarınızı saklamak için bir R Projesi ([R projeleri](r_projects.tr.qmd) sayfasına bakın)\
2)  Dosyaları bulmak için **here** paketi ([İçe ve dışa aktarma](importing.tr.qmd) sayfasına bakın)\
3)  Dosyaları içe/dışa aktarmak için **rio** paketi ([İçe ve dışa aktarma](importing.tr.qmd) sayfasına bakın)
...
+------------------+---------------------------------------------------------------------------------------+
| `as.Date()`      | Tarih sınıfına dönüştürür - Not: Ayrıntılar için [dates](dates.tr.qmd) bölümüne bakıns             |
+------------------+---------------------------------------------------------------------------------------+
