...
Nạp bộ `linelist` đã được làm sạch bằng hàm `get_data()` từ package **appliedepidata** (xem chương [Tải sách và dữ liệu](data_used.vn.qmd)).
...
Tất cả các phân cấp hành chính của Sierra Leone đến từ [website của Humanitarian Data Exchange (HDX) ](https://data.humdata.org/dataset/sierra-leone-all-ad-min-level-boundaries). Chương này nạp chúng bằng `get_data()` từ package **appliedepidata**, như được giải thích trong trang [Tải sách và dữ liệu](data_used.vn.qmd).
...
Để đọc shapefile của riêng bạn, hãy dùng hàm `read_sf()` từ package **sf** với đường dẫn tệp của nó (xem chương [Nhập xuất dữ liệu](importing.vn.qmd) và [Dự án R](r_projects.vn.qmd) để biết thêm chi tiết).
...
Những dữ liệu này nằm trong R package **appliedepidata** (xem chương [Tải sách và dữ liệu](data_used.vn.qmd)). Chúng cũng có trên HDX (link [tại đây](https://data.humdata.org/dataset/sierra-leone-population)). Chúng ta nạp dữ liệu bằng hàm `get_data()`, sau đó chuyển kết quả tới hàm `clean_names()` để chuẩn hóa cú pháp tên cột.
...
Thông tin vị trí của các cơ sở y tế đến từ HDX [tại đây](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities). Chương này nạp chúng bằng `get_data()` từ package **appliedepidata**, như được giải thích trong chương [Tải sách và dữ liệu](data_used.vn.qmd).
