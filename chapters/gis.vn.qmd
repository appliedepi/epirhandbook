...
Để nạp shapefile, chúng ta sử dụng hàm `get_data()` từ package **appliedepidata**; hàm này trả về shapefile dưới dạng một đối tượng **sf**. Để đọc shapefile của riêng bạn, hãy dùng hàm `read_sf()` từ package **sf** với đường dẫn tệp của nó (xem chương [Nhập xuất dữ liệu](importing.vn.qmd) và [Dự án R](r_projects.vn.qmd) để biết thêm chi tiết).
...
Bộ dữ liệu dân số trông sẽ như bên dưới. Cuộn sang bên phải để xem các cột: dân số nam (`male` population), dân số nữ (`female` population), tổng dân số (`total` population), và dân số theo từng nhóm tuổi.
...
Chúng ta nạp tọa độ điểm các cơ sở với hàm `get_data()`. Chúng ta làm sạch lại tên cột, sau đó lọc để chỉ giữ lại các điểm được gắn thẻ là "hospital", "clinic", hoặc "doctors".
