---
aliases:
  - /new_pages/tables_presentation.es.html
---
...
Comprende que *antes de* utilizar **flextable**, debes crear la tabla resumen como un dataframe. Utiliza los métodos de las páginas [Tablas descriptivas](tables_descriptive.es.qmd) y [Pivotar de datos](pivoting.es.qmd), como tabulaciones, tabulaciones cruzadas, pivoteo y cálculo de estadísticas descriptivas. El dataframe resultante puede pasarse a **flextable** para ponerle el formato.

Hay muchos otros paquetes de R que se pueden utilizar para elaborar tablas para su presentación - hemos elegido destacar **flextable** en esta página. Un ejemplo que utiliza el paquete **knitr** y su función `kable()` se puede encontrar en la página [rastreo de contactos](contact_tracing.es.qmd). Asimismo, el paquete **DT** se destaca en la página [Dashboards con Shiny](shiny_basics.es.qmd). Otros como **GT** y **huxtable** se mencionan en la página de [Paquetes recomendados](packages_suggested.es.qmd).
...
Instala y carga **flextable**. En este manual destacamos `p_load()` de **pacman**, que instala el paquete si es necesario *y* lo carga para su uso. También puedes cargar paquetes con `library()` de R **base**. Consulta la página sobre [fundamentos de R](basics.es.qmd) para obtener más información sobre los paquetes de R. 
...
Para empezar, importamos los datos limpios de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.es.qmd) para más detalles).
...
*Antes de* empezar a utilizar **flextable** tendrás que *crear* tu tabla como un dataframe. Consulta la página sobre [Tablas descriptivas](tables_descriptive.es.qmd) y [Pivotar datos](pivoting.es.qmd) para aprender a crear un dataframe utilizando paquetes como **janitor** y **dplyr**. Debes organizar el contenido en filas y columnas tal y como quieres que se muestre. Luego, el dataframe se pasará a **flextable** para mostrarlo con colores, encabezados, fuentes, etc.

A continuación se muestra un ejemplo de la página de [tablas descriptivas](tables_descriptive.es.qmd) para convertir la lista de casos en un dataframe que resume los resultados de los pacientes y los valores de TC por hospital, con una fila de totales en la parte inferior. El resultado se guarda como `table`. 
...
Mira los detalles en la página de [Informes con R Markdown](rmarkdown.es.qmd) de este manual.
