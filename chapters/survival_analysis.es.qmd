---
aliases:
  - /new_pages/survival_analysis.es.html
---
...
En este manual destacamos `p_load()` de **pacman**, que instala el paquete si es necesario *y* lo carga para su uso. También puedes cargar los paquetes instalados con `library()` de R **base**. Consulta la página sobre [fundamentos de R](basics.es.qmd) para obtener más información sobre los paquetes de R.
...
- pacientes censurados son aquellos que se recuperaron o para los que no se conoce el resultado final, es decir, no se observó el evento "muerte" (`event=0`).
...
linelist_surv <-  linelist_case_data %>% 
  ...
  dplyr::mutate(
       # crear la var evento que es 1 si el paciente falleció y 0 si fue censurado por la derecha
...
También podemos cruzar las columnas `age_cat_small` y `gender` para tener más detalles sobre la distribución de esta nueva columna por género. Utilizamos `tabyl()` y las funciones de *adorno* de **janitor** como se describe en la página de [tablas descriptivas](tables_descriptive.es.qmd).
...
```{r survobj }
# Utilizar la sintaxis Suv() para datos censurados por la derecha
...
Y aquí están los primeros 10 elementos de `survobj`. Se imprime esencialmente como un vector de tiempo de seguimiento, con "+" para representar si una observación fue censurada por la derecha. Mira cómo los números se alinean arriba y abajo.  
...
¿Qué ocurre si tienes interés en una covariable que se mide **después** de que comience el tiempo de seguimiento? O, ¿qué pasa si tienes una covariable que puede cambiar con el tiempo?
...
De nuevo, visualizaremos los resultados de nuestro modelo de Cox utilizando la función `ggforest()` del paquete **survminer**.:
