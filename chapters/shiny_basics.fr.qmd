---
aliases:
  - /new_pages/shiny_basics.fr.html
---
...
Pour une méthode alternative de création de tableaux de bord qui est plus rapide, plus facile, mais peut-être moins personnalisable, voir la page sur **flextable** ([Tableaux de bord avec R Markdown](flexdashboard.fr.qmd)).  
...
Dans ce manuel, nous mettons l'accent sur `p_load()` de **pacman**, qui installe le package si nécessaire *et* le charge pour l'utiliser. Vous pouvez aussi charger les packages installés avec `library()` de **base** R. Voir la page sur [bases de R](basics.fr.qmd) pour plus d'informations sur les packages R.  
...
Si vous souhaitez suivre cette page, consultez la section du [manuel pour le téléchargement des données](data_used.fr.qmd). Il y a des liens pour télécharger les scripts R et les fichiers de données qui produisent l'application Shiny finale.  
...
Les conducteurs réactifs sont des objets qui sont créés dans le serveur shiny de manière *réactive*, mais qui ne sont pas envoyés en sortie - ils peuvent simplement être utilisés par d'autres parties du serveur. Il existe un certain nombre de types différents de *conducteurs réactifs*, mais nous allons passer en revue les deux principaux.
...
Si vous préférez partager le code, vous pouvez créer un fichier .zip de l'application ou, mieux encore, *publier votre application sur github et ajouter des collaborateurs*. Vous pouvez vous reporter au chapitre sur github pour plus d'informations.
