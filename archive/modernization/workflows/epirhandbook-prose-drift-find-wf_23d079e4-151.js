...
const STEMS = args && args.stems ? args.stems : null

if (!STEMS) throw new Error('args.stems is required (array of 48 declared chapter stems)')
...
1. Split BOTH files into sections at markdown headings (lines matching ^#{1,6} followed by whitespace, OUTSIDE code chunks). Before matching heading text, STRIP the trailing attribute block: {#anchor}, {.unnumbered}, {#id .class}. 41% of headings carry one and they are often untranslated, so they align the sections for you.
...
log(`Phase 1b find stage: ${PAIRS.length} chapter-language pairs, ${STEMS.length} stems x ${LANGS.length} languages`)
