...
The example dataset used in this section is weekly counts of campylobacter cases reported in Germany between 2001 and 2011. Load it directly with `appliedepidata::get_data(name = "campylobacter_germany")` (see the [Download handbook and data](data_used.qmd) page for details).
...
The chunk below writes the 10 years of ".nc" climate data files into a temporary folder. It calls `save_data()` from **appliedepidata**, then unzips the archive.  
