...
We import the dataset of cases from a simulated Ebola epidemic. Load it directly with `appliedepidata::get_data(name = "linelist_cleaned_rds")` (see the [Download handbook and data](data_used.qmd) page for details).
...
The script is an R Markdown script, and so has the same components and organization as described in the page on [Reports with R Markdown](rmarkdown.qmd). We briefly re-visit these and highlight differences from other R Markdown output formats.  
...
Outside of an R code "chunk", you can write narrative text. As described in the page on [Reports with R Markdown](rmarkdown.qmd), you can italicize text by surrounding it with one asterisk (*), or bold by surrounding it with two asterisks (**). Recall that bullets and numbering schemes are sensitive to newlines, indentation, and finishing a line with two spaces.  

You can also insert in-line R code into text as described in the [Reports with R Markdown](rmarkdown.qmd) page, by surrounding the code with backticks and starting the command with "r": `` `r knitr::inline_expr("1+1")` ``(see example with date above).  



### Headings {.unnumbered}  

Different heading levels are established with different numbers of hash symbols, as described in the [Reports with R Markdown](rmarkdown.qmd) page.  
...
You can easily include standard R outputs such as text, ggplots, and tables (see [Tables for presentation](tables_presentation.qmd) page). Simply code them within an R code chunk as you would for any other R Markdown script.  

Note: you can download the finished Rmd script and HTML dashboard output - see the [Download handbook and data](data_used.qmd) page.  


### Text {.unnumbered}  

You can type in Markdown text and include *in-line* code as for any other R Markdown output. See the [Reports with R Markdown](rmarkdown.qmd) page for details. 
...
You may elect to have all code within the R Markdown **flexdashboard** script. Alternatively, to have a more clean and concise dashboard script you may choose to call upon code/figures that are hosted or created in external R scripts. This is described in greater detail in the [Reports with R Markdown](rmarkdown.qmd) page. 
...
You can use **shiny** to produce apps/dashboards *without* flexdashboard too. The handbook page on [Dashboards with Shiny](shiny_basics.qmd) gives an overview of this approach, including primers on **shiny** syntax, app file structure, and options for sharing/publishing (including free server options). These syntax and general tips translate into the **flexdashboard** context as well.  
