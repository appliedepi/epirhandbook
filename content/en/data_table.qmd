...
We import the dataset of cases from a simulated Ebola epidemic. Load it directly with `appliedepidata::get_data(name = "linelist_cleaned_excel")` (see the [Download handbook and data](data_used.qmd) page for details). From here we use `data.table()` to convert the data frame to a data table.
...
The first example retrieves the first, third and fifth columns of the data table, the second example selects all columns except the height, weight, age and gender columns. The third example uses the `.()` wrap to select the **case_id** and **outcome** columns.
