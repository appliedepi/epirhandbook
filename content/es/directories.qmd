...
Proporciona la ruta de la carpeta a `path = `. Utiliza `recurse = FALSE` para mostrar un solo nivel, o `recurse = TRUE` para mostrar todos los archivos en todos los subniveles. A continuación utilizamos `here()` como abreviatura del proyecto R y especificamos su subcarpeta "data", que contiene los datos de ejemplo de este manual. Lo configuramos para que muestre todos los archivos dentro de "data" y sus subcarpetas.
...
Esto puede ser especialmente útil si quieres extraer la última hora de modificación del archivo, por ejemplo si quieres importar la versión más reciente de un archivo. Para ver un ejemplo de esto, consulta la página de [importación y exportación](importing.qmd).
...
`render()` es una variación de `source()` que se utiliza más a menudo para los scripts de R markdown. Tu pescribes `input = ` que es el archivo R markdown, y también `output_format = ` (normalmente "html_document", "pdf_document", "word_document", "")

Mira la página sobre [Informes con R Markdown](rmarkdown.qmd) para más detalles. También consulta la documentación de `render()` [aquí](https://rmarkdown.rstudio.com/docs/reference/render.html) o escribiendo `?render`.
...
Consulta la página sobre [importación y exportación](importing.qmd) para importar y exportar archivos individuales.

Consulta también la página de [importación y exportación](importing.qmd) para conocer los métodos para importar automáticamente el archivo más reciente, basándose en una fecha del nombre del archivo *o* mirando los metadatos del mismo.

Consulta la página sobre [Iteración, bucles y listas](iteration.qmd) para ver un ejemplo con el paquete **purrr** demostrando:
