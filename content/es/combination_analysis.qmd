...
Consulta la página sobre [Caracteres y cadenas](characters_strings.qmd) para saber más sobre la función `unite()` de **stringr**
...
Si está interesado en un comando más eficiente, puede aprovechar la función `+()`, que convierte en 1s y 0s basándose en una sentencia lógica. Este comando utiliza la función `across()` para cambiar varias columnas a la vez (lea más en [Limpieza de datos y funciones básicas](cleaning.qmd#clean_across)).  
