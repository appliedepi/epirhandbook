...
* Consulta la página de [Paquetes recomendados](packages_suggested.qmd) para ayudarte a instalar los paquetes de R adecuados antes de que pierdas la conectividad a Internet
...
Después puedes importarlo con `import()` como cualquier archivo de tu ordenador; consulta la página de [Importar y exportar](importing.qmd) para más detalles. Ten en cuenta que `save_data()` no descomprime automáticamente los conjuntos de datos distribuidos como archivo .zip (por ejemplo los shapefiles de varios archivos, o las carpetas de archivos); utiliza después `utils::unzip()` sobre el .zip guardado.
...
Se trata de datos ficticios de una encuesta tipo Likert, utilizados en la página sobre [Pirámides de población y escalas de Likert](age_pyramid.qmd). Puedes cargar estos datos directamente en R ejecutando los siguientes comandos:
...
A continuación se encuentran los enlaces a los archivos asociados a la página sobre [Dashboards con R Markdown](flexdashboard.qmd). Son archivos fuente de R Markdown/HTML, no conjuntos de datos de ejemplo, por lo que no forman parte del paquete **appliedepidata**. Descárgalos directamente desde Github:
...
La página de [rastreo de contactos](contact_tracing.qmd) muestra el análisis de los datos de rastreo de contactos, utilizando como ejemplo datos de [Go.Data](https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting). Estos datos no forman parte de **appliedepidata**; los datos utilizados en la página pueden descargarse como archivos .rds clicando en los siguientes enlaces:
...
<span style="color: darkgreen;">**_CONSEJO:_** Si estás utilizando Go.Data y quieres conectarte a tu instancia de la API, consulta la página de importación y exportación [(sección API)](importing.qmd#import_api) y la [Comunidad de Prácticas de Go.Data](https://community-godata.who.int/).</span>
...
La página de [Conceptos básicos de los SIG](gis.qmd) carga los tres conjuntos de datos: los límites admin-3 `sle_adm3`, los puntos de centros sanitarios `sle_hf` y la tabla de población `sle_admpop_adm3_2020`. Para leer un shapefile desde tu propia ruta de archivo, utiliza `read_sf()` del paquete **sf**. Esa página también enlaza al sitio *Humanitarian Data Exchange*, donde puedes descargar más shapefiles como archivos comprimidos. Los puntos de centros sanitarios, por ejemplo, están [aquí](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities).

#### Árboles filogenéticos {.unnumbered}  

Mira la página sobre [árboles filogenéticos](phylogenetic_trees.qmd). El archivo Newick con el árbol filogenético construido a partir de la secuenciación del genoma completo de 299 muestras de Shigella sonnei y los datos de las muestras correspondientes (convertidos en un archivo de texto). Las muestras belgas y los datos resultantes han sido proporcionados amablemente por el NRC belga para Salmonella y Shigella en el marco de un proyecto dirigido por un fellow del programa ECDC EUPHEM, y también se publicarán en un manuscrito. Los datos internacionales están disponibles en bases de datos públicas (ncbi) y han sido publicados previamente.
...
Consulta la página de [tasas estandarizadas](standardization.qmd). Puedes cargar los datos directamente con los siguientes comandos:
...
Para el capítulo sobre [análisis de encuestas](survey_analysis.qmd) utilizamos datos ficticios de encuestas de mortalidad basados en las plantillas de encuestas de MSF OCA. Estos datos ficticios se generaron como parte del [proyecto "R4Epis"](https://r4epis.netlify.app/).
...
El capítulo sobre [Dashboards con Shiny](shiny_basics.qmd) demuestra la construcción de una sencilla aplicación para mostrar datos sobre la malaria.
