...
En esta página cubriremos consejos y trucos para hacer que tus ggplots sean nítidos y elegantes. Consulta la página sobre [Conceptos básicos de ggplot](ggplot_basics.qmd) para conocer los fundamentos.
...
Para esta página, importamos el conjunto de datos de casos de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.qmd) para más detalles).
...
Los matices de las escalas de fechas se tratan más ampliamente en la página de [curvas epidémicas](epicurves.qmd).
...
Para demostrar cómo ajustar una escala de colores continua, utilizaremos unos datos de la página de [Rastreo de contactos](contact_tracing.qmd) que contiene las edades de los casos y de sus casos de origen.
...
Aquí hay un ejemplo de la página de [Conceptos básicos de ggplot](ggplot_basics.qmd), utilizando varios esquemas de color. 
...
Consulta un ejemplo completo en la sección sobre [etiquetas de fecha de varios niveles](epicurves.qmd) de la página de curvas epidémicas, pero a continuación se muestra un ejemplo rápido como referencia:
...
Observa que hay otro ejemplo de esta técnica en la página de [curvas epidémicas](epicurves.qmd): la superposición de la incidencia acumulada sobre la curva epidemica.
