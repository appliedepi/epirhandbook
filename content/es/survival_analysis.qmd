...
En este manual destacamos `p_load()` de **pacman**, que instala el paquete si es necesario *y* lo carga para su uso. También puedes cargar los paquetes instalados con `library()` de R **base**. Consulta la página sobre [fundamentos de R](basics.qmd) para obtener más información sobre los paquetes de R.
...
También podemos cruzar las columnas `age_cat_small` y `gender` para tener más detalles sobre la distribución de esta nueva columna por género. Utilizamos `tabyl()` y las funciones de *adorno* de **janitor** como se describe en la página de [tablas descriptivas](tables_descriptive.qmd).
