...
Para reunir los tres componentes (contenedor, secuencia y operaciones) vamos a intentar trazar una epicurva para cada hospital (véase la página sobre [curvas epidémicas](epicurves.qmd)). 
...
Ten en cuenta que **purrr** es una *herramienta de programación funcional*. Es decir, las operaciones que se van a aplicar de forma iterativa están envueltas en *funciones*. Consulta la página [Escribir funciones](writing_functions.qmd) para aprender a escribir tus propias funciones. 
...
Recuerda de la página de [importación y exportación](importing.qmd) que cuando se utiliza en libros de Excel, `import()` puede aceptar el argumento `which = ` especificando la hoja a importar. Dentro de la función `.f` en  `import()`, proporcionamos `which = .x`, cuyo valor cambiará con cada iteración a través del vector `sheet_names` - primero "Central Hospital", luego "Military Hospital", etc. 
...
De nuevo utilizamos `map()`: tomamos el vector de nombres de elementos de la lista (mostrado arriba) y utilizamos `map()` para iterar a través de ellos, aplicando `export()` (del paquete **rio**, véase la página [Importar y exportar](importing.qmd)) en el dataframe de lista `linelist_split` que tiene ese nombre. También utilizamos el nombre para crear un nombre de archivo único. Así es como funciona: 
...
* La ruta del archivo proporcionada a `export()` es dinámica mediante el uso de `str_glue()` (ver página de [caracteres y cadenas](characters_strings.qmd)): 
...
Recuerda de la página sobre [Test estadísticos simples](stat_tests.qmd) que `t.test()` puede tomar entradas en un formato de fórmula, como `t.test(columna numérica ~ columna binaria)`. En este ejemplo, hacemos lo siguiente: 
...
Nota: Recuerda que si deseas aplicar una función sólo a determinadas columnas de un dataframe, puedes utilizar simplemente `mutate()` y `across()`, como se explica en la página [Limpieza de datos y funciones básicas](cleaning.qmd). A continuación se muestra un ejemplo de aplicación de `as.character()` sólo a las columnas "age". Observa la colocación de los paréntesis y las comas.   
