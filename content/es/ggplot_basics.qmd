...
En esta página cubriremos los fundamentos de la creación de gráficos con **ggplot2**. Consulta la página [Consejos de ggplot](ggplot_tips.qmd) para sugerencias y técnicas avanzadas para lograr que sus gráficos se vean realmente bien.
...
Importamos el conjunto de datos de casos de una epidemia de Ebola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.qmd) para más detalles).
...
* Limpia los valores de algunas columnas para cambiar texto "amigable con los datos" con barra baja, etc. a texto normal o a mayúsculas y minúsculas (ver [Caracteres y cadenas](characters_strings.qmd))
...
Como una cuestión de estructura de datos, para **ggplot2** a menudo queremos pivotar nuestros datos en formatos *largos*. Lee más sobre esto en la página de [Pivoteo de datos](pivoting.qmd).  
...
Para ajustar el orden de los grupos en un gráfico, consulta la página de [Consejos de ggplot](ggplot_tips.qmd) o la página sobre [Factores](factors.qmd). Hay muchos ejemplos de gráficos agrupados en las secciones siguientes sobre el trazado de datos continuos y categóricos. 
...
Recuerda que podés utilizar la sintaxis `here()` para proporcionar la ruta de archivo deseada. Consulta la página de [importación y exportación](importing.qmd) para obtener más información.  
...
Consulta esta [sección sobre las escalas de color de la página de consejos de ggplot](ggplot_tips.qmd#ggplot_tips_colors).  
...
* Gráficos para una variable continua:
  ...
  * **Gráficos de calor** para tres variables continuas (enlazado a la página de [gráficos de calor](heatmaps.qmd))
...
Para ver el código para añadir un gráfico de caja a los bordes de un gráfico de dispersión (gráficos "marginales"), consulta la página [Consejos de ggplot](ggplot_tips.qmd).
...
Los datos categóricos pueden ser valores de carácter, pueden ser lógicos (TRUE/FALSE), o factores (ver la página de [Factores](factors.qmd)).
...
Podemos ver que los valores de la variable hospital son caracteres, ya que son nombres de hospitales, y por defecto están ordenados alfabéticamente. Hay valores `Other` y `Missing`, que preferiríamos que fueran las últimas subcategorías al presentar los desgloses. Así que convertimos esta columna a clase factor y la reordenamos. Esto se trata con más detalle en la página de [Factores](factors.qmd).
...
Para mostrar los desgloses por hospital, necesitaríamos que nuestra tabla contuviera más información, y que estuviera en formato "largo". Creamos esta tabla con las frecuencias de las categorías combinadas `outcome` y `hospital` (véase la página de [Agrupar datos](grouping.qmd) para obtener consejos de agrupación). 
