...
Para la regresión de riesgos proporcionales de Cox, véase la página de [análisis de supervivencia](survival_analysis.qmd).
...
Importaremos los datos de casos de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.qmd) para más detalles).  
...
Al igual que en la página sobre [Tablas descriptivas](tables_descriptive.qmd), en función de la tarea que vayas a realizar, podremos elegir que función emplear. A continuación presentamos dos opciones para realizar análisis univariantes:
...
Para modificar el nivel de referencia de una variable determinada, asegúrate de que la columna es del tipo Factor y mueve el nivel deseado a la primera posición con `fct_relevel()` (véase la página sobre [Factores](factors.qmd)). Por ejemplo, a continuación tomamos la columna `age_cat` y establecemos "20-29" como línea de base antes de conectar mediante pipes el dataframe modificado con `glm()`.
...
2.  Combina estos resultados del modelo con una tabla de recuentos. A continuación, creamos la tabla cruzada de recuentos con la función `tabyl()` de **janitor**, como se explica en la página de [tablas descriptivas](tables_descriptive.qmd).
...
Este es el aspecto del dataframe combinado, impreso de forma agradable como una imagen con una función de **flextable**. En [Tablas para presentación](tables_presentation.qmd) se explica cómo personalizar dichas tablas con **flextable**, o bien puede utilizar otros paquetes como **knitr** o **GT**. 
...
Para ejecutar los modelos en varias variables de exposición para producir odds ratios univariantes (es decir, sin controlar entre sí), se puede utilizar el enfoque siguiente. Utiliza `str_c()` de **stringr** para crear fórmulas univariantes (véase [Caracteres y cadenas](characters_strings.qmd)), ejecuta la regresión `glm()` en cada fórmula, pasa cada resultado de `glm()` a `tidy()` y finalmente junta todos los resultados de los modelos resultantes con `bind_rows()` de **tidyr**. Este enfoque utiliza `map()` del paquete **purrr** para iterar - véase la página sobre [Iteración, bucles y listas](iteration.qmd) para más información sobre esta herramienta.
...
A continuación se muestra el aspecto del dataframe. Consulta la página sobre [Tablas para presentación](tables_presentation.qmd) para obtener ideas sobre cómo convertir esta tabla en una bonita tabla HTML (por ejemplo, con **flextable**).
...
Consulta la página sobre [Conceptos básicos de ggplot](ggplot_basics.qmd) si no estás familiarizado con el paquete de gráficos **ggplot2**.
