...
Para empezar, importamos la lista de casos limpiada de una epidemia de ébola simulada. Cárguelo directamente con `appliedepidata::get_data(name = "linelist_cleaned_rds")` (véase la página [Descargando el manual y los datos](data_used.qmd) para más detalles).  
...
La coincidencia probabilística también puede utilizarse para de-duplicar unos datos. Consulta la página sobre [de-duplicación](deduplication.qmd) para conocer otros métodos de de-duplicación. 
...
* Puedes utilizar `bind_rows()` en una lista de dataframes de estructura similar para combinarlos en un dataframe. Mira un ejemplo en la página [Iteración, bucles y listas](iteration.qmd) que implica la importación de múltiples listas de líneas con **purrr**. 
...
Mira otros ejemplos trabajados como éste en las páginas de [Tablas descriptivas](tables_descriptive.qmd) y [Tablas para presentaciones](tables_presentation.qmd). 
...
Ahora podemos ver el resultado. Observa cómo en la última fila se rellena un valor `NA` vacío para la columna hospital que no estaba en hosp_summary. Como se explica en la página de [Tablas para presentaciones](tables_presentation.qmd), podrías "rellenar" esta celda con "Total" utilizando replace_na().   
