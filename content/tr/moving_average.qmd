...
Simüle edilmiş Ebola salgını vakalarını içeren veri setini içe aktarıyoruz. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.qmd) sayfasına bakın).    
...
Her tarih için değer içeren temiz bir zaman serisi veriniz varsa, burada [Zaman Serisi ve Salgın Tespiti](time_series.qmd#timeseries_moving) sayfasında gösterildiği gibi `slide_dbl()` fonksiyonunu kullanabilirsiniz.
