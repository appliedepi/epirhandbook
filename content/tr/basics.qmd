---
aliases:
  - /new_pages/basics.tr.html
---

# R'ın Temelleri {#rbasics}
...
STATA, SAS veya Excel'den R'a geçişle ilgili ipuçları için [R'a Geçiş](transition_to_r.qmd) hakkındaki sayfaya bakabilirsiniz.
...
**Functions** - Fonksiyon, girdileri kabul eden ve çıktıya dönüştüren bir kod işlemidir. Daha fazlasını [Fonksiyonlar bölümünde](#functions) okuyabilirsiniz.
...
- Haritacılık (GIS)\
- **leaflet** paketi (etkileşimli haritalar)\
...
Bu El Kitabı hazırlanırken hangi R, RStudio veya paket sürümlerinin kullanıldığını görmek için [Editoryal ve teknik notlar](editorial_style.qmd) sayfasına bakın.
...
Fonksiyonlar, R'ı kullanmanın merkezinde yer alır. Fonksiyonlar, görevleri ve işlemleri nasıl gerçekleştirdiğinizdir. Pek çok fonksiyon R ile kurulu olarak gelir, daha pek çoğu *paketlerde* indirilebilir ([paketler](#packages) bölümünde açıklanmıştır) ve hatta kendi özel Fonksiyonlarınızı bile yazabilirsiniz!
...
```{r, basics_functions_pyramid2, eval = FALSE, warning=FALSE, message=FALSE, out.width = "75%", out.height="75%"}
...
- Sayılar yerine oranları göster (varsayılan `FALSE` olduğunda `proportional = TRUE` olarak ayarlayın)\
- Kullanılacak iki rengi belirtin (`pal =`, "palet"in kısaltmasıdır ve iki renk adından oluşan bir vektörle sağlanır. ""c()" fonksiyonunun nasıl vektör oluşturduğunu öğrenmek için [nesneler](#objects) sayfasına bakın)

[***NOT:*** İki taraflı belirtilecek değişkenler için (ör. `proportional = TRUE`) değişkenlerin sıraları önemsizdir.]{style="color: black;"}
...
Bir fonksiyonun nasıl yazılacağı [Fonksiyon Yazma](writing_functions.qmd) sayfasında derinlemesine ele alınmıştır.
...
`.libPaths()` (boş parantezler) girerek kitaplığınızın dosya yolunu yazdırabilirsiniz. [Ağ sürücülerinde R](network_drives.qmd) ile çalışırken bu özellikle önemlidir.
...
Github sürümü ile bilgisayarınızdaki sürüm arasında herhangi bir fark yoksa herhangi bir işlem yapılmayacaktır. Bunun yerine "p_load_current_gh()" komutunu "update = TRUE" argümanıyla kullanarak yeniden yüklemeyi "zorlayabilirsiniz" (force). Bu [çevrimiçi gösterimde] http://trinker.github.io/pacman/vignettes/Introduction_to_pacman.html adresinde **pacman** hakkında daha fazla bilgi edinin)
...
Günlük epidemiyoloji için önerdiğimiz paketlerin listesi için [Önerilen paketler](packages_suggested.qmd) sayfasına bakabilirsiniz.
...
\#'dan sonra yazılan hiçbir kod çalıştırılmayacaktır. Bu nedenle, kodun önüne bir \# koymak, silmek istemiyorsanız bir kod satırını geçici olarak engellemenin ("yorum yapma") da yararlı bir yoludur. Birden fazla satırı seçip (vurgulayıp) Ctrl+Shift+c (Mac'te Cmd+Shift+c) tuşlarına basarak bu satırları aynı anda yorum satırı haline getirebilir veya yorumdan çıkarabilirsiniz.
...
Yeni başlayan R kullanıcılarının da R Markdown'ı kullanabileceğini belirtmekte fayda var - gözünüz korkmasın! Daha fazla bilgi edinmek için, [R Markdown ile Raporlar](rmarkdown.qmd) belgelerindeki el kitabı sayfasına bakabilirsiniz.
...
[Shiny ile Panolar](shiny_basics.qmd)'daki el kitabı sayfasına veya bu çevrimiçi eğiticiye bakabilirsiniz: [Parlak öğretici](https://shiny.rstudio.com/tutorial/write-tutorial/lesson1/)
...
### Önerilen yaklaşım {.unnumbered}

**Çalışma dizininizi yönetmek için önerdiğimiz yaklaşımın ayrıntıları için [R projeleri](r_projects.qmd) sayfasına bakın.**\
Çalışma dizininizi ve dosya yollarınızı yönetmenin yaygın, verimli ve sorunsuz bir yolu, şu 3 öğeyi bir [R projesi](r_projects.qmd) odaklı iş akışında bir araya getirmektir:

1)  Tüm dosyalarınızı saklamak için bir R Projesi ([R projeleri](r_projects.qmd) sayfasına bakın)\
2)  Dosyaları bulmak için **here** paketi ([İçe ve dışa aktarma](importing.qmd) sayfasına bakın)\
3)  Dosyaları içe/dışa aktarmak için **rio** paketi ([İçe ve dışa aktarma](importing.qmd) sayfasına bakın)

### Komutla ayarlama {.unnumbered}

Yakın zamana kadar, R öğrenen birçok kişiye komut dosyalarına `setwd()` komutuyla başlamaları öğretiliyordu. Bunun yerine lütfen [R projesi](r_projects.qmd) odaklı bir iş akışı kullanmayı düşünün ve [`setwd()` kullanmama nedenleriniz için okuyunuz](https://www.tidyverse.org/blog/2017/12/workflow-vs-script/). Kısacası, işiniz bilgisayarınıza özel hale gelir, dosyaları içe ve dışa aktarmak için kullanılan dosya yolları "kırılgan" hale gelir ve bu, işbirliğini ve kodunuzun başka herhangi bir bilgisayarda kullanılmasını ciddi şekilde engeller. Kolay alternatifler var!
...
[***TEHLİKE:*** `setwd()` ile bir çalışma dizini ayarlamak dosya yolu bir bilgisayara özelse "kırılgan" *olabilir*. Bunun yerine, bir R Project kök dizinine göre dosya yollarını kullanın (**here** paketiyle). ]{style="renk: kırmızı;"}
...
Bir R markdown komut dosyasında, varsayılan çalışma dizini, Rmarkdown dosyasının (`.Rmd`) içinde kaydedildiği klasördür. Bir R projesi ve **here** paketi kullanılıyorsa, bu geçerli değildir ve çalışma dizini [R projeleri](r_projects.qmd) sayfasında açıklandığı gibi 'here()' olacaktır.
...
`<-` atama operatörünü "olarak tanımlanır" diye okuyabilirsiniz. Atama komutları genellikle standart bir sıra izler:
...
[İçe aktarma ve dışa aktarma](importing.qmd) bölümünden veri kümelerini içe ve dışa aktarma hakkında daha fazla bilgi edinebilirsiniz.
...
Bununla birlikte, bu el kitabında, **dplyr** paketindeki `mutate()` fonksiyonunu ve tünel operatörüyle (`%>%`) *piping* fonksiyonunu kullanan sütunları tanımlamaya yönelik farklı bir yaklaşımı vurguluyoruz. Sözdiziminin okunması daha kolaydır ve [Temizleme verileri ve temel fonksiyonlar](cleaning.qmd) sayfasında açıklanan başka avantajları da vardır. *tünelleme* hakkında daha fazla bilgiyi aşağıdaki Tünelleme bölümünde okuyabilirsiniz.
...
Aşağıdaki grafik [bu çevrimiçi R eğitiminden](http://venus.ifca.unican.es/Rintro/dataStruct.html) ödünç alınmıştır. Bazı yaygın veri yapılarını ve adlarını gösterir. [GIS temelleri](gis.qmd) sayfasında tartışılan uzamsal veriler bu görüntüye dahil değildir.
...
+------------------+--------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------+
| Veri Çerçeveleri   | Hepsi aynı sayıda satıra sahip olan birbirine bağlı vektörler (örneğin sütunlar).           | `linelist` bir veri çerçevesidir.                                                         |
+------------------+--------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------+
...
+------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------+
| Tarih       |**R'a belirli verilerin Tarih olduğu söylendiğinde**, bu veriler özel yollarla manipüle edilebilir ve görüntülenebilir. Daha fazla bilgi için [Tarihlerle çalışma](dates.qmd) sayfasına bakın.| 2018-04-12 veya 15/3/1954 veya Çrş 4 Ocak 1980                                                             |
+------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------+
| Mantıksal (Logical)    | Değerler TRUE veya FALSE olmak üzere iki özel değerden biri olmalıdır (bunların tırnak içindeki "TRUE" ve "FALSE" **olmadığına** dikkat edin)                                                              | TRUE veya FALSE                                                                                       |
+------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------+
...
+------------------+---------------------------------------------------------------------------------------+
| `as.Date()`      | Tarih sınıfına dönüştürür - Not: Ayrıntılar için [dates](dates.qmd) bölümüne bakıns             |
+------------------+---------------------------------------------------------------------------------------+
...
Parantezler ayrıca belirli satırları ve sütunları görüntülemek için veri çerçeveleri üzerinde de çalışır. Bunu, `dataframe[rows, columns]` sözdizimini kullanarak yapabilirsiniz:
...
**dplyr** sözdizimini (satırlar için 'filter()' ve sütunlar için 'select()' fonksiyonları) kullanarak veri çerçeveleri ve tibbles üzerinde yukarıdaki satır/sütun indekslemeyi de gerçekleştirebileceğinizi unutmayın. [Temizleme verileri ve temel fonksiyonlar](cleaning.qmd) sayfasında bu temel fonksiyonlar hakkında daha fazla bilgi edinin.
...
Bu, **ggtree** paketi ile filogenetik ağaçlara veri eklemek için kullanılır. [Filogenetik ağaçlar](phylogenetic_trees.qmd) sayfasına veya bu çevrimiçi [kaynak kitap](https://yulab-smu.top/treedata-book/) sayfasına bakabilirsiniz.
...
+--------------------------+------------+--------------+--------------------------------------------------------------------------------------------------------------------------------------------------------+
| Değer eksiktir           | `is.na()`  | `is.na(7)`   | `FALSE` [Kayıp Veri](missing_data.qmd) sayfasına bakabilirsiniz)                                                                                                                   |
+--------------------------+------------+--------------+--------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+------------------------------------------------------------------------------------------------+--------------------------------------------+
| "rdt_result" içindeki değer "Positive" ise                                                     | "Confirmed"                                 |
+------------------------------------------------------------------------------------------------+--------------------------------------------+
| "rdt_result" içindeki değer "Positive" DEĞİLSE VE "other_cases_in_home" içindeki değer "Yes" ise | "Probable"                                 |
+------------------------------------------------------------------------------------------------+--------------------------------------------+
| Yukarıdaki kriterlerden biri karşılanmazsa                                                    | "Suspected"                              |
+------------------------------------------------------------------------------------------------+--------------------------------------------+
...
R'da, eksik değerler "NA" ("ayrılmış" bir değer) özel değeriyle temsil edilir (büyük harfler N ve A - tırnak içinde değil). Eksik verileri başka bir şekilde kaydeden verileri içe aktarırsanız (ör. 99, "Eksik" veya .), bu değerleri "NA" olarak yeniden kodlamak isteyebilirsiniz. Bunun nasıl yapılacağı [İçe ve dışa aktar](importing.qmd) sayfasında ele alınmaktadır.

**Bir değerin "NA" olup olmadığını test etmek için `TRUE` veya `FALSE` döndüren özel "is.na()"** fonksiyonunu kullanın.
...
[Eksik veri](missing_data.qmd) sayfasındaki eksik, sonsuz, "NULL" ve imkansız değerler hakkında daha fazla bilgi edinin. [İçe ve dışa aktar](importing.qmd) sayfasındaki verileri içe aktarırken eksik değerleri nasıl dönüştüreceğinizi öğrenin.
...
[***DİKKAT:*** Aşağıdaki fonksiyonlar varsayılan olarak hesaplamalarda eksik değerleri içerecektir. "na.rm = TRUE" bağımsız değişkeni belirtilmedikçe, eksik değerler `NA` çıktısıyla sonuçlanır. Bu, kısaca `na.rm = T` şeklinde yazılabilir.]{style="color: orange;"}
...
- "*quantile()": "x" incelenecek sayısal vektördür ve "probs =" olasılıkları 0 ile 1.0 arasında olan sayısal bir vektördür, ör. "c(0.5, 0.8, 0.85)"
...
Bir değerin bir vektörün **`%in%`'inde olmadığını** sormak için mantık ifadesinin **önüne** bir ünlem işareti (!) koyun:
...
"%in%", **dplyr** fonksiyonu "case_when()" kullanılırken çok kullanışlıdır. Bir vektörü önceden tanımlayabilir ve daha sonra ona başvurabilirsiniz. Örneğin:
...
Not: Belki **stringr**'den `str_detect()` kullanarak kısmi bir dize algılamak istiyorsanız, `c("1", "Yes", "yes", "y")` gibi bir karakter vektörünü kabul etmeyecektir. Bunun yerine, "1\|Yes\|yes\|y" gibi VEYA çubukları olan bir yoğunlaştırılmış dize *normal ifade* verilmelidir. Örneğin, `str_detect(hospitalized, "1|Yes|yes|y")`. Daha fazla bilgi için [Karakterler ve dizeler](characters_strings.qmd) hakkındaki sayfaya bakın.
...
Genel hatalar, uyarılar ve sorun giderme ipuçları [Hatalar ve yardım](errors.qmd) sayfasında bulunabilir.
