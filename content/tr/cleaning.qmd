---
aliases:
  - /new_pages/cleaning.tr.html
output:
  word_document: default
  html_document: default
---
# Veri temizliği ve temel fonksiyonlar {#cleaning}
...
Bu komutların Stata ya da SAS komutları ile karşılaştırılmasını görmek için [R’a geçiş](transition_to_r.qmd) sayfasını inceleyin. 

Alternatif bir veri yönetim çerçevesi olarak data.table R paketi := gibi operatörler ve sıkça köşeli parantezler [ ] kullanır. Bu yaklaşım ve sintaksı kısaca [Veri Tablosu](data_table.qmd) sayfasında anlatılmıştır. 
...
Doğrudan `appliedepidata::get_data(name = "case_linelists_linelist_raw")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.qmd) sayfasına bakın).    
...
Tüm veri çerçevesine genel bir bakış elde etmek için skimr paketindeki skim() fonksiyonunu kullanabilirsiniz (daha fazla bilgi için Tanımlayıcı tablolar sayfasına bakın). Sütunlar karakter, sayısal gibi sınıflara göre özetlenir. Not: “POSIXct” bir tür ham tarih sınıfıdır (bkz. [Tarihlerle çalışma](dates.qmd))  
...
Kısa yol olarak, dplyr select() ve summarise() fonksiyonları ile sütunları yeniden adlandırabilirsiniz. select() yalnızca belirli sütunları tutmak için kullanılır (ve bu sayfanın ilerleyen kısımlarında ele alınacaktır). summarise(),[Verilerin gruplanması](grouping.qmd) ve [Tanımlayıcı tablolar](tables_descriptive.qmd) sayfalarında ele alınmıştır. Bu fonksiyonlar da yeni_adı = eski_adı biçimini kullanır. İşte bir örnek: 
...
Bir Excel dosyasındaki birleştirilmiş hücreler, veri alırken sık karşılaşılan bir durumdur. [R'a Geçiş](transition_to_r.qmd) bölümünde açıklandığı gibi, birleştirilmiş hücreler verinin insan tarafından okunması için iyi olabilir, ancak "düzenli-tidy veri" değildir ve verilerin makine tarafından okunması için birçok soruna neden olur. R, birleştirilmiş hücreleri barındıramaz.
...
•	Fonksiyonu purr stilinde yazabilirsinizi (örneğin ~ mean(.x, na.rm = TRUE)) ([bu sayfaya bakabilirsiniz](iteration.qmd))  
* •	Bir liste ile çoklu fonksiyonlar belirlenebilir  (örneğin list(mean = mean, n_miss = ~ sum(is.na(.x))). 
•	Birden çok fonkisyon yazarsanız, giriş sütunu başına farklı adlarla col_fn biçiminde birden çok dönüştürülmüş sütun oluşturulur. Yeni sütunların .names = argümanıyla nasıl adlandırılacağını, {.col} ve {.fn}'nin giriş sütunu ve fonksiyonu için kısayol olduğu tutkal sintaksının  [Karakterler ve dizeler](characters_strings.qmd) sayfasına bakınız) kullanarak ayarlayabilirsiniz.
...
Epikurve ile kümülatif insidansın nasıl çizileceğini öğrenmek için [Epidemik eğriler](epicurves.qmd) sayfasına bakın.
...
<span style="color: red;">**_TEHLİKE:_** **Sağ taraftaki değerlerin tümü aynı sınıfta olmalıdır** - sayısal, karakter, tarih, mantıksal vb. Eksik (NA) atamak için NA_character_, NA_real_ (sayısal veya POSIX) ve as.Date(NA) gibi NA'nın özel varyasyonlarını kullanmanız gerekebilir. [Tarihlerle çalışma](dates.qmd) bölümünden daha fazlasını okuyabilirsiniz.</span>  
...
2. Sözlük dosyasını R'a aktarın. Bu örnek, [El kitabı ve veri indirme](data_used.qmd) sayfasındaki talimatlar aracılığıyla indirilebilir.
...
İlk olarak, uygun kesme noktaları yapmak için verilerinizin dağılımını inceleyin. [ggplot temelleri](ggplot_basics.qmd) ile ilgili sayfaya bakabilirsiniz.
...
<span style="color: orange;">**_UYARI:_** Bazen sayısal değişkenler "karakter" sınıfı olarak içe aktarılır. Bu durum, bazı değerlerde sayısal olmayan karakterler varsa, örneğin yaş için “2 ay” girişi veya (yerel ayarlarınıza bağlı olarak) ondalık basamakta virgül kullanılmışsa (örn. “4,5” dört buçuk yıl anlamına gelir) karşınıza çıkar.</span>
...
NA değerlerine “Eksik” gibi bir etiket atamak isteyebilirsiniz. Yeni sütun faktör sınıfından (kısıtlı değerler) olduğundan replace_na() ile değiştiremezsiniz. Bunun yerine, [Faktörler](factors.qmd) sayfasında açıklandığı gibi forcat'lerden fct_explicit_na() kullanın.
...
Kesim noktası oluşturma ve vektörleri etiketlemenin hızlı bir yolu için aşağıdaki yöntemi kullanın. seq() ve rep() ile ilgili referanslar için [R temelleri](basics.qmd) sayfasına bakınız.
...
Bir veri çerçevesinin satırlarını başka bir veri çerçevesinin altına bağlayarak veri kümelerini birleştirmek için dplyr'den bind_rows() öğesini kullanabilirsiniz. Bu yöntem, [Verileri birleştirme](joining_matching.qmd) sayfasında daha ayrıntılı olarak açıklanmaktadır. 
...
Verilerinizdeki eksiklikleri analiz etmeye ve yönetmeye yönelik birçok teknik için [Eksik veriler](missing_data.qmd) sayfasına bakın. 
