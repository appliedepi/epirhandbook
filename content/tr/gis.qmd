...
Temizlenmiş `linelist` verisini **appliedepidata** paketindeki `get_data()` ile yükleyin ([El kitabını ve verileri indirin](data_used.qmd) sayfasına bakın).
...
Sierra Leone için tüm idari sınırlar İnsani Veri Değişimi'nden (HDX) [web sitesi burada](https://data.humdata.org/dataset/sierra-leone-all-ad-min-level-boundaries) gelmektedir. Bu bölüm, [El kitabını ve verileri indir](data_used.qmd) sayfasında açıklandığı gibi, bunları **appliedepidata** paketindeki `get_data()` ile yükler.
...
Kendi şekil dosyanızı okumak için **sf** paketindeki `read_sf()` fonksiyonunu dosya yoluyla kullanın (daha fazla bilgi için [İçe aktarma ve dışa aktarma](importing.qmd) ve [R projeleri](r_projects.qmd) sayfalarına bakabilirsiniz).
...
Bu veriler **appliedepidata** R paketimizden gelir ([El kitabını ve verileri indirin](data_used.qmd) sayfasına bakın). Veriler HDX'te de bulunur (bağlantı [buradan](https://data.humdata.org/dataset/sierra-leone-population)). Verileri `get_data()` ile yükler, ardından sütun adı sözdizimini standartlaştırmak için sonucu `clean_names()` öğesine iletiriz.
...
Sağlık tesislerinin konumları HDX'ten [buradan](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities) gelmektedir. Bu bölüm, [el kitabını ve verileri indir](data_used.qmd) sayfasında açıklandığı gibi, bunları **appliedepidata** paketindeki `get_data()` ile yükler.
...
Bu örnekte, ggplot()'a 'linelist_adm' ile başlıyoruz, böylece çubukları frekansa göre sıralayan 'fct_infreq()' gibi faktör fonksiyonlarını uygulayabiliriz (ipuçları için [Faktörler](factors.qmd) sayfasına bakınız).
...
1) `sle_adm3_pop` popülasyon verileri, `case_adm3` veri çerçevesindeki `admin3pcod` sütunundaki ve `sle_adm3_pop` içindeki `adm_pcode` sütunundaki ortak değerler temelinde **dplyr**'den bir `left_join()` kullanılarak birleştirilir. [Birleştirme verileri](joining_matching.qmd) ile ilgili sayfaya bakın.
...
Yukarıda yazdırılan matris, `sle_adm3_dat` verilerimizdeki bölgeler arasındaki ilişkileri göstermektedir. 0 puan, iki bölgenin komşu olmadığını gösterirken, 0 dışında herhangi bir değer, bir komşu ilişkisini gösterir. Matristeki değerler, her bölgenin toplam satır ağırlığı 1 olacak şekilde ölçeklendirilir.
