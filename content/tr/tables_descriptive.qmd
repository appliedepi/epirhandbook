---
aliases:
  - /new_pages/tables_descriptive.tr.html
---

# Tanımlayıcı tablolar {#tables_descriptive}
...
Simüle edilmiş bir Ebola salgını veri setini içe aktarıyoruz. Doğrudan `appliedepidata::get_data(name = "linelist_cleaned_rds")` ile yükleyin (ayrıntılar için [El kitabı ve veri indirme](data_used.qmd) sayfasına bakın).
...
Daha fazla kod ve istatistiklerle ilgili ipuçları için [Temel istatistik testleri](stat_tests.qmd) sayfasına bakın. 
...
• Daha sonraki hesaplamalar için (örneğin payda oluşturmak için) sütunlar oluşturabilir ve bu sütunları en sonunda select() ile veri çerçevenizden çıkarabilirsiniz. 
...
Ayrıca istatistiksel testlerin sonuçlarını gtsummary tablolarına ekleyebilirsiniz. Bu işlem, Basit istatistik testleri sayfasının gtsummary bölümünde açıklanmıştır.[Basit istatistiksel testler](stat_tests.qmd#stats_gt) sayfası.
...
• Denklemin sağ tarafı, **stringr**'den `str_glue()` sözdizimini kullanır (bkz. [Karakterler ve dizeler](characters_strings.qmd)); istenen görüntüleme dizesi tırnak içinde, istatistiklerin kendisi ise küme parantezleri içinde yazılır. “n” (sayılar için), “N” (payda için), “mean”, “median”, “sd”, “max”, “min”, yüzdelikleri ( “p##” olarak; örneğin p25, toplamın yüzdeliği p) dahil edebilirsiniz. Ayrıntılar için ?tbl_summary komutunu uygulayın.
...
<span style="color: orange;">**_Uyarı:_** NA (eksik) değerler, useNA = "always" (aynı zamanda "no" veya "ifany" olarak da ayarlanabilir) değişkenini eklemediğiniz sürece tablo haline getirilmeyecektir.</span>

<span style="color: darkgreen;">**_İPUCU:_** Baz fonksiyonlarda tekrarlanan veri çerçevesi belirtme ihtiyacını ortadan kaldırmak için magrittr'den %$% operatörünü kullanabilirsiniz. Örneğin aşağıdaki tablo `linelist %$% table(outcome, useNA = "always")` şeklinde yazılabilir.</span>
