...
Chương sách này nhấn mạnh việc sử dụng các hàm từ hệ sinh thái của package [**tidyverse**](https://www.tidyverse.org/). Các hàm R thiết yếu được trình bày trong chương này được liệt kê dưới đây.
...
Nếu bạn muốn so sánh các hàm này với các câu lệnh trong Stata hoặc SAS, hãy xem chương [Chuyển đổi sang R](transition_to_r.qmd).

Bạn cũng có thể bắt gặp một giải pháp quản lý dữ liệu khác được cung cấp bởi package **data.table** trong R với các toán tử như `:=` và toán tử dấu ngoặc vuông `[ ]`. Cách tiếp cận và cú pháp này được giải thích ngắn gọn trong chương [Data Table](data_table.qmd).
...
Ở đây, chúng ta sẽ nhập dữ liệu gốc từ file Excel bằng cách sử dụng hàm `import()` từ package **rio**. Package **rio** có thể xử lý linh hoạt nhiều loại tệp tin (ví dụ: .xlsx, .csv, .tsv, .rds). Xem thêm chương [Nhập xuất dữ liệu](importing.qmd) để biết thêm thông tin và mẹo về các tình huống bất thường (ví dụ: loại bỏ hàng, thiết lập giá trị trống, nạp trang tính Google, v.v.).

Tải trực tiếp bằng `appliedepidata::get_data(name = "case_linelists_linelist_raw")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).
...
Bạn có thể sử dụng hàm `skim()` từ package **skimr** để có cái nhìn tổng quan về toàn bộ bộ dữ liệu (xem chương [Bảng mô tả](tables_descriptive.qmd) để biết thêm thông tin). Các cột được tóm tắt theo phân lớp/định dạng như ký tự và số. Lưu ý: "POSIXct" là một loại phân lớp ngày thô (xem chương [Làm việc với ngày tháng](dates.qmd).
...
Các phần mềm thống kê khác như SAS và STATA sử dụng *"nhãn"* là tên cột phiên bản dài hơn khi in, cùng tồn tại song song với tên cột. Mặc dù R cung cấp tính năng thêm nhãn của cột vào dữ liệu, nhưng điều này hầu hết không được nhấn mạnh trong thực tế. Để đặt tên cột "dễ nhìn" cho các bảng biểu, người ta thường điều chỉnh hiển thị của chúng trong các lệnh vẽ biểu đồ để tạo ra kết quả (ví dụ: tiêu đề trục hoặc chú giải của một biểu đồ hoặc tiêu đề cột trong bảng - xem thêm [mục scales trong chương Các mẹo với ggplot](ggplot_tips.qmd#ggplot_tips_scales) và chương [Trình bày bảng](tables_presentation.qmd)). Nếu bạn muốn gán nhãn cột trong dữ liệu, hãy đọc thêm tài liệu trực tuyến [tại đây](https://cran.r-project.org/web/packages/expss/vignettes/labels-support.html) và [tại đây](https//cran.r-project.org/web/packages/labelt/vignettes/intro_labelling.html).
...
Như một lối tắt, bạn cũng có thể đổi tên các cột bằng hàm `select()` và `summarise()` thuộc package **dplyr**. Hàm `select()` được sử dụng để chỉ giữ một số cột nhất định (sẽ được đề cập sau trong chương này). Hàm `summarise()` được đề cập trong các chương [Nhóm dữ liệu](grouping.qmd) và [Bảng mô tả](tables_descriptive.qmd). Các hàm này cũng sử dụng định dạng `new_name = old_name`. Đây là một ví dụ:
...
Các ô được hợp nhất trong file Excel là một sự cố thường xảy ra khi nhận dữ liệu. Như đã giải thích trong chương [Chuyển đổi sang R](transition_to_r.qmd), các ô được hợp nhất có thể dễ nhìn cho con người đọc dữ liệu, nhưng không phải là "cấu trúc dữ liệu cơ bản" và gây ra nhiều vấn đề cho máy khi nhận diện dữ liệu. R không thể chứa các ô đã hợp nhất.
...
Hàm `matches()` hoạt động tương tự như hàm `contains()` nhưng có thể được cung cấp một biểu thức chính quy (xem chương [Ký tự và chuỗi](characters_strings.qmd)), chẳng hạn như nhiều chuỗi được phân tách bằng toán tự OR trong dấu ngoặc đơn:
...
`select()` cũng có thể được sử dụng như một lệnh độc lập (không bắt buộc trong chuỗi các bước làm sạch). Trong trường hợp này, đối số đầu tiên là data frame gốc cần thao tác.
...
Xem chương [Loại bỏ trùng lặp](deduplication.qmd) để biết các tùy chọn mở rộng về cách loại bỏ dữ liệu trùng lặp. Chỉ một ví dụ rất đơn giản về cách loại bỏ hàng trùng lặp được trình bày ở chương này.

Package **dplyr** cung cấp hàm `distinct()`. Hàm này kiểm tra tất cả các hàng và rút gọn bộ dữ liệu cho chỉ còn các hàng là duy nhất. Nghĩa là, nó loại bỏ các hàng là bản sao 100% của hàng khác.
...
Nếu tạo nhiều cột mới, hãy tách riêng từng cột bằng dấu phẩy và dòng mới. Dưới đây là các ví dụ về tạo các cột mới, bao gồm một cột mà chứa các giá trị từ các cột khác được kết hợp bằng hàm `str_glue()` từ package **stringr** (xem chương [Ký tự và chuỗi](characters_strings.qmd)).
...
Có nhiều cách để thiết lập kiểu dữ liệu cho cột khi nhập dữ liệu, nhưng chúng thường phức tạp. Xem chương [R cơ bản](basics.qmd) về kiểu dữ liệu để tìm hiểu thêm về cách chuyển đổi kiểu dữ liệu đối tượng và cột.
...
Theo cách tương tự, bạn có thể sử dụng hàm `as.character()` và hàm `as.logical()`. Để chuyển đổi sang kiểu danh mục, bạn có thể sử dụng hàm `factor()` từ **base** R hoặc hàm `as_factor()` từ package **forcats**. Đọc thêm ở chương [Factors](factors.qmd).

Bạn phải cẩn thận khi định dạng lại cột sang kiểu Ngày, các giải thích chi tiết hơn được đề cập ở chương [Làm việc với ngày tháng](dates.qmd). Thông thường, tất cả các giá trị ngày ban đầu phải ở cùng một định dạng cần chuyển đổi để có thể sử dụng (ví dụ: "MM/DD/YYYY" hoặc "DD MM YYYY"). Sau khi chuyển đổi thành thành kiểu Ngày, hãy kiểm tra dữ liệu của bạn để xác nhận rằng mỗi giá trị đã được chuyển đổi chính xác.

### Dữ liệu được nhóm {.unnumbered}

Nếu bộ dữ liệu của bạn đã được *nhóm* (xem chương [Nhóm dữ liệu](grouping.qmd)), hàm `mutate()` có thể hoạt động khác so với khi bộ dữ liệu không được nhóm. Bất kỳ hàm tổng hợp nào, như `mean()`, `median()`, `max()`, v.v. sẽ tính theo nhóm, không phải theo tất cả các hàng.
...
Xem chương [Đường cong dịch bệnh](epicurves.qmd) để biết cách lập biểu đồ các trường hợp mới mắc tích lũy với epicurve.
...
Đây là một hàm từ package **forcats**. Package **forcats** xử lý các cột dạng Factor. Factor là cách của R để xử lý các giá trị có thứ tự như `c("First", "Second", "Third")` hoặc để thiết lập thứ tự cho các giá trị (ví dụ: bệnh viện) xuất hiện trong bảng và biểu đồ. Xem chương [Factors](factors.qmd).
...
2)  Nạp tệp từ điển vào R. Ví dụ này có thể được tải xuống theo hướng dẫn trong chương [Tải sách và dữ liệu](data_used.qmd).
...
Trước tiên, hãy kiểm tra phân phối dữ liệu của bạn để đưa ra các điểm cắt thích hợp. Xem chương [ggplot cơ bản](ggplot_basics.qmd).
...
Để nhanh chóng tạo điểm cắt và gắn nhãn vectơ, hãy thực hành như bên dưới. Xem chương [R cơ bản](basics.qmd) về hàm `seq()` và `rep()`.
...
Để kết hợp các bộ dữ liệu với nhau bằng cách liên kết các hàng của một bộ dữ liệu này với phần cuối bộ dữ liệu khác, bạn có thể sử dụng hàm `bind_rows()` từ **dplyr**. Điều này được giải thích chi tiết hơn trong chương [Nối dữ liệu](joining_matching.qmd).
...
Xem chương về [Dữ liệu Missing](missing_data.qmd) để biết nhiều kỹ thuật phân tích và quản lý dữ liệu missing.
...
*(Xem chương [Làm việc với ngày tháng](dates.qmd) để biết thêm thông tin về cách xử lý với ngày tháng và package **lubridate**)*
...
Đơn giản hãy liệt kê các cột theo thứ tự mà chúng cần được sắp xếp. Thêm `.by_group = TRUE` nếu bạn muốn việc sắp xếp xảy ra trước bất kỳ *hành động nhóm dữ liệu* nào được áp dụng trong dữ liệu (xem chương [Nhóm dữ liệu](grouping.qmd)).
...
Việc sắp xếp dữ liệu với hàm `arrange()` đặc biệt hữu ích khi [Trình bày bảng](tables_descriptive.qmd), sử dụng hàm `slice()` để lấy các hàng "trên cùng" trên mỗi nhóm hoặc thiết lập thứ tự danh mục theo thứ tự xuất hiện.
