...
Trong chương này, chúng tôi trình bày cách tạo ra đường cong dịch bệnh bằng package **ggplot2**, cho phép khả năng tùy chỉnh nâng cao.
...
Package **incidence2** cung cấp một cách tiếp cận thay thế với các lệnh đơn giản hơn, nhưng tại thời điểm viết cuốn sách này thì package đang được sửa đổi và chưa ổn định. Package này sẽ được thêm lại vào chương khi đã ổn định.
...
Đoạn code này hiển thị cách gọi các package cần thiết cho các phân tích. Trong cuốn sách này, chúng tôi nhấn mạnh đến hàm `p_load()` từ package **pacman**, sẽ cài đặt package nếu cần *và* gọi package để sử dụng. Bạn cũng có thể gọi các package đã được cài đặt với hàm `library()` từ **base** R. Xem chương [R cơ bản](basics.qmd) để biết thêm thông tin về các package trong R.
...
Các bộ dữ liệu được nhập bằng hàm `import()` từ package **rio**. Xem chương [Nhập xuất dữ liệu](importing.qmd) để biết các cách nhập dữ liệu khác nhau.
...
Bạn phải kiểm soát *thủ công* việc tổng hợp các trường hợp theo thời gian (theo tuần, tháng, v.v.) *và* khoảng thời gian của các nhãn trên trục ngày tháng. Việc này phải được quản lý rất cẩn thận.
...
-   Chúng ta dùng `closed = "left"` trong `geom_histogram()` để đảm bảo các ngày được đếm vào đúng bins\
...
Các cột histogram có thể được tô màu theo nhóm và "xếp chồng lên nhau". Để chỉ định cột nhóm, hãy thực hiện các thay đổi sau. Xem chương [ggplot cơ bản](ggplot_basics.qmd) để biết thêm chi tiết.
...
-   Để biết thêm mẹo về thang màu và bảng màu, hãy xem chương [ggplot cơ bản](ggplot_basics.qmd).
...
Thứ tự mà các cột được nhóm xếp chồng lên nhau được điều chỉnh tốt nhất bằng cách phân loại cột nhóm dưới dạng Factor. Sau đó, bạn có thể chỉ định thứ tự cấp độ phân loại (và nhãn hiển thị của chúng). Xem chương về [Factors](factors.qmd) hoặc [Các mẹo với ggplot](ggplot_tips.qmd) để biết thêm chi tiết.

Trước khi tạo biểu đồ, hãy sử dụng hàm `fct_relevel()` từ package **forcats** để chuyển đổi cột phân nhóm thành kiểu Factor và điều chỉnh thứ tự cấp độ theo cách thủ công, như được trình bày chi tiết trong chương về [Factors](factors.qmd).
...
Đọc thêm về chú thích và scale trong chương [Các mẹo với ggplot](ggplot_tips.qmd). Dưới đây là một vài điểm nổi bật:
...
-   Xem [trang này](https://rdrr.io/r/base/strptime.html) hoặc chương [Làm việc với ngày tháng](dates.qmd) để biết thêm mẹo tạo nhãn ngày.
...
Xem chương về [Đường trung bình động](moving_average.qmd) để có mô tả chi tiết và một số tùy chọn. Dưới đây là một tùy chọn để tính toán đường trung bình động với package **slider**. Theo cách tiếp cận này, *trung bình động được tính toán trong bộ dữ liệu trước khi vẽ biểu đồ*:

1)  Tổng hợp dữ liệu thành số lượng nếu cần thiết (hàng ngày, hàng tuần, v.v.) (xem chương [Nhóm dữ liệu](grouping.qmd))\
...
Như với các ggplots khác, bạn có thể tạo các biểu đồ được chia nhỏ ("nhiều biểu đồ con"). Như đã giải thích trong chương [Các mẹo với ggplot](ggplot_tips.qmd) trong cuốn sách này, bạn có thể sử dụng `facet_wrap()` hoặc `facet_grid()`. Ở đây chúng tôi sẽ minh họa bằng hàm `facet_wrap()`. Đối với đường cong dịch bệnh, sử dụng `facet_wrap()` thường dễ dàng hơn vì khả năng bạn thường chỉ cần chia nhỏ biểu đồ theo một biến.
...
Để hiển thị tổng vụ dịch trong nền của mỗi biểu đồ con, hãy thêm hàm `gghighlight()` với dấu ngoặc đơn trống vào ggplot. Hàm này thuộc package **gghighlight**. Lưu ý rằng trục y tối đa trong tất cả các biểu đồ con hiện dựa vào đỉnh của toàn bộ vụ dịch. Có nhiều ví dụ hơn về package này trong chương [Các mẹo với ggplot](ggplot_tips.qmd).
...
-   Tùy chọn: để có các biểu đồ con xếp chồng lên nhau theo chiều dọc: cột chia được chuyển sang các hàng bên cạnh của phương trình và ở bên phải được thay thế bằng "." (`facet_wrap(facet~.)`), và `ncol = 1`. Bạn cũng có thể cần điều chỉnh chiều rộng và chiều cao của ảnh biểu đồ đã lưu dưới dạng png (xem `ggsave()` trong chương [Các mẹo với ggplot](ggplot_tips.qmd)).
...
Hãy nhớ rằng - bạn có thể sử dụng các công cụ như `\n` *trong* các đối số `date_labels` hoặc `labels` để đặt các phần của mỗi nhãn trên một dòng mới bên dưới. Tuy nhiên, đoạn code dưới đây giúp bạn thực hiện nhiều năm hoặc tháng (ví dụ) ở dòng thấp hơn *và chỉ một lần*.

Cách đơn giản nhất là gán đối số `labels = ` trong `scale_x_date()` cho hàm `label_date_short()` từ package **scales** (lưu ý: đừng quên viết cặp ngoặc đơn rỗng ()). Hàm này sẽ tự động tạo ra các nhãn ngày tháng hiệu quả (đọc thêm [tại đây](https://scales.r-lib.org/reference/label_date.html)). Một lợi ích khác của hàm này là các nhãn sẽ tự động điều chỉnh khi dữ liệu của bạn mở rộng theo thời gian: từ ngày, sang tuần, sang tháng và năm.
...
Cách thứ hai là sử dụng faceting. Dưới đây:
...
Nó cũng có thể được phủ đè lên đường cong dịch bệnh, với trục kép bằng cách sử dụng phương pháp **cowplot** được mô tả ở trên và trong chương [Các mẹo với ggplot](ggplot_tips.qmd):
