...
Hầu hết nội dung trong chương này không yêu cầu bộ dữ liệu. Tuy nhiên, trong phần sơ đồ Sankey, chúng ta sẽ sử dụng bộ dữ liệu linelist từ một vụ dịch Ebola mô phỏng. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).  
...
* `label` (ký tự, trong dấu ngoặc đơn nếu nhiều từ)  
* `fillcolor` (nhiều màu)  
* `fontcolor`  
