---
aliases:
  - /new_pages/ggplot_basics.vn.html
---
...
Trong chương này, chúng tôi sẽ trình bày các nguyên tắc cơ bản về vẽ biểu đồ bằng **ggplot2**. Xem chương [Các mẹo với ggplot](ggplot_tips.qmd) để biết các gợi ý và các kỹ thuật nâng cao giúp cho biểu đồ của bạn thực sự trông đẹp mắt.
...
Đoạn code này hiển thị việc gọi các package cần thiết cho các phân tích. Trong cuốn sách này, chúng tôi nhấn mạnh việc sử dụng hàm `p_load()` từ package **pacman**, giúp cài đặt các package cần thiết *và* gọi chúng ra để sử dụng. Bạn cũng có thể gọi các packages đã cài đặt với hàm `library()` của **base** R. Xem thêm chương [R cơ bản](basics.qmd) để có thêm thông tin về các packages trong R.

```{r}
pacman::p_load(
  tidyverse,      # gồm ggplot2 và các công cụ quản lý dữ liệu khác
  janitor,        # làm sạch dữ liệu và bảng tóm tắt
  ggforce,        # các tiện ích bổ sung cho ggplot
  rio,            # nhập và xuất dữ liệu
  here,           # xác định vị trí tệp
  stringr, # làm việc với ký tự
  appliedepidata # các bộ dữ liệu minh họa dùng trong sổ tay này
)
```

### Nhập dữ liệu {.unnumbered}

Để bắt đầu, chúng ta nhập bộ dữ liệu có tên linelist đã làm sạch bao gồm các trường hợp từ vụ dịch Ebola mô phỏng. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).
...
50 hàng đầu tiên của bộ dữ liệu được hiển thị như bên dưới. Chúng ta sẽ tập trung vào các biến liên tục là `age`, `wt_kg` (cân nặng được tính bằng kilogram), `ct_blood` (giá trị CT), và`days_onset_hosp` (sự khác biệt giữa ngày khởi phát và nhập viện).
...
Khi chuẩn bị dữ liệu để vẽ biểu đồ, tốt nhất là làm cho dữ liệu tuân thủ [tiêu chuẩn dữ liệu "gọn gàng"](https://r4ds.had.co.nz/tidy-data.html) nhất có thể. Để đạt được điều này, bạn có thể tham khảo chương [Làm sạch số liệu và các hàm quan trọng](cleaning.qmd) trong cuốn sổ tay này.
...
-   Làm sạch một số cột để các giá trị "thân thiện với dữ liệu" của chúng với dấu gạch dưới, v.v. được thay đổi thành văn bản bình thường hoặc chữ hoa tiêu đề (xem chương [Ký tự và chuỗi](characters_strings.qmd))

Sau đây là một số ví dụ:

```{r, }
# tạo phiên bản hiển thị của các cột với tên thân thiện hơn
linelist <- linelist %>%
  mutate(
    gender_disp = case_when(gender == "m" ~ "Male",        # m thành Male
                            gender == "f" ~ "Female",      # f thành Female,
                            is.na(gender) ~ "Unknown"),    # NA thành Unknown
    
    outcome_disp = replace_na(outcome, "Unknown")          # thay giá trị NA của outcome bằng "unknown"
  )
```

### Chuyển đổi dữ liệu sang dạng dọc {.unnumbered}

Về cấu trúc dữ liệu, đối với **ggplot2** chúng ta thường muốn xoay trục dữ liệu sang định dạng *dọc*. Xem thêm ở chương [Xoay trục dữ liệu](pivoting.qmd).
...
```{r, }
symptoms_data_long <- symptoms_data %>%    # bắt đầu với linelist thu nhỏ tên là symptoms_data
  
  pivot_longer(
    cols = -case_id,                       # xoay dọc mọi cột trừ case_id (tất cả các cột triệu chứng)
    names_to = "symptom_name",             # đặt tên cho cột mới chứa các triệu chứng
    values_to = "symptom_is_present") %>%  # đặt tên cho cột mới chứa các giá trị (yes/no)
  
  mutate(symptom_is_present = replace_na(symptom_is_present, "unknown")) # chuyển NA thành "unknown"
...
```{r, eval=F}
# vẽ dữ liệu từ các cột của my_data thành các điểm màu đỏ
ggplot(data = my_data)+                   # dùng bộ dữ liệu "my_data"
  geom_point(                             # thêm một lớp điểm
    mapping = aes(x = col1, y = col2),    # gán cột dữ liệu vào các trục
    color = "red")+                       # thiết lập khác cho geom
  labs()+                                 # nơi bạn thêm tiêu đề, nhãn trục, v.v.
  theme()                                 # nơi bạn chỉnh màu, phông chữ, cỡ chữ của các thành phần không phải dữ liệu (trục, tiêu đề, v.v.)
```
...
```{r, eval=F}
# Lệnh này sẽ tạo một biểu đồ là khung nền trống
...
# biểu đồ phân tán
ggplot(data = linelist, mapping = aes(x = age, y = wt_kg))+  # đặt dữ liệu và phép gán trục
  geom_point(color = "darkgreen", size = 0.5, alpha = 0.2)         # đặt thuộc tính tĩnh cho điểm

# biểu đồ histogram
ggplot(data = linelist, mapping = aes(x = age))+       # đặt dữ liệu và các trục
  geom_histogram(              # hiển thị histogram
    binwidth = 7,                # độ rộng của bins
    color = "red",               # màu đường viền của bin
    fill = "blue",               # màu bên trong của bin
    alpha = 0.1)                 # độ trong suốt của bin
```
...
```{r, out.width=c('50%', '50%'), fig.show='hold', warning=F, message=F}
# biểu đồ phân tán
ggplot(data = linelist,   # đặt dữ liệu
       mapping = aes(     # gán thuộc tính biểu đồ với giá trị của cột
         x = age,           # gán trục x với age
         y = wt_kg,         # gán trục y với cân nặng
         color = age)
       )+     # gán màu với age
  geom_point()         # hiển thị dữ liệu dạng điểm
...
ggplot(data = linelist,   # đặt dữ liệu
       mapping = aes(     # gán thuộc tính biểu đồ với giá trị của cột
         x = age,           # gán trục x với age
         y = wt_kg,         # gán trục y với cân nặng
         color = age,       # gán màu với age
         size = age))+      # gán kích thước với age
  geom_point(             # hiển thị dữ liệu dạng điểm
    shape = "diamond",      # điểm hiển thị dạng hình thoi
    alpha = 0.3)            # độ trong suốt của điểm là 30%
...
Điều quan trọng là phải theo dõi các lớp của biểu đồ và các đối tượng thuộc tính khi vẽ các biểu đồ phức tạp - ví dụ biểu đồ được cấu thành từ nhiều geoms. Trong ví dụ dưới đây, `geom_point()` đặt `size = 1` và `geom_smooth()` đặt `linewidth = 2`. Cả hai đều là giá trị tĩnh.

```{r, warning=F, message=F}
ggplot(data = linelist,
       mapping = aes(           # gán thuộc tính biểu đồ với các cột
         x = age,
         y = wt_kg,
         color = age_years)
       ) + 
  geom_point(                   # thêm điểm cho từng hàng dữ liệu
    size = 1,
    alpha = 0.5) +  
  geom_smooth(                  # thêm đường xu hướng
    method = "lm",              # theo phương pháp tuyến tính
    linewidth = 2)                   # độ rộng đường là 2
```
...
# Các lệnh này tạo ra cùng một biểu đồ
ggplot(data = linelist, mapping = aes(x = age))+
  geom_histogram()
...
```{r, eval=F}
# Đoạn code thay thế này tạo ra cùng một biểu đồ
ggplot(data = linelist,
...
Để thay đổi thứ tự của các nhóm trong biểu đồ, xem chương [Các mẹo với ggplot](ggplot_tips.qmd) hoặc chương [Factors](factors.qmd). Ngoài ra cũng có rất nhiều ví dụ về các biểu đồ được nhóm trong các phần bên dưới đối với dữ liệu dạng liên tục và danh mục.
...
Faceting là một chức năng đi kèm với **ggplot2**, vì vậy các chú giải và trục của các biểu đồ nhỏ được căn chỉnh tự động. Có các package khác được thảo luận trong chương [Các mẹo với ggplot](ggplot_tips.qmd) được sử dụng để kết hợp các biểu đồ hoàn toàn khác nhau (**cowplot** và **patchwork**) thành một biểu đồ.
...
Facets có thể chứa một lượng lớn thông tin - vì vậy bạn nên đảm bảo những biến số được lựa chọn để facets không có quá nhiều thứ bậc. Dưới đây là một số ví dụ nhanh với tập dữ liệu sốt rét (xem chương [Tải sách và dữ liệu](data_used.qmd)) bao gồm số lượng trường hợp sốt rét hàng ngày của các cơ sở theo nhóm tuổi..

Sau đây chúng ta nhập số liệu vào và thực hiện một số biến đổi để đơn giản hóa:

```{r, , warning=F, message=F}
# Dữ liệu này là số ca sốt rét hàng ngày, theo cơ sở và ngày
malaria_data <- appliedepidata::get_data(name = "malaria_facility_count_data") %>%  # nhập dữ liệu
  select(-submitted_date, -Province, -newid)                                 # loại bỏ các cột không cần thiết
...
ggplot(malaria_data, aes(x = data_date, y = malaria_tot)) +
  ...
  facet_wrap(~District, scales = "free")        # các facet được tạo ra
```
...
Hãy nhớ rằng bạn có thể sử dụng cú pháp `here()` để cung cấp đường dẫn tệp mong muốn. Xem chương [Nhập xuất dữ liệu](importing.qmd) để biết thêm thông tin.
...
-   Căn lề caption với `element_text(hjust = 0)` (từ phải qua trái)\
-   Tiêu đề phụ được in nghiêng với `element_text(face = "italic")`

```{r, , warning=F, message=F}
age_by_wt + 
  theme_classic()+                                 # chỉnh theo chủ đề đã định sẵn
  theme(
    legend.position = "bottom",                    # chuyển chú giải xuống dưới
    
    plot.title = element_text(size = 30),          # cỡ chữ tiêu đề là 30
    plot.caption = element_text(hjust = 0),        # căn trái chú thích
    plot.subtitle = element_text(face = "italic"), # in nghiêng phụ đề
    
    axis.text.x = element_text(color = "red", size = 15, angle = 90), # chỉ chỉnh chữ của trục x
    axis.text.y = element_text(size = 15),         # chỉ chỉnh chữ của trục y
    
    axis.title = element_text(size = 20)           # chỉnh tiêu đề của cả hai trục
    )     
```
...
Xem mục [thang đo cho màu sắc trong chương Các mẹo với ggplot](ggplot_tips.qmd#ggplot_tips_colors).
...
```{r, warning=F, message=F}
linelist %>%                                                     # bắt đầu với linelist
  select(c(case_id, fever, chills, cough, aches, vomit)) %>%     # chọn các cột
  pivot_longer(                                                  # xoay dọc
    cols = -case_id,                                  
    names_to = "symptom_name",
    values_to = "symptom_is_present") %>%
  mutate(                                                        # thay thế các giá trị missing
    symptom_is_present = replace_na(symptom_is_present, "unknown")) %>% 
  
  ggplot(                                                        # bắt đầu vẽ ggplot!
...
-   **Biểu đồ nhiệt** dành cho ba biến liên tục (xem chương [Biểu đồ nhiệt](heatmaps.qmd))
...
```{r fig.show='hold', message=FALSE, warning=FALSE, out.width=c('50%', '50%')}
# A) Histogram thông thường
ggplot(data = linelist, aes(x = age))+  # cung cấp biến x
  geom_histogram()+
...
```{r, fig.show='hold', message=FALSE, warning=FALSE, out.width=c('50%', '50%')}
# Tần suất với trục tỷ lệ, đã làm mượt
ggplot(data = linelist, mapping = aes(x = age)) +
  geom_density(linewidth = 2, alpha = 0.2)+
  labs(title = "Proportional density")

# Tần suất chồng với trục tỷ lệ, đã làm mượt
...
# A) Boxplot tổng thể
ggplot(data = linelist)+  
  geom_boxplot(mapping = aes(y = age))+   # chỉ gán trục y, không gán trục x
  labs(title = "A) Overall boxplot")

# B) Box plot theo nhóm
ggplot(data = linelist, mapping = aes(y = age, x = gender, fill = gender)) + 
  geom_boxplot()+                     
  theme(legend.position = "none")+   # bỏ chú giải vì không cần thiết
  labs(title = "B) Boxplot by gender")      
```

Code để thêm một box plot vào các cạnh của biểu đồ phân tán (còn gọi là "marginal" plots), vui lòng xem chương [Các mẹo với ggplot](ggplot_tips.qmd) .
...
# A) Jitter plot theo nhóm
ggplot(data = linelist %>% drop_na(outcome),      # loại bỏ các giá trị missing
       mapping = aes(y = age,                     # Biến liên tục
           x = outcome,                           # Biến phân nhóm
           color = outcome))+                     # Biến quy định màu
  geom_jitter()+                                  # Tạo biểu đồ violin
  labs(title = "A) jitter plot by gender")     



# B) Violin plot theo nhóm
ggplot(data = linelist %>% drop_na(outcome),       # loại bỏ các giá trị missing
       mapping = aes(y = age,                      # Biến liên tục
           x = outcome,                            # Biến phân nhóm
           fill = outcome))+                       # Biến quy định màu nền
  geom_violin()+                                   # tạo biểu đồ violin
...
# A) Sina plot theo nhóm
ggplot(
  data = linelist %>% drop_na(outcome), 
  aes(y = age,           # biến dạng số
      x = outcome)) +    # biến phân nhóm
  geom_violin(
    aes(fill = outcome), # fill (màu nền của violin)
    color = "white",     # viền màu trắng
    alpha = 0.2)+        # độ trong suốt
  geom_sina(
    size=1,                # Đổi kích thước của điểm jitter
    aes(color = outcome))+ # color (màu của các điểm)
  scale_fill_manual(       # Định nghĩa màu nền violin theo tử vong hoặc hồi phục
    values = c("Death" = "#bf5300", 
              "Recover" = "#11118c")) + 
  scale_color_manual(      # Định nghĩa màu các điểm theo tử vong hoặc hồi phục
    values = c("Death" = "#bf5300", 
              "Recover" = "#11118c")) + 
  theme_minimal() +                                # Bỏ nền màu xám
  theme(legend.position = "none") +                # Bỏ chú giải không cần thiết
...
```{r fig.show='hold', message=FALSE, warning=FALSE, out.width=c('50%', '50%')}
# Biểu đồ phân tán cơ bản của cân nặng và tuổi
ggplot(data = linelist, 
       mapping = aes(y = wt_kg, x = age))+
  geom_point() +
  labs(title = "A) Scatter plot of weight and age")

# Biểu đồ phân tán của cân nặng và tuổi theo giới tính và kết cục Ebola
ggplot(data = linelist %>% drop_na(gender, outcome), # bộ lọc giữ lại gender và outcome không bị missing
       mapping = aes(y = wt_kg, x = age))+
  geom_point() +
...
Bạn có thể hiển thị ba biến liên tục bằng cách sử dụng đối số `fill =` để tạo một *biểu đồ nhiệt*. Màu của mỗi “ô” sẽ phản ánh giá trị của biên liên tục thứ ba trong bộ dữ liệu. Xem chương [Các mẹo với ggplot](ggplot_tips.qmd) và chương [Biều đồ nhiệt](heatmaps.qmd) để biết thêm chi tiết và một số ví dụ.
...
Dữ liệu dạng danh mục có thể là các giá trị ký tự, có thể là logic (TRUE/FALSE), hoặc factors (Xem chương [Factors](factors.qmd)).
...
```{r}
# Xem class của cột hospital, ta thấy đó là kiểu character
class(linelist$hospital)

# Xem các giá trị và tỷ lệ trong cột hospital
linelist %>% 
  tabyl(hospital)
```

Chúng ta có thể thấy các giá trị bên trong là các ký tự, vì chúng là tên bệnh viện và theo mặc định, chúng được sắp xếp theo thứ tự bảng chữ cái. Ngoài ra còn có các giá trị 'other' và 'missing', thứ mà chúng ta muốn nó sẽ là danh mục cuối cùng khi trình bày. Vì thế chúng ta cần chuyển cột này sang dạng factor và sắp xếp lại chúng. Xem chương [Factors](factors.qmd) để biết chi tiết cách thực hiện.

```{r}
# Chuyển sang factor và định nghĩa thứ tự level để "Other" và "Missing" nằm cuối
linelist <- linelist %>% 
  mutate(
...
# A) Kết cục ở tất cả các ca bệnh
...
# B) Kết cục ở tất cả các ca bệnh theo bệnh viện
ggplot(linelist %>% drop_na(outcome)) + 
...
outcomes <- linelist %>% 
  ...
  group_by(outcome) %>% 
  count %>% 
  ungroup() %>% # Bỏ nhóm để tỷ lệ tính trên tổng số
  mutate(proportion = n/sum(n)*100) # Tính phần trăm
  
outcomes #Xem toàn bộ bảng
```
...
Để trình bày kết quả tách theo từng bệnh viện, chúng ta cần bảng phải chứa nhiều thông tin hơn, và ở định dạng "dài". Chúng ta tạo bảng này với các tần số là sự kết hợp của các danh mục của biến `outcome` và `hospital` (xem chương [Nhóm dữ liệu](grouping.qmd) để biết thêm các mẹo).

```{r, fig.height = 4, fig.width=6}
outcomes2 <- linelist %>% 
  drop_na(outcome) %>% 
  count(hospital, outcome) %>%  # lấy số đếm theo bệnh viện và kết cục
  group_by(hospital) %>%        # Nhóm lại để tỷ lệ tính trên tổng của bệnh viện
  mutate(proportion = n/sum(n)*100) # tính tỷ lệ trên tổng của bệnh viện

head(outcomes2) # Xem trước dữ liệu
```
...
# Kết cục ở tất cả các ca bệnh theo bệnh viện
ggplot(outcomes2) +  
  geom_col(
    mapping = aes(
      x = proportion,                 # hiển thị các giá trị tỷ lệ đã tính sẵn
      y = fct_rev(hospital),          # đảo thứ tự level để missing và other nằm dưới cùng
      fill = outcome),                # chồng theo kết cục
    width = 0.5)+                    # cột mỏng hơn (trên thang 1)
  theme_minimal() +                  # Chủ đề tối giản
  theme(legend.position = "bottom")+
  labs(subtitle = "Number of recovered and dead Ebola cases, by hospital",
       fill = "Outcome",             # tiêu đề chú giải
       y = "Count",                  # tiêu đề trục y
       x = "Hospital of admission")+ # tiêu đề trục x
  scale_fill_manual(                 # thêm màu thủ công
...
Histograms có thể trông giống biểu đồ cột, nhưng khác biệt vì nó đo lường sự phân bố của một biến *liên tục*. Không có khoảng cách giữa các "cột", và chỉ có một cột được cung cấp cho hàm `geom_histogram()`. Có các đối số cụ thể cho biểu đồ chẳng hạn như `bin_width =` và `breaks =` ể chỉ định cách dữ liệu nên được xếp. Phần bên trên về dữ liệu liên tục và chương [Đường cong dịch bệnh](epicurves.qmd) sẽ cung cấp thêm các hi tiết.
