...
R Markdown là một công cụ được sử dụng rộng rãi nhằm tạo ra tài liệu tự động, tái lập và chia sẻ được, như các báo cáo. Nó có thể tạo ra kết quả tĩnh hoặc tương tác, ở định dạng Word, pdf, html, powerpoint, và các định dạng khác. 
...
* Chương [Tổ chức báo cáo định kỳ](reportfactory.qmd) trình bày cách tự động hóa các báo cáo sản phẩm với các tập tin tự tạo kèm theo thời gian.  
* Chương [Dashboards với R Markdown](flexdashboard.qmd) giải thích cách định dạng một báo cáo R Markdown dưới dạng một dashboard.  
...
Cài đặt R package **rmarkdown**. Trong quyển sổ tay này chúng tôi nhấn mạnh việc sử dụng hàm `p_load()` từ package **pacman**, giúp cài đặt package nếu cần *và* gọi ra để sử dụng. Bạn cũng có thể gọi ra các package đã được cài đặt với `library()` từ **base** R. Xem thêm ở chương [R cơ bản](basics.qmd) để biết thêm thông tin về R packages.  
...
Để tìm kiếm các tệp này ở chỗ khác, bạn sẽ cần sử dụng đường dẫn tệp đầy đủ hoặc sử dụng package **here**. Package **here** cài đặt thư mục làm việc tới thư mục gốc của R project và sẽ được giải thích chi tiết trong các chương [Dự án R](r_projects.qmd) và [Nhập xuất dữ liệu](importing.qmd) của sổ tay này. Ví dụ, để nhập một tệp tên "data.csv" từ trong thư mục `projectX`, code sẽ là `import(here(“data.csv”))`.  
...
Vì R Markdown có thể gặp các vấn đề với pandoc khi chạy trên một ổ cứng mạng được chia sẻ, thư mục của bạn nên nằm ở ở cứng vật lý, ví dụ trong dự án thuộc 'My Documents'. Nếu bạn dùng Git (rất khuyến khích!), nó sẽ thân thuộc hơn. Để biết thêm chi tiết, xem thêm chương [R trên ổ cứng mạng](network_drives.qmd) và [Các lỗi thường gặp](errors.qmd).  
...
Quy trình làm việc cũng liên quan đến cấu trúc thư mục tổng thể, chẳng hạn như có thư mục 'đầu ra' cho các tài liệu và số liệu đã tạo và thư mục 'dữ liệu' hoặc 'đầu vào' cho dữ liệu đã được làm sạch. Chúng tôi không đi sâu vào chi tiết ở đây, nhưng hãy xem chương [Tổ chức báo cáo định kỳ](reportfactory.qmd).  
...
Đọc thêm về quy trình công việc này trong chương về [Tổ chức báo cáo định kỳ](reportfactory.qmd).  
...
* Đánh dấu: Định cấu hình này sẽ thay đổi giao diện của chữ được đánh dấu (ví dụ: code trong các đoạn được hiển thị). Các kiểu được hỗ trợ bao gồm mặc định, tango, pygments, kate, monochrome, espresso, zenburn, haddock, breezedark và textmate.  
...
* Plotly (được sử dụng trong chương sổ tay này và trong chương [Biểu đồ tương tác](interactive_plots.qmd))
...
Hàm `ggplotly()` từ **plotly** đặc biệt dễ sử dụng. Xem chương [Biểu đồ tương tác](interactive_plots.qmd).  
