...
Chương này sẽ hướng dẫn cách chuẩn hóa một kết quả như là tỷ suất tử vong hoặc nhập viện theo các đặc điểm của tuổi và giới. 

Trang này sử dụng package **PHEindicatormethods**.
...
Trong trường hợp này, số liệu của chúng ta có thể có các định dạng khác nhau. Có lẽ số liệu của chúng ta theo tỉnh, thành phố, hoặc khu vực dịch vụ khác. Chúng ta có thể có một hàng cho mỗi ca tử vong và các thông tin về tuổi và giới cho mỗi trường hợp (hoặc một tỷ lệ có ý nghĩa) của các ca tử vong này. Trong trường hợp này, hãy xem các chương về [Nhóm dữ liệu](grouping.qmd), [Xoay trục dữ liệu](pivoting.qmd), và [Bảng mô tả](tables_descriptive.qmd) để tạo một bộ số liệu với số lượng tử vong và dân số theo tầng của tuổi-giới tính.  
...
Xem chương [Tải sách và dữ liệu](data_used.qmd) để biết các hướng dẫn về cách tải tất cả các bộ số liệu mẫu trong cuốn số tay này. Bạn có thể nhập trực tiếp dữ liệu của chương Chuẩn hóa này vào R bằng các lệnh `appliedepidata::get_data()` sau:  
...
Đầu tiên, chúng ta liên kết các bộ số liệu dân số quốc gia, chuyển đổi thành số liệu dạng dọc/dài hơn, và làm sạch số liệu. Xem chương [Xoay trục dữ liệu](pivoting.qmd) để biết thêm chi tiết.  
...
standard_pop_clean <- standard_pop_data %>%
     ...
     rename(pop = WorldStandardPopulation)   # change col name to "pop", as this is expected by phe_dsr()
```
...
Chúng ta tính tỷ suất chuẩn hóa bằng package **PHEindicatormethods**. Package này cho phép chúng ta tính toán tỷ suất chuẩn hóa trực tiếp cũng như gián tiếp. Chúng tôi sẽ hướng dẫn cả hai.  
