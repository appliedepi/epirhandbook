---
aliases:
  - /new_pages/tables_presentation.vn.html
---
...
Hãy hiểu rằng *trước khi* sử dụng **flextable**, bạn phải tạo bảng tóm tắt dữ liệu dưới dạng một data frame. Sử dụng các phương pháp trong các chương [Bảng mô tả](tables_descriptive.qmd) và [Xoay trục dữ liệu](pivoting.qmd) như tạo bảng đơn, tạo bảng chéo, xoay trục, và tính toán các thống kê mô tả. Kết quả là một data frame sau đó có thể được chuyển đến **flextable** để định dạng hiển thị.  


Có nhiều các R packages khác có thể được sử dụng để tạo bảng cho mục đích trình bày - trong chương này chúng tôi nhấm mạnh vào package **flextable**. Một ví dụ sử dụng **knitr** package và hàm của nó `kable()` có thể được tìm thấy trong chương [Truy vết tiếp xúc](contact_tracing.qmd). Tương tự như vậy, package **DT** cũng được nhấn mạnh trong chương [Dashboards với Shiny](shiny_basics.qmd). Các package khác như **GT** và **huxtable** được đề cập trong chương [Package đề xuất](packages_suggested.qmd).  
...
Để bắt đầu, chúng ta nhập bộ dữ liệu linelist đã được làm sạch về các ca bệnh Ebola mô phỏng. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).  
...
*Trước khi* bắt đầu sử dụng **flextable** bạn cần phải *tạo* bảng của bạn dưới một data frame. Xem chương [Bảng mô tả](tables_descriptive.qmd) và [Xoay trục dữ liệu](pivoting.qmd) để biết cách tạo một data frame sử dụng các packages như **janitor** và **dplyr**. Đầu tiên, bạn phải sắp xếp nội dung theo hàng và cột như cách bạn muốn nội dung hiển thị. Sau đó, data frame sẽ được chuyển đến **flextable** để hiển thị nó với màu sắc, tiêu đề, phông chữ, v.v. 
  
Dưới đây là một ví dụ trong chương [Bảng mô tả](tables_descriptive.qmd) về cách biến đổi các trường hợp bệnh trong `linelist` thành một data frame để tóm tắt các outcomes của bệnh nhân và giá trị CT theo bệnh viện, với hàng Tổng ở cuối bảng. Đầu ra được lưu dưới dạng `table`.  
...
* `function(table, i = X, j = X, part = "X")`, where:
  ...
  * `part = ` đề cập đến phần nào của bảng mà hàm đang được áp dụng. Ví dụ. "header", "body" hoặc "all". 
  ...
  * `j = ` chỉ định *cột* mà hàm sẽ được áp dụng, trong đó 'X' là số thứ tự cột hoặc tên cột. Nếu nhiều cột được chọn, ví dụ: cột thứ năm và cột thứ sáu, ta có thể viết: `j = c(5,6)`. 
...
Chúng ta có thể highlight tất cả các giá trị trong một cột đáp ứng một quy tắc nhất định, ví dụ các ô có hơn 55% trường hợp tử vong. Đơn giản chỉ cần đặt điều kiện so sánh vào trong đối số `i = ` hoặc `j = `, phía sau dấu `~`. Bạn cần tham chiếu tới cột trong data frame, không phải các giá trị tiêu đề hiển thị.  
...
Lưu ý là bạn cần cài đặt package `webshot` hoặc `webshot2` để lưu bảng từ flextable dưới dạng ảnh. Hình ảnh xuất ra có thể có nền trong suốt.
...
Xem thêm chi tiết trong chương [Báo cáo với R Markdown](rmarkdown.qmd) của cuốn sổ tay này. 
