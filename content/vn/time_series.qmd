...
VÍ dụ dưới đây chúng ta sẽ sử dụng bộ dữ liệu về Campylobacter ở Đức thuộc package **surveillance** (xem chương [Tải sách và dữ liệu](data_used.qmd) để biết thêm chi tiết). Tuy nhiên, nếu bạn muốn thử chạy code này trên bộ dữ liệu lớn hơn (nhiều quốc gia hoặc tầng), bạn có thể tham khảo code mẫu tại [repo github của r4epis](https://github.com/R4EPI/epitsa). 
...
Bạn có thể tải xuống tất cả dữ liệu được sử dụng trong sổ tay này thông qua các hướng dẫn trong chương [Tải sách và dữ liệu](data_used.qmd).  

Bộ dữ liệu minh họa được sử dụng trong phần này là số lượng các trường hợp campylobacter hàng tuần được báo cáo ở Đức từ năm 2001 đến 2011. Tải trực tiếp bằng `appliedepidata::get_data(name = "campylobacter_germany")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).  
...
Code dưới đây đảm bảo rằng cột ngày thág đã ở đúng định dạng. Trong chương này chúng ta sẽ sử dụng package **tsibble** và hàm `yearweek` sẽ được sử dụng để tạp biến lịch theo tuần. Có một số cách để thực hiện việc này (Xem chương [Làm việc với ngày tháng](dates.qmd) để biết thêm chi tiết), tuy nhiên đối với dữ liệu chuỗi thời gian thì tốt nhất nên sử dụng thống nhất một framework (**tsibble**). 
...
Bạn có thể tải xuống từng tệp dữ liệu khí hậu này từ chương [Tải sách và dữ liệu](data_used.qmd).  
...
Đoạn code bên dưới ghi 10 năm tệp dữ liệu khí hậu ".nc" vào một thư mục tạm. Nó gọi hàm `save_data()` từ package **appliedepidata**, sau đó giải nén tệp lưu trữ.  
...
Xem chương [Loại bỏ trùng lặp](deduplication.qmd) để biết thêm chi tiết về cách lựa chọn các hàng bạn muốn. 
...
Xem chương [Dữ liệu Missing](missing_data.qmd) để biết các tùy chọn khác của imputation.  

Một giải pháp thay thế khác sẽ là tính toán đường trung bình động, để thử và giải quyết các vấn đề báo cáo rõ ràng này (xem phần tiếp theo và chương [Đường trung bình động](moving_average.qmd). 
...
Nếu dữ liệu rất nhiễu (dao động lên và xuống), việc tính toán đường trung bình động có thể sẽ hữu ích. Trong ví dụ dưới đây, với mỗi tuần chúng ta sẽ tính toán số trường hợp trung bình từ bốn tuần trước đó. Việc này sẽ giúp dữ liệu dễ diễn giải hơn. Trong trường hợp của chúng ta, điều này không thực sự bổ sung nhiều, vì vậy chúng ta sẽ bám vào dữ liệu nội suy để phân tích thêm. Xem chương [Đường trung bình động](moving_average.qmd) để biết thêm chi tiết. 
...
Sau đây chúng ta sẽ định nghĩa một hàm để tạo một biểu đồ chu kỳ. Xem chương [Viết hàm](writing_functions.qmd) để biết cách tạo một hàm trong R.  
...
Sử dụng hàm `PACF()` (cho tự tương quan một phần) hiển thị cùng một loại quan hệ nhưng được hiệu chỉnh cho tất cả các tuần khác nằm giữa. Nó sẽ ít thông tin hơn để xác định tính chu kỳ.
...
Bạn có thể nối các tập dữ liệu của mình sử dụng biến tuần. Để biết thêm về nối dữ liệu, xem chương [Nối dữ liệu](joining_matching.qmd).
...
Đầu tiên hãy trực quan hóa dữ liệu của bạn để kiểm tra xem có bất kỳ mối tương quan rõ ràng nào không. Biểu đồ dưới đây cho thấy một mối quan hệ rõ ràng về tính mùa vụ của các biến, và nhiệt độ có thể đạt đỉnh vài tuần trước khi các trường hợp xảy ra. Để biết thêm về xoay trục dữ liệu, xem chương [Xoay trục dữ liệu](pivoting.qmd). 
...
Xem chương [Lặp, vòng lặp, và danh sách](iteration.qmd) để biết thêm về **purrr**.  
