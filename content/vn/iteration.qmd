...
Chúng tôi nhập dataset về các trường hợp từ một vụ dịch Ebola mô phỏng. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).    
...
Cú pháp cơ bản là: `for (item in sequence) {do operations using item}`. Lưu ý dấu ngoặc đơn và dấu ngoặc nhọn. Kết quả có thể được in ra console hoặc được lưu trữ trong một đối tượng vùng chứa.  
...
Với ví dụ này, chúng tôi bao gồm code thao tác bên trong *vòng lặp for*, được chạy cho mọi giá trị trong chuỗi. Trong code này, các giá trị trình tự (tên cột) được sử dụng để *chỉ mục* (tập hợp con) từng phần tử một trong `linelist`. Như đã dạy trong chương [R cơ bản](basics.qmd), dấu ngoặc vuông kép `[[]]` được sử dụng cho tập hợp con. Cột kết quả được chuyển đến `is.na()`, sau đó đến `sum()` để tạo ra số giá trị trong cột bị thiếu. Kết quả được in ra console - một số cho mỗi cột.  
...
Sử dụng `vector()` và chỉ định `mode =` dựa trên lớp phân loại dự kiến của các đối tượng mà bạn sẽ chèn - "double" (để giữ số), "character" hoặc "logical". Bạn cũng nên đặt trước `length = `. Đây phải là độ dài chuỗi *vòng lặp for* của bạn.  

Giả sử bạn muốn lưu trữ trung vị thời gian nhập viện trễ cho mỗi bệnh viện. Bạn sẽ sử dụng "double" và đặt độ dài là số đầu ra dự kiến (số bệnh viện duy nhất trong data set).  
...
Để đặt tất cả ba thành phần lại với nhau (vùng chứa, trình tự và thao tác), chúng ta hãy cố gắng vẽ biểu đồ đường cong dịch bệnh cho mỗi bệnh viện (xem chương [Đường cong dịch bệnh](epicurves.qmd)).  
...
Hãy nhớ rằng **purrr** là một *công cụ lập trình theo hàm*. Có nghĩa là, các thao tác lặp đi lặp lại được gói gọn trong *các hàm*. Xem chương [Viết hàm](writing_functions.qmd) để tìm hiểu cách viết hàm của riêng bạn.  
...
Một lần nữa, chúng tôi sử dụng `map()`: chúng tôi lấy vector của tên phần tử danh sách (được hiển thị ở trên) và sử dụng `map()` để lặp lại chúng, áp dụng `export()` (từ package **rio**, xem chương [Nhập và xuất dữ liệu](importing.qmd)) trên data frame với danh sách `linelist_split` có tên đó. Chúng tôi cũng sử dụng tên để tạo một tên tệp duy nhất. Đây là các bước thực hiện:  
...
* Đường dẫn tệp được cung cấp cho `export()` là động thông qua việc sử dụng `str_glue()` (xem chương [Ký tự và chuỗi](characters_strings.qmd)):  
...
Nhớ lại từ chương về [Các kiểm định thống kê cơ bản](stat_tests.qmd) rằng `t.test()` có thể nhận đầu vào ở định dạng công thức, chẳng hạn như `t.test(numeric column ~ binary column)`. Trong ví dụ này, chúng tôi làm như sau:  
...
* Trong dấu ngoặc đơn của `t.test()`:  
  ...
  * hàm `t.test()` yêu cầu một cột nhị phân ở phía bên phải của phương trình. Chúng tôi đưa vector `linelist$gender` một cách độc lập và cố định (lưu ý rằng nó không được bao gồm trong `select()`).  
...
Lưu ý: Hãy nhớ rằng nếu bạn chỉ muốn áp dụng một hàm cho một số cột nhất định trong data frame, bạn cũng có thể chỉ cần sử dụng `mutate()` và `across()`, như được giải thích trong chương [Làm sạch số liệu và các hàm quan trọng](cleaning.qmd). Dưới đây là một ví dụ về việc áp dụng `as.character()` cho chỉ các cột "age". Lưu ý vị trí của dấu ngoặc đơn và dấu phẩy.  
...
Để trích xuất các phần tử danh sách theo tên hoặc theo vị trí, bạn có thể sử dụng dấu ngoặc `[[ ]]` như được mô tả trong chương [R cơ bản](basics.qmd). Dưới đây chúng tôi sử dụng dấu ngoặc kép để lập chỉ mục danh sách `t.tests_results` và hiển thị phần tử đầu tiên là kết quả của t-test trên `age`.  
