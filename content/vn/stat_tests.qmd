...
...nhiều kiểm định khác có thể được thực hiện, nhưng chúng tôi chỉ trình bày các kiểm định thông dụng và cung cấp đường dẫn tới các tài liệu tham khảo thêm.  
...
Chúng ta nhập bộ số liệu của các ca bệnh về một vụ dịch Ebola mô phỏng. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).    
...
Các lệnh trong **base** R có thể được sử dụng để thực hiện các kiểm định thống kê. Các câu lệnh tương đối đơn giản và kết quả sẽ hiển thị trong bảng điều khiển R Console. Tuy nhiên, kết quả đầu ra thường dưới dạng liệt kê, vì thế sẽ khó thao tác hơn nếu muốn sử dụng kết quả trong các thao tác tiếp theo. 
...
Nó cũng có thể được sử dụng với số liệu đã được nhóm, sao cho một hàng được trả về cho mỗi biến nhóm:  
...
Sử dụng cú pháp công thức để chỉ định cột biến liên tục và cột biến phân loại:  
...
Hàm kiểm định Chi bình phương chấp nhận một bảng, vì vậy đầu tiên là tạo một bảng chéo. Có nhiều cách để tạo một bảng chéo (xem chương [Bảng mô tả](tables_descriptive.qmd)) nhưng ở đây chúng ta sử dụng hàm `tabyl()` từ **janitor** avà bỏ cột ngoài cùng bên trái của nhãn giá trị trước khi đưa vào hàm `chisq_test()`.  
...
Sử dụng package **gtsummary** nếu bạn đang muốn thêm kết quả của một kiểm định thống kê vào một bảng đẹp được tạo ra bằng package này (như đã được mô tả trong phần **gtsummary** của chương [Bảng mô tả](tables_descriptive.qmd#tbl_gt)).  
...
Được sử dụng để so sánh sự khác biệt về trung bình của một biến liên tục trong hai nhóm. Ví dụ như so sánh tuổi trung bình với kết cục của bệnh nhân. 
```{r ttest_gt}
...
Mối tương quan giữa các biến định lượng có thể được khảo sát bằng cách sử dụng package **corrr** thuộc **tidyverse**. Package này cho phép tính các hệ số tương quan bằng phương pháp Pearson, Kendall hoặc Spearman. Gói lệnh này tạo ra một bảng kết quả và cũng có chức năng tự động vẽ các giá trị. 
