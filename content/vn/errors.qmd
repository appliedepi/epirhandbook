...
Nếu sau nhiều lần tìm kiếm, bạn không thể tìm thấy câu trả lời cho vấn đề của mình, hãy cân nhắc tạo một *ví dụ có thể tái tạo* ("reprex") và tự đăng câu hỏi. Xem chương [Nhờ sự trợ giúp](help.qmd) để biết các mẹo về cách tạo và đăng một ví dụ có thể tái tạo lên diễn đàn. 
...
Nếu bạn gặp lỗi thông báo rằng bạn cần xóa tệp "00LOCK", hãy vào thư mục thư viện "R" trên máy tính của bạn (ví dụ: R/win-library/) và tìm thư mục có tên "00LOCK". Hãy xóa thư mục này theo cách thủ công, rồi thử cài đặt lại package. Nguyên nhân có lẽ là một lần cài đặt trước đó đã bị gián đoạn.  
...
Nếu thông báo lỗi có chứa nội dung tương tự như `Error in options[[sprintf("fig.%s", i)]]`, kiểm tra xem các tùy chọn của knitr ở đầu mỗi đoạn code có sử dụng chính xác `out.width = ` hoặc `out.height = ` và *không phải* `fig.width=` và `fig.height=` hay không.
