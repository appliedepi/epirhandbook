---
aliases:
  - /new_pages/survival_analysis.vn.html
---
...
Trong nghiên cứu y học hiện nay, phân tích sống còn được sử dụng rộng rãi trong các nghiên cứu lâm sàng để đánh giá hiệu quả của một phương pháp điều trị hoặc trong dịch tễ học ung thư để đánh giá rất nhiều chỉ số sống còn khác nhau của bệnh ung thư. 
...
Trong cuốn sổ tay này, chúng tôi nhấn mạnh đến hàm `p_load()` từ package **pacman**, giúp cài đặt package nếu cần thiết *và* gọi chúng ra để sử dụng. Bạn cũng có thể gọi các package đã được cài đặt bằng hàm `library()` trong **base** R. Xem chương [R cơ bản](basics.qmd) để biết thêm thông tin về các package trong R.  
...
Chúng ta cũng có thể tạo bảng chéo giữa cột biến `age_cat_small` và `gender` để biết thêm chi tiết về sự phân bố của biến số mới này theo giới tính. Chúng ta sử dụng hàm `tabyl()` và hàm *adorn* từ package **janitor** như được mô tả trong chương [Bảng mô tả](tables_descriptive.qmd). 
...
<span style="color: orange;">**_THẬN TRỌNG:_**  Các hàm từ package **survminer** đòi hỏi chúng ta cụ thể đối tượng sống sót *và* cụ thể lại bộ số liệu đã dùng để fit đối tượng sống sót đó. Hãy nhớ làm điều này để tránh thông báo lỗi do không cụ thể. </span>
...
<span style="color: black;">**_LƯU Ý:_** đối số thứ hai được gọi là *method* có thể được định rõ khi tính toán mô hình Cox để xác định cách xử lý các giá trị thời gian trùng nhau (ties). Phương pháp *mặc định* là "efron", và các tùy chọn khác là "breslow" và "exact".</span>
