...
-   Xem chương [Package đề xuất](packages_suggested.qmd) để hỗ trợ bạn cài đặt các R package thích hợp trước khi bạn mất kết nối internet

**Để tải sổ tay xuống:**

Để truy cập nhanh, hãy **bấm chuột phải** vào [liên kết này](https://github.com/appliedepi/epirhandbook/raw/main/offline_long/index.html) **và chọn "Save link as"**.

Nếu dùng máy Mac, hãy dùng Cmd+click. Nếu dùng điện thoại, hãy nhấn giữ liên kết và chọn "Save link". Sổ tay sẽ được tải xuống thiết bị của bạn. Nếu màn hình hiển thị mã HTML thô, hãy kiểm tra lại xem bạn đã làm theo các hướng dẫn trên chưa.
...
Sau đó bạn có thể dùng `import()` như với bất kỳ tệp nào trên máy tính — xem chương [Nhập xuất dữ liệu](importing.qmd) để biết chi tiết. Lưu ý rằng `save_data()` không tự động giải nén các bộ dữ liệu được phân phối dưới dạng .zip (ví dụ shapefile gồm nhiều tệp, hoặc thư mục chứa tệp); hãy dùng `utils::unzip()` với tệp .zip đã lưu.
...
Đây là dữ liệu giả định từ một cuộc khảo sát sử dụng thang đo Likert, được sử dụng trong chương [Tháp dân số và thang đo Likert](age_pyramid.qmd). Bạn có thể tải những dữ liệu này trực tiếp vào R bằng cách chạy các lệnh sau:
...
Dưới đây là các liên kết đến tệp được dùng trong chương [Dashboards với R Markdown](flexdashboard.qmd). Đây là các tệp nguồn R Markdown/HTML, không phải bộ dữ liệu minh họa, nên chúng không thuộc package **appliedepidata** - hãy tải chúng trực tiếp từ Github:
...
Chương [Truy vết tiếp xúc](contact_tracing.qmd) trình bày phân tích dữ liệu truy vết tiếp xúc, sử dụng dữ liệu minh họa từ [Go.Data](https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting). Dữ liệu này không thuộc package **appliedepidata**; dữ liệu được sử dụng trong chương này có thể được tải xuống dưới dạng tệp .rds bằng cách bấm vào các liên kết sau:
...
[***LƯU Ý:*** Dữ liệu truy vết tiếp xúc có cấu trúc từ phần mềm khác (ví dụ: KoBo, DHIS2 Tracker, CommCare) có thể sẽ khác. Nếu bạn muốn đóng góp dữ liệu hoặc nội dung mẫu thay thế cho trang này, vui lòng [liên hệ chúng tôi](index.qmd).]{style="color: black;"}

[***MẸO:*** Nếu bạn đang triển khai Go.Data và muốn kết nối với API phiên bản của bạn, vui lòng xem chương Nhập xuất dữ liệu [(mục API)](importing.qmd#import_api) và [Go.Data Cộng đồng thực hành](https://community-godata.who.int/).]{style="color: darkgreen;"}
...
Chương [Cơ bản về GIS](gis.qmd) nạp cả ba bộ dữ liệu: ranh giới admin-3 `sle_adm3`, các điểm cơ sở y tế `sle_hf` và bảng dân số `sle_admpop_adm3_2020`. Để đọc một shapefile từ đường dẫn tệp của riêng bạn, hãy dùng `read_sf()` từ package **sf**. Chương đó cũng dẫn tới trang *Humanitarian Data Exchange*, nơi bạn có thể tải thêm shapefile dưới dạng tệp nén. Ví dụ, dữ liệu điểm cơ sở y tế có [tại đây](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities).

#### Cây phả hệ {.unnumbered}

Xem chương [Cây phả hệ](phylogenetic_trees.qmd). Tệp có tên Newick về cây phả hệ được xây dựng từ việc giải trình tự toàn bộ bộ gen của 299 mẫu Shigella sonnei và dữ liệu mẫu tương ứng (được chuyển đổi thành tệp văn bản). Các mẫu và kết quả từ nước Bỉ được cung cấp thông qua Trung tâm tham khảo quốc gia về Salmonella và Shigella (NRC Bỉ) trong phạm vi dự án do EUPHEM Fellow của ECDC thực hiện, và cũng sẽ được xuất bản dưới dạng bản thảo. Dữ liệu quốc tế được cung cấp công khai trên cơ sở dữ liệu công cộng (ncbi) và đã được xuất bản trước đó.
...
Xem trong chương [Tỷ lệ chuẩn hóa](standardization.qmd). Bạn có thể tải dữ liệu trực tiếp bằng các lệnh sau:
...
Đối với chương [phân tích điều tra](survey_analysis.qmd), chúng tôi sử dụng dữ liệu khảo sát tử vong giả định dựa trên mẫu khảo sát của MSF OCA. Dữ liệu giả định này là một phần của [Dự án "R4Epis"](https://r4epis.netlify.app/).
...
Chương [Dashboards với Shiny](shiny_basics.qmd) trình diễn việc xây dựng một ứng dụng đơn giản để hiển thị dữ liệu bệnh sốt rét.
...
Bạn có thể <a href='https://github.com/appliedepi/epirhandbook/raw/main/data/malaria_app/funcs/plot_epicurve.R' class='download-button'> bấm vào đây để tải tệp plot_epicurve.R<span></a> được global.R nạp bằng source(). Lưu ý rằng bạn có thể cần phải lưu trữ nó trong thư mục “funcs” để các đường dẫn tệp here () hoạt động chính xác.
