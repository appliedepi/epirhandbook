---
aliases:
  - /new_pages/ggplot_tips.vn.html
---

# Các mẹo với ggplot {#ggplot-tips}

Trong trang này, chúng tôi sẽ đề cập đến các mẹo và thủ thuật để làm cho biểu đồ ggplots của bạn đẹp và bóng bẩy hơn. Xem thêm chương [ggplot cơ bản](ggplot_basics.qmd) để có các kiến thức nền tảng.  
...
Các điều chỉnh với thang đo ngày tháng được trình bãy rõ hơn trong chương [Đường cong dịch bệnh](epicurves.qmd).  
...
Để trình bày cách điều chỉnh thang màu liên tục, chúng tôi sẽ sử dụng bộ dữ liệu từ chương [Truy vết tiếp xúc](contact_tracing.qmd) chứa độ tuổi của các trường hợp và các trường hợp tiếp xúc gốc của chúng.  
...
Sau đây là một ví dụ từ chương [ggplot cơ bản](ggplot_basics.qmd), sử dụng các phối màu khác nhau. 
...
Một cách đơn giản để có các nhãn ngày tháng hiệu quả trên trục x là gán đối số `labels = ` trong `scale_x_date()` cho hàm `label_date_short()` từ package **scales**. Hàm này sẽ tự động tạo ra các nhãn ngày tháng hiệu quả (đọc thêm [tại đây](https://scales.r-lib.org/reference/label_date.html)). Một lợi ích khác của hàm này là các nhãn sẽ tự động điều chỉnh khi dữ liệu của bạn mở rộng theo thời gian, từ ngày, sang tuần, sang tháng và năm.

Xem một ví dụ đầy đủ trong phần [nhãn ngày nhiều cấp độ](https://epirhandbook.com/en/epidemic-curves.html#multi-level-date-labels) của chương Đường cong dịch bệnh, nhưng dưới đây là một ví dụ nhanh để tham khảo:
...
Lưu ý rằng có một ví dụ khác về kỹ thuật này trong chương [Đường cong dịch bệnh](epicurves.qmd) - phủ tỷ lệ mới mắc tích lũy lên trên đỉnh đường cong dịch bệnh.  
