---
aliases:
  - /new_pages/flexdashboard.vn.html
---
...
Trong sổ tay này, chúng tôi nhấn mạnh đến hàm `p_load()` từ package **pacman**, cài đặt package nếu cần *và* gọi nó để sử dụng. Bạn cũng có thể gọi các package đã cài đặt với hàm `library()` từ **base** R. Xem chương [R cơ bản](basics.qmd) để biết thêm thông tin về các R packages.  
...
pacman::p_load(
  ...
  appliedepidata # dữ liệu ví dụ được sử dụng trong sổ tay này
)
```

### Nhập dữ liệu {.unnumbered}  

Chúng ta nhập bộ dữ liệu về các trường hợp từ một vụ dịch Ebola mô phỏng. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).    
...
Script là một script R Markdown và do đó có các thành phần và tổ chức tương tự như được mô tả trong chương [Báo cáo với R Markdown](rmarkdown.qmd). Chúng ta sẽ xem lại một cách ngắn gọn định dạng này và nêu rõ sự khác biệt so với các định dạng đầu ra R Markdown khác.  
...
Bên ngoài "đoạn" code R, bạn có thể viết văn bản tường thuật. Như được mô tả trong chương [Báo cáo với R Markdown](rmarkdown.qmd), bạn có thể in nghiêng văn bản bằng cách bao quanh nó bằng một dấu hoa thị (*) hoặc in đậm bằng cách bao quanh nó bằng hai dấu hoa thị (**). Nhớ lại rằng dấu đầu dòng và lược đồ đánh số nhạy cảm với dòng mới, thụt đầu dòng và kết thúc một dòng có hai dấu cách.  

Bạn cũng có thể chèn code R nội dòng vào văn bản như được mô tả trong chương [Báo cáo với R Markdown](rmarkdown.qmd), bằng cách bao quanh code bằng dấu nháy đơn ngược và bắt đầu lệnh bằng "r": `` ` 1+1` `` (xem ví dụ với ngày ở trên).  



### Đầu mục {.unnumbered}  

Các cấp đầu mục khác nhau được thiết lập với số lượng ký hiệu thăng khác nhau, như được mô tả trong chương [Báo cáo với R Markdown](rmarkdown.qmd).  
...
Bạn có thể dễ dàng bao gồm các đầu ra R tiêu chuẩn như văn bản, ggplots và bảng (xem chương [Trình bày bảng](tables_presentation.qmd)). Đơn giản chỉ cần code chúng trong một đoạn code R như bạn làm đối với bất kỳ script R Markdown nào khác.

Lưu ý: bạn có thể tải xuống script Rmd hoàn chỉnh và đầu ra dashboard HTML - xem chương [Tải sách và dữ liệu](data_used.qmd).  


### Chữ {.unnumbered}  

Bạn có thể nhập văn bản Markdown và bao gồm code *nội dòng* như đối với bất kỳ đầu ra R Markdown nào khác. Xem chương [Báo cáo với R Markdown](rmarkdown.qmd) để biết chi tiết.  
...
Bạn có thể in các đồ thị vào cửa sổ dashboard như bạn làm trong script R. Trong ví dụ của chúng tôi, chúng tôi sử dụng package **incidence2** để tạo một "epicurve" theo nhóm tuổi bằng hai lệnh đơn giản (xem trang [Đường cong dịch bệnh](epicurves.qmd)). Tuy nhiên, bạn có thể sử dụng `ggplot()` và in một đồ thị theo cách tương tự.  
...
Bạn cũng có thể chuyển một ggplot tiêu chuẩn hoặc đối tượng đồ thị khác vào `ggplotly()` từ package **plotly** (xem chương [Đồ thị tương tác](interactive_plots.qmd)). Điều này sẽ làm cho đồ thị của bạn có tính tương tác, cho phép người đọc "phóng to" và hiển thị khi di chuột qua giá trị của mọi điểm dữ liệu (trong trường hợp này là số trường hợp mỗi tuần và nhóm tuổi trong đường cong).  
...
* Plotly (được sử dụng trong chương sổ tay này và trong chương [Đồ thị tương tác](interactive_plots.qmd))
* visNetwork (được sử dụng trong chương [Chuỗi lây nhiễm](transmission_chains.qmd) của sổ tay này)
* Leaflet (được sử dụng trong chương [GIS cơ bản](gis.qmd) của sổ tay này)
...
Dưới đây, chúng tôi biểu diễn thêm một chuỗi lây nhiễm dịch sử dụng visNetwork vào dashboard. Script chỉ hiển thị code mới được thêm vào phần "Cột 2" của script R Markdown. Bạn có thể tìm thấy code trong chương [Chuỗi lây nhiễm](transmission_chains.qmd)  của sổ tay này.  
...
Bạn có thể chọn có tất cả code trong script R Markdown **flexdashboard**. Ngoài ra, để có một script dashboard rõ ràng và ngắn gọn hơn, bạn có thể chọn gọi code/biểu đồ được lưu trữ hoặc tạo trong các scripts R bên ngoài. Điều này được mô tả chi tiết hơn trong chương [Báo cáo với R Markdown](rmarkdown.qmd).
...
Bạn có thể sử dụng **shiny** để tạo ứng dụng/dashboards *mà không cần đến* flexdashboard (bảng điều khiển linh hoạt). Chương [Dashboards với Shiny](shiny_basics.qmd) trong sổ tay cung cấp thông tin tổng quan về cách tiếp cận này, bao gồm phần bổ sung về cú pháp **shiny**, cấu trúc tệp ứng dụng và các tùy chọn chia sẻ/xuất bản (bao gồm cả tùy chọn máy chủ miễn phí). Cú pháp và các mẹo chung này cũng chuyển sang ngữ cảnh **flexdashboard**.  
