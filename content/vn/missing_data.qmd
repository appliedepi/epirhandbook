---
aliases:
  - /new_pages/missing_data.vn.html
---
...
Chúng ta nhập bộ số liệu của các ca bệnh được mô phỏng từ một vụ dịch Ebola. Tải trực tiếp bằng `appliedepidata::get_data(name = "linelist_cleaned_rds")` (xem trang [Tải sách và dữ liệu](data_used.qmd) để biết chi tiết).
...
Xem phần nhập số liệu trong chương [Dữ liệu missing](importing.qmd#import_missing) để biết thêm chi tiết, vì cú pháp chính xác sẽ khác nhau tùy theo loại tệp.
...
Số liệu của chúng ta có thể có các cách khác để biểu thị giá trị bị thiếu, như là "99", hoặc "Giá trị bị thiếu / Missing", hoặc "Không biết / Unknown", thậm chí chúng ta có giá trị ký tự trống "" như "để trống / blank", hoặc một khoảng trắng " ". Cần lưu ý những giá trị này và xem xét có nên [chuyển đổi chúng thành NA khi nhập vào R](importing.qmd#import_missing) hoặc trong khi làm sạch số liệu bằng hàm `na_if()` hay không.
...
`NULL` là một giá trị dành riêng khác nữa trong R. Nó đại điện cho một mệnh đề không đúng cũng không sai. Nó là kết quả của các biểu thức hoặc hàm có giá trị không xác định. Nói chung, không gán NULL như một giá trị, trừ khi viết hàm hoặc khi viết một [Ứng dụng **shiny**](shiny_basics.qmd) để trả về `NULL` trong các tình huống cụ thể.
...
Sau đó, chúng ta vẽ biểu đồ tỷ lệ các giá trị bị thiếu dưới dạng một dòng theo tuần. Xem thêm chương [ggplot cơ bản](ggplot_basics.qmd) nếu bạn chưa quen với package vẽ biểu đồ **ggplot2**.
...
Bạn có thể chỉ định loại bỏ các hàng có giá trị bị thiếu trong một số cột nhất định:
...
Bạn có thể liệt kê lần lượt từng cột một, hoặc sử dụng [các hàm trợ giúp trong “tidyselect”](cleaning.qmd#clean_tidyselect):
...
-   Lưu ý rằng nếu nhiều cột cùng góp phần khiến các giá trị không được vẽ trên biểu đồ (ví dụ như tuổi hoặc giới nếu chúng được phản ánh trên biểu đồ), thì chúng ta cũng phải lọc trên các cột đó để tính toán chính xác số lượng không được hiển thị.
...
Nếu cột đang quan tâm là một factor, sử dụng hàm `fct_explicit_na()` thuộc package **forcats**  để chuyển đổi các giá trị `NA` thành giá trị ký tự. Xem thêm chi tiết trong chương [Factors](factors.qmd). Theo mặc định, giá trị mới là "(Missing)" nhưng nó có thể được điều chỉnh thông qua đối số `na_level =`.
