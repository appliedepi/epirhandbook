...
Você pode baixar todos os dados utilizados neste manual através das instruções na página de [download do manual e dos dados](data_used.qmd).  

Os dados utilizados nessa seção para fins de exemplificação são as contagens semanais de casos de campylobacter notificados na Alemanha entre 2001 e 2011. Carregue diretamente com `appliedepidata::get_data(name = "campylobacter_germany")` (consulte a página [Baixar manual e dados](data_used.qmd) para mais detalhes).  
...
maneiras de fazer isso (veja a página [trabalhando com datas](dates.qmd)
...
Você pode baixar cada um desses arquivos de dados climáticos da página [Download do manual e dos dados](data_used.qmd).  
...
Veja a página sobre [Eliminando duplicidades](deduplication.qmd)
...
Veja a página sobre [Dados faltantes](missing_data.qmd) para mais informações sobre imputação.

Outra alternativa seria calcular a média móvel, para tentar suavizar
esses problemas aparentes de notificação (veja a próxima seção, e a página sobre [médias móveis](moving_average.qmd)). 
...
Veja a página sobre [Médias móveis](moving_average.qmd) para mais detalhes.
...
Abaixo, nós definimos uma função customizada para criar um periodograma. Veja a página sobre [Escrevendo funções](writing_functions.qmd) para informações sobre como escrever funções no R.
...
manual a seção sobre [unindo dados](joining_matching.qmd).
...
Para mais sobre o pivoteamento dos dados, veja a seção sobre [pivoteando dados](pivoting.qmd) neste manual. 
...
Veja a página sobre [Iteração, loops, e listas](iteration.qmd) para aprender mais sobre o pacote **purrr**.  
