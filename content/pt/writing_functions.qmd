...
Este "chuk" (pedaço) de código mostra o carregamento de pacotes necessários para as análises. Neste manual, enfatizamos `p_load()` de **pacman**, que instala o pacote, se necessário, *e* carrega ele para ser utilizado. Você também pode carregar pacotes instalados com  `library()` do R **base**. Veja em [Introdução ao R](basics.qmd) para mais informações sobre pacotes R.  
...
Importamos os dados de casos de uma simulação de epidemia de Ebola. Se desejar fazer o download dos dados para seguir passo a passo, veja as instruções na página [Baixar livro e dados](data_used.qmd). O conjunto de dados é importado usando a função `import()` do pacote **rio**. Consulte a página [Importar e exportar](importing.qmd) para várias maneiras de importar dados.
