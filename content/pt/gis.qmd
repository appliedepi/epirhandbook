...
A matriz exibida acima mostra as relações entre as regiões de nossa base `sle_adm3_dat`. Um score de 0 indica duas regiões que não são vizinhas, enquanto quaisquer outros valores diferentes de zero indicam uma relação de vizinhança. Os valores na matriz estão escalados de forma que a linha de cada região tenha um peso total de 1. 
