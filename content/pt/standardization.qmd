...
No seu cenário, os dados podem estar em um formato diferente. Talvez seus dados sejam por províncias, cidades, ou outros tipos de área. Talvez você tenha uma linha para cada óbito, e informações (ou uma proporção significante) sobre idade e sexo para cada um desses óbitos. Neste caso, veja as páginas sobre [Agrupando dados](grouping.qmd), [Pivoteando os dados](pivoting.qmd), e [Tabelas descritivas](tables_descriptive.qmd) para criar um conjunto de dados com quantidades de eventos e população por estrato idade-sexo.
...
Veja a página [Download do manual e dados](data_used.qmd) para instruções sobre como baixar todos os dados de exemplos utilizados neste manual. Você pode carregar os dados da página de Padronização diretamente no R com as chamadas `appliedepidata::get_data()` abaixo:
...
Primeiro, nós iremos unir os dados de população por país, transformar para o formato longo, e realizar uma limpeza mínima dos dados. Veja a página sobre [Pivoteando os dados](pivoting.qmd) para mais detalhes.
...
Então, podemos classificar `Sex`, `age_cat5`, e `Country` como factors, e ajustar a ordem dos níveis com a função `fct_relevel()` do pacote **forcats**, como descrito na página [Fatores](factors.qmd). Nota, visivelmente, classificar os níveis dos factors não altera os dados, mas o comando `arrange()` ordena os dados por país (Country), categoria de idade (age), e sexo (sex).  
...
No momento, os valores da coluna `age_cat5`, do conjunto `standard_pop_data`, contém as palavras "years" e "plus", enquanto os dados em `country_data` não possuem essas palavras. Nós precisamos garantir que os valores da categoria "age" coincidam. Para tanto, utilizaremos a função `str_replace_all()`, do pacote **stringr**, como descrito na página sobre [Caracteres e strings](characters_strings.qmd), para substituir estes padrões para "sem espaço" `""`.  
