---
aliases:
  - /new_pages/missing_data.pt.html
---
...
Veja a seção sobre como importar os dados na página [Campos em branco](importing.qmd#import_missing) para mais detalhes, uma vez que a sintaxe muda de acordo com o tipo de arquivo.
...
`NULL` é outro valor reservado (especial) no R. É a representação lógica de uma declaração que não é nem verdadeira (true), tampouco falsa (false). Ele é gerado por expressões ou funções em que os valores são indefinidos. Geralemnte, não atribua NULL como um valor, exceto ao escrever funções ou, talvez, uma [aplicação em **shiny**](shiny_basics.qmd) para gerar `NULL` em cenários específicos.
...
Esta é uma função do pacote **tidyr**, que é útil em um [pipeline de limpeza de dados](cleaning.qmd). Se executada com os parênteses vazios, ela remove linhas com *qualquer* um dos campos esteja em brancos. Se o nome das colunas são especificados em parênteses, linhas com campos em branco apenas nestas colunas serão excluídas. Você pode também utilizar a sintaxe do "tidyselect" para especificar as colunas.
...
O pacote **naniar** oferece uma solução com a função `geom_miss_point()`. Ao criar um gráfico de dispersão de duas colunas, os registros em que um dos valores está em branco e o outro está preenchido são exibidos atribuindo-se aos campos em branco valores 10% menores do que o menor valor da coluna, que são então coloridos de forma distinta dos demais pontos.
...
linelist %>%
  ...
  summarise(across(
    ...
    .fns = list("mean" = ~mean(.x, na.rm = TRUE),     # estatísticas calculadas
                "sd" = ~sd(.x, na.rm = TRUE),
                "var" = ~var(.x, na.rm = TRUE),
                "min" = ~min(.x, na.rm = TRUE),
                "max" = ~max(.x, na.rm = TRUE))))                 # outros argumentos para o cálculo das estatísticas
```
...
Então nóstraçamos um gráfico das proporções dos campos em branco como uma linha, de acordo com a semana. A página sobre [básico do ggplot](ggplot_basics.qmd) pode ser utilizada se você não tiver familiaridade com o pacote **ggplot2** de visuzalização de dados.
...
Você pode listar as colunas uma após a outra, ou utilizar as [funções auxiliares do "tidyselect"](cleaning.qmd#clean_tidyselect):  
...
A função `fill()`, do pacote **tidyr**, pode ser utilizada para realizar imputação por LOCF e BOCF (entretanto, outros pacotes como **HMISC**, **zoo**, e **data.table** também incluem métodos para fazer isto). Para mostrar a sintaxe do `fill()`, nós iremos criar um simples banco de dados com série temporal contendo o número de casos de uma doença para cada trimestre dos anos 2000 e 2001. Entretanto, os valores para os trimestres após Q1 estão faltando, então nós iremos imputá-los. A sintaxe do `fill()` também é demonstrada na página sobre [Pivoteamento dos dados](pivoting.qmd).  
...
Nota: tenha certeza de que seus dados estão ordenados corretamente antes de utilizar a função `fill()`. Por padrão, `fill()` irá preencher de cima para baixo, mas você também pode imputar valores em direções diferentes ao mudar o parâmetro `.direction`. Nós podemos criar um banco de dados similar onde os valores dos anos estão registrados apenas no final do ano, e ausentes para os trimestres anteriores:
...
Então, assim que você tiver criado estes novos bancos de dados com os valores imputados, é possível aplicar quaisquer modelos estatísticos ou análises que estava planejando realizar em cada um dos bancos, e então unir os resultados destes modelos. Isto funciona muito bem para reduzir o enviesamento dos resultados nas situações de MCAR e MAR, e, frequentemente, resulta em estimativas de erro-padrão mais acuradas.
