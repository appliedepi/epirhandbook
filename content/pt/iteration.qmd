...
Para fins de exemplo, incluímos o código de operações dentro do *loop for*, que é executado para cada valor na sequência. Neste código, os valores de sequência (nomes das colunas) são usados para *indexar* (subconjunto) `linelist`, um de cada vez. Conforme ensinado na página [Introdução ao R](basics.qmd), colchetes duplos `[[ ]]` são usados para subconjunto. A coluna resultante é passada para `is.na()`, então para `sum()` para produzir o número de valores na coluna que estão faltando. O resultado é impresso no console - um número para cada coluna.
...
Para juntar todos os três componentes (contêiner, sequência e operações), vamos tentar traçar uma epicurva para cada hospital (consulte a página em [Curvas epidêmicas](epicurves.qmd)).
...
Tenha em mente que o **purrr** é uma *ferramenta de programação funcional*. Ou seja, as operações que devem ser aplicadas iterativamente são agrupadas em *funções*. Consulte a página [Escrevendo funções](writing_functions.qmd) para aprender a escrever suas próprias funções.
...
Lembre-se da página [Importar e exportar](importing.qmd) que quando usado em arquivos do Excel, `import()` pode aceitar o argumento `which =` (qual) especificando a aba a ser importada. Dentro da função `.f` `import()`, fornecemos `which = .x`, cujo valor mudará a cada iteração através do vetor `sheet_names` - primeiro "Central Hospital" ("Hospital Central"), depois "Military Hospital" ("Hospital Militar") etc.
...
Novamente usamos `map()`: pegamos o vetor de nomes de elementos da lista (mostrado acima) e usamos `map()` para iterar por eles, aplicando `export()` (do pacote **rio**, veja página [Importar e exportar](importing.qmd)) no data frame na lista `linelist_split` que tem esse nome. Também usamos o nome para criar um nome de arquivo exclusivo. Aqui está como funciona:
...
-   O caminho do arquivo fornecido para `export()` é dinâmico através do uso de `str_glue()` (consulte a página [Caracteres e strings](characters_strings.qmd)):
...
Se este código `map()` parecer muito confuso, você pode obter o mesmo resultado salvando seu comando `ggplot()` específico como uma função personalizada definida pelo usuário, por exemplo, podemos chamá-lo de `make_epicurve())`. Esta função é então usada dentro do `map()`. `.x` será substituído iterativamente pelo nome do hospital e usado como `hosp_name` na função `make_epicurve()`. Consulte a página sobre [Escrevendo funções](writing_functions.qmd).
...
Lembre-se da página em [Testes estatísticos simples](stat_tests.qmd) que `t.test()` pode receber entradas em um formato de fórmula, como `t.test(coluna numérica ~ coluna binária)`. Neste exemplo, fazemos o seguinte:
...
Para extrair elementos da lista por nome ou por posição, você pode usar colchetes `[[ ]]` conforme descrito na página [Introdução ao R](basics.qmd). Abaixo usamos colchetes duplos para indexar a lista `t.tests_results` e exibir o primeiro elemento que é o resultado do teste t em `age` (idade).
