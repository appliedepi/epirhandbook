...
Para iniciar, importaremos a versão limpa da linelist de casos de uma epidemia simulada de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.qmd) para mais detalhes).    
...
* Você pode usar `bind_rows()` em uma `list` de dataframes que tenham estrutura semelhante para combiná-los em um só. Veja um exemplo na página [Iteração, loops e listas](iteration.qmd) que envolve a importação de múltimplas linelists com **purrr**.  
...
Veja outros exemplos práticos como esse nas páginas [Tabelas descritivas](tables_descriptive.qmd) e [Tabelas para apresentação](tables_presentation.qmd).  
...
Agora podemos visualizar o resultado. Veja como na última linha, um valor vazio, `NA`, preenche a coluna `hospital` que não estava em `hosp_summary`. Como explicado na página [Tabelas para apresentação](tables_presentation.qmd), você pode preencher essa célula com "Total" utilizando `replace_na()`.  
