...
que instala o pacote se necessário *e* carrega-o para utilização. Também se pode carregar pacotes instalados com `library()` a partir do **R base**. Veja a página [Introdução ao R](basics.qmd) para mais informações sobre pacotes R.
...
Você pode fazer o download de todos os dados de exemplo para este manual a partir da página [Download do manual e dados](data_used.qmd).
...
Na sua forma original nos arquivos passíveis de download, os dados são exibidos como fornecidos pela API do Go.Data (saiba mais sobre [APIs aqui](importing.qmd#import_api)). Para fins de exemplo, aqui vamos limpar os dados para facilitar a leitura nesta página. Se estiver usando uma extensão Go.Data, pode ver instruções completas sobre como recuperar os seus dados [aqui](https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting).

Abaixo, os conjuntos de dados são importados utilizando a função `import()` do pacote **rio**. Veja a página [Importar e exportar](importing.qmd) para várias formas de importação de dados. Utilizamos `here()` para especificar o caminho do arquivo - forneça o caminho específico do arquivo para o seu computador. Em seguida, utilizamos `select()` para selecionar apenas certas colunas dos dados, simplificando para efeitos de demonstração.
...
Como demonstrado neste manual [Pirâmides demográficas](age_pyramid.qmd), é possível visualizar a idade e a distribuição por sexo (aqui utilizamos o pacote **apyramid**).
...
Estes dados podem existir em uma variedade de formatos, como um formato Excel "largo/amplo", com uma linha por contato e uma coluna por dia de acompanhamento. Veja [Pivoteando dados](pivoting.qmd) para descrições de dados "longos" e "largos", e como pivotar dados mais amplos/largos ou mais longos.
...
Para ver todos os valores, podemos executar um rápido `tabyl()` (do **janitor**) ou `table()` (do **R Base**) (ver [Tabelas descritivas](tables_descriptive.qmd)) por 'followup_status' para ver a frequência de cada um dos resultados.
...
Veja mais detalhes na página [Gráfico de calor](heatmaps.qmd).
...
A criação de tabelas como esta, dependerá da sua estrutura de dados de rastreamento de contatos. Visite a página [Tabelas descritivas](tables_descriptive.qmd) para aprender como resumir os dados usando as funções do **dplyr**.
...
Como foi discutido na página [Gráficos de calor](heatmaps.qmd), você pode criar uma matriz de "quem infectou quem" utilizando `geom_tile()`.
