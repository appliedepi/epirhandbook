...
* Consulte a página [Pacotes sugeridos](packages_suggested.qmd) para ajudá-lo a instalar os pacotes R apropriados antes que você perca a conectividade com a Internet  

**Para baixar o livro:**

Para acesso rápido, **clique com o botão direito** [neste link](https://github.com/appliedepi/epirhandbook/raw/main/offline_long/index.html) **e selecione "Salvar link como"**.

Se estiver em um Mac, use Cmd+clique. Em um dispositivo móvel, pressione e segure o link e selecione "Salvar link". O livro será baixado para o seu dispositivo. Se aparecer uma tela com código HTML bruto, verifique se você seguiu as instruções acima.
...
Estes são dados fictícios de uma pesquisa no estilo Likert, usados na página [Pirâmides demográficas e escalas Likert](age_pyramid.qmd). Você pode carregar esses dados diretamente no R executando os seguintes comandos:    
...
Abaixo estão os links para o arquivo associado à página em [Painéis (Dashboards) com R Markdown](flexdashboard.qmd). Estes são arquivos-fonte R Markdown/HTML, e não conjuntos de dados de exemplo, portanto não fazem parte do pacote **appliedepidata** - baixe-os diretamente do Github:  
...
A página [Rastreamento de contato](contact_tracing.qmd) demonstra a análise dos dados de rastreamento de contato, usando dados de exemplo de [Go.Data] (https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting). Estes dados não fazem parte do **appliedepidata**; os dados usados na página podem ser baixados como arquivos .rds clicando nos seguintes links:  
...
<span style = "color: black;"> **_NOTA:_** Dados de rastreamento de contato estruturado de outro software (por exemplo, KoBo, DHIS2 Tracker, CommCare) podem parecer diferentes. Se desejar contribuir com dados de amostra ou conteúdo alternativo para esta página, [entre em contato](index.qmd). </span> 

<span style = "color: darkgreen;"> **_DICA:_** Se você estiver implantando Go.Data e quiser se conectar à API da sua instância, consulte a página Importar e exportar [(seção API)](importing.qmd#import_api) e a [Go.Data Community of Practice](https://community-godata.who.int/). </span>
...
A página [GIS básico](gis.qmd) carrega os três conjuntos de dados: os limites admin-3 `sle_adm3`, os pontos dos estabelecimentos de saúde `sle_hf` e a tabela de população `sle_admpop_adm3_2020`. Para ler um shapefile a partir do seu próprio caminho de arquivo, use `read_sf()` do pacote **sf**. Essa página também traz links para o site *Humanitarian Data Exchange*, onde você pode baixar outros shapefiles como arquivos compactados. Os pontos dos estabelecimentos de saúde, por exemplo, estão [aqui](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities).

#### Árvores filogenéticas {.unnumbered}  

Veja a página sobre [Árvores filogenéticas](phylogenetic_trees.qmd). Arquivo Newick da árvore filogenética construída a partir do sequenciamento do genoma completo de 299 amostras de Shigella sonnei e dados de amostra correspondentes (convertidos em um arquivo de texto). As amostras belgas e os dados resultantes são gentilmente fornecidos pelo NRC belga para Salmonella e Shigella no âmbito de um projeto conduzido por um bolsista ECDC EUPHEM, e também serão publicados em um manuscrito. Os dados internacionais estão disponíveis abertamente em bases de dados públicas (NCBI) e foram publicados previamente.  
...
Consulte a página sobre [Taxas padronizadas](standardization.qmd). Você pode carregar os dados diretamente com os seguintes comandos:  
...
Consulte a página em [Séries temporais e detecção de surto](time_series.qmd). Usamos casos de Campylobacter relatados na Alemanha 2002-2011, conforme disponível no pacote **surveillance** R. (*nb.* este conjunto de dados foi adaptado do original, em que 3 meses de dados foram excluídos do final de 2011 para fins de demonstração).
...
Para a página [análise da pesquisa](survey_analysis.qmd), usamos dados fictícios de pesquisa de mortalidade baseados em modelos de pesquisa OCA do MSF. Esses dados fictícios foram gerados como parte do [projeto "R4Epis"](https://r4epis.netlify.app/).
...
A página em [Painéis com Shiny](shiny_basics.qmd) demonstra a construção de um aplicativo simples para exibir dados da malária.
...
Você pode <a href='https://github.com/appliedepi/epirhandbook/raw/main/data/malaria_app/funcs/plot_epicurve.R' class='download-button'>
	clique aqui para baixar o arquivo plot_epicurve.R <span> </a> que é carregado com source() pelo global.R. Observe que pode ser necessário armazená-lo em uma pasta "funcs" para que os caminhos de arquivo here() funcionem corretamente.
