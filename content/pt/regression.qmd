...
verificar as associações entre as variáveis (ex.: *odds ratios*, riscos relativos e *hazard ratio*).
...
Para realizar a regressão proporcional de riscos (regressão Cox), veja a página [Análise de sobrevivência](survival_analysis.qmd).  
...
Este código realiza o carregamento dos pacotes necessários para as análises. Neste manual, nós enfatizamos o uso da função `p_load()`, do pacote **pacman**, que instala os pacotes, caso necessário, *e* os carrega para utilização. Você também pode utilizar a função `library()`, do pacote R **base** , para carregar pacotes instalados. Veja a página sobre o [Introdução ao R](basics.qmd) para mais informações sobre os pacotes R.
...
Nós iremos importar o banco de dados dos casos de uma simulação de epidemia de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.qmd) para mais detalhes).
...
Assim como na página sobre [Tabelas descritivas](tables_descriptive.qmd), os seus objetivos irão determinar quais pacotes R utilizar. Aqui, nós apresentamos duas opções para realizar análises univariadas:
...
`glm()` pode ser utilizada para regressões logísticas univariadas e multivariadas (ex.: para obter razões de chances, ou *Odds Ratios*). Aqui estão as partes principais:
...
* `family = ` Isto determina o tipo de modelo a ser executado. Para regressão logística, utilize `family = "binomial"`, para poisson utilize `family = "poisson"`. Outros exemplos estão na tabela abaixo.
...
1) Obtenha as estimativas do log da razão de chances na forma *exponenciada* e os intervalos de confiança ao aplicar o modelo na função `tidy()`, e ajustar os atributos `exponentiate = TRUE` e `conf.int = TRUE`.  

```{r odds_base_single}

model <- glm(outcome ~ age_cat, family = "binomial", data = linelist) %>% 
  tidy(exponentiate = TRUE, conf.int = TRUE) %>%        # realize a potenciação e produza intervalos de confiança
  mutate(across(where(is.numeric), ~ round(.x, digits = 2)))  # arredonde todas as colunas numéricas
```
...
2) Combine os resultados dessa modelagem com uma tabela de contagens. Abaixo, nós criamos uma tabela cruzada de contagens com a função `tabyl()` do pacote **janitor**, como descrito na página sobre [Tabelas descritivas](tables_descriptive.qmd):
...
combined <- counts_table %>%           # inicie com uma tabela de contagens
  ...
  mutate(across(where(is.numeric), ~ round(.x, digits = 2))) # arredonde para 2 casas decimais
```

Aqui está o resultado da combinação de duas tabelas, exportado como um bela imagem utilizando uma função do **flextable**. A página [Tabelas para apresentação](tables_presentation.qmd) explica como customizar tais tabelas com o **flextable**, mas você pode utilizar outros inúmeros pacotes do R, como **knitr** ou **GT**.
...
Para rodar os modelos com diferentes variáveis e produzir razões de chances univariadas (ex.: sem dependência entre elas), você pode utilizar a abordagem abaixo. Ela utiliza a função `str_c()`, do pacote **stringr**, para criar fórmulas univaridas (veja a página [caracteres e strings](characters_strings.qmd)), e rodar a regressão `glm()`com cada fórmula, aplicando cada resultado `glm()` no `tidy()`, e, finalmente, unindo todos os resultados dos modelos com a função `bind_rows()`, do pacote **tidyr**. Esta abordagem utiliza a função `map()`, do pacote **purrr**, para repetir as funções - veja a página sobre [Iteração, loops, e listas](iteration.qmd) para mais informações sobre essa ferramenta.
...
Abaixo é como o quadro de dados fica após a execução do código. Veja a página sobre [Tabelas para apresentação](tables_presentation.qmd) para ideias de como converter essa tabela para um formato bonito em HTML (ex.: com **flextable**).
...
Abaixo, nós apresentamos o uso da função `tbl_uvregression()` do pacote **gtsummary**. Assim como na página sobre [Tabelas descritivas](tables_descriptive.qmd), as funções do **gtsummary** fazem um bom trabalho executando estatísticas *e* produzindo resultados com aparência profissional. A função `tbl_uvregression()` produz uma tabela com os resultados de uma regressão univariada.
...
É possível construir seu modelo passo a passo, salvando diferentes modelos que incluem certas variáveis explicativas. Esses modelos podem ser comparados com testes de razão de verossimilhança utilizando a função `lrtest()`, do pacote **lmtest**, como mostrado abaixo:  
...
Como descrito na seção sobre análise univariada, aplique o resultado da modelagem na função `tidy()` para potencializar o log das chances e os intervalos de confiança. Finalmente,  todas as colunas numéricas são arredondadads para duas casas decimais. Role o cursor para visualizar todas as linhas.

```{r mv_regression_base}

mv_tab_base <- final_mv_reg %>% 
  broom::tidy(exponentiate = TRUE, conf.int = TRUE) %>%  ## obtenha um quadro de dados organizado das estimativas
  mutate(across(where(is.numeric), ~ round(.x, digits = 2)))          ## arredonde 
```
...
Veja a página sobre [básico do ggplot](ggplot_basics.qmd) se você não é familiar com o pacote de plotagem **ggplot2**.  
