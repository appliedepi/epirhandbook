...
Neste manual, enfatizamos `p_load()` de **pacman**, que instala o pacote se necessário *e* o carrega para uso. Você também pode carregar pacotes instalados com `library()` de **base** R. Veja a página em [Introdução ao R](basics.qmd) para mais informações sobre pacotes R.  
...
Importamos o conjunto de dados de casos de uma simulação de epidemia de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.qmd) para mais detalhes).  
...
O script é um script R Markdown e, portanto, tem os mesmos componentes e organização descritos na página [Relatórios com R Markdown](reportfactory.qmd). Nós os revisamos brevemente e destacamos as diferenças de outros formatos de saída do R Markdown.  
...
Fora de um "bloco" de código R, você pode escrever um texto narrativo. Conforme descrito na página em [Relatórios com R Markdown](reportfactory.qmd), você pode colocar o texto em itálico circundando-o com um asterisco (*) ou em negrito circundando-o com dois asteriscos (**). Lembre-se de que os marcadores e os esquemas de numeração são sensíveis a novas linhas, recuo e acabamento de uma linha com dois espaços.  

Você também pode inserir o código R nas próprias linhas de texto (*in-line*), conforme descrito na página [Relatórios com R Markdown](reportfactory.qmd), circundando o código com crases e iniciando o comando com "r": `` `1 + 1` ``(veja o exemplo com data acima).  



### Títulos {.unnumbered}  

Diferentes níveis de título são estabelecidos com diferentes números de símbolos hash ou jogo da velha, conforme descrito na página [Relatórios com R Markdown](reportfactory.qmd).  
...
Você pode incluir facilmente saídas R padrão, como texto, ggplots e tabelas (consulte a página [Tabelas para apresentação](tables_presentation.qmd)). Simplesmente codifique-os dentro de um fragmento de código R, como faria para qualquer outro script R Markdown.  

Observação: você pode baixar o script Rmd finalizado e a saída do painel HTML - consulte a página [Baixar manual e dados](data_used.qmd).  


### Texto {.unnumbered}  

Você pode digitar o texto do Markdown e incluir o código *in-line* como para qualquer outra saída do R Markdown. Consulte a página [Relatórios com R Markdown](reportfactory.qmd) para obter detalhes. 
...
- Plotly (usado nesta página do manual e na página [Gráficos interativos](interactive_plots.qmd))
- visNetwork (usado na página [Cadeias de transmissão](transmission_chains.qmd) deste manual) \  
- Leaflet (usado na página [Noções básicas de GIS](gis.qmd) deste manual) \  
...
Abaixo, demonstramos como adicionar uma cadeia de transmissão epidêmica que usa visNetwork ao painel. O script mostra apenas o novo código adicionado à seção "Coluna 2" do script R Markdown. Você pode encontrar o código na página [Cadeias de transmissão](transmission_chains.qmd) deste manual.  
