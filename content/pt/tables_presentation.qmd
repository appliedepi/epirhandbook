...
Instalar e carregar o pacote **flextable**. Neste manual, nós destacamos o `p_load()` do **pacman**, o qual instala o pacote, se necessário, *e* o carrega para ser utilizado. Você também pode carregar os pacotes com `library()` presente no R **base**. Veja a seção sobre [Introdução ao R](basics.qmd) para obter mais informações sobre os pacotes “R”.
...
Para começar, nós importamos a *linelist* dos casos que simulam uma epidemia de Ebola. Carregue diretamente com `appliedepidata::get_data(name = "linelist_cleaned_rds")` (consulte a página [Baixar manual e dados](data_used.qmd) para mais detalhes).
...
*Antes* de começar a utilizar o pacote **flextable** você precisará *criar* a sua tabela como um data frame. Veja a seção em [Tabelas descritivas](tables_descriptive.qmd) e [Pivotando dados](pivoting.qmd) para aprender a criar um quadro de dados utilizando pacotes como **janitor*** e **dplyr***. 
...
Abaixo está um exemplo da página [Tabelas descritivas](tables_descriptive.qmd) de conversão dos casos na `linelist` dentro de um conjunto de dados que resume os resultados dos pacientes e os valores de CT por hospital, com uma linha de Totais na parte inferior. A saída é salva como o objeto `table`.
...
Veja detalhes na seção [Relatórios com R Markdown](rmarkdown.qmd) deste manual.
