...
* Se vários pacotes têm funções com o mesmo nome, o mascaramento (*masking*) das funções pode ocorrer entre os pacotes. Ou seja, quando a função do pacote mais recentemente carregado prevalece. Leia mais na página [Introdução ao R](basics.qmd). Considere o uso do pacote **conflicted** para gerenciar tais conflitos.  
* Consulte a seção [Introdução ao R](basics.qmd) sobre pacotes para mais informações sobre **pacman** e mascaramento.  

Para ver as versões dos pacotes R, RStudio e R utilizados durante a produção deste manual, veja a página em [Notas editoriais e técnicas](editorial_style.qmd).  
