...
Nous importons le jeu de données des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_excel")` (voir la page [Télécharger le manuel et les données](data_used.qmd) pour plus de détails). A partir de là, nous utilisons `data.table()` pour convertir le cadre de données en un tableau de données.
