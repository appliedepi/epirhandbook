...
Voir la page sur la [transition a R](transition_to_r.qmd) pour des conseils sur le passage de STATA, SAS ou Excel à R.
...
Pour savoir quelles versions de R, RStudio ou des paquets ont été utilisées lors de la réalisation de ce manuel, consultez la page sur [Notes techniques et choix éditoriaux](editorial_style.qmd).
...
-   Spécifier les deux couleurs à utiliser (`pal =` est l'abréviation de "palette" et est fourni avec un vecteur de deux noms de couleurs. Voir la page [objets](#objects) pour savoir comment la fonction `c()` fabrique un vecteur).
...
L'écriture d'une fonction est traitée en détail à la page [Écriture de fonctions](writing_functions.qmd).
...
Vous pouvez imprimer le chemin d'accès à votre bibliothèque en entrant`.libPaths()` (parenthèses vides). Ceci devient particulièrement important si vous travaillez avec [R sur des lecteurs réseau](network_drives.qmd).
...
Voir la page [Paquets suggérés](packages_suggested.qmd) pour une liste de paquets que nous recommandons pour l'épidémiologie quotidienne.
...
Il convient de noter que les utilisateurs débutants de R peuvent également utiliser R Markdown - ne vous laissez pas intimider !Pour en savoir plus, consultez la page du manuel consacrée aux [rapports avec R Markdown](rmarkdown.qmd).
...
**Voir la page sur** [projets R](r_projects.qmd) pour plus de détails sur notre approche recommandée pour gérer votre répertoire de travail.

Un moyen commun, efficace et sans problème de gérer votre répertoire de travail et vos chemins de fichier consiste à combiner ces trois éléments dans un flux de travail du [projets R](r_projects.qmd) orienté comme expliqué ci-dessous:

1.  Un projet R pour stocker tous vos fichiers (voir page sur [projets R](r_projects.qmd))
2.  Le paquet **here** pour localiser les fichiers (voir page sur [importer et exporter](importing.qmd))
3.  Le paquet **rio** pour importer ou exporter des fichiers (voir page sur [importer et exporter](importing.qmd))

<!-- ======================================================= -->

### Définir le répertoire de travail par commande {.unnumbered}

Jusqu'à récemment, de nombreuses personnes apprenant R ont appris à commencer leurs scripts avec une commande `setwd()`. Veuillez plutôt envisager d'utiliser un flux de travail orienté par [projets R](r_projects.qmd) et lire les [raisons de ne pas utiliser `setwd()`](https://www.tidyverse.org/blog/2017/12/workflow-vs-script/).
...
Dans un script R Markdown, le répertoire de travail par défaut est le dossier ou le fichier RMarkdown (`.Rmd`) est enregistré. Si vous utilisez un projet R et le paquet **here**, cela ne s'applique pas et le répertoire de travail sera `here()`, comme expliqué dans la page [projets R](r_projects.qmd).
...
Vous pouvez en savoir plus sur l'importation et l'exportation d'ensembles de données dans la section sur [importer et exporter](importing.qmd).
...
Le graphique ci-dessous est emprunté à [ce tutoriel R en ligne](http://venus.ifca.unican.es/Rintro/dataStruct.html). Il montre certaines structures de données courantes et leurs noms. Les données spatiales ne sont pas incluses dans cette image, qui sont abordées dans la page [bases de GIS](gis.qmd).
...
Si [Fonctions d'écriture](writing_functions.qmd), ou si vous utilisez R de
...
Ceci est utilisé pour ajouter des données aux arbres phylogénétiques avec le package **ggtree**. Voir la page sur les [arbres phylogénétiques](phylogenetic_trees.qmd) ou ce [livre de ressources en ligne](https://yulab-smu.top/treedata-book/).
...
dans la page [importer et exporter](importing.qmd).
...
impossibles dans la page sur [les valeur manquantes](missing_data.qmd).
Découvrez comment convertir les valeurs manquantes lors de l'importation
de données dans la page sur [importer et exporter](importing.qmd).
...
Les erreurs et avertissements courants ainsi que des conseils de dépannage sont disponibles sur la page [erreurs frequentes](errors.qmd).
