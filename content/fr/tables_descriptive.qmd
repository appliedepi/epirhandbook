---
aliases:
  - /new_pages/tables_descriptive.fr.html
---

# Tableaux descriptifs {#tables_descriptive}
...
*Cette page explique comment* créer les tableaux de base, tandis que la page [Tableaux pour la présentation](tables_presentation.qmd) explique comment les mettre en forme et les imprimer.
...
Nous importons le jeu de données des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.qmd) pour plus de détails).
...
Vous pouvez renvoyer des statistiques individuelles avec des fonctions **extension/package R base** comme `max()`, `min()`, `median()`, `mean()`, `quantile()`, `sd()`, et `range()`. Consultez la page [Bases de R](basics.qmd) pour obtenir une liste complète.
...
Si la colonne est un facteur de classe et que seuls certains niveaux sont présents dans vos données, tous les niveaux apparaîtront quand même dans le tableau. Vous pouvez supprimer cette fonctionnalité en spécifiant `show_missing_levels = FALSE`. Pour en savoir plus, consultez la page [Facteurs](factors.qmd).
...
+--------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `adorn_percentages()`    | Convertir les nombres en proportions, avec `denominator = ` "row", "col", ou "all".                                                                                                                        |
+--------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
+--------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| `adorn_ns()`             | Ajoutez des nombres à un tableau de proportions ou de pourcentages. Indiquez `position =` "rear" pour montrer les nombres entre parenthèses, ou "front" pour mettre les pourcentages entre parenthèses.    |
+--------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
Consultez la page sur les [Tests statistiques simples](stat_tests.qmd) pour obtenir plus de code et de conseils sur les statistiques.
...
Les résultats de la mise en tableau de deux colonnes de regroupement ou plus sont toujours renvoyés au format "long", avec les effectifs dans la colonne `n`. Consultez la page [Pivoter les données](pivoting.qmd) pour en savoir plus sur les formats de données "long" et "large".
...
Cette technique est utile pour standardiser vos tableaux/graphiques. Par exemple, si vous créez des figures pour plusieurs sous-groupes, ou si vous créez plusieurs fois la même figure pour des rapports de routine. Dans chacune de ces circonstances, la présence de valeurs dans les données peut fluctuer, mais vous pouvez définir des niveaux qui restent constants.

Consultez la page sur les [Facteurs](factors.qmd) pour plus d'informations.
...
Afficher un tableau "long" comme celui ci-dessus avec `ggplot()` est relativement simple. Les données sont naturellement au format "long", qui est naturellement accepté par `ggplot()`. Voir d'autres exemples dans les pages [ggplot basics](ggplot_basics.qmd) et [Astuces de ggplot](ggplot_tips.qmd).
...
Dans la page [Caractères et chaînes de caractères](characters_strings.qmd), diverses options pour combiner des colonnes sont discutées, notamment `unite()`, et `paste0()`. Dans ce cas d'utilisation, nous préconisons `str_glue()` parce qu'il est plus flexible que `unite()` et a une syntaxe plus simple que `paste0()`.
...
Maintenant, vous pouvez passer à **flextable** et imprimer le tableau dans Word, .png, .jpeg, .html, Powerpoint, RMarkdown, etc. ! (voir la page [Tableaux pour la présentation](tables_presentation.qmd)).
...
linelist %>% 
  ...
  summarise(across(.cols = c(age_years, temp, wt_kg, ht_cm),  # colonnes
                   .fns = ~ mean(.x, na.rm=T)))                                  # arguments supplémentaires
```
...
linelist %>% 
  group_by(outcome) %>% 
  summarise(across(.cols = c(age_years, temp, wt_kg, ht_cm), # colonnes
                   .fns = list("mean" = ~mean(.x, na.rm=T), "sd" = ~sd(.x, na.rm=T))))                                 # arguments supplémentaires
```
...
Si vous préférez votre tableau en format "large", vous pouvez le transformer en utilisant la fonction **tidyr** `pivot_wider()`. Vous devrez probablement renommer les colonnes avec `rename()`. Pour plus d'informations, consultez la page sur [le pivotement des données](pivoting.qmd).
...
Et vous pouvez ensuite afficher ce tableau sous la forme d'une image. Vous trouverez ci-dessous le résultat imprimé avec **flextable**. Vous pouvez lire plus en détail cet exemple et la façon d'obtenir ce "joli" tableau sur la page [Tableaux pour la présentation](tables_presentation.qmd).
...
Vous pouvez également ajouter les résultats des tests statistiques aux tableaux **gtsummary**. Ce processus est décrit dans la section **gtsummary** de la page [Tests statistiques simples](stat_tests.qmd).
...
-   Le côté droit de l'équation utilise la syntaxe de `str_glue()` de **stringr** (voir [Caractères et chaînes de caractères](characters_strings.qmd)), avec la chaîne d'affichage souhaitée entre guillemets et les statistiques elles-mêmes entre accolades. Vous pouvez inclure des statistiques comme "n" (pour les comptes), "N" (pour le dénominateur), "mean", "median", "sd", "max", "min", les percentiles comme "p##" comme "p25", ou le pourcentage du total comme "p". Voir `?tbl_summary` pour plus de détails.
...
**`missing_text =`**
Ajustez la façon dont les valeurs manquantes sont affichées. La valeur par défaut est "Unknown".
...
* `type = all_categorical() ~ "categorical"` Force les colonnes dichotomiques (par exemple, `fever` oui/non) à montrer tous les niveaux au lieu de ne montrer que la ligne "oui"
...
5)  Affichez, visualisez ou exportez comme vous le souhaitez. Dans cet exemple, nous utilisons `flextable()` du package **flextable** comme décrit dans la page [Tableaux pour la présentation](tables_presentation.qmd). Cela permettra d'afficher dans le volet de visualisation de RStudio une jolie image HTML.
