---
aliases:
  - /new_pages/cleaning.fr.html
---

# Nettoyage de données et fonctions essentielles {#cleaning}
...
Si vous souhaitez comparer ces fonctions aux commandes de Stata ou de SAS, consultez la page [Transition vers R](transition_to_r.qmd).  

Vous pouvez rencontrer une méthode alternative de gestion des données à partir du package  R **data.table** avec des opérateurs comme `:=` et l'utilisation fréquente de crochets `[ ]`. Cette approche et cette syntaxe sont brièvement expliquées dans la page [Data Table](data_table.qmd).  
...
Chargez-les directement avec `appliedepidata::get_data(name = "case_linelists_linelist_raw")` (voir la page [Télécharger le manuel et les données](data_used.qmd) pour plus de détails).    
...
* Certains noms contiennent des espaces (par exemple `infection date`)  
...
* La préférence de capitalisation pour les nouveaux noms de colonnes peut être spécifiée en utilisant l'argument `case = ` ("snake" est le défaut, les alternatives incluent "sentence", "title", "small_camel"...)   
...
Comme raccourci, vous pouvez aussi renommer les colonnes dans les fonctions **dplyr** `select()` et `summarise()`. `select()` est utilisé pour ne garder que certaines colonnes (et est couvert plus loin dans cette page). La fonction `summarise()` est traitée dans les pages [Regroupement des données](grouping.qmd) et [Tableaux descriptifs](tables_descriptive.qmd). Ces fonctions utilisent également le format `nouveau_nom = ancien_nom`. Voici un exemple :  
...
La fonction `matches()` fonctionne de la même manière que `contains()` mais on peut lui fournir une expression régulière (voir la page sur les [Caractères et chaînes de caractères](characters_strings.qmd)), comme plusieurs chaînes de caractères séparées par des barres OR à l'intérieur des parenthèses :  
...
Il existe des moyens de définir la classe des colonnes avec les commandes d'importation, mais cela est souvent fastidieux. Consultez la section [Bases de R](basics.qmd) sur les classes d'objets pour en savoir plus sur la conversion de la classe des objets et des colonnes.  
...
Si votre dataframe est déjà *groupée* (voir la page sur [Travailler sur des données groupées](grouping.qmd)), `mutate()` peut se comporter différemment que si la base de données n'est pas groupée. Toutes les fonctions de résumé, comme `mean()`, `median()`, `max()`, etc. seront calculées par groupe, et non par toutes les lignes.     
...
`mutate(col_a_changer = replace(col_a_changer, condition sur les lignes, nouvelle valeur))`.  
...
`ifelse(condition, valeur à renvoyer si la condition vaut TRUE, valeur à renvoyer si la condition vaut FALSE)`. 
...
<span style="color: red;">**_DANGER:_** **Les valeurs du côté droit doivent toutes être de la même classe** - soit numérique, caractère, date, logique, etc. Pour attribuer des valeurs manquantes (`NA`), Il est dnas certaine situation important d'utiliser  des variantes spéciales de `NA` telles que `NA_character_`, `NA_real_` (pour les numériques ou POSIX), et `as.Date(NA)`. Pour en savoir plus, lisez [Manipuler les dates](dates.qmd).</span>  
...
2) Importez le fichier du dictionnaire dans R. Cet exemple peut être téléchargé via les instructions de la page [Télécharger le manuel et les données](data_used.qmd).  
...
Maintenant, faites défiler vers la droite pour voir comment les valeurs ont changé - en particulier le `gender`(de minuscule à majuscule), et toutes les colonnes de symptômes ont été transformées de oui/non à 1/0.  
...
Tout d'abord, examinez la distribution de vos données, afin de définir limites  appropriés. Voir la page sur [les bases de ggplot](ggplot_basics.qmd).  
...
Pour une manière rapide de faire des pauses et de labelliser des vecteurs, utilisez quelque chose comme ci-dessous. Voir la page [Bases de R](basics.qmd) pour les références sur `seq()` et `rep()`.  
...
Pour combiner des ensembles de données ensemble en liant les lignes d'un cadre de données au bas d'un autre cadre de données, vous pouvez utiliser `bind_rows()` de **dplyr**. Ceci est expliqué plus en détail dans la page [Joindre des données](joining_matching.qmd).  
...
Il est assez courant de vouloir filtrer les lignes qui ont des valeurs manquantes. Résistez à l'envie d'écrire `filter(!is.na(column) & !is.na(column))` et utilisez plutôt la fonction **tidyr** qui est spécialement conçue à cet effet : `drop_na()`. Si elle est exécutée avec des parenthèses vides, elle supprime les lignes ayant *au moins une* valeur manquante. Alternativement, vous pouvez fournir des noms de colonnes spécifiques à évaluer pour les valeurs manquantes, ou utiliser les fonctions d'aide "tidyselect" décrites [ci-dessus](#clean_tidyselect).  
...
*(Voir la page [Manipuler les dates](dates.qmd) pour plus d'informations sur le travail avec des dates et le paquet **lubridate**)*.
...
Listez simplement les colonnes dans l'ordre où elles doivent être triées. Spécifiez `.by_group = TRUE` si vous voulez que le tri se fasse d'abord par tout *groupement* appliqué aux données (voir la page sur [Travailler sur des données groupées](grouping.qmd)).  
...
Le tri des données avec `arrange()` est particulièrement utile lorsque vous créez des [Tableaux de présentation](tables_presentation.qmd), lorsque vous utilisez `slice()` pour prendre les lignes "supérieures" par groupe, ou lorsque vous définissez l'ordre des niveaux de facteurs par ordre d'apparition.  
