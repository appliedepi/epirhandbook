...
Dans la section **purrr** [section](#iter_purrr), nous fournirons également plusieurs exemples de création et de manipulation de `lists`.  
...
3) le **conteneur** des résultats (facultatif).  

La syntaxe de base est la suivante : `for (élément in séquence) {faire des opérations avec l'élément}`. Notez les parenthèses et les accolades. Les résultats peuvent être imprimés sur la console, ou stockés dans un objet R conteneur.   
...
Nous avons choisi le terme `hosp` pour représenter les valeurs du vecteur `hospital_names`. Pour la première itération de la boucle, la valeur de `hosp` sera `hospital_names[[1]]`. Pour la deuxième boucle, elle sera `hospital_names[[2]]`. Et ainsi de suite...  
...
Utilisez `vector()` et spécifiez le `mode = ` en fonction de la classe attendue des objets que vous allez insérer - soit "double" (pour contenir des nombres), "character", ou "logical". Vous devez également définir la valeur `length = ` à l'avance. Il s'agit de la longueur de votre séquence *for loop*.  
...
Dans l'exemple ci-dessous, la séquence est un vecteur de caractères explicite, qui est utilisé pour sélectionner un sous-ensemble de la linelist par hôpital. Les résultats ne sont pas stockés dans un conteneur, mais sont imprimés sur la console avec la fonction `print()`.    
...
Pour rassembler les trois composants (conteneur, séquence et opérations), essayons de tracer une épicurve pour chaque hôpital (voir la page sur les [Courbes épidémiques](epicurves.qmd)).  
...
Gardez à l'esprit que **purrr** est un outil de programmation *fonctionnel*. C'est-à-dire que les opérations qui doivent être appliquées de manière itérative sont regroupées dans des *fonctions*. Consultez la page [Écrire des fonctions](writing_functions.qmd) pour apprendre à écrire vos propres fonctions.  
...
Nous enregistrons la liste de sortie comme `combined`.  
...
```{r, eval=F}
appliedepidata::save_data(name = "example_hospital_linelists", path = getwd())
sheet_names <- readxl::excel_sheets("example_hospital_linelists.xlsx") # extrait les noms des feuilles

combined <- sheet_names %>% # commence avec les noms de feuilles
  purrr::set_names() %>% # définit leurs noms
  map(.f = ~import("example_hospital_linelists.xlsx", which = .x)) %>% # itère, importe, sauvegarde dans la liste
...
names(linelist_split) <- groupings %>% 
     mutate(across(everything(), ~ replace_na(.x, "Missing"))) %>% # Remplacer NA par "Missing" dans toutes les colonnes
...
* Le chemin du fichier fourni à `export()` est dynamique grâce à l'utilisation de `str_glue()` (voir la page [Caractères et chaînes de caractères](characters_strings.qmd)) :  
...
Un autre cas d'utilisation courant est de mapper une fonction sur plusieurs colonnes. Ci-dessous, nous mappons (`map()`) la fonction `t.test()` sur les colonnes numériques du cadre de données `linelist`, en comparant les valeurs numériques par sexe.  

Rappelez-vous de la page sur les [Tests statistiques simples](stat_tests.qmd) que `t.test()` peut prendre des entrées dans un format de formule, comme `t.test(colonne numérique ~ colonne binaire)`. Dans cet exemple, nous faisons ce qui suit :    
...
Pour extraire les éléments d'une liste par nom ou par position, vous pouvez utiliser des crochets "[[ ]]" comme décrit dans la page [bases de R](basics.qmd). Ci-dessous, nous utilisons des doubles crochets pour indexer la liste `t.tests_results` et afficher le premier élément qui est le résultat du test t sur `age`.  
