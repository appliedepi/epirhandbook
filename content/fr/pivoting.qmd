...
<!-- ======================================================= -->
<!-- ======================================================= -->
# Restructurer les données {#pivoting}
...
Dans ce chapitre, nous allons nous focaliser sur le second processus. Résumer ses données dans des tableau est une étape cruciale de l'analyse des données et est traitée dans les chapitres sur [le regroupement des données](grouping.qmd) et les [tableaux descriptifs](tables_descriptive.qmd). 
...
## Étapes préliminaires {#pivot_prep}
...
Dans ce chapitre, nous utiliserons un jeu de données fictif de cas quotidiens de paludisme, par établissement et par groupe d'âge. Chargez-les directement avec `appliedepidata::get_data(name = "malaria_facility_count_data")` (voir la page [Télécharger le manuel et les données](data_used.qmd) pour plus de détails).    
...
A la fin de ce chapitre, nous utiliserons également une liste des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.qmd) pour plus de détails).    
...
sont contenus dans les colonnes les plus à droites, dans un format "large" : l'information de chaque groupe d'âge pour un établissement à une date donnée est contenue sur une seule ligne.
...
La colonne `date` est actuellement sous la forme d'une *chaîne de caractères* mais nous pouvons facilement la convertir en classe date en utilisant les fonctions `mutate()` et `as_date()` décrites dans la page [travailler avec des dates](dates.qmd).  

Nous pouvons aussi améliorer la colonne `observation` en supprimant le préfixe "obs"  et en la convertissant en format `numeric`. Nous pouvons le faire avec `str_remove_all()` du paquet **stringr** (voir la page sur les [chaînes de caractères](characters_strings.qmd)). 
...
Un cas d'utilisation typique est lorsque qu'il faut transformer les résultats d'une analyse dans un format plus digeste pour le lecteur, tel qu'un [tableau résumé](tables_presentation.qmd). En général, il s'agit de transformer un dataframe dans lequel les informations relatives à un sujet sont réparties sur plusieurs lignes en un format dans lequel ces informations sont stockées sur une seule ligne.
