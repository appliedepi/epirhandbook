...
Pour commencer, nous importons la linelist nettoyée des cas d'une épidémie d'Ebola simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.qmd) pour plus de détails).    
...
Voir la page sur [Caractères et chaînes de caractères](characters_strings.qmd) pour en savoir plus sur la fonction `unite()` de **stringr**.
...
Si vous êtes intéressé par une commande plus efficace, vous pouvez profiter de la fonction `+()`, qui convertit en 1 et en 0 en fonction d'une déclaration logique. Cette commande utilise la fonction `across()` pour modifier plusieurs colonnes à la fois (pour en savoir plus, lisez la section [Nettoyage des données et des fonctions de base](cleaning.qmd#clean_across)).  
