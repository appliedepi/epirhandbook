...
Fournissez le chemin d'accès au dossier dans `path = `. Utilisez `recurse = FALSE` pour afficher un seul niveau, ou `recurse = TRUE` pour afficher tous les fichiers de tous les sous-niveaux. Ci-dessous, nous utilisons `here()` comme raccourci pour le projet R et spécifions son sous-dossier "data", qui contient les données d'exemple de ce manuel. Nous le paramétrons pour afficher tous les fichiers contenus dans "data" et ses sous-dossiers.  
...
Pour lister uniquement les noms de fichiers d'un répertoire, vous pouvez utiliser `dir()` à partir de **base** R. Par exemple, cette commande liste les noms des fichiers contenus dans le sous-dossier "linelists" du dossier "example" d'un projet R. Le chemin de fichier relatif est fourni en utilisant `here()`. (dont vous trouverez plus d'informations sur la page [Importer et exporter des données](importing.qmd) ).  

```{r}
# noms de fichiers
dir(here("data", "example", "linelists"))
```
...
dir_ls(here("data", "example", "linelists"))
```
...
Cela peut être particulièrement utile si vous voulez extraire la date de dernière modification du fichier, par exemple si vous voulez importer la version la plus récente d'un fichier. Pour un exemple de ceci, voir la page [Importer et exporter des données](importing.qmd).     

```{r, eval=F}
# informations sur le fichier
dir_info(here("data", "example", "linelists"))
```

Voici le cadre de données renvoyé. Faites défiler vers la droite pour voir toutes les colonnes.  

```{r, echo=F}
DT::datatable(dir_info(here("data", "example", "linelists")), rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
...
Cela revient à afficher le script R ci-dessus et à cliquer sur le bouton "Source" en haut à droite du script. Ceci exécutera le script mais le fera silencieusement (pas de sortie sur la console R) sauf si cela est spécifiquement prévu.  
...
Voir la page sur les [Production de rapports avec R Markdown](rmarkdown.qmd) pour plus de détails. Consultez également la documentation de `render()` [ici](https://rmarkdown.rstudio.com/docs/reference/render.html) ou en entrant `?render`.  
...
Voir la page [Importer et exporter des données](importing.qmd) pour importer et exporter des fichiers individuels.  

Consultez également la page [Importer et exporter des données](importing.qmd) pour connaître les méthodes permettant d'importer automatiquement le fichier le plus récent, en fonction d'une date figurant dans le nom du fichier *ou* en examinant les métadonnées du fichier.  

Voir la page [Itération, boucles et listes](iteration.qmd) pour un exemple avec le paquet **purrr** démontrant :  
...
list.files(path = here("data", "example"), pattern = ".csv")
# dir(path = here("data", "example"), pattern = ".csv")

list.files(path = here("data", "example"), pattern = "hospital", ignore.case = TRUE)
