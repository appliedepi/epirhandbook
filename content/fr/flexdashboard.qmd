...
Dans ce manuel, nous mettons l'accent sur `p_load()` de **pacman**, qui installe le paquet si nécessaire *et* le charge pour l'utiliser. Vous pouvez aussi charger les paquets installés avec `library()` de **base** R. Voir la page sur [bases de R](basics.qmd) pour plus d'informations sur les paquets R.  
...
Le script est un script R Markdown, et a donc les mêmes composants et la même organisation que les scripts décrits dans la page sur les [Rapports avec R Markdown](rmarkdown.qmd). Nous allons brièvement les revoir et souligner les différences avec les autres formats de sortie R Markdown.  
...
En dehors d'un "chunk" de code R, vous pouvez écrire un texte narratif. Comme décrit dans la page [Rapports avec R Markdown](rmarkdown.qmd), vous pouvez mettre du texte en italique en l'entourant d'un astérisque (*texte italique*), ou en gras en l'entourant de deux astérisques (**texte gras**). Rappelez-vous que les puces et les schémas de numérotation sont sensibles aux nouvelles lignes, à l'indentation et au fait de terminer une ligne par deux espaces.  

Vous pouvez également insérer du code R en ligne dans du texte, comme décrit à la page [Rapports avec R Markdown](rmarkdown.qmd), en entourant le code de crochets arrière et en commençant la commande par "r" : `` `r knitr::inline_expr("1+1")` `` (voir l'exemple avec la date ci-dessus).  



### Titres {.unnumbered}  

Différents niveaux de titres sont établis avec différents nombres de symboles de hachage, comme décrit dans la page [Rapports avec R Markdown](rmarkdown.qmd).  
...
Vous pouvez facilement inclure des sorties R standard telles que du texte, des ggplots et des tableaux (voir la page [Tableaux pour la présentation](tables_presentation.qmd)). Il suffit de les coder dans un chunk de code R comme vous le feriez pour tout autre script R Markdown.  

Remarque : vous pouvez télécharger le script Rmd terminé et la sortie du tableau de bord HTML - voir la page [Télécharger le manuel et les données](data_used.qmd).  


### Texte {.unnumbered}  

Vous pouvez saisir du texte Markdown et inclure du code *en ligne* comme pour toute autre sortie R Markdown. Voir la page [Rapports avec R Markdown](rmarkdown.qmd) pour plus de détails. 
...
Vous pouvez également passer un ggplot standard ou un autre objet de tracé à `ggplotly()` du paquet **plotly** (voir la page [Graphiques interactifs](interactive_plots.qmd)). Cela rendra votre graphique interactif, permettra au lecteur de "zoomer" et affichera en surimpression la valeur de chaque point de données (dans ce scénario, le nombre de cas par semaine et le groupe d'âge dans la courbe).  
...
* Plotly (utilisé dans cette page du manuel et dans la page [Graphiques interactifs](interactive_plots.qmd)).
* visNetwork (utilisé dans la page [Chaînes de transmission](transmission_chains.qmd) de ce manuel)  
* Leaflet (utilisé dans la page [bases de GIS](gis.qmd) de ce manuel)  
