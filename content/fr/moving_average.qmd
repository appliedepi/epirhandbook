...
Si vous avez des séries temporelles propres avec des valeurs pour chaque date, vous pouvez utiliser `slide_dbl()`, comme démontré ici dans la page [Série chronologique et détection des épidémies](time_series.qmd).  
...
Voir la page du manuel sur le [Regroupement des données](grouping.qmd) pour plus de détails sur le regroupement des données.
