...
Il s'agit d'une fonction de **tidyr** utile pour [nettoyer des données dans un pipeline](cleaning.qmd). Si elle est exécutée sans argument, elle supprime également les lignes dont *au moins une* valeur est manquante. Mais si des noms de colonnes sont spécifiés comme arguments, seules les lignes avec des valeurs manquantes dans ces colonnes seront supprimées.   
...
Référez vous à la page [bases de ggplot](ggplot_basics.qmd) si vous n'êtes pas familier avec le package **ggplot2**.  
...
On peut passer plusieurs colonnes l'une après l'autre à la fonction, ou utiliser des [fonctions utilitaires de "tidyselect"](cleaning.qmd#clean_tidyselect):  
...
Si votre colonne d'intérêt est un facteur, utilisez `fct_explicit_na()` du package **forcats** pour convertir les valeurs `NA` en une chaîne de caractères (plus de détails dans la page [Facteurs](factors.qmd). Par défaut, la nouvelle valeur est "(Missing)" mais cela peut être ajusté via l'argument `na_level =`.   
