...
Pour commencer, nous importons la liste linéaire ("linelist") nettoyée des cas d'une épidémie d'Ebola qui a été simulée. Chargez-les directement avec `appliedepidata::get_data(name = "linelist_cleaned_rds")` (voir la page [Télécharger le manuel et les données](data_used.qmd) pour plus de détails).
...
Comme expliqué plus haut, *avant* de commencer à utiliser **flextable**, vous devez d'abord *créer* la table que vous voulez présenter sous forme de tableau de données. Consultez la page sur les [Tables descriptives](tables_descriptive.qmd) et les [Données pivotantes](pivoting.qmd) pour apprendre à créer un tableau de données à l'aide de "packages" tels que **janitor** et **dplyr**. Vous devez disposer le contenu en lignes et en colonnes comme vous voulez qu'il soit affiché. C'est à dire on part de notre base de données principales, on lui applique les modifications et opérations nécessaires pour synthétiser l'information que l'on veut présenter dans notre table finale et ce résultat sera enregistré dans un tableu de données. Ensuite, ce tableau de données sera soumis à **flextable** pour l'afficher avec la mise en forme voulue ajoutant des couleurs, des en-têtes, des polices, etc.

Voici un exemple tiré de la page [Tables descriptives](tables_descriptive.qmd) sur la conversion de la "liste linéaire des cas" en un tableau de données qui résume/synthétise l'issue finale des patients et les valeurs CT par hôpital, avec une ligne de totaux en bas. Le résultat est enregistré sous le nom de `table`.
...
Voir les détails dans la page [Rapports avec R Markdown](rmarkdown.qmd) de ce manuel.
