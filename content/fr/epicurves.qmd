...
Les jeux de données sont importés à l'aide de la fonction `import()` du paquetage **rio**. Voir la page [Importation et exportation](importing.qmd) pour les différentes maniéres d'importer des données.  
...
Les barres de l'histogramme peuvent étre colorées par groupe et "empilées". Pour désigner la colonne de regroupement, effectuez les modifications suivantes. Voir la page [ggplot basics](ggplot_basics.qmd) pour plus de détails.  
...
* Pour plus d'informations sur les échelles et les palettes de couleurs, consultez la page sur [les bases de ggplot](ggplot_basics.qmd).  
...
Le meilleur moyen d'ajuster l'ordre dans lequel les barres groupées sont empilées est de classer la colonne de groupage en tant que classe Facteur. Vous pouvez alors désigner l'ordre des niveaux de facteurs (et leurs étiquettes d'affichage). Voir la page sur [facteurs](factors.qmd) ou [ggplot tips](ggplot_tips.qmd) pour plus de détails.  

Avant de réaliser le tracé, utilisez la fonction `fct_relevel()` du paquet **forcats** pour convertir la colonne de regroupement en classe facteur et ajuster manuellement l'ordre des niveaux, comme détaillé dans la page sur les [facteurs](factors.qmd).  
...
Pour en savoir plus sur les légendes et les échelles, consultez la page [ggplot tips](ggplot_tips.qmd). Voici quelques points saillants :  
...
* Voir [cette page](https://rdrr.io/r/base/strptime.html) ou la page [travailler avec des dates](dates.qmd) pour des conseils sur la création d'étiquettes de date.  
...
Voir la page sur les [Moyennes mobiles](moving_average.qmd) pour une description détaillée et plusieurs options. Vous trouverez ci-dessous une option pour calculer des moyennes mobiles avec le package **slider**. Dans cette approche, *la moyenne mobile est calculée dans l'ensemble de données avant le tracé* :  

1) Regroupez les données en comptes si nécessaire (quotidien, hebdomadaire, etc.) (voir la page sur [groupage des données](grouping.qmd)).  
...
On peut aussi le superposer à l'épicurve, avec un double axe en utilisant la méthode **cowplot** décrite ci-dessus et dans la page [ggplot tips](ggplot_tips.qmd) :
