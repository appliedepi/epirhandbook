...
Pour convertir des nombres à un chiffre en nombres à deux chiffres (par exemple, pour ajouter des zéros aux heures ou aux minutes afin d'obtenir deux chiffres), consultez la section ["Longueur des caractères" de la page caractères et chaînes de caractères](characters_strings.qmd#str_pad). 
...
Pour en savoir plus sur la fonction `case_when()`, consultez la page [Nettoyage des données et des fonctions de base](cleaning.qmd). 
...
Consultez la page [Regroupement de données](grouping.qmd) pour des exemples plus complets de regroupement de données par date. Ci-dessous, nous décrivons briévement le regroupement des données par semaine.   
...
Allez à la page [Regroupement des données](grouping.qmd) pour une explication détaillée du regroupement des données avec `count()`, `group_by()`, et `summarise()`. Voici un bref exemple.  
...
Une autre alternative pour les séries temporelles qui fonctionne également bien pour afficher un format "semaine" ("2020 W12") est `yearweek()` du paquet **tsibble**, comme démontré dans la page sur [Série temporelle et détection des épidémies](time_series.qmd).  
