...
-   La page [Organisation des rapports de routine](reportfactory.qmd) montre comment organiser la production de vos rapports avec des dossiers horodatés générés automatiquement.\
-   La page [Tableaux de bord avec R Markdown](flexdashboard.qmd) explique comment formater un rapport R Markdown en tant que tableau de bord.
...
Pour faire référence à des fichiers ailleurs, vous devrez soit utiliser le chemin complet du fichier, soit utiliser le package **here**. Le package **here** définit le répertoire de travail comme étant le dossier racine du projet R et est expliqué en détail dans les pages [Projets R](r_projects.qmd) et [Importer et exporter des données](importing.qmd) de ce manuel. Par exemple, pour importer un fichier appelé "data.csv" depuis le dossier `projectX`, le code serait `import(here("data.csv"))`.
...
Pour en savoir plus sur ce flux de travail, consultez la page [Organisation des rapports de routine](reportfactory.qmd).
...
- Leaflet (utilisé dans la page [Bases des GIS](gis.qmd) de ce manuel)
...
La fonction `ggplotly()` de **plotly** est particulièrement facile à utiliser. Voir la page [Graphiques interactifs](interactive_plots.qmd).
