---
aliases:
  - /new_pages/ggplot_tips.fr.html
---

# Trucs et Astuces avec ggplot {#ggplot_tips}

Dans cette page, nous couvrirons les trucs et astuces pour rendre vos ggplots nets et esthétiques. Voir la page sur les [bases de ggplot](ggplot_basics.qmd) pour les principes de base.
...
Les nuances des "scales" au format date sont traitées plus en détail dans la page [Courbes épidémiques](epicurves.qmd).
...
Pour illustrer comment ajuster une "scale" de couleur continue, nous utiliserons un ensemble de données de la page [Suivi des contacts](contact_tracing.qmd) qui contient les âges des cas et de leurs cas sources.

```{r, warning=F, message=F}
case_source_relationships <- appliedepidata::get_data(name = "relationships_clean") %>%
...
Voici un exemple tiré de la page [ggplot basics](ggplot_basics.qmd), utilisant différents schémas de couleurs.
...
Vous trouverez un exemple complet dans la section de la page [Courbes épidémiques](epicurves.qmd) sur les étiquettes de date à plusieurs niveaux, mais un exemple rapide est présenté ci-dessous pour référence :
...
Si vos graphiques ont la même légende, il est relativement simple de les combiner. Utilisez simplement l'approche **cowplot** ci-dessus pour combiner les graphiques, mais supprimez la légende de l'une d'entre elles (dé-duplication).
...
Notez qu'il existe un autre exemple de cette technique sur la page [Courbes épidémiques](epicurves.qmd) - superposition de l'incidence cumulée sur l'épicurve.
...
"Cet addin vous permet d'explorer interactivement vos données en les visualisant avec le package ggplot2. Il vous permet de dessiner des diagrammes en barres, des courbes, des diagrammes de dispersion, des histogrammes, des boxplot et des objets sf, puis d'exporter le graphique ou de récupérer le code pour reproduire le graphique."
...
Centre européen de prévention et de contrôle des maladies [Guide pour la présentation des données de surveillance](https://ecdc.europa.eu/sites/portal/files/documents/Guidelines%20for%20presentation%20of%20surveillance%20data-final-with-cover-for-we....pdf)
