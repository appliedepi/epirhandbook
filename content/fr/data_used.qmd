...
* Ouvrez `index.html` dans le dossier décompressé pour commencer la lecture.
* La barre de recherche ne fonctionne pas hors ligne. Utilisez Ctrl+f (Cmd+f sur un Mac) pour chercher dans la page que vous lisez.
* Consultez la page [Paquets conseillés](packages_suggested.qmd) pour installer les paquets R dont vous avez besoin avant de perdre votre connexion.

**Pour télécharger le manuel :**

<a href='https://github.com/appliedepi/epirhandbook/releases/download/offline/epirhandbook-fr.zip' class='download-button'>Télécharger le manuel (zip)</a>
...
La liste de cas "brute" est une feuille de calcul Excel contenant des données désordonnées. Utilisez-la pour suivre la page [Nettoyer les données et fonctions essentielles](cleaning.qmd).
...
Il s'agit de données fictives issues d'une enquête de type Likert, utilisées dans la page [Pyramides démographiques et échelles de Likert](age_pyramid.qmd). Vous pouvez charger ces données directement dans R en exécutant les commandes suivantes :    
...
Vous trouverez ci-dessous des liens vers les fichiers associés à la page [Tableaux de bord avec R Markdown](flexdashboard.qmd). Il s'agit de fichiers sources R Markdown/HTML, et non de jeux de données d'exemple ; ils ne font donc pas partie du paquet **appliedepidata** - téléchargez-les directement depuis Github :
...
<span style="color: black;">**_NOTE:_** Les données structurées de recherche des contacts provenant d'autres logiciels (par exemple KoBo, DHIS2 Tracker, CommCare) peuvent être organisées différemment. Si vous souhaitez contribuer à l'élaboration d'un échantillon de données ou d'un contenu alternatif pour cette page, veuillez [nous contacter](index.qmd).</span> 

<span style="color: darkgreen;">**_TIP:_** Si vous déployez Go.Data et souhaitez vous connecter à l'API de votre instance, consultez la page Importation et exportation, [(section API)](importing.qmd#import_api) et la [Communauté de pratique Go.Data](https://community-godata.who.int/).</span>
...
Pour la page sur la [standardisation des données](standardization.qmd), vous pouvez charger les données directement avec les commandes suivantes :  
...
Voir la page sur [les séries temporelles et la détection des épidémies](time_series.qmd). Nous utilisons les cas de campylobacter rapportés en Allemagne de 2002 à 2011, tels que disponibles dans le paquet R **surveillance**. (*note* cet ensemble de données a été adapté de l'original, en ce sens que 3 mois de données ont été supprimés à partir de la fin de 2011 à des fins de démonstration).
...
Pour la page [analyse d'enquête](survey_analysis.qmd), nous utilisons des données d'enquêtes de mortalité fictives basées sur les modèles d'enquête MSF OCA. Ces données fictives ont été générées dans le cadre du projet ["R4Epis"](https://r4epis.netlify.app/).
...
La page sur les [tableaux de bord avec Shiny](shiny_basics.qmd) illustre la construction d'une application simple pour afficher les données sur le paludisme. Les données de l'application font partie de **appliedepidata** :
