...
Chargez la `linelist` propre avec `get_data()` de **appliedepidata** (voir [Télécharger le manuel et les données](data_used.qmd)).  
...
Pour lire votre propre shapefile, utilisez `read_sf()` de **sf** avec son chemin de fichier (voir [Importation et exportation](importing.qmd) et [Projets R](r_projects.qmd)).  
```{r}
...
Ces données proviennent du package **appliedepidata** (voir [Télécharger le manuel et les données](data_used.qmd)). Elles sont aussi sur HDX (lien [ici](https://data.humdata.org/dataset/sierra-leone-population)). Nous les chargeons avec `get_data()`, puis nous passons le résultat à `clean_names()` pour standardiser la syntaxe des noms de colonnes.   
...
Les emplacements des établissements de santé proviennent de HDX [site Web ici](https://data.humdata.org/dataset/hotosm_sierra_leone_health_facilities). Le chapitre les charge avec `get_data()` de **appliedepidata**, comme expliqué dans la page [Télécharger le manuel et les données](data_used.qmd).   
...
Dans cet exemple, nous commençons le `ggplot()` avec la `linelist_adm`, afin de pouvoir appliquer des fonctions de facteurs comme `fct_infreq()` qui ordonne les barres par fréquence (voir la page sur les [Facteurs](factors.qmd) pour des conseils).  
...
1) Les données de population `sle_adm3_pop` sont jointes à l'aide d'un `left_join()` de **dplyr** sur la base des valeurs communes à la colonne `admin3pcod` dans le dataframe `case_adm3`, et à la colonne `adm_pcode` dans le dataframe `sle_adm3_pop`. Voir la page [Joindre des données](joining_matching.qmd).  
