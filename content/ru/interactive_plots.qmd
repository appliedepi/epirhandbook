---
aliases:
  - /new_pages/interactive_plots.ru.html
---
...
На этой странице мы предполагаем, что вы начинаете с графика `ggplot()`, который вы хотите преобразовать в интерактивный. На этой странице мы построим несколько таких графиков, используя случай `linelist`, используемый на многих страницах данного руководства.  


### Импорт данных {.unnumbered}

Для начала мы импортируем построчный список случаев из смоделированной эпидемии лихорадки Эбола. Загрузите их напрямую с помощью `appliedepidata::get_data(name = "linelist_cleaned_rds")` (подробности см. на странице [Скачивание руководства и данных](data_used.qmd)).    
...
Начнем с создания сводного набора данных по каждой эпидемиологической неделе и проценту случаев с известным исходом, закончившихся смертью.  
...
Практически любой график `ggplot()` можно сделать интерактивным, в том числе и тепловые графики. На странице [Тепловые графики](heatmaps.qmd) вы можете прочитать о том, как сделать приведенный ниже график, который отображает долю дней в неделю, когда определенные учреждения предоставляли данные в свою провинцию.  
