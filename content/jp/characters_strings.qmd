...
複数の行の値を結合して 1 つの行にまとめる場合など、データフレームの値を列に沿って結合したい場合は、[重複データの排除](deduplication.qmd) の章内の [ロールアップした値](deduplication.qmd#str_rollup) のセクションを参照してください。
...
この例では、最初に評価された文字列から 2 番目の値（"fever"）を抽出します。要素の抽出の詳細については、[R の基礎](basics.qmd) の章をご覧ください。
...
`str_detect()` は **dplyr** パッケージの `case_when()` と組み合わせて用いられることが多いです。例として、`occupations` がラインリスト内の列であるとします。以下の `mutate()` は `case_when()` による条件分岐に従って新しい列 `is_educator` を作ります。`case_when()` についての詳細は、[データクリーニングと主要関数](cleaning.qmd) の章を参照してください。
