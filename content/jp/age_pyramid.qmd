...
例示のため、ラインリストデータを年齢カテゴリごと、性別ごとの発症数に集約し、「横長」 形式にします。これにより、データがもともと集約されていたかのように、擬似的に作り出すことができます。[データのグループ化](grouping.qmd)と[データの縦横変換](pivoting.qmd)については、それぞれの章で詳しく解説しています。
...
上記では、"m" と "f" の因子の順序が異なる（ピラミッドが逆）ことに注意してください。順序を調整するには、集計データの性別を因子型として再定義し、希望するレベルの順序に修正する必要があります。[因子（ファクタ）型データ](factors.qmd)の章をご参照ください。
...
人口のデータをインポートして表示します（[ハンドブックとデータのダウンロード](data_used.qmd)の章をご参照ください）。
...
データを取り込みます（必要に応じて[ハンドブックとデータのダウンロード](data_used.qmd)章をご参照ください）。
