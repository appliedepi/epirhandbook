...
データフレームの一意の行のみ（重複しない行のみ）を保持するには、**dplyr** の `distinct()` を使います（[データクリーニングと主要関数](cleaning.qmd) の章でさらに詳しく説明しています）。重複している行のうち、最初の行のみが保持され、それ以外の行は削除されます。デフォルトでは、「最初」とは最も上位の `rownumber` （行の順序、上から下へ番号が振られている）を意味します。その結果、一意の行だけが残ります。
...
重複している可能性が高いものを特定する方法についての説明は、[データの結合](joining_matching.qmd) の章を参照してください。「確率的マッチング」のセクションでは、確率的マッチングアルゴリズムを適用して、データフレームを*それ*<u>自体</u>と比較し、確率的な重複排除を行う例を紹介しています。
