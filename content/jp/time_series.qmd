...
以下の例では、ドイツのカンピロバクターに関する**surveillance**パッケージのデータセットを使用していることに注意してください（詳細はハンドブックの[ハンドブックとデータのダウンロード](data_used.qmd)を参照してください）。しかし、同じコードを複数の国や他の層を持つデータセットで実行したい場合は、[r4epis github repo](https://github.com/R4EPI/epitsa) にコードテンプレートの例がありますので、そちらを参照してください。
...
本ハンドブックで使用しているすべてのデータは、[ハンドブックとデータのダウンロード](data_used.qmd)の章の手順でダウンロードできます。 

ここでは、2001年から2011年にドイツで報告されたカンピロバクター症例の週次カウントをデータセットとして例示しています。`appliedepidata::get_data(name = "campylobacter_germany")` で直接読み込めます（詳細は[ハンドブックとデータのダウンロード](data_used.qmd)のページを参照）。
...
以下のコードで、日付カラムが適切なフォーマットであることを確認します。このタブでは **tsibble** パッケージを使用し、 `yearweek()` が暦週変数を作成するために使用されています。他にもいくつかの方法がありますが（詳細は [日付型データ](dates.qmd) の章を参照してください）、時系列の場合は1つのフレームワーク（**tsibble**）に収めるのがベストです。
...
これらの各気候データファイルは、[ハンドブックとデータのダウンロード](data_used.qmd)の章からダウンロードできます。 
...
必要な行の選択方法の詳細については[重複データの排除](deduplication.qmd)の章を参照してください。
...
代入の他のオプションについては、[欠損データの処理](missing_data.qmd)の章を参照してください。

別の方法として、移動平均を計算して用いることでこれらの明らかな報告の問題をスムーズにすることができます（次のセクション、および[移動平均](moving_average.qmd)の章を参照してください）。
...
詳細は、[移動平均](moving_average.qmd)の章を参照してください。
...
以下では、周期表を作成するためのカスタム関数を定義します。R での関数の書き方は[関数の作成](writing_functions.qmd)の章を参照してください。
...
week の変数を使ってデータセットを結合することができます。結合の詳細については、ハンドブックの[データの結合](joining_matching.qmd)のセクションを参照してください。
...
データピボットについての詳細は、ハンドブックの[データの縦横変換](pivoting.qmd)のセクションを参照してください。
...
**purrr** パッケージについては[ループと反復処理・リストの操作](iteration.qmd)の章を参照してください。
