...
新しい年齢分類の性別ごとのより詳細な分布を得るために、列 `age_cat_small` と `gender` のクロス集計も行うことができます。ここでは、[記述統計表の作り方](tables_descriptive.qmd)の章で説明した **janitor** パッケージの `tabyl()` と *adorn* 関数を使用します。
